...
func log_imm_action(messg *byte, _rest ...interface{}) {
	...
	var time_s *byte = libc.AscTime(libc.LocalTime(&ct))
	var args libc.ArgList
	args.Start(messg, _rest)
	*(*byte)(unsafe.Add(unsafe.Pointer(time_s), libc.StrLen(time_s)-1)) = '\x00'
	stdio.Fprintf(fl, "%-15.15s :: ", (*byte)(unsafe.Add(unsafe.Pointer(time_s), 4)))
	stdio.Vfprintf(fl, libc.GoString(messg), args)
	stdio.Fprintf(fl, "\n")
	args.End()
	fl.Close()
}
...
func do_rpp(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if selection > 2 {
		if selection == 3 {
			if ch.Rp < 30 {
				...
				ch.Desc.Obj_name = libc.CString("Generic Armor Vest")
				ch.Desc.Obj_short = libc.CString("@cGeneric @DArmor @WVest@n")
				ch.Desc.Obj_long = libc.CString("@wA @cgeneric @Darmor @Wvest@w is lying here@n")
				ch.Desc.Obj_type = 1
		...
		if selection == 6 {
			...
			if ch.Rp < pay {
				...
				if libc.StrCaseCmp(&arg2[0], libc.CString("str")) == 0 {
					if (ch.Bonuses[BONUS_WIMP]) > 0 && int(ch.Real_abils.Str) >= 45 {
						send_to_char(ch, libc.CString("You can't because that stat maxes at 45 due to a trait negative.\r\n"))
						return
					} else if int(ch.Aff_abils.Str) >= 100 {
						send_to_char(ch, libc.CString("100 is the maximum for any stat.\r\n"))
						return
					}
					send_to_char(ch, libc.CString("You increase your strength by 2.\r\n"))
					ch.Real_abils.Str += 2
				} else if libc.StrCaseCmp(&arg2[0], libc.CString("con")) == 0 {
					if (ch.Bonuses[BONUS_FRAIL]) > 0 && int(ch.Real_abils.Con) >= 45 {
						send_to_char(ch, libc.CString("You can't because that stat maxes at 45 due to a trait negative.\r\n"))
						return
					} else if int(ch.Aff_abils.Con) >= 100 {
						send_to_char(ch, libc.CString("100 is the maximum for any stat.\r\n"))
						return
					}
					send_to_char(ch, libc.CString("You increase your constitution by 2.\r\n"))
					ch.Real_abils.Con += 2
				} else if libc.StrCaseCmp(&arg2[0], libc.CString("int")) == 0 {
					if (ch.Bonuses[BONUS_DULL]) > 0 && int(ch.Real_abils.Intel) >= 45 {
						send_to_char(ch, libc.CString("You can't because that stat maxes at 45 due to a trait negative.\r\n"))
						return
					} else if int(ch.Aff_abils.Intel) >= 100 {
						send_to_char(ch, libc.CString("100 is the maximum for any stat.\r\n"))
						return
					}
					send_to_char(ch, libc.CString("You increase your intelligence by 2.\r\n"))
					ch.Real_abils.Intel += 2
				} else if libc.StrCaseCmp(&arg2[0], libc.CString("wis")) == 0 {
					if (ch.Bonuses[BONUS_FOOLISH]) > 0 && int(ch.Real_abils.Wis) >= 45 {
						send_to_char(ch, libc.CString("You can't because that stat maxes at 45 due to a trait negative.\r\n"))
						return
					} else if int(ch.Aff_abils.Wis) >= 100 {
						send_to_char(ch, libc.CString("100 is the maximum for any stat.\r\n"))
						return
					}
					send_to_char(ch, libc.CString("You increase your wisdom by 2.\r\n"))
					ch.Real_abils.Wis += 2
				} else if libc.StrCaseCmp(&arg2[0], libc.CString("spd")) == 0 {
					if (ch.Bonuses[BONUS_SLOW]) > 0 && int(ch.Real_abils.Cha) >= 45 {
						send_to_char(ch, libc.CString("You can't because that stat maxes at 45 due to a trait negative.\r\n"))
						return
					} else if int(ch.Aff_abils.Cha) >= 100 {
						send_to_char(ch, libc.CString("100 is the maximum for any stat.\r\n"))
						return
					}
					send_to_char(ch, libc.CString("You increase your speed by 2.\r\n"))
					ch.Real_abils.Cha += 2
				} else if libc.StrCaseCmp(&arg2[0], libc.CString("agl")) == 0 {
					if (ch.Bonuses[BONUS_CLUMSY]) > 0 && int(ch.Real_abils.Dex) >= 45 {
						send_to_char(ch, libc.CString("You can't because that stat maxes at 45 due to a trait negative.\r\n"))
						return
					} else if int(ch.Aff_abils.Dex) >= 100 {
						send_to_char(ch, libc.CString("100 is the maximum for any stat.\r\n"))
						return
					}
					send_to_char(ch, libc.CString("You increase your speed by 2.\r\n"))
		...
		if selection == 8 {
			...
			if ch.Rp < pay {
				...
				char_from_room(ch)
				if ch.Droom != int(-1) && ch.Droom != 0 && ch.Droom != 1 {
					char_to_room(ch, real_room(ch.Droom))
				} else if int(ch.Chclass) == CLASS_ROSHI {
				...
				} else if int(ch.Chclass) == CLASS_GINYU {
				...
				ch.Deathtime = 0
				act(libc.CString("$n's body forms in a pool of @Bblue light@n."), 1, ch, nil, nil, TO_ROOM)
				...
				if GET_LEVEL(ch) > 9 {
					...
					if GET_LEVEL(ch) > 9 {
						...
						var con int = -8
						var agl int = -8
						if int(ch.Real_abils.Str) <= 16 {
							str = -4
						}
						if int(ch.Real_abils.Intel) <= 16 {
							intel = -4
						}
						if int(ch.Real_abils.Cha) <= 16 {
							spd = -4
						}
						if int(ch.Real_abils.Dex) <= 16 {
							agl = -4
						}
						if int(ch.Real_abils.Wis) <= 16 {
							wis = -4
						}
						if int(ch.Real_abils.Con) <= 16 {
							con = -4
						}
		...
		if selection == 11 {
			...
			if ch.Rp < pay {
				...
				return
			} else if int(ch.Race) == RACE_ARLIAN {
	...
	if selection <= 2 {
		...
		if selection == 1 {
			stdio.Fprintf(fl, "@D[@cName@W: @C%-20s @cRequest@W: @Y%-20s@D]\n", GET_NAME(ch), "House")
			send_to_imm(libc.CString("RPP Request: %s paid for house"), GET_NAME(ch))
			BOARDNEWCOD = libc.GetTime(nil)
...
func do_commune(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if prob < perc {
		ch.Mana -= cost
		reveal_hiding(ch, 0)
		act(libc.CString("@cYou close your eyes and try to commune with the Eldritch Star. You are unable to concentrate though.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@W$n closes $s eyes for a moment. Then $e reopens them and frowns.@n"), 1, ch, nil, nil, TO_ROOM)
		...
		reveal_hiding(ch, 0)
		act(libc.CString("@cYou close your eyes and commune with the Eldritch Star spiritually. You feel your stamina replenish some.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@W$n closes $s eyes for a moment. Then $e reopens them and smiles.@n"), 1, ch, nil, nil, TO_ROOM)
		WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
		return
	}
}
	...
	if ch.Majinize <= 0 {
		...
		if fail == 1 {
			...
			if rand_number(10, 100)-int(ch.Aff_abils.Intel) > 60 {
				reveal_hiding(ch, 0)
				act(libc.CString("@WYou focus all your knowledge and will on breaking free. Dark purple energy swirls around your body and the M on your forehead burns brightly. After a few moments you give up, having failed to overcome the majinization!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@W$n focuses hard with $s eyes closed. Dark purple energy swirls around $s body and the M on $s head burns brightly. After a few moments $n seems to give up and the commotion dies down.@n"), 1, ch, nil, nil, TO_ROOM)
				return
			} else {
				ch.Exp = 0
				ch.Player_specials.Class_skill_points[ch.Chclass] -= 100
...
func do_grapple(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if ch.Grappling != nil {
		act(libc.CString("@RYou stop grappling with @r$N@R!@n"), 1, ch, nil, unsafe.Pointer(ch.Grappling), TO_CHAR)
		act(libc.CString("@r$n@R stops grappling with @rYOU!!@n"), 1, ch, nil, unsafe.Pointer(ch.Grappling), TO_VICT)
		act(libc.CString("@r$n@R stops grappling with @r$N@R!@n"), 1, ch, nil, unsafe.Pointer(ch.Grappling), TO_NOTVICT)
		ch.Grappling.Grap = -1
	...
	var pass int = 0
	_ = pass
	if libc.StrCaseCmp(libc.CString("hold"), &arg2[0]) == 0 || libc.StrCaseCmp(libc.CString("choke"), &arg2[0]) == 0 || libc.StrCaseCmp(libc.CString("grab"), &arg2[0]) == 0 || libc.StrCaseCmp(libc.CString("wrap"), &arg2[0]) == 0 {
		pass = 1
		...
		if (!IS_NPC(vict) && int(vict.Race) == RACE_ICER && rand_number(1, 30) >= 28 || AFF_FLAGGED(vict, AFF_ZANZOKEN)) && vict.Move >= 1 && int(vict.Position) != POS_SLEEPING {
			if !AFF_FLAGGED(ch, AFF_ZANZOKEN) || AFF_FLAGGED(ch, AFF_ZANZOKEN) && GET_SPEEDI(ch)+rand_number(1, 5) < GET_SPEEDI(vict)+rand_number(1, 5) {
				reveal_hiding(ch, 0)
				act(libc.CString("@C$N@c disappears, avoiding your grapple attempt before reappearing!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@cYou disappear, avoiding @C$n's@c grapple attempt before reappearing!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@C$N@c disappears, avoiding @C$n's@c grapple attempt before reappearing!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				...
				reveal_hiding(ch, 0)
				act(libc.CString("@C$N@c disappears, trying to avoid your grapple but your zanzoken is faster!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@cYou zanzoken to avoid the grapple attempt but @C$n's@c zanzoken is faster!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@C$N@c disappears, trying to avoid @C$n's@c grapple attempt but @C$n's@c zanzoken is faster!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				REMOVE_BIT_AR(vict.Affected_by[:], AFF_ZANZOKEN)
				REMOVE_BIT_AR(ch.Affected_by[:], AFF_ZANZOKEN)
			}
		}
		...
		if (float64(ch.Hit)*0.02)*float64(ch.Aff_abils.Str) < (float64(vict.Hit)*0.01)*float64(vict.Aff_abils.Str) {
			reveal_hiding(ch, 0)
			act(libc.CString("@RYou try to grapple with @r$N@R, but $E manages to overpower you!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@r$n@R tries to grapple with YOU, but you manage to overpower $m!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@r$n@R tries to grapple with @r$N@R, but $E manages to overpower @r$n@R!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			ch.Move -= int64(cost)
			...
			reveal_hiding(ch, 0)
			act(libc.CString("@RYou try to grapple with @r$N@R, but $E manages to avoid it!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@r$n@R tries to grapple with YOU, but you manage to avoid it!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@r$n@R tries to grapple with @r$N@R, but $E manages to avoid it!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			ch.Move -= int64(cost)
			improve_skill(ch, SKILL_GRAPPLE, 1)
			WAIT_STATE(ch, (int(1000000/OPT_USEC))*4)
			return
		} else if !HAS_ARMS(vict) && libc.StrCaseCmp(libc.CString("grab"), &arg2[0]) == 0 {
			send_to_char(ch, libc.CString("They don't even have an arm to grab onto!\r\n"))
			return
		} else if libc.StrCaseCmp(libc.CString("hold"), &arg2[0]) == 0 {
			reveal_hiding(ch, 0)
			act(libc.CString("@RYou rush at @r$N@R and manage to get $M in a hold from behind!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@r$n@R rushes at YOU and manages to get you in a hold from behind!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@r$n@R rushes at @r$N@R and manages to get $M in a hold from behind!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			ch.Grappling = vict
			ch.Grap = 1
			vict.Grappled = ch
			vict.Grap = 1
			...
			WAIT_STATE(ch, (int(1000000/OPT_USEC))*4)
			return
		} else if libc.StrCaseCmp(libc.CString("choke"), &arg2[0]) == 0 {
			reveal_hiding(ch, 0)
			act(libc.CString("@RYou rush at @r$N@R and manage to grab $S throat with both hands!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@r$n@R rushes at YOU and manages to grab your throat with both hands!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@r$n@R rushes at @r$N@R and manages to grab $S throat with both hands!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			ch.Grappling = vict
			ch.Grap = 2
			vict.Grappled = ch
			vict.Grap = 2
			...
			WAIT_STATE(ch, (int(1000000/OPT_USEC))*4)
			return
		} else if libc.StrCaseCmp(libc.CString("wrap"), &arg2[0]) == 0 {
			if int(ch.Race) != RACE_MAJIN {
				send_to_char(ch, libc.CString("Your body is not flexible enough to wrap around a target!\r\n"))
				return
			}
			act(libc.CString("@MMoving quickly you stretch your body out and wrap it around the length of @c$N's@M body! You tighten your body until you begin crushing @c$N@M!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@C$n@M quickly stretches out $s body and wraps it around @RYOU@M! You feel $s body begin to crush your own!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@C$n@M quickly stretches out $s body and wraps it around @c$N@M! It appears that @c$N's@M body is being crushed slowly!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			ch.Grappling = vict
			ch.Grap = 4
			vict.Grappled = ch
			vict.Grap = 4
			...
			WAIT_STATE(ch, (int(1000000/OPT_USEC))*4)
			return
		} else if libc.StrCaseCmp(libc.CString("grab"), &arg2[0]) == 0 {
			reveal_hiding(ch, 0)
			act(libc.CString("@RYou rush at @r$N@R and manage to lock your arm onto $S!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@r$n@R rushes at YOU and manages to lock $s arm onto your's!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@r$n@R rushes at @r$N@R and manages to lock $s arm onto @r$N's@R!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			ch.Grappling = vict
			ch.Grap = 3
			vict.Grappled = ch
			vict.Grap = 3
...
func do_trip(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if vict != nil {
		...
		if (!IS_NPC(vict) && int(vict.Race) == RACE_ICER && rand_number(1, 30) >= 28 || AFF_FLAGGED(vict, AFF_ZANZOKEN)) && vict.Move >= 1 && int(vict.Position) != POS_SLEEPING {
			if !AFF_FLAGGED(ch, AFF_ZANZOKEN) || AFF_FLAGGED(ch, AFF_ZANZOKEN) && GET_SPEEDI(ch)+rand_number(1, 5) < GET_SPEEDI(vict)+rand_number(1, 5) {
				reveal_hiding(ch, 0)
				act(libc.CString("@C$N@c disappears, avoiding your trip before reappearing!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@cYou disappear, avoiding @C$n's@c trip before reappearing!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@C$N@c disappears, avoiding @C$n's@c trip before reappearing!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				if AFF_FLAGGED(ch, AFF_ZANZOKEN) {
					REMOVE_BIT_AR(ch.Affected_by[:], AFF_ZANZOKEN)
				}
				REMOVE_BIT_AR(vict.Affected_by[:], AFF_ZANZOKEN)
				...
				reveal_hiding(ch, 0)
				act(libc.CString("@C$N@c disappears, trying to avoid your trip but your zanzoken is faster!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@cYou zanzoken to avoid the trip but @C$n's@c zanzoken is faster!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@C$N@c disappears, trying to avoid @C$n's@c trip but @C$n's@c zanzoken is faster!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				REMOVE_BIT_AR(vict.Affected_by[:], AFF_ZANZOKEN)
				REMOVE_BIT_AR(ch.Affected_by[:], AFF_ZANZOKEN)
			}
		}
		if perc < prob {
			reveal_hiding(ch, 0)
			act(libc.CString("@mYou move to trip $N@m, but you screw up and $E keeps $S footing!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@m$n@m moves to trip YOU, but $e screws up and you manage to keep your footing!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@m$n@m moves to trip $N@m, but $e screws up and $N@m manages to keep $S footing!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			...
			reveal_hiding(ch, 0)
			act(libc.CString("@mYou move to trip $N@m, and manage to knock $M off $S feet!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@m$n@m moves to trip YOU, and manages to knock you off your feet!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@m$n@m moves to trip $N@m, and manages to knock $N@m off $S feet!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			improve_skill(ch, SKILL_TRIP, 0)
			ch.Move -= int64(cost)
...
func do_train(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	var concap int = 5000
	var aglcap int = 5000
	strcap += int(ch.Real_abils.Str) * 500
	intcap += int(ch.Real_abils.Intel) * 500
	wiscap += int(ch.Real_abils.Wis) * 500
	spdcap += int(ch.Real_abils.Cha) * 500
	concap += int(ch.Real_abils.Con) * 500
	aglcap += int(ch.Real_abils.Dex) * 500
	...
	if libc.StrCaseCmp(libc.CString("str"), &arg[0]) == 0 {
		if int(ch.Real_abils.Str) == 80 {
			send_to_char(ch, libc.CString("Your base strength is maxed!\r\n"))
			return
		}
		if int(ch.Real_abils.Str) == 25 && (ch.Bonuses[BONUS_WIMP]) > 0 {
		...
		case 1:
			act(libc.CString("@WYou throw a flurry of punches into the air at an invisible opponent.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@W$n throws a flurry of punches into the air.@n"), 1, ch, nil, nil, TO_ROOM)
		case 2:
			act(libc.CString("@WYou leap into the air and throw a wild kick at an invisible opponent@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@W$n leaps into the air and throws a wild kick at nothing.@n"), 1, ch, nil, nil, TO_ROOM)
		case 3:
			act(libc.CString("@WYou leap high into the air and unleash a flurry of punches and kicks at an invisible opponent@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@W$n leaps high into the air and unleashes a flurry of punches and kicks at nothing.@n"), 1, ch, nil, nil, TO_ROOM)
		}
		...
		if ch.Player_specials.Trainstr >= strcap {
			...
			ch.Real_abils.Str += 1
			if int(ch.Chclass) == CLASS_PICCOLO && int(ch.Race) == RACE_NAMEK && level_exp(ch, GET_LEVEL(ch)+1)-int(ch.Exp) > 0 {
	...
	} else if libc.StrCaseCmp(libc.CString("spd"), &arg[0]) == 0 {
		if int(ch.Real_abils.Cha) == 80 {
			send_to_char(ch, libc.CString("Your base speed is maxed!\r\n"))
			return
		}
		if int(ch.Real_abils.Cha) == 25 && (ch.Bonuses[BONUS_SLOW]) > 0 {
		...
		switch rand_number(1, 3) {
		case 1:
			act(libc.CString("@WYou dash quickly around the surrounding area as fast as you can!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@W$n dashes quickly around the surrounding area as fast as $e can!@n"), 1, ch, nil, nil, TO_ROOM)
		case 2:
			act(libc.CString("@WYou dodge to the side as fast as you can!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@W$n dodges to the side as fast as $e can!@n"), 1, ch, nil, nil, TO_ROOM)
		case 3:
			act(libc.CString("@WYou dash backwards as fast as you can!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@W$n dashes backwards as fast as $e can!@n"), 1, ch, nil, nil, TO_ROOM)
		}
		plus = int((((total / 20) + ch.Max_move/50) * 100) / ch.Max_move)
		if GET_LEVEL(ch) > 80 {
			plus += 50
		...
		if ch.Player_specials.Trainspd >= spdcap {
			...
			ch.Real_abils.Cha += 1
			if int(ch.Chclass) == CLASS_PICCOLO && level_exp(ch, GET_LEVEL(ch)+1)-int(ch.Exp) > 0 {
				ch.Exp += int64(float64(level_exp(ch, GET_LEVEL(ch)+1)) * 0.25)
				send_to_char(ch, libc.CString("You gained quite a bit of experience from that!\r\n"))
			}
			save_char(ch)
		}
	} else if libc.StrCaseCmp(libc.CString("con"), &arg[0]) == 0 {
		if int(ch.Real_abils.Con) == 80 {
			send_to_char(ch, libc.CString("Your base constitution is maxed!\r\n"))
			return
		}
		if int(ch.Real_abils.Con) == 25 && (ch.Bonuses[BONUS_WIMP]) > 0 {
		...
		switch rand_number(1, 3) {
		case 1:
			act(libc.CString("@WYou leap into the air and then slam into the ground with your feet outstretched!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@W$n leaps into the air and then slams into the ground with $s feet outstretched!?@n"), 1, ch, nil, nil, TO_ROOM)
		case 2:
			act(libc.CString("@WYou leap into the air and then slam into the ground with your fists!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@W$n leaps into the air and then slams into the ground with $s fists!?@n"), 1, ch, nil, nil, TO_ROOM)
		case 3:
			act(libc.CString("@WYou leap into the air and then slam into the ground with your body!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@W$n leaps into the air and then slams into the ground with $s body!?@n"), 1, ch, nil, nil, TO_ROOM)
		}
		plus = int((((total / 20) + ch.Max_move/50) * 100) / ch.Max_move)
		if GET_LEVEL(ch) > 80 {
			plus += 50
	...
	} else if libc.StrCaseCmp(libc.CString("agl"), &arg[0]) == 0 {
		if int(ch.Real_abils.Dex) == 80 {
			send_to_char(ch, libc.CString("Your base agility is maxed!\r\n"))
			return
		}
		if int(ch.Real_abils.Dex) == 25 && (ch.Bonuses[BONUS_CLUMSY]) > 0 {
		...
		switch rand_number(1, 3) {
		case 1:
			act(libc.CString("@WYou do a series of backflips through the air, landing gracefully on one foot a moment later.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@W$n does a series of backflips through the air, landing gracefully on one foot a moment later.@n"), 1, ch, nil, nil, TO_ROOM)
		case 2:
			act(libc.CString("@WYou flip forward and launch off your hands into the air. You land gracefully on one foot a moment later.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@W$n flips forward and launches off $s hands into the air. Then $e lands gracefully on one foot a moment later.@n"), 1, ch, nil, nil, TO_ROOM)
		case 3:
			act(libc.CString("@WYou flip to the side off one hand and then land on your feet.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@W$n flips to the side off one hand and then lands on $s feet.@n"), 1, ch, nil, nil, TO_ROOM)
		}
		plus = int((((total / 20) + ch.Max_move/50) * 100) / ch.Max_move)
		if GET_LEVEL(ch) > 80 {
			plus += 50
	...
	} else if libc.StrCaseCmp(libc.CString("int"), &arg[0]) == 0 {
		if int(ch.Real_abils.Intel) == 80 {
			send_to_char(ch, libc.CString("Your base intelligence is maxed!\r\n"))
			return
		}
		if int(ch.Real_abils.Intel) == 25 && (ch.Bonuses[BONUS_DULL]) > 0 {
		...
		switch rand_number(1, 3) {
		case 1:
			act(libc.CString("@WConcentrating you fly high into the air as fast as you can before settling slowly back to the ground.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@W$n flies high into the air as fast as $e can before settling slowly back to the ground.@n"), 1, ch, nil, nil, TO_ROOM)
		case 2:
			act(libc.CString("@WYou focus your ki at your outstretched hand and send a mild shockwave in that direction!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@W$n focuses $s ki at $s outstretched hand and sends a mild shockwave in that direction!@n"), 1, ch, nil, nil, TO_ROOM)
		case 3:
			act(libc.CString("@WYou concentrate on your ki and force torrents of it to rush out from your body randomly!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@W$n seems to concentrate before torrents of ki randomly blasts out from $s body!@n"), 1, ch, nil, nil, TO_ROOM)
		}
	...
	} else if libc.StrCaseCmp(libc.CString("wis"), &arg[0]) == 0 {
		if int(ch.Real_abils.Wis) == 80 {
			send_to_char(ch, libc.CString("Your base wisdom is maxed!\r\n"))
			return
		}
		if int(ch.Real_abils.Wis) == 25 && (ch.Bonuses[BONUS_FOOLISH]) > 0 {
		...
		switch rand_number(1, 3) {
		case 1:
			act(libc.CString("@WYou close your eyes and wage a mental battle against an imaginary opponent.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@W$n closes $s eyes for a moment and an expression of intensity forms on it.@n"), 1, ch, nil, nil, TO_ROOM)
		case 2:
			act(libc.CString("@WYou look around and contemplate battle tactics for an imaginary scenario.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@W$n looks around and appears to be imagining things that aren't there.@n"), 1, ch, nil, nil, TO_ROOM)
		case 3:
			act(libc.CString("@WYou invent a battle plan for a battle that doesn't exist!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@W$n seems to have thought of something.@n"), 1, ch, nil, nil, TO_ROOM)
		}
		plus = int((((total / 20) + ch.Max_mana/50) * 100) / ch.Max_mana)
		if GET_LEVEL(ch) > 80 {
			plus += 50
...
func do_rip(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if ch != vict && int(ch.Position) > POS_SLEEPING {
		if ch.Move < ch.Max_move/20 {
			...
			if ch.Hit > vict.Hit*2 {
				reveal_hiding(ch, 0)
				act(libc.CString("@rYou rush at @R$N@r and grab $S tail! With a powerful tug you pull it off!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@R$n@r rushes at YOU and grabs your tail! With a powerful tug $e pulls it off!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$n@R rushes at @R$N@r and grab $S tail! With a powerful tug $e pulls it off!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				...
				reveal_hiding(ch, 0)
				act(libc.CString("@rYou rush at @R$N@r and grab $S tail! You are too weak to pull it off though!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@R$n@r rushes at YOU and grabs your tail! $e is too weak to pull it off though!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$n@R rushes at @R$N@r and grab $S tail! $e is too weak to pull it off though!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				return
			}
		} else {
			ch.Move -= ch.Max_move / 20
			reveal_hiding(ch, 0)
			act(libc.CString("@rYou rush at @R$N@r and try to grab $S tail, but fail!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@R$n@r rushes at YOU and tries to grab your tail, but fails!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@R$n@R rushes at @R$N@r and tries to grab $S tail, but fails!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			return
		}
	} else if ch == vict {
		reveal_hiding(ch, 0)
		act(libc.CString("@rYou grab your own tail and yank it off!@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@R$n@r grabs $s own tail and yanks it off!@n"), 1, ch, nil, nil, TO_ROOM)
		...
		ch.Move -= ch.Max_move / 20
		reveal_hiding(ch, 0)
		act(libc.CString("@rYou reach and grab @R$N's@r tail! With a powerful tug you pull it off!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@RYou feel your tail pulled off!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("@R$n@R reaches and grabs @R$N's@r tail! With a powerful tug $e pulls it off!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		REMOVE_BIT_AR(vict.Act[:], PLR_TAIL)
		REMOVE_BIT_AR(vict.Act[:], PLR_STAIL)
		oozaru_drop(vict)
		return
...
func do_infuse(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if AFF_FLAGGED(ch, AFF_INFUSE) {
		act(libc.CString("You stop infusing ki into your attacks."), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("$n stops infusing ki into $s attacks."), 1, ch, nil, nil, TO_ROOM)
	...
	reveal_hiding(ch, 0)
	act(libc.CString("You start infusing ki into your attacks."), 1, ch, nil, nil, TO_CHAR)
	act(libc.CString("$n starts infusing ki into $s attacks."), 1, ch, nil, nil, TO_ROOM)
...
func do_paralyze(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if (vict.Bonuses[BONUS_INSOMNIAC]) != 0 {
		...
		act(libc.CString("@RYou focus ki and point both your arms at @r$N@R. However $N seems to shake off your paralysis attack!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@r$n @Rfocuses ki and points both $s arms at YOU! Your insomnia makes you immune to $s feeble paralysis attempt.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("@r$n @Rfocuses ki and points both $s arms at @r$N@R. However $N seems to shake off $s paralysis attack!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		return
	} else if prob < perc {
		reveal_hiding(ch, 0)
		act(libc.CString("@RYou focus ki and point both your arms at @r$N@R. However $E manages to avoid your attempt to paralyze $M!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@r$n @Rfocuses ki and points both $s arms at YOU! You manage to avoid $s technique though...@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("@r$n @Rfocuses ki and points both $s arms at @r$N@R. However $E manages to avoid @r$n's@R attempted technique...@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		ch.Mana -= vict.Hit/6 + ch.Max_mana/20
		improve_skill(ch, SKILL_PARALYZE, 0)
	} else {
		reveal_hiding(ch, 0)
		act(libc.CString("@RYou focus ki and point both your arms at @r$N@R. Your ki flows into $S body and partially paralyzes $M!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@r$n @Rfocuses ki and points both $s arms at YOU! You are caught in $s paralysis technique and now can barely move!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("@r$n @Rfocuses ki and points both $s arms at @r$N@R. @r$n's@R ki flows into @r$N@R body and partially paralyzes $M!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
...
func do_taisha(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if prob < perc {
		reveal_hiding(ch, 0)
		act(libc.CString("@WYou hold up your hands while channeling ki. Your technique fails to produce an aura though....@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@g$n@W holds up $s hands while channeling ki. $s technique fails to produce an aura though...."), 1, ch, nil, nil, TO_ROOM)
		ch.Mana -= ch.Max_mana / 3
		improve_skill(ch, SKILL_TAISHA, 1)
		return
	} else {
		reveal_hiding(ch, 0)
		act(libc.CString("@WYou hold up your hands while channeling ki. Suddenly a @wburst@W of calming @Cblue@W light covers the surrounding area!@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@g$n holds up $s hands while channeling ki. Suddenly a @wburst@W of calming @Cblue@W light covers the surrounding area!@n"), 1, ch, nil, nil, TO_ROOM)
		ch.Mana -= ch.Max_mana / 3
		improve_skill(ch, SKILL_TAISHA, 1)
...
func do_kura(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if skill <= axion_dice(0) {
		ch.Move -= cost
		reveal_hiding(ch, 0)
		act(libc.CString("You crouch down and scream as your eyes turn red. You attempt to tap into your dark energies but you fail!"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@c$n@w crouches down and screams as $s eyes turn red and $e attempts to tap into dark energies but fails!"), 1, ch, nil, nil, TO_ROOM)
		...
		reveal_hiding(ch, 0)
		act(libc.CString("You crouch down and scream as your eyes turn red. You attempt to tap into your dark energies and succeed as a rush of energy explodes around you!"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@c$n@w crouches down and screams as $s eyes turn red. Suddenly $e manages to tap into dark energies and a rush of energy explodes around $m!"), 1, ch, nil, nil, TO_ROOM)
		improve_skill(ch, SKILL_KURA, 0)
		WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
		return
	}
...
func do_candy(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if rand_number(1, 6) == 6 {
		...
		reveal_hiding(ch, 0)
		act(libc.CString("@cYou aim your forelock at @R$N@c and fire a beam of energy but it is dodged!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@C$n@c aims $s forelock at @R$N@c and fires a beam of energy but the beam is dodged!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		if ch.Fighting == nil {
			set_fighting(ch, vict)
		}
		if vict.Fighting == nil {
		...
		ch.Mana -= ch.Max_mana / 15
		reveal_hiding(ch, 0)
		act(libc.CString("@cYou aim your forelock at @R$N@c and fire a beam of energy that envelopes $S entire body and changes $M into candy!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@C$n@c aims $s forelock at @R$N@c and fires a beam of energy that envelopes $S entire body and changes $M into candy!@n "), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
...
func do_future(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	one_argument(argument, &arg[0])
	if IS_NPC(ch) || int(ch.Race) != RACE_KANASSAN {
	...
	if vict != ch {
		if int(vict.Real_abils.Cha)+5 > 25 && (vict.Bonuses[BONUS_SLOW]) > 0 {
			send_to_char(ch, libc.CString("They can't handle having their speed increased beyond 25.\r\n"))
			return
		}
		if int(vict.Real_abils.Intel)+2 > 25 && (vict.Bonuses[BONUS_DULL]) > 0 {
		...
		ch.Player_specials.Class_skill_points[ch.Chclass] -= 100
		reveal_hiding(ch, 0)
		act(libc.CString("@CYou focus your energy into your fingers before stabbing your claws into $N and bestowing the power of Future Sight upon $M. Shortly after $E passes out.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@C$n focuses $s energy into $s fingers before stabbing $s claws into YOUR neck and bestowing the power of Future Sight upon you! Soon after you pass out!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("@C$n focuses $s energy into $s fingers before stabbing $s claws into $N's neck and bestowing the power of Future Sight upon $M! Soon after $E passes out!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		...
		if int(ch.Real_abils.Cha)+5 > 25 && (ch.Bonuses[BONUS_SLOW]) > 0 {
			send_to_char(ch, libc.CString("You can't handle having your speed increased beyond 25.\r\n"))
			return
		}
		if int(ch.Real_abils.Intel)+2 > 25 && (ch.Bonuses[BONUS_DULL]) > 0 {
		...
		ch.Player_specials.Class_skill_points[ch.Chclass] -= 100
		reveal_hiding(ch, 0)
		act(libc.CString("@CYou focus your energy into your mind and awaken your latent Future Sight powers!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@C$n focuses $s energy while closing $s eyes for a moment.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("@C$n focuses $s energy while closing $s eyes for a moment.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
...
func do_drag(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if ch.Drag != nil {
		...
		ch.Drag = nil
		vict.Dragged = nil
		act(libc.CString("@wYou stop dragging @C$N@W.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@C$n@W stops dragging @c$N@W.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_ROOM)
		return
	}
	...
	if int(vict.Position) != POS_SLEEPING {
		reveal_hiding(ch, 0)
		act(libc.CString("@wYou try to grab and pull @C$N@W with you, but $E resists!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@C$n@W tries to grab and pull you! However you resist!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("@C$n@W tries to grab and pull @c$N@W but $E resists!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		...
		reveal_hiding(ch, 0)
		act(libc.CString("@wYou try to grab and pull @C$N@W with you, but $E is too heavy!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@C$n@W tries to grab and pull @c$N@W but $E is too heavy!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_ROOM)
		return
	} else {
		reveal_hiding(ch, 0)
		act(libc.CString("@wYou grab and start dragging @C$N@W.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@C$n@W grabs and starts dragging @c$N@W.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		ch.Drag = vict
		vict.Dragged = ch
...
func do_stop(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if ch.Fighting == nil {
		...
		act(libc.CString("@CYou move out of your fighting posture.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@c$n@C moves out of $s fighting posture.@n"), 1, ch, nil, nil, TO_ROOM)
		stop_fighting(ch)
		WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
		return
	}
...
func do_suppress(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if IS_NONPTRANS(ch) && int(ch.Race) != RACE_ICER && IS_TRANSFORMED(ch) {
	...
	if libc.StrCaseCmp(&arg[0], libc.CString("release")) == 0 {
		if ch.Suppression != 0 {
			reveal_hiding(ch, 0)
			act(libc.CString("@GYou stop suppressing your current powerlevel!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@G$n smiles as a rush of power erupts around $s body briefly.@n"), 1, ch, nil, nil, TO_ROOM)
	...
	if ch.Hit < amt && ch.Suppression == 0 {
		...
		if ch.Suppressed+ch.Hit < amt {
			...
			reveal_hiding(ch, 0)
			act(libc.CString("@GYou alter your suppression level!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@G$n seems to concentrate for a moment.@n"), 1, ch, nil, nil, TO_ROOM)
	...
	reveal_hiding(ch, 0)
	act(libc.CString("@GYou suppress your current powerlevel!@n"), 1, ch, nil, nil, TO_CHAR)
	act(libc.CString("@G$n seems to concentrate for a moment.@n"), 1, ch, nil, nil, TO_ROOM)
	var diff int64 = ch.Hit - amt
	ch.Suppressed = diff
	ch.Hit = amt
...
func do_hass(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if perc < prob {
		reveal_hiding(ch, 0)
		act(libc.CString("@WYou try to move your arms at incredible speeds but screw up and waste some of your stamina.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@C$n@W tries to move $s arms at incredible speeds but screws up and wastes some of $s stamina.@n"), 1, ch, nil, nil, TO_ROOM)
		ch.Move -= ch.Max_move / 30
		improve_skill(ch, SKILL_HASSHUKEN, 0)
		return
	} else {
		reveal_hiding(ch, 0)
		act(libc.CString("@WYou concentrate and start to move your arms at incredible speeds.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@C$n@W concentrates and starts to move $s arms at incredible speeds.@n"), 1, ch, nil, nil, TO_ROOM)
...
func do_implant(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	for obj = ch.Carrying; obj != nil; obj = next_obj {
		...
		if found == 0 && GET_OBJ_VNUM(obj) == 66 && (!OBJ_FLAGGED(obj, ITEM_BROKEN) && !OBJ_FLAGGED(obj, ITEM_FORGED)) {
			found = 1
			limb = obj
		}
	}
	...
	if libc.StrCmp(&arg[0], libc.CString("rarm")) == 0 {
		if (vict.Limb_condition[0]) >= 1 {
			...
			if vict != ch {
				reveal_hiding(ch, 0)
				act(libc.CString("@WYou place the $p@W up to @c$N@W's body. It automaticly adjusts itself, becoming a new right arm!@n"), 1, ch, limb, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@C$n@W places a $p@W up to your body. It automaticly adjusts itself, becoming a new right arm!@n"), 1, ch, limb, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@C$n@W places a $p@W up to @c$N@W's body. It automaticly adjusts itself, becoming a new right arm!@n"), 1, ch, limb, unsafe.Pointer(vict), TO_NOTVICT)
			}
			if vict == ch {
				reveal_hiding(ch, 0)
				act(libc.CString("@WYou place the $p@W up to your body. It automaticly adjusts itself, becoming a new right arm!@n"), 1, ch, limb, nil, TO_CHAR)
				act(libc.CString("@C$n@W places the $p@W up to $s body. It automaticly adjusts itself, becoming a new right arm!@n"), 1, ch, limb, nil, TO_ROOM)
			}
		...
		if (vict.Limb_condition[1]) >= 1 {
			...
			if vict != nil && vict != ch {
				reveal_hiding(ch, 0)
				act(libc.CString("@WYou place the $p@W up to @c$N@W's body. It automaticly adjusts itself, becoming a new left arm!@n"), 1, ch, limb, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@C$n@W places a $p@W up to your body. It automaticly adjusts itself, becoming a new left arm!@n"), 1, ch, limb, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@C$n@W places a $p@W up to @c$N@W's body. It automaticly adjusts itself, becoming a new left arm!@n"), 1, ch, limb, unsafe.Pointer(vict), TO_NOTVICT)
			}
			if vict == ch {
				reveal_hiding(ch, 0)
				act(libc.CString("@WYou place the $p@W up to your body. It automaticly adjusts itself, becoming a new left arm!@n"), 1, ch, limb, nil, TO_CHAR)
				act(libc.CString("@C$n@W places the $p@W up to $s body. It automaticly adjusts itself, becoming a new left arm!@n"), 1, ch, limb, nil, TO_ROOM)
			}
		...
		if (vict.Limb_condition[2]) >= 1 {
			...
			if vict != nil && vict != ch {
				reveal_hiding(ch, 0)
				act(libc.CString("@WYou place the $p@W up to @c$N@W's body. It automaticly adjusts itself, becoming a new right leg!@n"), 1, ch, limb, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@C$n@W places a $p@W up to your body. It automaticly adjusts itself, becoming a new right leg!@n"), 1, ch, limb, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@C$n@W places a $p@W up to @c$N@W's body. It automaticly adjusts itself, becoming a new right leg!@n"), 1, ch, limb, unsafe.Pointer(vict), TO_NOTVICT)
			}
			if vict == ch {
				reveal_hiding(ch, 0)
				act(libc.CString("@WYou place the $p@W up to your body. It automaticly adjusts itself, becoming a new right leg!@n"), 1, ch, limb, nil, TO_CHAR)
				act(libc.CString("@C$n@W places the $p@W up to $s body. It automaticly adjusts itself, becoming a new right leg!@n"), 1, ch, limb, nil, TO_ROOM)
			}
		...
		if (vict.Limb_condition[3]) >= 1 {
			...
			if vict != nil && vict != ch {
				reveal_hiding(ch, 0)
				act(libc.CString("@WYou place the $p@W up to @c$N@W's body. It automaticly adjusts itself, becoming a new left leg!@n"), 1, ch, limb, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@C$n@W places a $p@W up to your body. It automaticly adjusts itself, becoming a new left leg!@n"), 1, ch, limb, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@C$n@W places a $p@W up to @c$N@W's body. It automaticly adjusts itself, becoming a new left leg!@n"), 1, ch, limb, unsafe.Pointer(vict), TO_NOTVICT)
			}
			if vict == nil || vict == ch {
				reveal_hiding(ch, 0)
				act(libc.CString("@WYou place the $p@W up to your body. It automaticly adjusts itself, becoming a new left leg!@n"), 1, ch, limb, nil, TO_CHAR)
				act(libc.CString("@C$n@W places the $p@W up to $s body. It automaticly adjusts itself, becoming a new left leg!@n"), 1, ch, limb, nil, TO_ROOM)
			}
...
func do_pose(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if int(ch.Real_abils.Str)+8 > 25 && (ch.Bonuses[BONUS_WIMP]) > 0 {
		send_to_char(ch, libc.CString("You can't handle having your strength increased beyond 25.\r\n"))
		return
	}
	if int(ch.Real_abils.Dex)+8 > 25 && (ch.Bonuses[BONUS_CLUMSY]) > 0 {
		send_to_char(ch, libc.CString("You can't handle having your agility increased beyond 25.\r\n"))
		return
	}
	if prob < perc {
		reveal_hiding(ch, 0)
		act(libc.CString("@WYou attempt to strike an awe inspiring pose, but end up falling on your face!@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@C$n@W attempts to strike an awe inspiring pose, but ends up falling on $s face!@n"), 1, ch, nil, nil, TO_ROOM)
		...
		case 1:
			act(libc.CString("@WYou turn around with your back to everyone. You bend forward dramatically and put your head between your legs!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@C$n@W turns around with $s back to you. $e bends forward dramatically and puts $s head between $s legs. Strange...@n"), 1, ch, nil, nil, TO_ROOM)
		case 2:
			act(libc.CString("@WYou turn to the side while flexing your muscles and extend your arms up at an angle dramatically!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@C$n@W turns to the side while flexing $s muscles and extending $s arms up at an angle dramatically!@n"), 1, ch, nil, nil, TO_ROOM)
		case 3:
			act(libc.CString("@WYou extend one leg outward while you bend forward, balancing on a single leg!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@C$n@W extends one leg outward while $e bends forward, balancing on a single leg!@n"), 1, ch, nil, nil, TO_ROOM)
		case 4:
			act(libc.CString("@WYou drop down to one knee while angling your arms up to either side and slanting your hands down like wings!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@C$n@W drops down to one knee while angling $s arms up to either side and slanting $s hands down like wings!@n"), 1, ch, nil, nil, TO_ROOM)
		}
...
func do_fury(ch *char_data, argument *byte, cmd int, subcmd int) {
	var arg [2048]byte
	one_argument(argument, &arg[0])
	if int(ch.Race) != RACE_HALFBREED || IS_NPC(ch) {
	...
	if arg[0] == 0 {
		...
		ch.Fury = 0
	} else if libc.StrCaseCmp(&arg[0], libc.CString("attack")) == 0 {
		ch.Fury = 50
	} else {
	...
	reveal_hiding(ch, 0)
	act(libc.CString("You release your fury! Your very next attack is guaranteed to rip your foes a new one!"), 1, ch, nil, nil, TO_CHAR)
	act(libc.CString("$n screams furiously as a look of anger appears on $s face!"), 1, ch, nil, nil, TO_ROOM)
...
func hint_system(ch *char_data, num int) {
	...
	if int(ch.Race) != RACE_ANDROID && int(ch.Race) != RACE_NAMEK {
...
func do_telepathy(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if libc.StrCmp(&arg[0], libc.CString("far")) == 0 {
		if IS_NPC(ch) {
			return
		} else if (func() *char_data {
			...
			if int(vict.Aff_abils.Intel) > int(ch.Aff_abils.Intel) {
	...
	if libc.StrCmp(&arg[0], libc.CString("link")) == 0 {
		if IS_NPC(ch) {
			return
		}
		if ch.Mindlink != nil {
			act(libc.CString("@CYou remove the link your mind had with @w$N.@n"), 1, ch, nil, unsafe.Pointer(ch.Mindlink), TO_CHAR)
			act(libc.CString("@w$n@C removes the link $s mind had with yours.@n"), 1, ch, nil, unsafe.Pointer(ch.Mindlink), TO_VICT)
			...
			act(libc.CString("@R$n@r tried to link $s mind with yours, but you manage to force a break in the link!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@R$N@r manages to sense the intrusion and with $S intelligence push you out!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			return
		} else {
			act(libc.CString("@CYou link your mind with @w$N.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@w$n@C links $s mind with yours. You can speak your thoughts to $m with 'think'.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
	...
	} else if libc.StrCmp(&arg[0], libc.CString("read")) == 0 {
		if (func() *char_data {
			vict = get_char_vis(ch, &arg2[0], nil, 1<<0)
			return vict
		}()) == nil {
			...
			if axion_dice(0) > GET_SKILL(ch, SKILL_TELEPATHY) {
				ch.Mana -= ch.Max_mana / 40
				act(libc.CString("@wYou attempt to read $N's@w mind, but fail to see it clearly.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				if rand_number(1, 15) >= 14 && !AFF_FLAGGED(ch, AFF_SHOCKED) {
					act(libc.CString("@MYour mind has been shocked!@n"), 1, ch, nil, nil, TO_CHAR)
				...
				ch.Mana -= ch.Max_mana / 40
				act(libc.CString("@wYou fail to read @c$N's@w mind and they seemed to have noticed the attempt!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@C$n@w attempts to read your mind, but you resist and force $m out!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				improve_skill(ch, SKILL_TELEPATHY, 0)
				return
			} else {
...
func do_potential(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if int(vict.Race) == RACE_MAJIN {
		...
		reveal_hiding(ch, 0)
		act(libc.CString("You place your hand on top of $N's head. After a moment of concentrating you release their hidden potential."), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("$n places $s hand on top of your head. After a moment you feel a rush of power as your hidden potential is released!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("$n places $s hand on $N's head. After a moment a rush of power explodes off of $N's body!"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
...
func do_majinize(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if vict.Majinize > 0 && vict.Majinize != int(ch.Id) {
		...
		reveal_hiding(ch, 0)
		act(libc.CString("You remove $N's majinization, freeing them from your influence, but also weakening them."), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("$n removes your majinization, freeing you from their influence, and weakening you!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("$n waves a hand at $N, and instantly the glowing M on $S forehead disappears!"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		vict.Majinize = 0
		ch.Boosts += 1
		var mult float64 = 1
		if int(vict.Race) == RACE_TRUFFLE && PLR_FLAGGED(vict, PLR_TRANS1) {
			...
			mult = 4
		} else if int(vict.Race) == RACE_MAJIN && PLR_FLAGGED(vict, PLR_TRANS1) {
			mult = 2
		} else if int(vict.Race) == RACE_MAJIN && PLR_FLAGGED(vict, PLR_TRANS2) {
			mult = 3
		} else if int(vict.Race) == RACE_MAJIN && PLR_FLAGGED(vict, PLR_TRANS3) {
			mult = 4.5
		} else if int(vict.Race) == RACE_HOSHIJIN && vict.Starphase == 1 {
			mult = 2
		} else if int(vict.Race) == RACE_HOSHIJIN && vict.Starphase == 2 {
		...
		reveal_hiding(ch, 0)
		act(libc.CString("You focus your power into $N, influencing their mind and increasing their strength! After the struggle ends in $S mind a glowing purple M forms on $S forehead."), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("$n focuses power into you, influencing your mind and increasing your strength! After the struggle in your mind ends a glowing purple M forms on your forehead."), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("$n focuses power into $N, influencing their mind and increasing their strength! After the struggle ends in $S mind a glowing purple M forms on $S forehead."), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
...
func do_spit(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if AFF_FLAGGED(vict, AFF_PARALYZE) {
		act(libc.CString("$N has already been turned to stone."), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		return
	}
	if vict.Fighting != nil {
	...
	if GET_SKILL(ch, SKILL_SPIT) < axion_dice(0) {
		ch.Mana -= int64(cost)
		reveal_hiding(ch, 0)
		act(libc.CString("@WGathering spit you concentrate ki into a wicked loogie and let it loose, but it falls short of hitting @c$N@W!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@C$n@W seems to focus ki before hawking a loogie at you! Fortunatly the loogie falls short.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("@C$n@W seems to focus ki before hawking a loogie at @c$N@W! Fortunatly for @c$N@W the loogie falls short.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		improve_skill(ch, SKILL_SPIT, 1)
		WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
		return
	} else if AFF_FLAGGED(vict, AFF_ZANZOKEN) && vict.Move >= 1 && int(vict.Position) != POS_SLEEPING {
		ch.Mana -= int64(cost)
		reveal_hiding(ch, 0)
		act(libc.CString("@C$N@c disappears, avoiding your spit before reappearing!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@cYou disappear, avoiding @C$n's@c @rstone spit@c before reappearing!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("@C$N@c disappears, avoiding @C$n's@c @rstone spit@c before reappearing!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		REMOVE_BIT_AR(vict.Affected_by[:], AFF_ZANZOKEN)
		WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
		improve_skill(ch, SKILL_SPIT, 1)
		return
		...
		ch.Mana -= int64(cost)
		reveal_hiding(ch, 0)
		act(libc.CString("@WGathering spit you concentrate ki into a wicked loogie and let it loose, and it smacks into @c$N@W turning $M into stone!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@C$n@W seems to focus ki before hawking a loogie at you! It manages to hit and you instantly turn to stone!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("@C$n@W seems to focus ki before hawking a loogie at @c$N@W! It manages to hit and $E instantly turns to stone!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		improve_skill(ch, SKILL_SPIT, 1)
		WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
		return
	}
...
func do_form(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	reveal_hiding(ch, 0)
	if libc.StrCmp(&arg[0], libc.CString("food")) == 0 {
		...
		if ch.Mana < cost {
			...
			if arg2[0] == 0 {
				...
				if libc.StrCaseCmp(&arg2[0], libc.CString("highest")) == 0 && skill >= 100 {
					level = 4
				} else if libc.StrCaseCmp(&arg2[0], libc.CString("high")) == 0 && skill >= 75 {
					level = 3
				} else if libc.StrCaseCmp(&arg2[0], libc.CString("mid")) == 0 && skill >= 50 {
					level = 2
				} else {
					level = 1
				}
			...
			reveal_hiding(ch, 0)
			ch.Con_cooldown = 10
			act(libc.CString("You hold out your hand and create $p out of your ki!"), 1, ch, obj, nil, TO_CHAR)
			act(libc.CString("$n holds out $s hand and creates $p out of thin air!"), 1, ch, obj, nil, TO_ROOM)
			ch.Mana -= cost
			return
		}
	} else if libc.StrCmp(&arg[0], libc.CString("water")) == 0 {
		cost = ch.Max_mana / int64(skill*2)
		cost *= int64(discount)
		if ch.Mana < cost {
			send_to_char(ch, libc.CString("You do not have enough ki to create %s\r\n"), &arg[0])
	...
	} else if libc.StrCmp(&arg[0], libc.CString("bag")) == 0 {
		cost = ch.Max_mana / int64(skill*2)
		cost *= int64(discount)
		if bag == 0 {
			send_to_char(ch, libc.CString("What do you want to create?\r\n"))
	...
	} else if libc.StrCmp(&arg[0], libc.CString("mattress")) == 0 {
	...
	} else if libc.StrCmp(&arg[0], libc.CString("weapon")) == 0 {
		cost = ch.Max_mana / 5
		cost *= int64(discount)
		if sword == 0 {
			send_to_char(ch, libc.CString("What do you want to create?\r\n"))
		...
		if ch.Mana < cost {
			...
			if arg3[0] == 0 {
				...
				if libc.StrCaseCmp(&arg3[0], libc.CString("highest")) == 0 && skill >= 100 {
					level = 5
				} else if libc.StrCaseCmp(&arg3[0], libc.CString("higher")) == 0 && skill >= 75 {
					level = 4
				} else if libc.StrCaseCmp(&arg3[0], libc.CString("high")) == 0 && skill >= 50 {
					level = 3
				} else if libc.StrCaseCmp(&arg3[0], libc.CString("mid")) == 0 && skill >= 30 {
					level = 2
				} else {
					level = 1
				}
			}
			if libc.StrCaseCmp(&arg2[0], libc.CString("sword")) == 0 {
				if level == 5 {
			...
			} else if libc.StrCaseCmp(&arg2[0], libc.CString("dagger")) == 0 {
				if level == 5 {
			...
			} else if libc.StrCaseCmp(&arg2[0], libc.CString("club")) == 0 {
				if level == 5 {
			...
			} else if libc.StrCaseCmp(&arg2[0], libc.CString("spear")) == 0 {
				if level == 5 {
			...
			} else if libc.StrCaseCmp(&arg2[0], libc.CString("gun")) == 0 {
				if level == 5 {
	...
	} else if libc.StrCmp(&arg[0], libc.CString("clothesbeam")) == 0 {
		cost = ch.Max_mana / 2
		cost *= int64(discount)
		if clothes == 0 {
			send_to_char(ch, libc.CString("What do you want to create?\r\n"))
	...
	} else if libc.StrCmp(&arg[0], libc.CString("gi")) == 0 {
		cost = ch.Max_mana / 5
		cost *= int64(discount)
		if gi == 0 {
			send_to_char(ch, libc.CString("What do you want to create?\r\n"))
	...
	} else if libc.StrCmp(&arg[0], libc.CString("shuriken")) == 0 {
		cost = ch.Max_mana / 4
		cost *= int64(discount)
		if shuriken == 0 {
			send_to_char(ch, libc.CString("What do you want to create?\r\n"))
	...
	} else if libc.StrCmp(&arg[0], libc.CString("pants")) == 0 {
		cost = ch.Max_mana / 5
		cost *= int64(discount)
		if pants == 0 {
			send_to_char(ch, libc.CString("What do you want to create?\r\n"))
	...
	} else if libc.StrCmp(&arg[0], libc.CString("wristband")) == 0 {
		cost = ch.Max_mana / 5
		cost *= int64(discount)
		if wrist == 0 {
			send_to_char(ch, libc.CString("What do you want to create?\r\n"))
	...
	} else if libc.StrCmp(&arg[0], libc.CString("boots")) == 0 {
		cost = ch.Max_mana / 5
		cost *= int64(discount)
		if boots == 0 {
			send_to_char(ch, libc.CString("What do you want to create?\r\n"))
	...
	} else if libc.StrCmp(&arg[0], libc.CString("light")) == 0 {
		cost = ch.Max_mana / int64(skill*2)
		cost *= int64(discount)
		if light == 0 {
			send_to_char(ch, libc.CString("What do you want to create?\r\n"))
	...
	} else if libc.StrCmp(&arg[0], libc.CString("kachin")) == 0 {
		cost = ch.Max_mana - 1
		cost *= int64(discount)
		if kachin == 0 {
			send_to_char(ch, libc.CString("What do you want to create?\r\n"))
	...
	} else if libc.StrCmp(&arg[0], libc.CString("elixir")) == 0 {
		cost = ch.Max_mana - 1
		cost *= int64(discount)
		if boost == 0 {
			send_to_char(ch, libc.CString("What do you want to create?\r\n"))
	...
	} else if libc.StrCmp(&arg[0], libc.CString("senzu")) == 0 {
		cost = ch.Max_mana
...
func do_recharge(ch *char_data, argument *byte, cmd int, subcmd int) {
	if IS_NPC(ch) || int(ch.Race) != RACE_ANDROID {
	...
	if !PLR_FLAGGED(ch, PLR_REPAIR) {
		...
		if ch.Mana < cost {
			...
			reveal_hiding(ch, 0)
			ch.Con_cooldown = 10
			act(libc.CString("You focus your ki into your energy reserves, recharging them some."), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("$n stops and glows green briefly."), 1, ch, nil, nil, TO_ROOM)
			ch.Mana -= cost
...
func do_srepair(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if !IS_NPC(ch) && !PLR_FLAGGED(ch, PLR_REPAIR) {
		...
		if ch.Move < cost {
			...
			reveal_hiding(ch, 0)
			ch.Con_cooldown = 10
			act(libc.CString("You repair some of your outer casings and internal systems, with the small nano-robots contained in your body."), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("$n stops a moment as small glowing particles move across $s body."), 1, ch, nil, nil, TO_ROOM)
			var repaired int = 0
			if !IS_NPC(ch) {
...
func do_upgrade(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if libc.StrCaseCmp(libc.CString("augment"), &arg[0]) == 0 {
		var (
			obj  *obj_data = nil
			gain int64     = 0
		)
		...
		if (func() *obj_data {
			...
			if libc.StrCaseCmp(libc.CString("powerlevel"), &arg2[0]) == 0 {
				obj_from_char(obj)
				extract_obj(obj)
				act(libc.CString("@WYou install the circuits and upgrade your maximum powerlevel.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n@W installs some circuits and upgrades $s systems.@n"), 1, ch, nil, nil, TO_ROOM)
				ch.Max_hit += gain
				ch.Basepl += gain
				send_to_char(ch, libc.CString("@gGain @D[@G+%s@D]\r\n"), add_commas(gain))
				return
			} else if libc.StrCaseCmp(libc.CString("ki"), &arg2[0]) == 0 {
				obj_from_char(obj)
				extract_obj(obj)
				act(libc.CString("@WYou install the circuits and upgrade your maximum ki.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n@W installs some circuits and upgrades $s systems.@n"), 1, ch, nil, nil, TO_ROOM)
				ch.Max_mana += gain
				ch.Baseki += gain
				send_to_char(ch, libc.CString("@gGain @D[@G+%s@D]\r\n"), add_commas(gain))
				return
			} else if libc.StrCaseCmp(libc.CString("stamina"), &arg2[0]) == 0 {
				obj_from_char(obj)
				extract_obj(obj)
				act(libc.CString("@WYou install the circuits and upgrade your maximum stamina.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n@W installs some circuits and upgrades $s systems.@n"), 1, ch, nil, nil, TO_ROOM)
				ch.Max_move += gain
				ch.Basest += gain
				send_to_char(ch, libc.CString("@gGain @D[@G+%s@D]\r\n"), add_commas(gain))
	...
	if arg2[0] == 0 && (libc.StrCaseCmp(libc.CString("powerlevel"), &arg[0]) == 0 || libc.StrCaseCmp(libc.CString("ki"), &arg[0]) == 0 || libc.StrCaseCmp(libc.CString("stamina"), &arg[0]) == 0) {
		send_to_char(ch, libc.CString("How many times do you want to increase %s?"), &arg[0])
		return
	}
	if libc.Atoi(libc.GoString(&arg2[0])) <= 0 && (libc.StrCaseCmp(libc.CString("powerlevel"), &arg[0]) == 0 || libc.StrCaseCmp(libc.CString("ki"), &arg[0]) == 0 || libc.StrCaseCmp(libc.CString("stamina"), &arg[0]) == 0) {
		send_to_char(ch, libc.CString("It needs to be between 1-1000\r\n"))
		return
	}
	if libc.Atoi(libc.GoString(&arg2[0])) > 1000 && (libc.StrCaseCmp(libc.CString("powerlevel"), &arg[0]) == 0 || libc.StrCaseCmp(libc.CString("ki"), &arg[0]) == 0 || libc.StrCaseCmp(libc.CString("stamina"), &arg[0]) == 0) {
		send_to_char(ch, libc.CString("It needs to be between 1-1000\r\n"))
		return
	}
	if libc.StrCaseCmp(libc.CString("powerlevel"), &arg[0]) == 0 {
	...
	} else if libc.StrCaseCmp(libc.CString("ki"), &arg[0]) == 0 {
		count = libc.Atoi(libc.GoString(&arg2[0]))
		for count > 0 {
			if GET_LEVEL(ch) >= 90 {
	...
	} else if libc.StrCaseCmp(libc.CString("stamina"), &arg[0]) == 0 {
		count = libc.Atoi(libc.GoString(&arg2[0]))
		for count > 0 {
			if GET_LEVEL(ch) >= 90 {
				bonus += GET_LEVEL(ch) * 3650
...
func do_ingest(ch *char_data, argument *byte, cmd int, subcmd int) {
	if int(ch.Race) == RACE_MAJIN {
		...
		reveal_hiding(ch, 0)
		if AFF_FLAGGED(vict, AFF_ZANZOKEN) && vict.Move >= 1 && int(vict.Position) != POS_SLEEPING {
			act(libc.CString("@C$N@c disappears, avoiding your attempted ingestion!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@cYou disappear, avoiding @C$n's@c attempted @ringestion@c before reappearing!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@C$N@c disappears, avoiding @C$n's@c attempted @ringestion@c before reappearing!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			REMOVE_BIT_AR(vict.Affected_by[:], AFF_ZANZOKEN)
			WAIT_STATE(ch, (int(1000000/OPT_USEC))*3)
			return
		}
		if GET_SPEEDI(ch)+rand_number(1, 5) < GET_SPEEDI(ch)+rand_number(1, 5) {
			act(libc.CString("@WYou fling a piece of goo at @c$N@W, and try to ingest $M! $E manages to avoid your blob of goo though!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@C$n@W flings a piece of goo at you, you manage to avoid it though!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@C$n@w flings a piece of goo at @c$N@W, but the goo misses $M@W!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			WAIT_STATE(ch, (int(1000000/OPT_USEC))*3)
			return
		} else {
			act(libc.CString("@WYou flings a piece of goo at @c$N@W! The goo engulfs $M and then returns to your body!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@C$n@W flings a piece of goo at you! The goo engulfs your body and then returns to @C$n@W!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@C$n@w flings a piece of goo at @c$N@W! The goo engulfs $M and then return to @C$n@W!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			ch.Absorbs += 1
...
func do_absorb(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if !IS_NPC(ch) {
		if int(ch.Race) == RACE_BIO && !PLR_FLAGGED(ch, PLR_TAIL) {
			send_to_char(ch, libc.CString("You have no tail!\r\n"))
			return
		}
	}
	if int(ch.Race) != RACE_ANDROID && int(ch.Race) != RACE_BIO {
	...
	if ch.Fighting != nil && int(ch.Race) != RACE_ANDROID {
	...
	if ch.Absorbing != nil {
		act(libc.CString("@WYou stop absorbing from @c$N@W!@n"), 1, ch, nil, unsafe.Pointer(ch.Absorbing), TO_CHAR)
		act(libc.CString("$n stops absorbing from you!"), 1, ch, nil, unsafe.Pointer(ch.Absorbing), TO_VICT)
		act(libc.CString("$n stops absorbing from $N!"), 1, ch, nil, unsafe.Pointer(ch.Absorbing), TO_NOTVICT)
	...
	if arg[0] == 0 && int(ch.Race) == RACE_ANDROID {
	...
	if int(ch.Race) == RACE_ANDROID {
		...
		if init_skill(ch, SKILL_ABSORB) < axion_dice(0) {
			act(libc.CString("@WYou rush at @c$N@W and try to absorb from them, but $E manages to avoid you!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@C$n@W rushes at you and tries to grab you, but you manage to avoid $m!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@C$n@w rushes at @c$N@W and tries to grab $M, but @c$N@W manages to avoid @c$n@W!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			...
			act(libc.CString("@WYou rush at @c$N@W and try to absorb from them, and manage to grab on!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@C$n@W rushes at you and tries to grab you, and manages to grab on!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@C$n@w rushes at @c$N@W and tries to grab $M, and manages to grab on!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			improve_skill(ch, SKILL_ABSORB, 1)
			ch.Absorbing = vict
			vict.Absorbby = ch
			WAIT_STATE(ch, (int(1000000/OPT_USEC))*3)
	...
	} else if int(ch.Race) == RACE_BIO && libc.StrCmp(&arg[0], libc.CString("swallow")) == 0 {
		if vict.Absorbby != nil {
			send_to_char(ch, libc.CString("%s is already absorbing from them!"), GET_NAME(vict.Absorbby))
			return
		}
		...
		if GET_SKILL(ch, SKILL_ABSORB) < axion_dice(0) {
			...
			act(libc.CString("@WYou rush at @c$N@W and your tail engulfs $M! You quickly suck $S squirming body into your tail, absorbing $m!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@C$n@W rushes at you and $s tail engulfs you! $e quickly sucks your squirming body into $s tail, absorbing you!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@C$n@w rushes at @c$N@W and $s tail engulfs $M! You quickly suck $S squirming body into your tail, absorbing @c$N@W!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			ch.Absorbs -= 1
	...
	} else if int(ch.Race) == RACE_BIO && libc.StrCmp(&arg[0], libc.CString("extract")) == 0 {
		...
		if GET_SKILL(ch, SKILL_ABSORB) < failthresh {
			...
			act(libc.CString("@WYou rush at @c$N@W and stab them with your tail! You quickly suck out all the bio extract you need and leave the empty husk behind!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@C$n@w rushes at @c$N@W and stabs $M with $s tail! $e quickly sucks out all the bio extract and leaves the empty husk of @c$N@W behind!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
...
func do_escape(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if ch.Absorbby != nil {
		...
		if num > skill {
			...
			act(libc.CString("@c$N@W struggles to break loose of @C$n's@W hold!@n"), 1, ch.Absorbby, nil, unsafe.Pointer(ch), TO_NOTVICT)
			act(libc.CString("@WYou struggle to break loose of @C$n's@W hold!@n"), 1, ch.Absorbby, nil, unsafe.Pointer(ch), TO_VICT)
			act(libc.CString("@c$N@W struggles to break loose of your hold!@n"), 1, ch.Absorbby, nil, unsafe.Pointer(ch), TO_CHAR)
			if rand_number(1, 3) == 3 {
				var dmg int64 = int64(float64(ch.Max_hit) * 0.025)
				hurt(0, 0, ch, ch.Absorbby, nil, dmg, 0)
				if int(ch.Absorbby.Position) == POS_SLEEPING {
					act(libc.CString("@c$N@W manages to break loose of @C$n's@W hold!@n"), 1, ch.Absorbby, nil, unsafe.Pointer(ch), TO_NOTVICT)
					act(libc.CString("@WYou manage to break loose of @C$n's@W hold!@n"), 1, ch.Absorbby, nil, unsafe.Pointer(ch), TO_VICT)
					act(libc.CString("@c$N@W manages to break loose of your hold!@n"), 1, ch.Absorbby, nil, unsafe.Pointer(ch), TO_CHAR)
					ch.Absorbby.Absorbing = nil
	...
	if ch.Grappled != nil {
		...
		if num > skill {
			if ch.Grappled.Grap == 4 {
				act(libc.CString("@c$N@M flexes with all $S might and causes your body to explode outward into gooey chunks!@n"), 1, ch.Grappled, nil, unsafe.Pointer(ch), TO_CHAR)
				act(libc.CString("@MYou flex with all your might and cause @C$n's@M body to explode outward into gooey chunks!@n"), 1, ch.Grappled, nil, unsafe.Pointer(ch), TO_VICT)
				act(libc.CString("@c$N@M flexes with all $S might and causes @C$n's@M body to explode outward into gooey chunks!@n"), 1, ch.Grappled, nil, unsafe.Pointer(ch), TO_NOTVICT)
				act(libc.CString("@MYou reform your body mere moments later.@n"), 1, ch.Grappled, nil, nil, TO_CHAR)
				act(libc.CString("@C$n@M reforms $s body mere moments later."), 1, ch.Grappled, nil, nil, TO_ROOM)
			} else {
				act(libc.CString("@c$N@W manages to break loose of @C$n's@W hold!@n"), 1, ch.Grappled, nil, unsafe.Pointer(ch), TO_NOTVICT)
				act(libc.CString("@WYou manage to break loose of @C$n's@W hold!@n"), 1, ch.Grappled, nil, unsafe.Pointer(ch), TO_VICT)
				act(libc.CString("@c$N@W manages to break loose of your hold!@n"), 1, ch.Grappled, nil, unsafe.Pointer(ch), TO_CHAR)
			}
			if ch.Fighting == nil {
			...
			act(libc.CString("@c$N@W struggles to break loose of @C$n's@W hold!@n"), 1, ch.Grappled, nil, unsafe.Pointer(ch), TO_NOTVICT)
			act(libc.CString("@WYou struggle to break loose of @C$n's@W hold!@n"), 1, ch.Grappled, nil, unsafe.Pointer(ch), TO_VICT)
			act(libc.CString("@c$N@W struggles to break loose of your hold!@n"), 1, ch.Grappled, nil, unsafe.Pointer(ch), TO_CHAR)
			if rand_number(1, 3) == 3 {
				var dmg int64 = int64(float64(ch.Max_hit) * 0.025)
				hurt(0, 0, ch, ch.Grappled, nil, dmg, 0)
				if int(ch.Grappled.Position) == POS_SLEEPING {
					act(libc.CString("@c$N@W manages to break loose of @C$n's@W hold!@n"), 1, ch.Grappled, nil, unsafe.Pointer(ch), TO_NOTVICT)
					act(libc.CString("@WYou manage to break loose of @C$n's@W hold!@n"), 1, ch.Grappled, nil, unsafe.Pointer(ch), TO_VICT)
					act(libc.CString("@c$N@W manages to break loose of your hold!@n"), 1, ch.Grappled, nil, unsafe.Pointer(ch), TO_CHAR)
					ch.Grappled.Grap = -1
...
func do_regenerate(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if ch.Hit >= gear_pl(ch) {
		act(libc.CString("You concentrate your ki and regenerate your body completely."), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("$n concentrates and regenerates $s body completely."), 1, ch, nil, nil, TO_ROOM)
	} else if amt < ch.Max_hit/10 {
		act(libc.CString("You concentrate your ki and regenerate your body a little."), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("$n concentrates and regenerates $s body a little."), 1, ch, nil, nil, TO_ROOM)
	} else if amt < ch.Max_hit/5 {
		act(libc.CString("You concentrate your ki and regenerate your body some."), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("$n concentrates and regenerates $s body some."), 1, ch, nil, nil, TO_ROOM)
	} else if amt < ch.Max_hit/2 {
		act(libc.CString("You concentrate your ki and regenerate your body a great deal."), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("$n concentrates and regenerates $s body a great deal."), 1, ch, nil, nil, TO_ROOM)
	} else if ch.Hit < ch.Max_hit {
		act(libc.CString("You concentrate your ki and regenerate you nearly completely."), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("$n concentrates and regenerates $s body nearly completely."), 1, ch, nil, nil, TO_ROOM)
	}
	improve_skill(ch, SKILL_REGENERATE, 0)
	if AFF_FLAGGED(ch, AFF_BURNED) {
		send_to_char(ch, libc.CString("Your burns are healed now.\r\n"))
		act(libc.CString("$n@w's burns are now healed.@n"), 1, ch, nil, nil, TO_ROOM)
		null_affect(ch, AFF_BURNED)
	}
	if !IS_NPC(ch) {
		if (ch.Limb_condition[0]) <= 0 {
			act(libc.CString("You regrow your right arm!"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("$n regrows $s right arm!"), 1, ch, nil, nil, TO_ROOM)
			ch.Limb_condition[0] = 100
		} else if (ch.Limb_condition[0]) >= 0 && (ch.Limb_condition[0]) < 50 {
			act(libc.CString("Your broken right arm mends itself!"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("$n regenerates $s broken right arm!"), 1, ch, nil, nil, TO_ROOM)
			ch.Limb_condition[0] = 100
		}
		if (ch.Limb_condition[1]) <= 0 {
			ch.Limb_condition[1] = 100
			act(libc.CString("You regrow your left arm!"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("$n regrows $s left arm!"), 1, ch, nil, nil, TO_ROOM)
		} else if (ch.Limb_condition[1]) > 0 && (ch.Limb_condition[1]) < 50 {
			act(libc.CString("Your broken left arm mends itself!"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("$n regenerates $s broken left arm!"), 1, ch, nil, nil, TO_ROOM)
			ch.Limb_condition[1] = 100
		}
		if (ch.Limb_condition[3]) <= 0 {
			ch.Limb_condition[3] = 100
			act(libc.CString("You regrow your left leg!"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("$n regrows $s left leg!"), 1, ch, nil, nil, TO_ROOM)
		} else if (ch.Limb_condition[3]) > 0 && (ch.Limb_condition[3]) < 50 {
			act(libc.CString("Your broken left leg mends itself!"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("$n regenerates $s broken left leg!"), 1, ch, nil, nil, TO_ROOM)
			ch.Limb_condition[3] = 100
		}
		if (ch.Limb_condition[2]) <= 0 {
			ch.Limb_condition[2] = 100
			act(libc.CString("You regrow your right leg!"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("$n regrows $s right leg!"), 1, ch, nil, nil, TO_ROOM)
		} else if (ch.Limb_condition[2]) > 0 && (ch.Limb_condition[2]) < 50 {
			ch.Limb_condition[2] = 100
			act(libc.CString("Your broken right leg mends itself!"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("$n regenerates $s broken right leg!"), 1, ch, nil, nil, TO_ROOM)
		}
		if !PLR_FLAGGED(ch, PLR_TAIL) && int(ch.Race) == RACE_BIO {
			SET_BIT_AR(ch.Act[:], PLR_TAIL)
			act(libc.CString("You regrow your tail!"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("$n regrows $s tail!"), 1, ch, nil, nil, TO_ROOM)
		}
		improve_skill(ch, SKILL_REGENERATE, 0)
	}
}
func do_focus(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if AFF_FLAGGED(ch, AFF_CURSE) {
		...
		return
	} else if libc.StrCmp(&arg[0], libc.CString("tough")) == 0 {
		...
		if name[0] == 0 {
			if AFF_FLAGGED(ch, AFF_STONESKIN) {
				...
				reveal_hiding(ch, 0)
				act(libc.CString("You focus ki into your skin, but fail in making it tough!"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("$n focuses ki into $s skin, but fails in making it tough!"), 1, ch, nil, nil, TO_ROOM)
				return
			} else {
				...
				ch.Mana -= ch.Max_mana / 20
				reveal_hiding(ch, 0)
				act(libc.CString("You focus ki into your skin, making it tough!"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("$n focuses ki into $s skin, making it tough!"), 1, ch, nil, nil, TO_ROOM)
				return
			}
		} else {
			if (func() *char_data {
			...
			if !can_kill(ch, vict, nil, 2) {
				...
				if AFF_FLAGGED(vict, AFF_STONESKIN) {
					...
					ch.Mana -= ch.Max_mana / 20
					reveal_hiding(ch, 0)
					act(libc.CString("You focus ki into $N's skin, but fail in making it tough!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("$n focuses ki into your skin, but fails in making it tough!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("$n focuses ki into $N's skin, but fails in making it tough!"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					return
				} else {
					...
					ch.Mana -= ch.Max_mana / 20
					reveal_hiding(ch, 0)
					act(libc.CString("You focus ki into $N's skin, making it tough!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("$n focuses ki into your skin, making it tough!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("$n focuses ki into $N's skin, making it tough!"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					return
				}
			}
		}
	} else if libc.StrCmp(&arg[0], libc.CString("might")) == 0 {
		...
		if name[0] == 0 {
			if AFF_FLAGGED(ch, AFF_MIGHT) {
				...
				return
			} else if (ch.Bonuses[BONUS_WIMP]) > 0 && int(ch.Aff_abils.Str)+10 > 25 {
				send_to_char(ch, libc.CString("Your body is not able to withstand increasing its strength beyond 25.\r\n"))
				return
			} else if (ch.Bonuses[BONUS_FRAIL]) > 0 && int(ch.Aff_abils.Str)+2 > 25 {
				send_to_char(ch, libc.CString("Your body is not able to withstand increasing its strength beyond 25.\r\n"))
				return
			} else if ch.Mana < ch.Max_mana/20 {
				...
				ch.Mana -= ch.Max_mana / 20
				reveal_hiding(ch, 0)
				act(libc.CString("You focus ki into your muscles, but fail in making them mighty!"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("$n focuses ki into $s muscles, but fails in making them mighty!"), 1, ch, nil, nil, TO_ROOM)
				return
			} else {
				...
				reveal_hiding(ch, 0)
				act(libc.CString("You focus ki into your muscles, making them mighty!"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("$n focuses ki into $s muscles, making them mighty!"), 1, ch, nil, nil, TO_ROOM)
				return
			}
		} else {
			if (func() *char_data {
			...
			if !can_kill(ch, vict, nil, 2) {
				...
				if AFF_FLAGGED(vict, AFF_MIGHT) {
					...
					return
				} else if (vict.Bonuses[BONUS_WIMP]) > 0 && int(vict.Aff_abils.Str)+10 > 25 {
					send_to_char(ch, libc.CString("Their body is not able to withstand increasing its strength beyond 25.\r\n"))
					return
				} else if (vict.Bonuses[BONUS_FRAIL]) > 0 && int(vict.Aff_abils.Con)+2 > 25 {
					...
					ch.Mana -= ch.Max_mana / 20
					reveal_hiding(ch, 0)
					act(libc.CString("You focus ki into $N's muscles, but fail in making them mighty!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("$n focuses ki into your muscles, but fails in making them mighty!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("$n focuses ki into $N's muscles, but fails in making them mighty!"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					return
				} else {
					ch.Mana -= ch.Max_mana / 20
					var duration int = roll_aff_duration(int(ch.Aff_abils.Intel), 2)
					...
					reveal_hiding(ch, 0)
					act(libc.CString("You focus ki into $N's muscles, making them mighty!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("$n focuses ki into your muscles, making them mighty!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("$n focuses ki into $N's muscles, making them mighty!"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					return
				}
			}
		}
	} else if libc.StrCmp(&arg[0], libc.CString("wither")) == 0 {
		...
		if AFF_FLAGGED(vict, AFF_WITHER) {
			...
			ch.Mana -= ch.Max_mana / 20
			reveal_hiding(ch, 0)
			act(libc.CString("You focus ki into $N's body, but fail in withering it!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("$n focuses ki into your body, but fails in withering it!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("$n focuses ki into $N's body, but fails in withering it!"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			return
		} else {
			...
			save_char(vict)
			reveal_hiding(ch, 0)
			act(libc.CString("You focus ki into $N's body, and succeed in withering it!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("$n focuses ki into your body, and succeeds in withering it!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("$n focuses ki into $N's body, and succeeds in withering it!"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			return
		}
	} else if libc.StrCmp(&arg[0], libc.CString("enlighten")) == 0 {
		...
		if name[0] == 0 {
			if AFF_FLAGGED(ch, AFF_ENLIGHTEN) {
				...
				return
			} else if (ch.Bonuses[BONUS_FOOLISH]) > 0 && int(ch.Aff_abils.Wis)+10 > 25 {
				send_to_char(ch, libc.CString("You're not able to withstand increasing your wisdom beyond 25.\r\n"))
				return
			} else if ch.Mana < ch.Max_mana/20 {
				...
				ch.Mana -= ch.Max_mana / 20
				reveal_hiding(ch, 0)
				act(libc.CString("You focus ki into your mind, but fail in awakening it to cosmic wisdom!"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("$n focuses ki into $s mind, but fails in awakening it to cosmic wisdom!"), 1, ch, nil, nil, TO_ROOM)
				return
			} else {
				var duration int = roll_aff_duration(int(ch.Aff_abils.Intel), 2)
				...
				ch.Mana -= ch.Max_mana / 20
				reveal_hiding(ch, 0)
				act(libc.CString("You focus ki into your mind, awakening it to cosmic wisdom!"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("$n focuses ki into $s mind, awakening it to cosmic wisdom!"), 1, ch, nil, nil, TO_ROOM)
				if int(ch.Chclass) == CLASS_JINTO && level_exp(ch, GET_LEVEL(ch)+1)-int(ch.Exp) > 0 && (ch.Player_specials.Class_skill_points[ch.Chclass]) >= 15 && rand_number(1, 4) >= 3 {
					var gain int64 = 0
			...
			if !can_kill(ch, vict, nil, 2) {
				...
				if AFF_FLAGGED(vict, AFF_ENLIGHTEN) {
					...
					return
				} else if (vict.Bonuses[BONUS_FOOLISH]) > 0 && int(vict.Aff_abils.Wis)+10 > 25 {
					...
					ch.Mana -= ch.Max_mana / 20
					reveal_hiding(ch, 0)
					act(libc.CString("You focus ki into $N's mind, but fail in awakening it to cosmic wisdom!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("$n focuses ki into your mind, but fails in awakening it to cosmic wisdom!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("$n focuses ki into $N's mind, but fails in awakening it to cosmic wisdom!"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					return
				} else {
					var duration int = roll_aff_duration(int(ch.Aff_abils.Intel), 2)
					...
					ch.Mana -= ch.Max_mana / 20
					reveal_hiding(ch, 0)
					act(libc.CString("You focus ki into $N's mind, awakening it to cosmic wisdom!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("$n focuses ki into your mind, awakening it to cosmic wisdom!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("$n focuses ki into $N's mind, awakening it to cosmic wisdom!"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					if int(ch.Chclass) == CLASS_JINTO && level_exp(vict, GET_LEVEL(vict)+1)-int(vict.Exp) > 0 && (ch.Player_specials.Class_skill_points[ch.Chclass]) >= 15 && rand_number(1, 4) >= 3 {
						var gain int64 = 0
						ch.Player_specials.Class_skill_points[ch.Chclass] -= 15
						if GET_SKILL(ch, SKILL_ENLIGHTEN) >= 100 {
	...
	} else if libc.StrCmp(&arg[0], libc.CString("genius")) == 0 {
		...
		if name[0] == 0 {
			if AFF_FLAGGED(ch, AFF_GENIUS) {
				...
				return
			} else if (ch.Bonuses[BONUS_DULL]) > 0 && int(ch.Aff_abils.Intel)+10 > 25 {
				send_to_char(ch, libc.CString("You're not able to withstand increasing your intelligence beyond 25.\r\n"))
				return
			} else if ch.Mana < ch.Max_mana/20 {
				...
				ch.Mana -= ch.Max_mana / 20
				reveal_hiding(ch, 0)
				act(libc.CString("You focus ki into your mind, but fail in making it work faster!"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("$n focuses ki into $s muscles, but fails in making it work faster!"), 1, ch, nil, nil, TO_ROOM)
				return
			} else {
				var duration int = roll_aff_duration(int(ch.Aff_abils.Intel), 2)
				...
				ch.Mana -= ch.Max_mana / 20
				reveal_hiding(ch, 0)
				act(libc.CString("You focus ki into your mind, making it work faster!"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("$n focuses ki into $s mind, making it work faster!"), 1, ch, nil, nil, TO_ROOM)
				return
			}
		} else {
			if (func() *char_data {
			...
			if !can_kill(ch, vict, nil, 2) {
				...
				if AFF_FLAGGED(vict, AFF_GENIUS) {
					...
					return
				} else if (vict.Bonuses[BONUS_DULL]) > 0 && int(vict.Aff_abils.Intel)+10 > 25 {
					...
					ch.Mana -= ch.Max_mana / 20
					reveal_hiding(ch, 0)
					act(libc.CString("You focus ki into $N's mind, but fail in making it work faster!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("$n focuses ki into your mind, but fails in making it work faster!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("$n focuses ki into $N's mind, but fails in making it work faster!"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					return
				} else {
					var duration int = roll_aff_duration(int(ch.Aff_abils.Intel), 2)
					...
					ch.Mana -= ch.Max_mana / 20
					reveal_hiding(ch, 0)
					act(libc.CString("You focus ki into $N's mind, making it work faster!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("$n focuses ki into your mind, making it work faster!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("$n focuses ki into $N's mind, making it work faster!"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					if (vict.Master == ch || ch.Master == vict || ch.Master == vict.Master) && AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP) {
						if int(ch.Race) == RACE_KAI && level_exp(ch, GET_LEVEL(ch)+1)-int(ch.Exp) > 0 && rand_number(1, 3) == 3 {
	...
	} else if libc.StrCmp(&arg[0], libc.CString("flex")) == 0 {
		...
		if name[0] == 0 {
			if AFF_FLAGGED(ch, AFF_FLEX) {
				...
				return
			} else if (ch.Bonuses[BONUS_CLUMSY]) > 0 && int(ch.Aff_abils.Dex)+10 > 25 {
				send_to_char(ch, libc.CString("You're not able to withstand increasing your agility beyond 25.\r\n"))
				return
			} else if ch.Mana < ch.Max_mana/20 {
				...
				ch.Mana -= ch.Max_mana / 20
				reveal_hiding(ch, 0)
				act(libc.CString("You focus ki into your limbs, but fail in making them more flexible!"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("$n focuses ki into $s muscles, but fails in making them more flexible!"), 1, ch, nil, nil, TO_ROOM)
				return
			} else {
				ch.Mana -= ch.Max_mana / 20
				var duration int = roll_aff_duration(int(ch.Aff_abils.Intel), 2)
				...
				reveal_hiding(ch, 0)
				act(libc.CString("You focus ki into your limbs, making them more flexible!"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("$n focuses ki into $s limbs, making them more flexible!"), 1, ch, nil, nil, TO_ROOM)
				return
			}
		} else {
			if (func() *char_data {
			...
			if !can_kill(ch, vict, nil, 2) {
				...
				if AFF_FLAGGED(vict, AFF_FLEX) {
					...
					return
				} else if (vict.Bonuses[BONUS_CLUMSY]) > 0 && int(vict.Aff_abils.Dex)+3 > 25 {
					...
					ch.Mana -= ch.Max_mana / 20
					reveal_hiding(ch, 0)
					act(libc.CString("You focus ki into $N's limbs, but fail in making them more flexible!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("$n focuses ki into your limbs, but fails in making them more flexible!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("$n focuses ki into $N's limbs, but fails in making them more flexible!"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					return
				} else {
					ch.Mana -= ch.Max_mana / 20
					var duration int = roll_aff_duration(int(ch.Aff_abils.Intel), 2)
					...
					reveal_hiding(ch, 0)
					act(libc.CString("You focus ki into $N's limbs, making them more flexible!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("$n focuses ki into your limbs, making them more flexible!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("$n focuses ki into $N's limbs, making them more flexible!"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					if (vict.Master == ch || ch.Master == vict || ch.Master == vict.Master) && AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP) {
						if int(ch.Race) == RACE_KAI && level_exp(ch, GET_LEVEL(ch)+1)-int(ch.Exp) > 0 && rand_number(1, 3) == 3 {
							ch.Exp += int64(float64(level_exp(ch, GET_LEVEL(ch)+1)) * 0.05)
						}
	...
	} else if libc.StrCmp(&arg[0], libc.CString("bless")) == 0 {
		...
		if name[0] == 0 {
			if AFF_FLAGGED(ch, AFF_BLESS) {
				...
				ch.Mana -= ch.Max_mana / 20
				reveal_hiding(ch, 0)
				act(libc.CString("You focus ki while chanting spiritual words. Your blessing does nothing though, you must have messed up!"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("$n focuses ki while chanting spiritual words. $n seems disappointed."), 1, ch, nil, nil, TO_ROOM)
				return
			} else {
				...
				act(libc.CString("You focus ki while chanting spiritual words. You feel your body recovering at above normal speed!"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("$n focuses ki while chanting spiritual words. $n smiles after finishing $s chant."), 1, ch, nil, nil, TO_ROOM)
				if AFF_FLAGGED(ch, AFF_CURSE) {
			...
			if !can_kill(ch, vict, nil, 2) {
				...
				if AFF_FLAGGED(vict, AFF_BLESS) {
					...
					ch.Mana -= ch.Max_mana / 20
					reveal_hiding(ch, 0)
					act(libc.CString("You focus ki while chanting spiritual words. Your blessing fails!"), 1, ch, nil, nil, TO_CHAR)
					act(libc.CString("$n focuses ki while chanting spiritual words. $n places a hand on your head, but nothing happens!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("$n focuses ki while chanting spiritual words. $n places a hand on $N's head, but nothing happens!"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					return
				} else {
					var duration int = roll_aff_duration(int(ch.Aff_abils.Intel), 3)
					...
					act(libc.CString("You focus ki while chanting spiritual words. Blessing $N with faster regeneration!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("$n focuses ki while chanting spiritual words. $n then places a hand on your head, blessing you!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("$n focuses ki while chanting spiritual words. $n then places a hand on $N's head, blessing them!"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					if (vict.Master == ch || ch.Master == vict || ch.Master == vict.Master) && AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP) {
						if int(ch.Race) == RACE_KAI && level_exp(ch, GET_LEVEL(ch)+1)-int(ch.Exp) > 0 && rand_number(1, 3) == 3 {
							ch.Exp += int64(float64(level_exp(ch, GET_LEVEL(ch)+1)) * 0.05)
						}
	...
	} else if libc.StrCmp(&arg[0], libc.CString("curse")) == 0 {
		...
		if name[0] == 0 {
			if AFF_FLAGGED(ch, AFF_CURSE) {
				...
				return
			} else if int(ch.Race) == RACE_DEMON {
				...
				ch.Mana -= ch.Max_mana / 20
				reveal_hiding(ch, 0)
				act(libc.CString("You focus ki while chanting demonic words. Your cursing does nothing though, you must have messed up!"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("$n focuses ki while chanting demonic words. $n seems disappointed."), 1, ch, nil, nil, TO_ROOM)
				return
			} else {
				var duration int = roll_aff_duration(int(ch.Aff_abils.Intel), 3)
				...
				ch.Mana -= ch.Max_mana / 20
				reveal_hiding(ch, 0)
				act(libc.CString("You focus ki while chanting demonic words. You feel your body recovering at below normal speed!"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("$n focuses ki while chanting demonic words. $n grins after finishing $s chant."), 1, ch, nil, nil, TO_ROOM)
				if AFF_FLAGGED(ch, AFF_BLESS) {
			...
			if !can_kill(ch, vict, nil, 0) {
				...
				if AFF_FLAGGED(vict, AFF_CURSE) {
					...
					send_to_char(ch, libc.CString("Whatever would you waste your ki on them for?\r\n"))
					return
				} else if int(vict.Race) == RACE_DEMON {
					...
					ch.Mana -= ch.Max_mana / 20
					reveal_hiding(ch, 0)
					act(libc.CString("You focus ki while chanting demonic words. Your cursing fails!"), 1, ch, nil, nil, TO_CHAR)
					act(libc.CString("$n focuses ki while chanting demonic words. $n places a hand on your head, but nothing happens!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("$n focuses ki while chanting demonic words. $n places a hand on $N's head, but nothing happens!"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					return
				} else {
					var duration int = roll_aff_duration(int(ch.Aff_abils.Intel), 3)
					assign_affect(vict, AFF_CURSE, SKILL_CURSE, duration, 0, 0, 0, 0, 0, 0)
					ch.Mana -= ch.Max_mana / 20
					reveal_hiding(ch, 0)
					act(libc.CString("You focus ki while chanting demonic words. cursing $N with slower regeneration!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("$n focuses ki while chanting demonic words. $n then places a hand on your head, cursing you!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("$n focuses ki while chanting demonic words. $n then places a hand on $N's head, cursing them!"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					if AFF_FLAGGED(vict, AFF_BLESS) {
	...
	} else if libc.StrCmp(&arg[0], libc.CString("yoikominminken")) == 0 || libc.StrCmp(&arg[0], libc.CString("yoik")) == 0 {
		...
		if !can_kill(ch, vict, nil, 0) {
			...
			if AFF_FLAGGED(vict, AFF_SLEEP) {
				...
				ch.Mana -= ch.Max_mana / 20
				reveal_hiding(ch, 0)
				act(libc.CString("You focus ki while moving your hands in lulling patterns, but $N doesn't look the least bit sleepy!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("$n focuses ki while moving $s hands in a lulling pattern, but you just don't feel tired."), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("$n focuses ki while moving $s hands in a lulling pattern, but $N doesn't look the least bit sleepy!"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				return
				...
				ch.Mana -= ch.Max_mana / 20
				reveal_hiding(ch, 0)
				act(libc.CString("You focus ki while moving your hands in lulling patterns, but fail to put $N to sleep!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("$n focuses ki while moving $s hands in a lulling pattern, but you resist the technique!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("$n focuses ki while moving $s hands in a lulling pattern, but $N resists the technique!"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				return
			} else {
				...
				ch.Mana -= ch.Max_mana / 20
				reveal_hiding(ch, 0)
				act(libc.CString("You focus ki while moving your hands in lulling patterns, putting $N to sleep!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("$n focuses ki while moving $s hands in a lulling pattern, before you realise it you are asleep!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("$n focuses ki while moving $s hands in a lulling pattern, putting $N to sleep!"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				vict.Position = POS_SLEEPING
				if AFF_FLAGGED(vict, AFF_FLYING) {
	...
	} else if libc.StrCmp(&arg[0], libc.CString("vigor")) == 0 {
		...
		if name[0] == 0 {
			if ch.Mana < ch.Max_mana/10 {
				...
				reveal_hiding(ch, 0)
				act(libc.CString("You focus ki into your very cells, but fail at re-engerizing them!"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("$n focuses ki and glows green for a moment, $e then frowns."), 1, ch, nil, nil, TO_ROOM)
				WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
				return
			} else if ch.Move >= ch.Max_move {
				...
				reveal_hiding(ch, 0)
				act(libc.CString("You focus ki into your very cells, and manage to re-energize them!"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("$n focuses ki and glows green for a moment, $e then smiles."), 1, ch, nil, nil, TO_ROOM)
				WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
				return
			}
		} else {
			...
			if !can_kill(ch, vict, nil, 2) {
				...
				if IS_NPC(vict) {
					...
					ch.Mana -= ch.Max_mana / 10
					reveal_hiding(ch, 0)
					act(libc.CString("You focus ki into $N's very cells, and fail at re-energizing them!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("$n focuses ki into your very cells, but nothing happens!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("$n focuses ki and $N glows green for a moment, $N frowns."), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
					return
				} else {
					if (ch.Bonuses[BONUS_HEALER]) > 0 {
					...
					reveal_hiding(ch, 0)
					act(libc.CString("You focus ki into $N's very cells, and manage to re-energize them!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("$n focuses ki into your very cells, and manages to re-energize them!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("$n focuses ki and $N glows green for a moment, $N smiles."), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
					return
				}
			}
		}
	} else if libc.StrCmp(&arg[0], libc.CString("cure")) == 0 {
		...
		if name[0] == 0 {
			if !AFF_FLAGGED(ch, AFF_POISON) {
				...
				ch.Mana -= ch.Max_mana / 20
				reveal_hiding(ch, 0)
				act(libc.CString("You focus ki and aim a pulsing light at your body. Nothing happens!"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("$n focuses ki and aims a pulsing light at $s body. Nothing seems to happen."), 1, ch, nil, nil, TO_ROOM)
				return
			} else {
				affect_from_char(ch, SPELL_POISON)
				ch.Mana -= ch.Max_mana / 20
				reveal_hiding(ch, 0)
				act(libc.CString("You focus ki and aim a pulsing light at your body. You feel the poison in your blood disappear!"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("$n focuses ki and aims a pulsing light at $s body. $n smiles."), 1, ch, nil, nil, TO_ROOM)
			...
			if !can_kill(ch, vict, nil, 2) {
				...
				if !AFF_FLAGGED(vict, AFF_POISON) {
					...
					ch.Mana -= ch.Max_mana / 20
					reveal_hiding(ch, 0)
					act(libc.CString("You focus ki and aim a pulsing light at $N's body. Nothing happens."), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("$n focuses ki and aims a pulsing light at your body. You are STILL poisoned!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("$n focuses ki and aims a pulsing light at $N's body. $N looks disappointed."), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					return
				} else {
					affect_from_char(vict, SPELL_POISON)
					ch.Mana -= ch.Max_mana / 20
					reveal_hiding(ch, 0)
					act(libc.CString("You focus ki and aim a pulsing light at $N's body. $e is cured."), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("$n focuses ki and aims a pulsing light at your body. You have been cured of your poison!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("$n focuses ki and aims a pulsing light at $N's body. $N smiles."), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					null_affect(vict, AFF_POISON)
					return
				}
			}
		}
	} else if libc.StrCmp(&arg[0], libc.CString("poison")) == 0 {
		...
		if !can_kill(ch, vict, nil, 0) {
			...
			if AFF_FLAGGED(vict, AFF_POISON) {
				...
				ch.Mana -= ch.Max_mana / 20
				reveal_hiding(ch, 0)
				act(libc.CString("You focus ki and fling poison at $N. You missed!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("$n focuses ki and flings poison at you, but misses!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("$n focuses ki and flings poison at $N, but misses!"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				return
			} else {
				ch.Mana -= ch.Max_mana / 20
				reveal_hiding(ch, 0)
				act(libc.CString("You focus ki and fling poison at $N! The poison burns into $s skin!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("$n focuses ki and flings poison at you! The poison burns into your skin!"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("$n focuses ki and flings poison at $N! The poison burns into $s skin!"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				if IS_NPC(vict) {
					set_fighting(vict, ch)
				}
				if int(vict.Race) == RACE_MUTANT && ((vict.Genome[0]) == 7 || (vict.Genome[1]) == 7) {
					act(libc.CString("However $N seems unaffected by the poison."), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("Your natural immunity to poison prevents it from affecting you."), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("However $N seems unaffected by the poison."), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				} else {
					vict.Poisonby = ch
					if ch.Charge > 0 {
...
func do_kaioken(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if ch.Kaioken > 0 {
		...
		act(libc.CString("$n@w drops out of kaioken.@n"), 1, ch, nil, nil, TO_ROOM)
	...
	if !IS_NPC(ch) {
		if (IS_TRANSFORMED(ch) || int(ch.Race) == RACE_HOSHIJIN && ch.Starphase > 0) && x > 5 {
	...
	if init_skill(ch, SKILL_KAIOKEN) < roll {
		...
		act(libc.CString("$n tries to use kaioken but messes up somehow."), 1, ch, nil, nil, TO_ROOM)
	...
	case 3:
		if ch.Max_hit < 5000 {
			act(libc.CString("@rA blazing red aura bursts up around your body, flashing intensely before your body gives out and you release the kaioken because of the pressure!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@rA blazing red aura bursts up around @R$n's @rbody, flashing intensely before $s body gives out and $e releases the kaioken because of the pressure!@n"), 1, ch, nil, nil, TO_ROOM)
		} else {
			pass = 1
		}
	case 4:
	...
	act(libc.CString("@rA dark red aura bursts up around @R$n@r as they achieve a level of Kaioken!@n"), 1, ch, nil, nil, TO_ROOM)
	improve_skill(ch, SKILL_KAIOKEN, 1)
...
func do_plant(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if int(ch.Feats[FEAT_DEFT_HANDS]) != 0 {
		roll += 2
	}
	if int(vict.Position) < POS_SLEEPING {
		detect = 0
	...
	if roll <= detect && roll <= fail {
		reveal_hiding(ch, 0)
		act(libc.CString("@C$n@w tries to plant $p@w on you!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("@C$n@w tries to plant $p@w on @c$N@w!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_NOTVICT)
		act(libc.CString("@wYou try and fail to plant $p@w on @c$N@w, and $E notices!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_CHAR)
		WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
		return
	} else if roll <= fail {
		act(libc.CString("@wYou try and fail to plant $p@w on @c$N@w! However no one seemed to notice.@n"), 1, ch, obj, unsafe.Pointer(vict), TO_CHAR)
		WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
		return
		...
		act(libc.CString("@C$n@w tries to plant $p@w on you!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("@C$n@w tries to plant $p@w on @c$N@w!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_NOTVICT)
		act(libc.CString("@wYou try and fail to plant $p@w on @c$N@w because $E can't carry the weight. It seems $E noticed the attempt!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_CHAR)
		WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
	} else if roll <= detect {
		act(libc.CString("@cYou feel like the weight of your inventory has changed.@n"), 1, ch, obj, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("@c$N@w looks around after feeling $S pockets.@n"), 1, ch, obj, unsafe.Pointer(vict), TO_NOTVICT)
		act(libc.CString("@wYou plant $p@w on @c$N@w! @c$N @wseems to notice the change in weight in their inventory.@n"), 1, ch, obj, unsafe.Pointer(vict), TO_CHAR)
		obj_from_char(obj)
		obj_to_char(obj, vict)
		WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
		return
	} else {
		act(libc.CString("@wYou plant $p@w on @c$N@w! No one noticed, whew....@n"), 1, ch, obj, unsafe.Pointer(vict), TO_CHAR)
		obj_from_char(obj)
		obj_to_char(obj, vict)
		WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
		return
...
func do_forgery(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	for obj = ch.Carrying; obj != nil; obj = next_obj {
		next_obj = obj.Next_content
		if found == 0 && GET_OBJ_VNUM(obj) == 19 && (!OBJ_FLAGGED(obj, ITEM_BROKEN) && !OBJ_FLAGGED(obj, ITEM_FORGED)) {
			found = 1
			obj4 = obj
		}
	}
	...
	reveal_hiding(ch, 0)
	act(libc.CString("@c$n@w looks at $p, begins to work on forging a fake copy of it.@n"), 1, ch, obj2, nil, TO_ROOM)
	...
	if GET_SKILL(ch, SKILL_FORGERY) < axion_dice(0) {
		...
		act(libc.CString("@c$n@w tried to duplicate $p but screws up somehow.@n"), 1, ch, obj2, nil, TO_ROOM)
		obj_from_char(obj4)
		extract_obj(obj4)
		WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
		return
	}
	var loadn int = GET_OBJ_VNUM(obj2)
	...
	act(libc.CString("@c$n@w makes a perfect forgery of $p.@n"), 1, ch, obj2, nil, TO_ROOM)
	WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
}
func do_appraise(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	reveal_hiding(ch, 0)
	act(libc.CString("@c$n@w looks at $p, turning it over in $s hands.@n"), 1, ch, obj, nil, TO_ROOM)
	...
	if GET_SKILL(ch, SKILL_APPRAISE) < axion_dice(-10) {
		...
		act(libc.CString("@c$n@w looks stumped about $p.@n"), 1, ch, obj, nil, TO_ROOM)
		WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
		return
	}
	if OBJ_FLAGGED(obj, ITEM_BROKEN) {
		send_to_char(ch, libc.CString("%s is broken!\r\n"), obj.Short_description)
		act(libc.CString("@c$n@w looks at $p and frowns.@n"), 1, ch, obj, nil, TO_ROOM)
		WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
		return
	}
	if OBJ_FLAGGED(obj, ITEM_FORGED) {
		send_to_char(ch, libc.CString("%s is fake and worthless!\r\n"), obj.Short_description)
		act(libc.CString("@c$n@w looks at $p with an angry face.@n"), 1, ch, obj, nil, TO_ROOM)
		WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
		return
	}
	found = 0
	...
	act(libc.CString("@c$n@w looks at $p and nods, a satisfied look on $s face.@n"), 1, ch, obj, nil, TO_ROOM)
	var percent int = 0
...
func do_disguise(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if PLR_FLAGGED(ch, PLR_DISGUISED) {
		...
		act(libc.CString("@C$n @wpulls off $s disguise and reveals $mself!"), 1, ch, nil, nil, TO_ROOM)
		return
	}
	...
	if skill > roll {
		...
		act(libc.CString("@C$n @wmanages to disguise $mself with some skilled manipulation of $s headwear."), 1, ch, nil, nil, TO_ROOM)
		...
		act(libc.CString("@C$n @wattempts and fails to disguise $mself properly and must try again."), 1, ch, nil, nil, TO_ROOM)
		if ch.Move >= ch.Move/50 {
			ch.Move -= ch.Move / 50
		} else {
			ch.Move = 0
...
func do_eavesdrop(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if (func() int {
		dir = search_block(&buf[0], &dirs[0], 0)
		return dir
	}()) < 0 {
	...
	if (world[ch.In_room].Dir_option[dir]) != nil {
		if IS_SET((world[ch.In_room].Dir_option[dir]).Exit_info, 1<<1) && (world[ch.In_room].Dir_option[dir]).Keyword != nil {
			...
			ch.Listenroom = int(libc.BoolToInt(GET_ROOM_VNUM((world[ch.In_room].Dir_option[dir]).To_room)))
			ch.Eavesdir = dir
...
func do_zanzoken(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	act(libc.CString("@wYou focus your ki, preparing to move at super speeds if necessary.@n"), 1, ch, nil, nil, TO_CHAR)
	ch.Mana -= cost
...
func do_block(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if arg[0] == 0 {
		...
		if ch.Blocks != nil {
			act(libc.CString("@wYou stop blocking @c$N@w.@n"), 1, ch, nil, unsafe.Pointer(ch.Blocks), TO_CHAR)
			act(libc.CString("@C$n@w stops blocking you.@n"), 1, ch, nil, unsafe.Pointer(ch.Blocks), TO_VICT)
			act(libc.CString("@C$n@w stops blocking @c$N@w.@n"), 1, ch, nil, unsafe.Pointer(ch.Blocks), TO_NOTVICT)
			vict = ch.Blocks
			vict.Blocked = nil
			ch.Blocks = nil
			return
	...
	if ch.Blocks != nil {
		...
		vict.Blocked = ch
		reveal_hiding(ch, 0)
		act(libc.CString("@wYou start blocking @c$N's@w escape.@n"), 1, ch, nil, unsafe.Pointer(ch.Blocks), TO_CHAR)
		act(libc.CString("@C$n@w starts blocking your escape.@n"), 1, ch, nil, unsafe.Pointer(ch.Blocks), TO_VICT)
		act(libc.CString("@C$n@w starts blocking @c$N's@w escape.@n"), 1, ch, nil, unsafe.Pointer(ch.Blocks), TO_NOTVICT)
		return
	} else {
		ch.Blocks = vict
		vict.Blocked = ch
...
func do_eyec(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if PLR_FLAGGED(ch, PLR_EYEC) {
		...
		act(libc.CString("@wYou open your eyes.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@C$n@w opens $s eyes.@n"), 1, ch, nil, nil, TO_ROOM)
	} else if !PLR_FLAGGED(ch, PLR_EYEC) {
		SET_BIT_AR(ch.Act[:], PLR_EYEC)
		act(libc.CString("@wYou close your eyes.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@C$n@w closes $s eyes.@n"), 1, ch, nil, nil, TO_ROOM)
	}
	WAIT_STATE(ch, (int(1000000/OPT_USEC))*1)
}
func do_solar(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	prob += bonus
	if prob < perc {
		act(libc.CString("@WYou raise both your hands to either side of your face, while closing your eyes, and shout '@YSolar Flare@W' but nothing happens!@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@C$n@W raises both $s hands to either side of $s face, while closing $s eyes, and shouts '@YSolar Flare@W' but nothing happens!@n"), 1, ch, nil, nil, TO_ROOM)
		ch.Mana -= int64(cost)
		WAIT_STATE(ch, (int(1000000/OPT_USEC))*3)
		improve_skill(ch, SKILL_SOLARF, 0)
		return
	}
	act(libc.CString("@WYou raise both your hands to either side of your face, while closing your eyes, and shout '@YSolar Flare@W' as a blinding light fills the area!@n"), 1, ch, nil, nil, TO_CHAR)
	act(libc.CString("@C$n@W raises both $s hands to either side of $s face, while closing $s eyes, and shouts '@YSolar Flare@W' as a blinding light fills the area!@n"), 1, ch, nil, nil, TO_ROOM)
	for vict = world[ch.In_room].People; vict != nil; vict = next_v {
		...
		if vict == ch {
			...
			act(libc.CString("@W$N@W is @YBLINDED@W!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@RYou are @YBLINDED@R!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@W$N@W is @YBLINDED@W!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		}
	}
	improve_skill(ch, SKILL_SOLARF, 0)
	ch.Mana -= int64(cost)
...
func do_heal(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if heal < gear_pl(vict) {
		heal += (heal / 100) * int64(int(ch.Aff_abils.Wis)/4)
	}
	if ch.Mana < cost {
		send_to_char(ch, libc.CString("You do not have enough ki.\r\n"))
		return
	...
	bonus = int64((int(ch.Aff_abils.Intel) / 2) + int(ch.Aff_abils.Wis)/3)
	prob += bonus
	if prob < perc {
		if vict != ch {
			act(libc.CString("@WYou place your hands near @c$N@W, but fail to concentrate enough to heal them!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@C$n@W places $s hands near you, but nothing happens!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@C$n@W places $s hands near @c$N@W, but nothing happens."), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			ch.Mana -= cost
		...
		if vict == ch {
			act(libc.CString("@WYou place your hands on your body, but fail to concentrate to heal yourself!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@C$n@W places $s hands on $s body, but nothing happens."), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			ch.Mana -= cost
			improve_skill(ch, SKILL_HEAL, 0)
			WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
			return
	...
	if vict != ch {
		...
		act(libc.CString("@WYou place your hands near @c$N@W and an orange glow surrounds $M!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@C$n@W places $s hands near you and an orange glow surrounds you!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("@C$n@W places $s hands near @c$N@W and an orange glow surrounds $M."), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		ch.Mana -= cost
		...
		if AFF_FLAGGED(vict, AFF_BURNED) {
			...
			act(libc.CString("$n@w's burns are now healed.@n"), 1, vict, nil, nil, TO_ROOM)
		...
		if AFF_FLAGGED(vict, AFF_HYDROZAP) {
			...
			act(libc.CString("$n@w no longer looks as if they could drink an ocean.@n"), 1, vict, nil, nil, TO_ROOM)
		...
		if vict.Master == ch || ch.Master == vict || ch.Master == vict.Master {
			if int(ch.Chclass) == CLASS_NAIL && int(ch.Race) == RACE_NAMEK && level_exp(ch, GET_LEVEL(ch)+1)-int(ch.Exp) > 0 && float64(vict.Hit) <= float64(gear_pl(vict))*0.85 && rand_number(1, 3) == 3 {
	...
	if vict == ch {
		...
		act(libc.CString("@WYou place your hands on your body and an orange glow surrounds you!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@C$n@W places $s hands on $s body and an orange glow surrounds $m."), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		ch.Mana -= cost
		if vict.Hit+heal <= gear_pl(vict) {
			vict.Hit += heal
		} else {
...
func do_barrier(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if AFF_FLAGGED(ch, AFF_SANCTUARY) && libc.StrCaseCmp(libc.CString("release"), &arg[0]) == 0 {
		act(libc.CString("@BYou dispel your barrier, releasing its energy.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@B$n@B dispels $s barrier, releasing its energy.@n"), 1, ch, nil, nil, TO_ROOM)
		ch.Barrier = 0
	...
	if size > prob {
		...
		act(libc.CString("@BYou shout as you form a barrier of ki around your body, but you imbalance it and it explodes outward!@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@B$n@B shouts as $e forms a barrier of ki around $s body, but it becomes imbalanced and explodes outward!@n"), 1, ch, nil, nil, TO_ROOM)
		ch.Charge -= cost
		if GET_SKILL(ch, SKILL_BARRIER) != 0 {
		...
		if GET_SKILL(ch, SKILL_BARRIER) != 0 {
			act(libc.CString("@BYou shout as you form a barrier of ki around your body!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@B$n@B shouts as $e forms a barrier of ki around $s body!@n"), 1, ch, nil, nil, TO_ROOM)
		} else {
			act(libc.CString("@BYou shout as you form a barrier of ki and raging waters around your body!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@B$n@B shouts as $e forms a barrier of ki and raging waters around $s body!@n"), 1, ch, nil, nil, TO_ROOM)
		}
...
func do_instant(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if tar != nil {
		if tar == ch {
			...
			return
		} else if int(tar.Race) == RACE_ANDROID || float64(tar.Hit) < (float64(ch.Hit)*0.001)+1 {
		...
		ch.Mana -= cost
		act(libc.CString("@wPlacing two fingers on your forehead you close your eyes and concentrate. Accelerating to such a speed that you move through the molecules of the universe faster than the speed of light. You stop as you arrive at $N@w!@n"), 1, ch, nil, unsafe.Pointer(tar), TO_CHAR)
		act(libc.CString("@w$n@w appears in an instant out of nowhere right next to you!@n"), 1, ch, nil, unsafe.Pointer(tar), TO_VICT)
		act(libc.CString("@w$n@w places two fingers on $s forehead and disappears in an instant!@n"), 1, ch, nil, unsafe.Pointer(tar), TO_NOTVICT)
		SET_BIT_AR(ch.Act[:], PLR_TRANSMISSION)
		handle_teleport(ch, tar, 0)
		improve_skill(ch, skill_num, 2)
	} else {
		ch.Mana -= cost
		act(libc.CString("@wPlacing two fingers on your forehead you close your eyes and concentrate. Accelerating to such a speed that you move faster than light and arrive almost instantly at your destination. Having located the planet by its collective population's ki.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@w$n@w places two fingers on $s forehead and disappears in an instant!@n"), 1, ch, nil, nil, TO_NOTVICT)
...
func wishSYS() {
	if SHENRON == 1 {
		if SELFISHMETER < 10 {
			switch DRAGONC {
			case 300:
				send_to_room(real_room(DRAGONR), libc.CString("@WThe dragon balls on the ground begin to glow yellow in slow pulses.@n\r\n"))
				send_to_planet(0, ROOM_EARTH, libc.CString("@DThe sky begins to grow dark and cloudy suddenly.@n\r\n"))
				DRAGONC -= 1
			case 295:
				send_to_room(real_room(DRAGONR), libc.CString("@WSuddenly lightning shoots into the sky, twisting about as a roar can be heard for miles!@n\r\n"))
				send_to_planet(0, ROOM_EARTH, libc.CString("@DThe sky flashes with lightning.@n\r\n"))
				DRAGONC -= 1
			case 290:
				send_to_room(real_room(DRAGONR), libc.CString("@WThe lightning takes shape and slowly the Eternal Dragon, Shenron, can be made out from the glow!@n\r\n"))
			...
			case 285:
				...
				DRAGONC -= 1
			case 280:
				send_to_room(real_room(DRAGONR), libc.CString("@WThe glow around Shenron becomes subdued as the Eternal Dragon coils so that his head is looking down on the dragon balls!@n\r\n"))
				DRAGONC -= 1
			case 275:
				send_to_room(real_room(DRAGONR), libc.CString("@wShenron says, '@CWho summoned me? I will grant you any two wishes that are within my power.@w'@n\r\n"))
				DRAGONC -= 1
			case 180:
				send_to_room(real_room(DRAGONR), libc.CString("@wShenron says, '@CMake your wish already, you only have 3 minutes remaining.@w'@n\r\n"))
				DRAGONC -= 1
			case 120:
				send_to_room(real_room(DRAGONR), libc.CString("@wShenron says, '@CMake your wish. I am losing patience, you only have 2 minutes left.@w'@n\r\n"))
				DRAGONC -= 1
			case 60:
				send_to_room(real_room(DRAGONR), libc.CString("@wShenron says, '@CMake your wish now! You only have 1 minute left.@w'@n\r\n"))
				DRAGONC -= 1
			case 0:
				send_to_room(real_room(DRAGONR), libc.CString("Shenron growls and disappears with a blinding flash that is absorbed into the dragon balls. The glowing dragon balls then float high into the sky, splitting into several directions and streaking across the sky!@n\r\n"))
			...
			send_to_room(real_room(DRAGONR), libc.CString("@RThe dragon balls suddenly begin to crack and darkness begins to pour out through the cracks! Shenron begins to turn pitch black slowly as the darkness escapes. Suddenly Shenron explodes out into the distance in seven parts. Each part taking a dragon ball with it!@n\r\n"))
...
func do_summon(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if summoned == 1 {
		reveal_hiding(ch, 0)
		act(libc.CString("@WYou place the dragon balls on the ground and with both hands outstretched towards them you say '@CArise Eternal Dragon Shenron!@W'@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@W$n places the dragon balls on the ground and with both hands outstretched towards them $e says '@CArise Eternal Dragon Shenron!@W'@n"), 1, ch, nil, nil, TO_ROOM)
		SHENRON = 1
		DRAGONC = 300
		DRAGONR = int(libc.BoolToInt(GET_ROOM_VNUM(ch.In_room)))
		if real_room(DRAGONR) == int(-1) {
			DRAGONR = int(libc.BoolToInt(GET_ROOM_VNUM(ch.In_room)))
		}
...
func do_transform(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if arg[0] == 0 {
		if int(ch.Race) == RACE_HUMAN {
			...
			send_to_char(ch, libc.CString("@b------------------------------------------------@n\r\n"))
		} else if int(ch.Race) == RACE_SAIYAN && !PLR_FLAGGED(ch, PLR_LSSJ) {
			...
			send_to_char(ch, libc.CString("@b------------------------------------------------@n\r\n"))
		} else if int(ch.Race) == RACE_KONATSU {
		...
		return
	} else if libc.StrCaseCmp(libc.CString("first"), &arg[0]) == 0 {
		if int(ch.Race) == RACE_HUMAN {
		...
		if int(ch.Race) == RACE_TRUFFLE {
			...
			if ch.Basepl < int64(trans_req(ch, 1)) {
				...
				var zone int = 0
				act(libc.CString("@WYour mind accelerates working through the mysteries of the universe while at the same time your body begins to change! Innate nano-technology within your body begins to activate, forming flexible metal plating across parts of your skin!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n@W begins to write complicated calculations in the air as though $e were possessed while at the same time $s body begins to change! Innate nano-technology within $s body begins to activate, forming flexible metal plating across parts of $s skin!@n"), 1, ch, nil, nil, TO_ROOM)
				if (func() int {
					zone = real_zone_by_thing(ch.In_room)
					return zone
				}()) != int(-1) {
		...
		if int(ch.Race) == RACE_KAI {
			...
			if ch.Basepl < int64(trans_req(ch, 1)) {
				...
				var zone int = 0
				act(libc.CString("@WThoughts begin to flow through your mind of events throughout your life. The progression leads up to more recent events and finally to this very moment. All of it's significance overwhelms you momentarily and your motivation and drive increase. As your attention is drawn back to your surroundings, you feel as though your thinking, senses, and reflexes have sharpened dramatically.  At the core of your being, a greater depth of power can be felt.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@W$n@W's face tenses, it becoming clear momentarily that they are deep in thought. After a brief lapse in focus, their attention seems to return to their surroundings. Though it's not apparent why they were so distracted, something definitely seems different about $m.@n"), 1, ch, nil, nil, TO_ROOM)
				if (func() int {
					zone = real_zone_by_thing(ch.In_room)
					return zone
				}()) != int(-1) {
		...
		if int(ch.Race) == RACE_KONATSU {
			...
			if ch.Basepl < int64(trans_req(ch, 1)) {
				...
				var zone int = 0
				act(libc.CString("@WA dark shadowy aura with flecks of white energy begins to burn around your body! Strength and agility can be felt rising up within as your form becomes blurred and ethereal looking. You smile as you realize your @GFirst @DShadow @BForm@W!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@WA dark shadowy aura with flecks of white energy begins to burn around @C$n@W's body! $s form becomes blurred and ethereal-looking as $s muscles become strong and lithe. $e smiles as $e achieves $s @GFirst @DShadow @BForm@W!@n"), 1, ch, nil, nil, TO_ROOM)
				if (func() int {
					zone = real_zone_by_thing(ch.In_room)
					return zone
				}()) != int(-1) {
		...
		if int(ch.Race) == RACE_ICER {
			...
			if ch.Basepl < int64(trans_req(ch, 1)) {
				...
				var zone int = 0
				act(libc.CString("@WYou yell with pain as your body begins to grow and power surges within! Your legs expand outward to triple their previous length. Soon after your arms, chest, and head follow. Your horns grow longer and curve upwards while lastly your tail expands. You are left confidently standing, having completed your @GFirst @cTransformation@W.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n@W yells with pain as $s body begins to grow and power surges outward! $s legs expand outward to triple their previous length. Soon after $s arms, chest, and head follow. $s horns grow longer and curve upwards while lastly $s tail expands. $e is left confidently standing, having completed $s @GFirst @cTransformation@W.@n"), 1, ch, nil, nil, TO_ROOM)
				if (func() int {
					zone = real_zone_by_thing(ch.In_room)
					return zone
				}()) != int(-1) {
		...
		if int(ch.Race) == RACE_MUTANT {
			...
			if ch.Basepl < int64(trans_req(ch, 1)) {
				...
				var zone int = 0
				act(libc.CString("@WYour flesh grows tougher as power surges up from within. Your fingernails grow longer, sharper, and more claw-like. Lastly your muscles double in size as you achieve your @GFirst @mMutation@W!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n@W flesh grows tougher as power surges up around $m. $s fingernails grow longer, sharper, and more claw-like. Lastly $s muscles double in size as $e achieves $s @GFirst @mMutation@W!@n"), 1, ch, nil, nil, TO_ROOM)
				if (func() int {
					zone = real_zone_by_thing(ch.In_room)
					return zone
				}()) != int(-1) {
		...
		if int(ch.Race) == RACE_HALFBREED {
			...
			if ch.Basepl < int64(trans_req(ch, 1)) {
				...
				var zone int = 0
				act(libc.CString("@WSomething inside your mind snaps as your rage spills over! Lightning begins to strike the ground all around you as you feel torrents of power rushing through every fiber of your being. Your hair suddenly turns golden as your eyes change to the color of emeralds. In a final rush of power a golden aura rushes up around your body! You have become a @CSuper @YSaiyan@W!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n@W screams in rage as lightning begins to crash all around! $s hair turns golden and $s eyes change to an emerald color as a bright golden aura bursts up around $s body! As $s energy stabilizes $e wears a fierce look upon $s face, having transformed into a @CSuper @YSaiyan@W!@n"), 1, ch, nil, nil, TO_ROOM)
				if (func() int {
					zone = real_zone_by_thing(ch.In_room)
					return zone
				}()) != int(-1) {
		...
		return
	} else if libc.StrCaseCmp(libc.CString("second"), &arg[0]) == 0 {
		if int(ch.Race) == RACE_HUMAN {
			...
			if ch.Basepl < int64(trans_req(ch, 2)) {
				...
				var zone int = 0
				act(libc.CString("@WSuddenly a bright white aura bursts into existance around your body, you feel the intensity of your hidden potential boil until it can't be contained any longer! Waves of ki shoot out from your aura streaking outwards in many directions. A roar that shakes everything in the surrounding area sounds right as your energy reaches its potential and you achieve @CSuper @cHuman @GSecond@W!"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n@W is suddenly covered with a bright white aura as $e grits $s teeth, apparently struggling with the power boiling to the surface! Waves of ki shoot out from $s aura, streaking in several directions as a mighty roar shakes everything in the surrounding area. As $s aura calms $e smiles, having achieved @CSuper @cHuman @GSecond@W!"), 1, ch, nil, nil, TO_ROOM)
				if (func() int {
					zone = real_zone_by_thing(ch.In_room)
					return zone
				}()) != int(-1) {
		...
		if int(ch.Race) == RACE_TRUFFLE {
			...
			if ch.Basepl < int64(trans_req(ch, 2)) {
				...
				var zone int = 0
				act(libc.CString("@WComplete understanding of every physical thing floods your mind as the nano-technology within you continues to change your body! Your eyes change; becoming glassy, hard, and glowing. Your muscles merge with a nano-fiber strengthening them at the molecular level! Finally your very bones become plated in nano-metals that have yet to be invented naturally!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n@.s nano-technology continues to change $s body! $s eyes change; becoming glassy, hard, and glowing. $s muscles merge with a nano-fiber strengthening them at the molecular level! Finally $s very bones become plated in nano-metals that have yet to be invented naturally!@n"), 1, ch, nil, nil, TO_ROOM)
				if (func() int {
					zone = real_zone_by_thing(ch.In_room)
					return zone
				}()) != int(-1) {
		...
		if int(ch.Race) == RACE_MUTANT {
			...
			if ch.Basepl < int64(trans_req(ch, 2)) {
				...
				var zone int = 0
				act(libc.CString("@WSpikes grow out from your elbows as your power begins to climb to new heights. The muscles along your forearms grow to double their former size as the spikes growing from your elbows flatten and sharpen into blades. You have achieved your @GSecond @mMutation@W!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@WSpikes grow out from @C$n@W's elbows as $s power begins to climb to new heights. The muscles along $s forearms grow to double their former size as the spikes growing from $s elbows flatten and sharpen into blades. $e has achieved your @GSecond @mMutation@W!@n"), 1, ch, nil, nil, TO_ROOM)
				if (func() int {
					zone = real_zone_by_thing(ch.In_room)
					return zone
				}()) != int(-1) {
		...
		if int(ch.Race) == RACE_HALFBREED {
			...
			if ch.Basepl < int64(trans_req(ch, 2)) {
				...
				var zone int = 0
				act(libc.CString("@WBlinding rage burns through your mind as a sudden eruption of energy surges forth! A golden aura bursts up around your body, glowing as bright as the sun. Rushing winds rocket out from your body in every direction as bolts of electricity begin to crackle in your aura. As your aura dims you are left standing confidently, having achieved @CSuper @YSaiyan @GSecond@W!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n@W stands up straight with $s head back as $e releases an ear piercing scream! A blindingly bright golden aura bursts up around $s body, glowing as bright as the sun. As rushing winds begin to rocket out from $m in every direction, bolts of electricity flash and crackle in $s aura. As $s aura begins to dim $e is left standing confidently, having achieved @CSuper @YSaiyan @GSecond@W!@n"), 1, ch, nil, nil, TO_ROOM)
				if (func() int {
					zone = real_zone_by_thing(ch.In_room)
					return zone
				}()) != int(-1) {
		...
		if int(ch.Race) == RACE_NAMEK {
			...
			if ch.Basepl < int64(trans_req(ch, 2)) {
				...
				var zone int = 0
				act(libc.CString("@WYou gasp in shock as a power within your body that you had not been aware of begins to surge to the surface! Your muscles grow larger as energy crackles between your antennae intensely! A shockwave of energy explodes outward as you achieve a new plateau in power, @CSuper @gNamek @GSecond@W!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n @Wgasps in shock as a power within $s body begins to surge out! $s muscles grow larger as energy crackles between $s antennae intensely! A shockwave of energy explodes outward as $e achieves a new plateau in power, @CSuper @gNamek @GSecond@W!@n"), 1, ch, nil, nil, TO_ROOM)
				if (func() int {
					zone = real_zone_by_thing(ch.In_room)
					return zone
				}()) != int(-1) {
		...
		if int(ch.Race) == RACE_SAIYAN {
			...
			if ch.Basepl < int64(trans_req(ch, 2)) {
				...
				if !PLR_FLAGGED(ch, PLR_LSSJ) {
					...
					act(libc.CString("@WYou roar and then stand at your full height. You flex every muscle in your body as you feel your strength grow! Your eyes begin to glow @wwhite@W with energy, your hair turns @Ygold@W, and at the same time a @wbright @Yg@yo@Yl@yd@Ye@yn@W aura flashes up around your body! You release your @YL@ye@Dg@We@wn@Yd@ya@Dr@Yy@W power upon the universe!@n"), 1, ch, nil, nil, TO_CHAR)
					act(libc.CString("@C$n @Wroars and then stands at $s full height. Then $s muscles start to buldge and grow as $e flexes them! Suddenly $s eyes begin to glow @wwhite@W with energy, $s hair turns @Ygold@W, and at the same time a @wbright @Yg@yo@Yl@yd@Ye@yn@W aura flashes up around $s body! @C$n@W releases $s @YL@ye@Dg@We@wn@Yd@ya@Dr@Yy@W power upon the universe!@n"), 1, ch, nil, nil, TO_ROOM)
				}
				if (func() int {
					zone = real_zone_by_thing(ch.In_room)
					return zone
		...
		return
	} else if libc.StrCaseCmp(libc.CString("third"), &arg[0]) == 0 {
		if int(ch.Race) == RACE_HUMAN {
			...
			if ch.Basepl < int64(trans_req(ch, 3)) {
				...
				var zone int = 0
				act(libc.CString("@WYou clench both of your fists as the bright white aura around your body is absorbed back into your flesh. As it is absorbed, your muscles triple in size and electricity crackles across your flesh. You grin as you feel the power of @CSuper @cHuman @GThird@W!"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n@W clenches both of $s fists as the bright white aura around $s body is absorbed back into $s flesh. As it is absorbed, $s muscles triple in size and bright electricity crackles across $s flesh. $e smiles as $e achieves the power of @CSuper @cHuman @GThird@W!"), 1, ch, nil, nil, TO_ROOM)
				if (func() int {
					zone = real_zone_by_thing(ch.In_room)
					return zone
				}()) != int(-1) {
		...
		if int(ch.Race) == RACE_TRUFFLE {
			...
			if ch.Basepl < int64(trans_req(ch, 3)) {
				...
				var zone int = 0
				act(libc.CString("@WYou have reached the final stage of enlightenment and the nano-technology thriving inside you begin to initiate the changes! Your neural pathways become refined, your reflexes honed, your auditory and ocular senses sharpening far beyond normal levels! Your gravitational awareness improves, increasing sensitivity and accuracy in your equilibrum!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n begins to mumble quietly, slowly at first and gradually picking up speed. A glint is seen from $s eyes and $s arms reach outwards briefly as $e appears to catch his balance. $s arms drop back to $s sides as balance is regained, a vicious smile on $s face.@n"), 1, ch, nil, nil, TO_ROOM)
				if (func() int {
					zone = real_zone_by_thing(ch.In_room)
					return zone
				}()) != int(-1) {
		...
		if int(ch.Race) == RACE_MUTANT {
			...
			if ch.Basepl < int64(trans_req(ch, 3)) {
				...
				var zone int = 0
				act(libc.CString("@WA dark cyan aura bursts up around your body as the ground begins to crack beneath you! You scream out in pain as your power begins to explode! Two large spikes grow out from your shoulder blades as you reach your @GThird @mMutation!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@WA dark cyan aura bursts up around @C$n@W's body as the ground begins to crack beneath $m and $e screams out in pain as $s power begins to explode! Two large spikes grow out from $s shoulder blades as $e reaches $s @GThird @mMutation!@n"), 1, ch, nil, nil, TO_ROOM)
				if (func() int {
					zone = real_zone_by_thing(ch.In_room)
					return zone
				}()) != int(-1) {
		...
		if int(ch.Race) == RACE_KONATSU {
			...
			if ch.Basepl < int64(trans_req(ch, 3)) {
				...
				var zone int = 0
				act(libc.CString("@WThe shadowy aura around you explodes outward as your power begins to rise!  You're overcome with a sudden realization, that the shadows are an extension of yourself, that light isn't needed for your shadows to bloom.  With this newfound wisdom comes ability and power!  The color in your aura drains as the shadows slide inward and cling to your body like a second, solid black skin!  Shockwaves roll off of you in quick succession, pelting the surrounding area harshly!  Accompanying the waves, a pool of darkness blossoms underneath you, slowly spreading the shadows to the whole area, projecting onto any surface nearby!  Purple and black electricity crackle in your solid white aura, and you grin as you realize your @GThird @DShadow @BForm@W!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@WThe shadowy aura around $n explodes outward as $s power begins to rise!  Realization dawns on $s face, followed shortly by confidence! The color in $s aura drains as the shadows slide inward to cling to $s body like a second, solid black skin! Shockwaves roll off of $n in quick succession, pelting the surrounding area harshly!  Accompanying the waves, a pool of darkness blossoms underneath them, slowly spreading the shadows to the whole area, projecting onto any surface nearby! Purple and black electricity crackle in $s solid white aura, and he grins as $e realizes $s @GThird @DShadow @BForm@W!@n"), 1, ch, nil, nil, TO_ROOM)
				if (func() int {
					zone = real_zone_by_thing(ch.In_room)
					return zone
				}()) != int(-1) {
		...
		if int(ch.Race) == RACE_NAMEK {
			...
			if ch.Basepl < int64(trans_req(ch, 3)) {
				...
				var zone int = 0
				act(libc.CString("@WA fierce clear aura bursts up around your body as you struggle to control a growing power within! Energy leaks off of your aura at an astounding rate filling the air around you with small orbs of ki. As your power begins to level off the ambient ki hovering around you is absorbed inward in a sudden shock that leaves your skin glowing! You have achieved a rare power, @CSuper @gNamek @GThird@W!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@WA fierce clear aura bursts up around @C$n@W's body as $e struggles to control $s own power! Energy leaks off of $s aura at an astounding rate filling the air around $m with small orbs of ki. As $s power begins to level off the ambient ki hovering around $m is absorbed inward in a sudden shock that leaves $s skin glowing! $e has achieved a rare power, @CSuper @gNamek @GThird@W!@n"), 1, ch, nil, nil, TO_ROOM)
				if (func() int {
					zone = real_zone_by_thing(ch.In_room)
					return zone
				}()) != int(-1) {
		...
		if int(ch.Race) == RACE_SAIYAN && !PLR_FLAGGED(ch, PLR_LSSJ) {
			if PLR_FLAGGED(ch, PLR_TRANS4) {
				send_to_char(ch, libc.CString("You are already beyond that form!\r\n"))
				return
			}
		...
		return
	} else if libc.StrCaseCmp(libc.CString("fourth"), &arg[0]) == 0 {
		if int(ch.Race) == RACE_HUMAN {
			if PLR_FLAGGED(ch, PLR_TRANS4) {
				send_to_char(ch, libc.CString("You are already in that form! Try 'revert'.\r\n"))
				return
		...
		if int(ch.Race) == RACE_NAMEK {
			...
			if ch.Basepl < int64(trans_req(ch, 4)) {
				...
				var zone int = 0
				act(libc.CString("@WAn inner calm fills your mind as your power surges higher than ever before. Complete clarity puts everything once questioned into perspective. While this inner calm is filling your mind, an outer storm of energy erupts around your body! The storm of energy boils and crackles while growing larger. You have achieved @CSuper @gNamek @GFourth@W, a mystery of the ages.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n@W smiles calmly as a look of complete understand fills $s eyes. While $e remains perfectly calm and detached a massivly powerful storm of energy erupts from his body. This storm of energy shimmers with the colors of the rainbow and boils and crackles with awesome power! $s smile disappears as he realizes a mysterious power of the ages, @CSuper @gNamek @GFourth@W!@n"), 1, ch, nil, nil, TO_ROOM)
				if (func() int {
					zone = real_zone_by_thing(ch.In_room)
					return zone
				}()) != int(-1) {
		...
		if int(ch.Race) == RACE_SAIYAN && !PLR_FLAGGED(ch, PLR_LSSJ) {
			...
			if ch.Basepl < int64(trans_req(ch, 4)) {
				...
				var zone int = 0
				act(libc.CString("@WHaving absorbed enough blutz waves, your body begins to transform! Red fur grows over certain parts of your skin as your hair grows longer and unkempt. A red outline forms around your eyes while the irises of those very same eyes change to an amber color. Energy crackles about your body violently as you achieve the peak of saiyan perfection, @CSuper @YSaiyan @GFourth@W!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@WHaving absorbed enough blutz waves, @C$n@W's body begins to transform! Red fur grows over certain parts of $s skin as $s hair grows longer and unkempt. A red outline forms around $s eyes while the irises of those very same eyes change to an amber color. Energy crackles about $s body violently as $e achieves the peak of saiyan perfection, @CSuper @YSaiyan @GFourth@W!@n"), 1, ch, nil, nil, TO_ROOM)
				if (func() int {
					zone = real_zone_by_thing(ch.In_room)
					return zone
				}()) != int(-1) {
		...
		return
	} else if libc.StrCaseCmp(libc.CString("mature"), &arg[0]) == 0 {
		if int(ch.Race) == RACE_BIO {
			if PLR_FLAGGED(ch, PLR_TRANS2) {
				send_to_char(ch, libc.CString("You are already beyond that form!\r\n"))
				return
		...
		return
	} else if libc.StrCaseCmp(libc.CString("semi-perfect"), &arg[0]) == 0 || libc.StrCaseCmp(libc.CString("Semi-Perfect"), &arg[0]) == 0 {
		if int(ch.Race) == RACE_BIO {
			if PLR_FLAGGED(ch, PLR_TRANS3) || PLR_FLAGGED(ch, PLR_TRANS4) {
				send_to_char(ch, libc.CString("You are already beyond that form!\r\n"))
				return
	...
	} else if libc.StrCaseCmp(libc.CString("perfect"), &arg[0]) == 0 {
		if int(ch.Race) == RACE_BIO {
			if PLR_FLAGGED(ch, PLR_TRANS4) {
				send_to_char(ch, libc.CString("You are already beyond that form!\r\n"))
				return
	...
	} else if (libc.StrCaseCmp(libc.CString("super"), &arg[0]) == 0 || libc.StrCaseCmp(libc.CString("super perfect"), &arg[0]) == 0) && int(ch.Race) == RACE_BIO {
		if int(ch.Race) == RACE_BIO {
			if PLR_FLAGGED(ch, PLR_TRANS4) {
				send_to_char(ch, libc.CString("You are already in that form!\r\n"))
				return
	...
	} else if libc.StrCaseCmp(libc.CString("affinity"), &arg[0]) == 0 {
		if int(ch.Race) == RACE_MAJIN {
			if PLR_FLAGGED(ch, PLR_TRANS2) || PLR_FLAGGED(ch, PLR_TRANS3) || PLR_FLAGGED(ch, PLR_TRANS4) {
				send_to_char(ch, libc.CString("You are already beyond that form!\r\n"))
				return
			...
			if ch.Basepl < int64(trans_req(ch, 1)) {
				...
				var zone int = 0
				act(libc.CString("@WA dark pink aura bursts up around your body as images of good and evil fill your mind! You feel the power within your body growing intensely, reflecting your personal alignment as your body changes!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@WA dark pink aura bursts up around @C$n@W's body as images of good and evil fill $s mind! $e feels the power within $s body growing intensely, reflecting $s personal alignment as $s body changes!@n"), 1, ch, nil, nil, TO_ROOM)
				if (func() int {
					zone = real_zone_by_thing(ch.In_room)
					return zone
				}()) != int(-1) {
	...
	} else if libc.StrCaseCmp(libc.CString("super"), &arg[0]) == 0 {
		if int(ch.Race) == RACE_MAJIN {
			if PLR_FLAGGED(ch, PLR_TRANS3) || PLR_FLAGGED(ch, PLR_TRANS4) {
				send_to_char(ch, libc.CString("You are already beyond that form!\r\n"))
				return
			...
			if ch.Absorbs > 0 && GET_LEVEL(ch) < 50 {
				...
				var zone int = 0
				act(libc.CString("@WAn intense pink aura surrounds your body as it begins to change, taking on the characteristics of those you have ingested! Explosions of pink energy burst into existence all around you as your power soars to sights unseen!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@WAn intense pink aura surrounds @C$n@W's body as it begins to change, taking on the characteristics of those $e has ingested! Explosions of pink energy burst into existence all around $m as $s power soars to sights unseen!@n"), 1, ch, nil, nil, TO_ROOM)
				if (func() int {
					zone = real_zone_by_thing(ch.In_room)
					return zone
				}()) != int(-1) {
	...
	} else if libc.StrCaseCmp(libc.CString("true"), &arg[0]) == 0 {
		if int(ch.Race) == RACE_MAJIN {
			if PLR_FLAGGED(ch, PLR_TRANS4) {
				send_to_char(ch, libc.CString("You are already beyond that form!\r\n"))
				return
			...
			if ch.Basepl < int64(trans_req(ch, 3)) {
				...
				var zone int = 0
				act(libc.CString("@WRipples of intense pink energy rush upwards around your body as it begins to morph into its truest form! The ground beneath your feet forms into a crater from the very pressure of your rising ki! Earthquakes shudder throughout the area as your finish morphing!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@WRipples of intense pink energy rush upwards around @C$n@W's body as it begins to morph into its truest form! The ground beneath $s feet forms into a crater from the very pressure of $s rising ki! Earthquakes shudder throughout the area as $e finishes morphing!@n"), 1, ch, nil, nil, TO_ROOM)
				if (func() int {
					zone = real_zone_by_thing(ch.In_room)
					return zone
				}()) != int(-1) {
	...
	} else if libc.StrCaseCmp(libc.CString("1.0"), &arg[0]) == 0 {
		if int(ch.Race) == RACE_ANDROID {
			if PLR_FLAGGED(ch, PLR_TRANS2) || PLR_FLAGGED(ch, PLR_TRANS3) || PLR_FLAGGED(ch, PLR_TRANS4) {
			...
			if ch.Basepl < int64(trans_req(ch, 1)) {
				...
				act(libc.CString("@WYou stop for a moment as the nano-machines within your body reprogram and restructure you. You are now more powerful and efficient!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n @Wstops for a moment as the nano-machines within $s body reprogram and restructure $m. $e is now more powerful and efficient!@n"), 1, ch, nil, nil, TO_ROOM)
				var add int = 5000000
				var mult float64 = 1
	...
	} else if libc.StrCaseCmp(libc.CString("2.0"), &arg[0]) == 0 {
		if int(ch.Race) == RACE_ANDROID {
			if PLR_FLAGGED(ch, PLR_TRANS3) || PLR_FLAGGED(ch, PLR_TRANS4) {
				send_to_char(ch, libc.CString("You are already beyond that upgrade!\r\n"))
				return
	...
	} else if libc.StrCaseCmp(libc.CString("3.0"), &arg[0]) == 0 {
		if int(ch.Race) == RACE_ANDROID {
			if PLR_FLAGGED(ch, PLR_TRANS4) {
				send_to_char(ch, libc.CString("You are already beyond that upgrade!\r\n"))
				return
	...
	} else if libc.StrCaseCmp(libc.CString("4.0"), &arg[0]) == 0 {
		if int(ch.Race) == RACE_ANDROID {
	...
	} else if libc.StrCaseCmp(libc.CString("5.0"), &arg[0]) == 0 {
		if int(ch.Race) == RACE_ANDROID {
	...
	} else if libc.StrCaseCmp(libc.CString("6.0"), &arg[0]) == 0 {
		if int(ch.Race) == RACE_ANDROID {
			if PLR_FLAGGED(ch, PLR_TRANS6) {
				send_to_char(ch, libc.CString("You are already in that upgrade!\r\n"))
				return
	...
	} else if libc.StrCaseCmp(libc.CString("revert"), &arg[0]) == 0 {
		if int(ch.Race) == RACE_HUMAN {
			if PLR_FLAGGED(ch, PLR_TRANS4) {
				act(libc.CString("@wYou revert from @CSuper @cHuman @GFourth@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CSuper @cHuman @GFourth@w.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS4)
				...
				act(libc.CString("@wYou revert from @CSuper @cHuman @GThird@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CSuper @cHuman @GThird@w.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS3)
				...
				act(libc.CString("@wYou revert from @CSuper @cHuman @GSecond@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CSuper @cHuman @GSecond@w.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS2)
				...
				act(libc.CString("@wYou revert from @CSuper @cHuman @GFirst@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CSuper @cHuman @GFirst.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS1)
			...
			if PLR_FLAGGED(ch, PLR_TRANS4) {
				act(libc.CString("@wYou revert from @CSuper @cSaiyan @GFourth@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CSuper @cSaiyan @GFourth@w.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS4)
				...
				act(libc.CString("@wYou revert from @CSuper @cSaiyan @GThird@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CSuper @cSaiyan @GThird@w.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS3)
				...
				if !PLR_FLAGGED(ch, PLR_LSSJ) {
					act(libc.CString("@wYou revert from @CSuper @cSaiyan @GSecond@w.@n"), 1, ch, nil, nil, TO_CHAR)
					act(libc.CString("@w$n@w reverts from @CSuper @cSaiyan @GSecond@w.@n"), 1, ch, nil, nil, TO_ROOM)
					if ch.Charge > 0 {
						do_charge(ch, libc.CString("release"), 0, 0)
					}
					REMOVE_BIT_AR(ch.Act[:], PLR_TRANS2)
					...
					act(libc.CString("@wYou revert from your @YLegendary @CSuper Saiyan@w form.@n"), 1, ch, nil, nil, TO_CHAR)
					act(libc.CString("@w$n@w reverts from $s @YLegendary @CSuper Saiyan@w form@w.@n"), 1, ch, nil, nil, TO_ROOM)
					if ch.Charge > 0 {
						do_charge(ch, libc.CString("release"), 0, 0)
					}
					REMOVE_BIT_AR(ch.Act[:], PLR_TRANS2)
				...
				act(libc.CString("@wYou revert from @CSuper @cSaiyan @GFirst@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CSuper @cSaiyan @GFirst.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS1)
			...
			if PLR_FLAGGED(ch, PLR_TRANS4) {
				act(libc.CString("@wYou revert from @CSuper @cNamek @GFourth@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CSuper @cNamek @GFourth@w.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS4)
				...
				act(libc.CString("@wYou revert from @CSuper @cNamek @GThird@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CSuper @cNamek @GThird@w.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS3)
				...
				act(libc.CString("@wYou revert from @CSuper @cNamek @GSecond@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CSuper @cNamek @GSecond@w.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS2)
				...
				act(libc.CString("@wYou revert from @CSuper @cNamek @GFirst@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CSuper @cNamek @GFirst.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS1)
			...
			if PLR_FLAGGED(ch, PLR_TRANS4) {
				act(libc.CString("@wYou revert from @CTransform @GFourth@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CTransform @GFourth@w.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS4)
				...
				act(libc.CString("@wYou revert from @CTransform @GThird@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CTransform @GThird@w.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS3)
				...
				act(libc.CString("@wYou revert from @CTransform @GSecond@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CTransform @GSecond@w.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS2)
				...
				act(libc.CString("@wYou revert from @CTransform @GFirst@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CTransform @GFirst.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS1)
			...
			if PLR_FLAGGED(ch, PLR_TRANS3) {
				act(libc.CString("@wYou revert from @CMutate @GThird@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CMutate @GThird@w.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS3)
				...
				act(libc.CString("@wYou revert from @CMutate @GSecond@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CMutate @GSecond@w.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS2)
				...
				act(libc.CString("@wYou revert from @CMutate @GFirst@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CMutate @GFirst.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS1)
			...
			if PLR_FLAGGED(ch, PLR_TRANS3) {
				act(libc.CString("@wYou revert from @CShadow @GThird@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CShadow @GThird@w.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS3)
			...
			if PLR_FLAGGED(ch, PLR_TRANS2) {
				act(libc.CString("@wYou revert from @CShadow @GSecond@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CShadow @GSecond@w.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS2)
				...
				act(libc.CString("@wYou revert from @CShadow @GFirst@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CShadow @GFirst.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS1)
			...
			if PLR_FLAGGED(ch, PLR_TRANS3) {
				act(libc.CString("@wYou revert from @CMystic @GThird@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CMystic @GThird@w.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS3)
			...
			if PLR_FLAGGED(ch, PLR_TRANS2) {
				act(libc.CString("@wYou revert from @CMystic @GSecond@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CMystic @GSecond@w.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS2)
				...
				act(libc.CString("@wYou revert from @CMystic @GFirst@w.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@w reverts from @CMystic @GFirst.@n"), 1, ch, nil, nil, TO_ROOM)
				if ch.Charge > 0 {
					do_charge(ch, libc.CString("release"), 0, 0)
				}
				REMOVE_BIT_AR(ch.Act[:], PLR_TRANS1)
		...
		return
	} else if int(ch.Race) == RACE_KANASSAN || int(ch.Race) == RACE_DEMON {
		send_to_char(ch, libc.CString("You do not have a transformation.\r\n"))
		return
	} else {
...
func do_situp(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if int(ch.Race) == RACE_ANDROID || int(ch.Race) == RACE_BIO || int(ch.Race) == RACE_MAJIN || int(ch.Race) == RACE_ARLIAN {
	...
	if AFF_FLAGGED(ch, AFF_FLYING) {
		...
		if world[ch.In_room].Gravity == 0 {
			bonus = 1
			act(libc.CString("@gYou do a situp.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@g$n does a situp.@n"), 1, ch, nil, nil, TO_ROOM)
		} else if world[ch.In_room].Gravity == 10 {
			bonus = rand_number(3, 7)
			act(libc.CString("@gYou do a situp, feeling the strain of gravity.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@g$n does a situp, while sweating.@n"), 1, ch, nil, nil, TO_ROOM)
		} else if world[ch.In_room].Gravity == 20 {
			bonus = rand_number(8, 14)
			act(libc.CString("@gYou do a situp, and feel gravity's pull.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@g$n does a situp, while sweating.@n"), 1, ch, nil, nil, TO_ROOM)
		} else if world[ch.In_room].Gravity == 30 {
			bonus = rand_number(14, 20)
			act(libc.CString("@gYou do a situp, and feel the burn.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@g$n does a situp, while sweating.@n"), 1, ch, nil, nil, TO_ROOM)
			...
			act(libc.CString("@gYou do a situp, and really strain against the gravity.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@g$n does a situp, while sweating.@n"), 1, ch, nil, nil, TO_ROOM)
			...
			act(libc.CString("@gYou do a situp, and it was a really hard one to finish.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@g$n does a situp, while sweating profusely.@n"), 1, ch, nil, nil, TO_ROOM)
		} else if world[ch.In_room].Gravity == 5000 {
...
func do_meditate(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if libc.StrCaseCmp(&arg[0], libc.CString("expand")) == 0 {
		var cost int = 3500
		if int(ch.Race) == RACE_SAIYAN {
			cost = 7000
		}
		...
		return
	} else if libc.StrCaseCmp(&arg[0], libc.CString("break")) == 0 {
		if ch.Mindlink == nil {
			...
			act(libc.CString("@rYou manage to break the mind link between you and @R$N@r!@n"), 0, ch, nil, unsafe.Pointer(ch.Mindlink), TO_CHAR)
			act(libc.CString("$n closes their eyes for a few seconds."), 0, ch, nil, unsafe.Pointer(ch.Mindlink), TO_ROOM)
			...
			act(libc.CString("@rYou struggle to free your mind of @R$N's@r link, but fail!@n"), 0, ch, nil, unsafe.Pointer(ch.Mindlink), TO_CHAR)
			act(libc.CString("$n closes their eyes for a few seconds, and appears to struggle quite a bit."), 0, ch, nil, unsafe.Pointer(ch.Mindlink), TO_ROOM)
	...
	if ch.Mana < cost {
		...
		act(libc.CString("@cYou close your eyes and concentrate, lifting $p@c with your ki.@n"), 1, ch, obj, nil, TO_CHAR)
		act(libc.CString("@c$n closes $s eyes and lifts $p@c with $s ki.@n"), 1, ch, obj, nil, TO_ROOM)
		if world[ch.In_room].Gravity == 0 {
			bonus = 1
		} else if world[ch.In_room].Gravity == 10 {
		...
		if bonus != 1 && int(ch.Race) == RACE_DEMON && rand_number(1, 100) >= 80 {
...
func do_pushup(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if AFF_FLAGGED(ch, AFF_FLYING) {
		...
		if world[ch.In_room].Gravity == 0 {
			bonus = 1
			act(libc.CString("@gYou do a pushup.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@g$n does a pushup.@n"), 1, ch, nil, nil, TO_ROOM)
		} else if world[ch.In_room].Gravity == 10 {
			bonus = rand_number(3, 7)
			act(libc.CString("@gYou do a pushup, feeling the strain of gravity.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@g$n does a pushup, while sweating.@n"), 1, ch, nil, nil, TO_ROOM)
		} else if world[ch.In_room].Gravity == 20 {
			bonus = rand_number(8, 14)
			act(libc.CString("@gYou do a pushup, and feel gravity's pull.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@g$n does a pushup, while sweating.@n"), 1, ch, nil, nil, TO_ROOM)
		} else if world[ch.In_room].Gravity == 30 {
			bonus = rand_number(14, 20)
			act(libc.CString("@gYou do a pushup, and feel the burn.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@g$n does a pushup, while sweating.@n"), 1, ch, nil, nil, TO_ROOM)
		} else if world[ch.In_room].Gravity == 40 {
			bonus = rand_number(20, 35)
			act(libc.CString("@gYou do a pushup, and feel the burn.@n"), 1, ch, nil, nil, TO_CHAR)
			...
			bonus = rand_number(180, 250)
			act(libc.CString("@gYou do a pushup, and really strain against the gravity.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@g$n does a pushup, while sweating.@n"), 1, ch, nil, nil, TO_ROOM)
		} else if world[ch.In_room].Gravity == 200 {
			bonus = rand_number(400, 600)
			act(libc.CString("@gYou do a pushup, and really strain against the gravity.@n"), 1, ch, nil, nil, TO_CHAR)
			...
			bonus = rand_number(9000, 10000)
			act(libc.CString("@gYou do a pushup, and it was a really hard one to finish.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@g$n does a pushup, while sweating profusely.@n"), 1, ch, nil, nil, TO_ROOM)
		} else if world[ch.In_room].Gravity == 5000 {
			bonus = rand_number(15000, 20000)
			act(libc.CString("@gYou do a pushup, and it was a really hard one to finish.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@g$n does a pushup, while sweating profusely.@n"), 1, ch, nil, nil, TO_ROOM)
...
func do_spar(ch *char_data, argument *byte, cmd int, subcmd int) {
	if IS_NPC(ch) {
		...
		if PLR_FLAGGED(ch, PLR_SPAR) {
			act(libc.CString("@wYou cease your sparring stance.@n"), 0, ch, nil, nil, TO_CHAR)
			act(libc.CString("@C$n@w ceases $s sparring stance.@n"), 0, ch, nil, nil, TO_ROOM)
		}
		if !PLR_FLAGGED(ch, PLR_SPAR) {
			act(libc.CString("@wYou move into your sparring stance.@n"), 0, ch, nil, nil, TO_CHAR)
			act(libc.CString("@C$n@w moves into $s sparring stance.@n"), 0, ch, nil, nil, TO_ROOM)
		}
...
func check_eq(ch *char_data) {
	...
	for i = 0; i < NUM_WEARS; i++ {
		if (ch.Equipment[i]) != nil {
			obj = ch.Equipment[i]
			if OBJ_FLAGGED(obj, ITEM_BROKEN) {
				act(libc.CString("@W$p@W falls apart and you remove it.@n"), 0, ch, obj, nil, TO_CHAR)
				act(libc.CString("@W$p@W falls apart and @C$n@W remove it.@n"), 0, ch, obj, nil, TO_ROOM)
				perform_remove(ch, i)
				return
			}
			if obj == (ch.Equipment[WEAR_WIELD1]) && (ch.Limb_condition[0]) <= 0 {
				act(libc.CString("@WWithout your right arm you let go of @c$p@W!@n"), 0, ch, obj, nil, TO_CHAR)
				act(libc.CString("@C$n@W lets go of @c$p@W!@n"), 0, ch, obj, nil, TO_ROOM)
				perform_remove(ch, i)
				return
			}
			if obj == (ch.Equipment[WEAR_WIELD2]) && (ch.Limb_condition[1]) <= 0 {
				act(libc.CString("@WWithout your left arm you let go of @c$p@W!@n"), 0, ch, obj, nil, TO_CHAR)
				act(libc.CString("@C$n@W lets go of @c$p@W!@n"), 0, ch, obj, nil, TO_ROOM)
				perform_remove(ch, i)
				return
			}
...
func base_update() {
	...
	if INTERESTTIME != 0 && INTERESTTIME <= libc.GetTime(nil) && libc.GetTime(nil) != 0 {
		INTERESTTIME = libc.GetTime(nil) + 86400
		LASTINTEREST = libc.GetTime(nil)
		save_mud_time(&time_info)
		cash = 1
		countch = 1
	}
	...
	for d = descriptor_list; d != nil; d = d.Next {
		...
		if IS_NPC(d.Character) {
			...
			if int(d.Character.Race) == RACE_ANDROID && d.Character.Absorbing != nil && rand_number(1, 10) >= 7 {
				var (
		...
		if PLR_FLAGGED(d.Character, PLR_GOOP) && d.Character.Gooptime == 60 {
			if int(d.Character.Race) == RACE_BIO {
				act(libc.CString("@GConciousness slowly returns to you. You realize quickly that some of your cells have survived. You take control of your regenerative processes and focus on growing a new body!@n"), 1, d.Character, nil, nil, TO_CHAR)
			} else {
				act(libc.CString("@MSlowly you regain conciousness. The various split off chunks of your body begin to likewise stir.@n"), 1, d.Character, nil, nil, TO_CHAR)
				act(libc.CString("@MYou think you notice the chunks of @m$n@M's moving slightly.@n"), 1, d.Character, nil, nil, TO_ROOM)
			}
			d.Character.Gooptime -= 1
		} else if PLR_FLAGGED(d.Character, PLR_GOOP) && d.Character.Gooptime == 30 {
			if int(d.Character.Race) == RACE_BIO {
				act(libc.CString("@GFrom the collection of cells growing a crude form of your body starts to take shape!@n"), 1, d.Character, nil, nil, TO_CHAR)
				act(libc.CString("@GYou start to notice a large mass of pulsing flesh growing before you!@n"), 1, d.Character, nil, nil, TO_ROOM)
			} else {
				act(libc.CString("@MYou will the various chunks of your body to return and slowly more and more of them begin to fly into you. Your body begins to grow larger and larger as this process unfolds!@n "), 1, d.Character, nil, nil, TO_CHAR)
				act(libc.CString("@MThe various chunks of @m$n@M's body start to fly into the largest chunk! As the chunks collide they begin to form a larger and still growing blob of goo!@n"), 1, d.Character, nil, nil, TO_ROOM)
			}
			d.Character.Gooptime -= 1
		} else if PLR_FLAGGED(d.Character, PLR_GOOP) && d.Character.Gooptime == 15 {
			if int(d.Character.Race) == RACE_BIO {
				act(libc.CString("@GYour body has almost reached its previous form! Only a little more regenerating is needed!@n"), 1, d.Character, nil, nil, TO_CHAR)
				act(libc.CString("@GThe lump of flesh has now grown to the size where the likeness of @g$n@G can be seen of it! It appears that $e is regenerating $s body from what was only a few cells!@n"), 1, d.Character, nil, nil, TO_ROOM)
			} else {
				act(libc.CString("@MYour body has reached half its previous size as your limbs ooze slowly out into their proper shape!@n"), 1, d.Character, nil, nil, TO_CHAR)
				act(libc.CString("@m$n@M's body has regenerated to half its previous size! Slowly $s limbs ooze out into their proper shape! It won't be long now till $e has fully regenerated!@n"), 1, d.Character, nil, nil, TO_ROOM)
			}
			d.Character.Gooptime -= 1
		} else if PLR_FLAGGED(d.Character, PLR_GOOP) && d.Character.Gooptime == 0 {
			if int(d.Character.Race) == RACE_BIO {
				d.Character.Hit = gear_pl(d.Character)
				act(libc.CString("@GYour body has fully regenerated! You flex your arms and legs outward with a rush of renewed strength!@n"), 1, d.Character, nil, nil, TO_CHAR)
				act(libc.CString("@g$n@G's body has fully regenerated! Suddenly $e flexes $s arms and legs and a rush of power erupts from off of $s body!@n"), 1, d.Character, nil, nil, TO_ROOM)
			} else if int(d.Character.Race) == RACE_SAIYAN {
				var (
					zenkaiPL int
					zenkaiKi int
					zenkaiSt int
				...
				act(libc.CString("@RYou collapse to the ground, body pushed beyond the typical limits of exhaustion. The passage of time distorts and an indescribable amount of time passes as raw emotions pass through your very being. Your eyes open and focus with a newfound clarity as your unadulterated emotions and feelings revive you for a second wind!@n"), 1, d.Character, nil, nil, TO_CHAR)
				act(libc.CString("@r$n@R collapses to the ground, seemingly dead. After a brief moment, their eyes flash open with a determined look on their face!"), 1, d.Character, nil, nil, TO_ROOM)
			} else {
				d.Character.Hit = gear_pl(d.Character)
				act(libc.CString("@MYour body has fully regenerated! You scream out in triumph and a short gust of steam erupts from your pores!@n"), 1, d.Character, nil, nil, TO_CHAR)
				act(libc.CString("@m$n@M's body has fully regenerated! Suddenly $e screams out in gleeful triumph and short gust of steam erupts from $s skin pores!"), 1, d.Character, nil, nil, TO_ROOM)
			}
		...
		if MOON_UP != 0 && (int(d.Character.Race) == RACE_SAIYAN || int(d.Character.Race) == RACE_HALFBREED) && !PLR_FLAGGED(d.Character, PLR_OOZARU) {
			oozaru_add(d.Character)
		}
		if cash == 1 && d.Character.Bank_gold > 0 {
		...
		if !IS_NPC(d.Character) && world[d.Character.In_room].Geffect >= 1 && rand_number(1, 100) >= 96 {
			if world[d.Character.In_room].Geffect <= 4 {
				switch rand_number(1, 4) {
				case 1:
					act(libc.CString("@RLava spews up violently from the cracks in the ground!@n"), 0, d.Character, nil, nil, TO_ROOM)
					act(libc.CString("@RLava spews up violently from the cracks in the ground!@n"), 0, d.Character, nil, nil, TO_CHAR)
				case 2:
					act(libc.CString("@RThe lava bubbles and gives off tremendous heat!@n"), 0, d.Character, nil, nil, TO_ROOM)
					act(libc.CString("@RThe lava bubbles and gives off tremendous heat!@n"), 0, d.Character, nil, nil, TO_CHAR)
				case 3:
					act(libc.CString("@RNoxious fumes rise from the bubbling lava!@n"), 0, d.Character, nil, nil, TO_ROOM)
					act(libc.CString("@RNoxious fumes rise from the bubbling lava!@n"), 0, d.Character, nil, nil, TO_CHAR)
				case 4:
					act(libc.CString("@RSome of the lava cools as it spreads further from the source!@n"), 0, d.Character, nil, nil, TO_ROOM)
					act(libc.CString("@RSome of the lava cools as it spreads further from the source!@n"), 0, d.Character, nil, nil, TO_CHAR)
				}
				world[d.Character.In_room].Geffect += 1
			} else if world[d.Character.In_room].Geffect == 5 {
				act(libc.CString("@RLava covers the entire area now!@n"), 0, d.Character, nil, nil, TO_ROOM)
				act(libc.CString("@RLava covers the entire area now!@n"), 0, d.Character, nil, nil, TO_CHAR)
				world[d.Character.In_room].Geffect += 1
			}
		}
		if d.Character.Absorbing != nil && d.Character.In_room != d.Character.Absorbing.In_room {
		...
		if int(d.Character.Race) == RACE_ANDROID && d.Character.Absorbing != nil {
			if d.Character.Absorbing.Move < (d.Character.Max_move/15) && d.Character.Absorbing.Mana < (d.Character.Max_mana/15) {
				act(libc.CString("@WYou stop absorbing stamina and ki from @c$N as they don't have enough for you to take@W!@n"), 1, d.Character, nil, unsafe.Pointer(d.Character.Absorbing), TO_CHAR)
				act(libc.CString("@C$n@W stops absorbing stamina and ki from you!@n"), 1, d.Character, nil, unsafe.Pointer(d.Character.Absorbing), TO_VICT)
				act(libc.CString("@C$n@W stops absorbing stamina and ki from @c$N@w!@n"), 1, d.Character, nil, unsafe.Pointer(d.Character.Absorbing), TO_NOTVICT)
		...
		if int(d.Character.Race) == RACE_ANDROID && d.Character.Absorbing != nil && rand_number(1, 9) >= 6 {
			if d.Character.Absorbing.Move > (d.Character.Max_move/15) || d.Character.Absorbing.Mana > (d.Character.Max_mana/15) {
				...
				act(libc.CString("@WYou absorb stamina and ki from @c$N@W!@n"), 1, d.Character, nil, unsafe.Pointer(d.Character.Absorbing), TO_CHAR)
				act(libc.CString("@C$n@W absorbs stamina and ki from you!@n"), 1, d.Character, nil, unsafe.Pointer(d.Character.Absorbing), TO_VICT)
				send_to_char(d.Character.Absorbing, libc.CString("@wTry 'escape'!@n\r\n"))
				act(libc.CString("@C$n@W absorbs stamina and ki from @c$N@w!@n"), 1, d.Character, nil, unsafe.Pointer(d.Character.Absorbing), TO_NOTVICT)
				...
				if d.Character.Move > d.Character.Max_move && d.Character.Mana < d.Character.Max_mana {
					...
					d.Character.Mana = d.Character.Max_mana
					d.Character.Move = d.Character.Max_move
					act(libc.CString("@WYou stop absorbing stamina and ki from @c$N as you are full@W!@n"), 1, d.Character, nil, unsafe.Pointer(d.Character.Absorbing), TO_CHAR)
					act(libc.CString("@C$n@W stops absorbing stamina and ki from you!@n"), 1, d.Character, nil, unsafe.Pointer(d.Character.Absorbing), TO_VICT)
					act(libc.CString("@C$n@W stops absorbing stamina and ki from @c$N@w!@n"), 1, d.Character, nil, unsafe.Pointer(d.Character.Absorbing), TO_NOTVICT)
					if d.Character.Fighting == nil || d.Character.Fighting != d.Character.Absorbing {
						set_fighting(d.Character, d.Character.Absorbing.Absorbby)
				...
				if sum == 1 {
					if rand_number(1, 8) >= 6 {
						...
						if int(libc.BoolToInt(group_bonus(d.Character, 2))) == 7 {
		...
		if d.Character.Overf == 1 {
			mudlog(NRM, ADMLVL_GOD, 1, libc.CString("OVERFLOW: %s has caused an overflow, check for illegal activity."), GET_NAME(d.Character))
			d.Character.Overf = 0
		}
...
func do_snet(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if libc.StrStr(&arg[0], libc.CString("#")) != nil {
	...
	if libc.StrCaseCmp(&arg[0], libc.CString("check")) == 0 {
	...
	if channel > 0 {
		...
		act(libc.CString("@wYou push some buttons on $p@w and change its channel."), 1, ch, obj, nil, TO_CHAR)
		act(libc.CString("@C$n@w pushes some buttons on $p@w and changes its channel."), 1, ch, obj, nil, TO_ROOM)
		return
	} else {
		...
		if libc.StrCaseCmp(&arg[0], libc.CString("*")) == 0 && call <= -1 {
			global = 1
		}
		if ch.Voice != nil {
		...
		for i = descriptor_list; i != nil; i = i.Next {
			...
			if (i.Character.Equipment[WEAR_EYE]) != nil {
				...
				if global == 0 && call <= -1 && obj2.Scoutfreq == obj.Scoutfreq && i.Character.Admlevel < 1 {
					...
					continue
				} else if global == 1 && call <= -1 && i.Character.Admlevel < 1 {
					...
					continue
				} else if call > -1 && int(i.Character.Id) == call {
		...
		if call <= -1 {
			if global == 0 {
				...
				act(&over[0], 1, ch, nil, nil, TO_ROOM)
...
func do_scouter(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if obj == nil {
		...
		reveal_hiding(ch, 3)
		if libc.StrCaseCmp(libc.CString("scan"), &arg[0]) == 0 {
			for i = descriptor_list; i != nil; i = i.Next {
				if i.Connected != CON_PLAYING {
					continue
		...
		if int(vict.Race) == RACE_ANDROID {
			act(libc.CString("$n points $s scouter at you."), 0, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("$n points $s scouter at $N."), 0, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			send_to_char(ch, libc.CString("@D,==================================|@n\r\n"))
			...
			if OBJ_FLAGGED(obj, ITEM_BSCOUTER) && vict.Hit >= 150000 {
				...
				act(libc.CString("$n's scouter explodes!"), 0, ch, nil, nil, TO_ROOM)
				extract_obj(obj)
				save_char(ch)
				return
...
func do_quit(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if subcmd != SCMD_QUIT {
	...
	} else if int(ch.Position) == POS_FIGHTING {
		send_to_char(ch, libc.CString("No way!  You're fighting for your life!\r\n"))
	} else if int(ch.Position) < POS_STUNNED {
		send_to_char(ch, libc.CString("You die before your time...\r\n"))
		die(ch, nil)
	} else {
		act(libc.CString("$n has left the game."), 1, ch, nil, nil, TO_ROOM)
		mudlog(NRM, int(MAX(ADMLVL_IMMORT, int64(ch.Player_specials.Invis_level))), 1, libc.CString("%s has quit the game."), GET_NAME(ch))
		...
		if ROOM_FLAGGED(ch.In_room, ROOM_PAST) {
			if !GET_ROOM_VNUM(ch.In_room) && GET_ROOM_VNUM(ch.In_room) && !GET_ROOM_VNUM(ch.In_room) {
				ch.Player_specials.Load_room = int(libc.BoolToInt(GET_ROOM_VNUM(real_room(1561))))
			}
		}
		Crash_rentsave(ch, 0)
		cp(ch)
...
func do_steal(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if (func() *char_data {
		...
		send_to_char(ch, libc.CString("Come on now, that's rather stupid!\r\n"))
		return
	} else if !can_kill(ch, vict, nil, 0) {
		return
	...
	if axion_dice(0) > 100 && int(vict.Position) != POS_SLEEPING {
		reveal_hiding(ch, 0)
		act(libc.CString("@r$N@R just happens to glance in your direction! What terrible luck!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@RYou just happen to glance behind you and spot @r$n@R trying to STEAL from you!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("@r$N@R just happens to glance in @r$n's@R direction and catches $m trying to STEAL!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		prob = -1000
	}
	if prob+20 < perc && int(vict.Position) != POS_SLEEPING {
		reveal_hiding(ch, 0)
		act(libc.CString("@rYou are caught trying to stick your hand in @R$N's@r possessions!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@rYou catch @R$n@r trying to rummage through your possessions!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("@R$n@R is caught by @R$N@r as $e sticks $s hand in @R$N's@r possessions!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		WAIT_STATE(ch, (int(1000000/OPT_USEC))*3)
		if IS_NPC(vict) {
			set_fighting(vict, ch)
		}
		...
		if libc.StrCaseCmp(&arg[0], libc.CString("zenni")) == 0 {
			if prob > perc {
				if vict.Gold > 0 {
					if vict.Gold > 100 {
					...
					if !IS_NPC(vict) {
						...
						mudlog(NRM, int(MAX(ADMLVL_GRGOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("THEFT: %s has stolen %s zenni@n from %s"), GET_NAME(ch), add_commas(int64(gold)), GET_NAME(vict))
					}
					if gold > 1 {
					...
					if axion_dice(0) > prob {
						...
						reveal_hiding(ch, 0)
						act(libc.CString("@R$n@r just stole zenni from @R$N@r!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_ROOM)
				...
				reveal_hiding(ch, 0)
				act(libc.CString("@rYou are caught trying to steal zenni from @R$N@r!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou catch @R$n's@r hand trying to snatch your zenni!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r catches @R$n's@r hand trying to snatch $S zenni!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				WAIT_STATE(ch, (int(1000000/OPT_USEC))*3)
				if IS_NPC(vict) {
					set_fighting(vict, ch)
				}
			...
			if (func() *obj_data {
				...
				for eq_pos = 0; eq_pos < NUM_WEARS; eq_pos++ {
					if (vict.Equipment[eq_pos]) != nil && isname(&arg[0], (vict.Equipment[eq_pos]).Name) && CAN_SEE_OBJ(ch, vict.Equipment[eq_pos]) {
						obj = vict.Equipment[eq_pos]
						break
					}
				}
				if obj == nil {
					act(libc.CString("$E isn't wearing that item."), 0, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					return
				} else if int(vict.Position) > POS_SLEEPING {
					...
					send_to_char(ch, libc.CString("You can't steal that!\r\n"))
					return
				} else if int(obj.Type_flag) == ITEM_KEY {
					...
					act(libc.CString("You unequip $p and steal it."), 0, ch, obj, unsafe.Pointer(vict), TO_CHAR)
					if axion_dice(0) > prob {
						send_to_char(ch, libc.CString("You think that your movements might have been a bit obvious.\r\n"))
						reveal_hiding(ch, 0)
						act(libc.CString("@R$n@r just stole $p@r from @R$N@r!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_ROOM)
					...
					reveal_hiding(ch, 0)
					vict.Position = POS_SITTING
					act(libc.CString("@rYou are caught trying to steal $p@r from @R$N@r!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("@rYou feel your body being shifted while you sleep and wake up to find @R$n@r trying to steal $p@r from you!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("@R$N@r catches @R$n's@r trying to $p@r from $M during $S sleep!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_NOTVICT)
					WAIT_STATE(ch, (int(1000000/OPT_USEC))*3)
					if IS_NPC(vict) {
				...
				if !give_otrigger(obj, vict, ch) || !receive_mtrigger(ch, vict, obj) {
					...
					act(libc.CString("You steal $p from $N."), 0, ch, obj, unsafe.Pointer(vict), TO_CHAR)
					obj_from_char(obj)
					obj_to_char(obj, ch)
					if !IS_NPC(vict) {
						SET_BIT_AR(vict.Act[:], PLR_STOLEN)
						mudlog(NRM, int(MAX(ADMLVL_GRGOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("THEFT: %s has stolen %s@n from %s"), GET_NAME(ch), obj.Short_description, GET_NAME(vict))
					}
					if axion_dice(0) > prob {
						reveal_hiding(ch, 0)
						send_to_char(ch, libc.CString("You think that your movements might have been a bit obvious.\r\n"))
					...
					reveal_hiding(ch, 0)
					act(libc.CString("@rYou are caught trying to steal $p@r from @R$N@r!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("@rYou catch @R$n@r trying to steal $p@r from you!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("@R$N@r catches @R$n's@r trying to $p@r!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_NOTVICT)
					WAIT_STATE(ch, (int(1000000/OPT_USEC))*3)
					if IS_NPC(vict) {
						vict.Position = POS_STANDING
						set_fighting(vict, ch)
...
func do_visible(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if AFF_FLAGGED(ch, AFF_ETHEREAL) && affectedv_by_spell(ch, ART_EMPTY_BODY) {
		...
		if AFF_FLAGGED(ch, AFF_ETHEREAL) {
			...
			if appeared == 0 {
				act(libc.CString("$n flashes into existence."), 0, ch, nil, nil, TO_ROOM)
			}
		}
		appeared = 1
	}
...
func do_title(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	} else if libc.StrStr(argument, libc.CString("(")) != nil || libc.StrStr(argument, libc.CString(")")) != nil {
		send_to_char(ch, libc.CString("Titles can't contain the ( or ) characters.\r\n"))
	} else if libc.StrLen(argument) > MAX_TITLE_LENGTH {
...
func perform_group(ch *char_data, vict *char_data, highlvl int, lowlvl int, highpl int64, lowpl int64) int {
	...
	if (vict.Bonuses[BONUS_LONER]) > 0 {
		act(libc.CString("$n is the loner type and refuses to be in your group."), 1, vict, nil, unsafe.Pointer(ch), TO_VICT)
		return 0
	}
	if GET_LEVEL(vict)+12 < highlvl {
		act(libc.CString("$n isn't experienced enough to be in your group with its current members."), 1, vict, nil, unsafe.Pointer(ch), TO_VICT)
		return 0
	}
	if GET_LEVEL(vict) > lowlvl+12 {
		act(libc.CString("$n is too experienced to be in your group with its current members."), 1, vict, nil, unsafe.Pointer(ch), TO_VICT)
		return 0
	}
	if highlvl >= 100 {
		if float64(gear_pl(vict)) > float64(highpl)*1.5 {
			act(libc.CString("$n is too powerful right now to be in a level 100 group with you."), 1, vict, nil, unsafe.Pointer(ch), TO_VICT)
			return 0
		}
		if float64(gear_pl(vict)) < float64(lowpl)*0.5 {
			act(libc.CString("$n is too weak right now to be in a level 100 group with you."), 1, vict, nil, unsafe.Pointer(ch), TO_VICT)
			return 0
		}
	}
	SET_BIT_AR(vict.Affected_by[:], AFF_GROUP)
	if ch != vict {
		act(libc.CString("$N is now a member of your group."), 0, ch, nil, unsafe.Pointer(vict), TO_CHAR)
	}
	act(libc.CString("You are now a member of $n's group."), 0, ch, nil, unsafe.Pointer(vict), TO_VICT)
	act(libc.CString("$N is now a member of $n's group."), 0, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
	return 1
}
func print_group(ch *char_data) {
	...
	if !AFF_FLAGGED(ch, AFF_GROUP) {
		...
		if AFF_FLAGGED(k, AFF_GROUP) {
			...
			act(&buf[0], 0, ch, nil, unsafe.Pointer(k), TO_CHAR)
		}
		for f = k.Followers; f != nil; f = f.Next {
			...
			act(&buf[0], 0, ch, nil, unsafe.Pointer(f.Follower), TO_CHAR)
		}
		send_to_char(ch, libc.CString("@D----------------@n\r\n"))
	}
}
func do_group(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if ch.Master != nil {
		act(libc.CString("You cannot enroll group members without being head of a group."), 0, ch, nil, nil, TO_CHAR)
		return
	}
	...
	var foundwas int = 0
	if libc.StrCaseCmp(&buf[0], libc.CString("all")) == 0 {
	...
	if (func() *char_data {
		...
		act(libc.CString("$N must follow you to enter your group."), 0, ch, nil, unsafe.Pointer(vict), TO_CHAR)
	} else {
		if !AFF_FLAGGED(vict, AFF_GROUP) {
			...
			if ch != vict {
				act(libc.CString("$N is no longer a member of your group."), 0, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			}
			act(libc.CString("You have been kicked out of $n's group!"), 0, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("$N has been kicked out of $n's group!"), 0, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
...
func do_ungroup(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if buf[0] == 0 {
		...
		for f = ch.Followers; f != nil; f = next_fol {
			...
			if AFF_FLAGGED(f.Follower, AFF_GROUP) {
				...
				act(libc.CString("$N has disbanded the group."), 1, f.Follower, nil, unsafe.Pointer(ch), TO_CHAR)
	...
	act(libc.CString("$N is no longer a member of your group."), 0, ch, nil, unsafe.Pointer(tch), TO_CHAR)
	act(libc.CString("You have been kicked out of $n's group!"), 0, ch, nil, unsafe.Pointer(tch), TO_VICT)
	act(libc.CString("$N has been kicked out of $n's group!"), 0, ch, nil, unsafe.Pointer(tch), TO_NOTVICT)
...
func do_report(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	for f = k.Followers; f != nil; f = f.Next {
		if AFF_FLAGGED(f.Follower, AFF_GROUP) && f.Follower != ch {
			act(&buf[0], 1, ch, nil, unsafe.Pointer(f.Follower), TO_VICT)
		}
	}
	if k != ch {
		act(&buf[0], 1, ch, nil, unsafe.Pointer(k), TO_VICT)
	}
...
func do_use(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	switch subcmd {
	case SCMD_QUAFF:
		if int(mag_item.Type_flag) != ITEM_POTION {
	...
	case SCMD_RECITE:
		if int(mag_item.Type_flag) != ITEM_SCROLL {
	...
	case SCMD_USE:
		if int(ch.Race) == RACE_ANDROID {
			...
			case 381:
				...
				act(libc.CString("@WYou place the $p@W against your chest and feel a rush of stamina as it automatically administers the dose.@n"), 1, ch, mag_item, nil, TO_CHAR)
				act(libc.CString("@C$n@W places an $p@W against $s chest and a loud click is heard.@n"), 1, ch, mag_item, nil, TO_ROOM)
			...
			case 382:
				if AFF_FLAGGED(ch, AFF_BURNED) {
					act(libc.CString("@WYou gently apply the salve to your burns.@n"), 1, ch, mag_item, nil, TO_CHAR)
					act(libc.CString("@C$n@W gently applies a burn salve to $s burns.@n"), 1, ch, mag_item, nil, TO_ROOM)
			...
			case 383:
				if AFF_FLAGGED(ch, AFF_POISON) {
					act(libc.CString("@WYou place the $p@W against your neck and feel a rush of relief as the antitoxiin enters your bloodstream.@n"), 1, ch, mag_item, nil, TO_CHAR)
					act(libc.CString("@C$n@W places an $p@W against $s neck and a loud click is heard.@n"), 1, ch, mag_item, nil, TO_ROOM)
					null_affect(ch, AFF_POISON)
					extract_obj(mag_item)
				} else {
					send_to_char(ch, libc.CString("You are not poisoned.\r\n"))
				...
				return
			case 385:
				act(libc.CString("@WYou drink the contents of the vial before disposing of it.@n"), 1, ch, mag_item, nil, TO_CHAR)
				act(libc.CString("@C$n@W dinks a $p and then disposes of it.@n"), 1, ch, mag_item, nil, TO_ROOM)
				if AFF_FLAGGED(ch, AFF_BLIND) {
					act(libc.CString("@WYour eyesight has returned!@n"), 1, ch, mag_item, nil, TO_CHAR)
					act(libc.CString("@C$n@W eyesight seems to have returned.@n"), 1, ch, mag_item, nil, TO_ROOM)
					null_affect(ch, AFF_BLIND)
				}
				var refreshed int = 0
...
func do_gen_write(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	ct = libc.GetTime(nil)
	tmp = libc.AscTime(libc.LocalTime(&ct))
	if IS_NPC(ch) {
	...
	stdio.Fprintf(fl, "@D[@WUser: @c%-10s@D] [@WChar: @C%-10s@D] [@WRoom: @G%-4d@D] [@WDate: @Y%6.6s@D]@b \n-----------@w\n%s\n", func() *byte {
		if GET_USER(ch) != nil {
			return GET_USER(ch)
		}
...
func do_gen_tog(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	case SCMD_AFK:
		...
		if PRF_FLAGGED(ch, PRF_AFK) {
			act(libc.CString("$n has gone AFK."), 1, ch, nil, nil, TO_ROOM)
		} else {
			act(libc.CString("$n has come back from AFK."), 1, ch, nil, nil, TO_ROOM)
	...
	case SCMD_BUILDWALK:
		...
		if PRF_FLAGGED(ch, PRF_BUILDWALK) {
			mudlog(CMP, GET_LEVEL(ch), 1, libc.CString("OLC: %s turned buildwalk on. Allowed zone %d"), GET_NAME(ch), ch.Player_specials.Olc_zone)
		} else {
			mudlog(CMP, GET_LEVEL(ch), 1, libc.CString("OLC: %s turned buildwalk off. Allowed zone %d"), GET_NAME(ch), ch.Player_specials.Olc_zone)
		}
	case SCMD_AUTOSPLIT:
...
func do_file(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if *argument == 0 {
		...
		for func() int {
			...
			if int(fields[i].Level) <= GET_LEVEL(ch) {
				stdio.Sprintf(&buf[libc.StrLen(&buf[0])], "%-15s%s\r\n", fields[i].Cmd, fields[i].File)
			}
		}
		send_to_char(ch, &buf[0])
		return
	...
	if libc.StrCaseCmp(&field[0], libc.CString("request")) == 0 {
		ch.Lboard[2] = libc.GetTime(nil)
	}
	if value[0] == 0 {
		req_lines = 15
	} else {
	...
	if (func() *stdio.File {
		req_file = stdio.FOpen(libc.GoString(fields[l].File), "r")
		return req_file
	}()) == nil {
		mudlog(BRF, ADMLVL_IMPL, 1, libc.CString("SYSERR: Error opening file %s using 'file' command."), fields[l].File)
		return
	}
...
func do_compare(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	} else if int(obj1.Type_flag) != int(obj2.Type_flag) {
	...
	act(msg, 0, ch, obj1, unsafe.Pointer(obj2), TO_CHAR)
}
func do_break(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	act(libc.CString("$n ruins $p."), 0, ch, obj, nil, TO_ROOM)
...
func do_fix(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if libc.StrCaseCmp(libc.CString("self"), &arg[0]) == 0 {
		if int(ch.Race) != RACE_ANDROID {
	...
	for rep = ch.Carrying; rep != nil; rep = next_obj {
		...
		if custom == 0 {
			if found == 0 && GET_OBJ_VNUM(rep) == 48 && (!OBJ_FLAGGED(rep, ITEM_BROKEN) && !OBJ_FLAGGED(rep, ITEM_FORGED)) {
				found = 1
				obj4 = rep
			}
		} else {
			if found == 0 && GET_OBJ_VNUM(rep) == 0x3519 && (!OBJ_FLAGGED(rep, ITEM_BROKEN) && !OBJ_FLAGGED(rep, ITEM_FORGED)) {
				found = 1
				obj4 = rep
			}
		}
	...
	if self == 0 {
		if GET_SKILL(ch, SKILL_REPAIR) < axion_dice(0) {
			act(libc.CString("You try to repair $p but screw up.."), 1, ch, obj, nil, TO_CHAR)
			act(libc.CString("$n tries to repair $p but screws up.."), 1, ch, obj, nil, TO_ROOM)
			extract_obj(obj4)
		...
		if (obj.Value[VAL_ALL_HEALTH])+GET_SKILL(ch, SKILL_REPAIR) < 100 {
			...
			act(libc.CString("$n repairs $p a bit."), 0, ch, obj, nil, TO_ROOM)
			...
			act(libc.CString("$n repairs $p completely."), 0, ch, obj, nil, TO_ROOM)
		...
		if ch.Hit >= gear_pl(ch) {
			...
			act(libc.CString("You try to repair your body but screw up.."), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("$n tries to repair $s body but screws up.."), 1, ch, nil, nil, TO_ROOM)
			extract_obj(obj4)
			improve_skill(ch, SKILL_REPAIR, 1)
			WAIT_STATE(ch, (int(1000000/OPT_USEC))*5)
			return
		} else {
			act(libc.CString("You use the repair kit to fix part of your body..."), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("$n works on their body with a repair kit."), 1, ch, nil, nil, TO_ROOM)
...
func is_innate(ch *char_data, spellnum int) bool {
	return spellnum == SPELL_FAERIE_FIRE && int(ch.Race) == RACE_ANIMAL
}
func is_innate_ready(ch *char_data, spellnum int) bool {
	var inn *innate_node
	for inn = ch.Innate; inn != nil; inn = inn.Next {
...
func spell_in_book(obj *obj_data, spellnum int) bool {
	var i int
	if obj.Sbinfo == nil {
		return false
	}
...
func spell_in_scroll(obj *obj_data, spellnum int) bool {
	if (obj.Value[VAL_SCROLL_SPELL1]) == spellnum {
		return true
	}
	return false
}
func spell_in_domain(ch *char_data, spellnum int) bool {
...
var freeres [9]int = [9]int{1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000}
...
func do_resurrect(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	for af = ch.Affected; af != nil; af = next_af {
		next_af = af.Next
		if af.Location == APPLY_NONE && int(af.Type) == -1 && (int(af.Bitvector) == AFF_SPIRIT || int(af.Bitvector) == AFF_ETHEREAL) {
			affect_remove(ch, af)
		}
	}
	if ch.Hit < 1 {
...
func do_clan(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if ch.Admlevel >= ADMLVL_IMPL {
		if libc.StrCmp(&arg1[0], libc.CString("create")) == 0 {
			if arg2[0] == 0 {
				show_clan_info(ch)
				...
				clanCreate(&arg2[0])
				mudlog(BRF, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("(GC) %s has created a clan named %s."), GET_NAME(ch), &arg2[0])
			}
			return
			...
			return
		} else if libc.StrCmp(&arg1[0], libc.CString("bset")) == 0 {
			if arg2[0] == 0 {
				show_clan_info(ch)
			} else if !isClan(&arg2[0]) {
				send_to_char(ch, libc.CString("No clan with the name %s exists.\r\n"), &arg2[0])
			...
			return
		} else if libc.StrCmp(&arg1[0], libc.CString("reload")) == 0 {
			if arg2[0] == 0 {
				show_clan_info(ch)
			} else if !isClan(&arg2[0]) {
				send_to_char(ch, libc.CString("No clan with the name %s exists.\r\n"), &arg2[0])
	...
	if libc.StrCmp(&arg1[0], libc.CString("apply")) == 0 {
		if arg2[0] == 0 {
			show_clan_info(ch)
		} else if !isClan(&arg2[0]) {
	...
	if libc.StrCmp(&arg1[0], libc.CString("join")) == 0 {
		if arg2[0] == 0 {
			show_clan_info(ch)
		} else if !isClan(&arg2[0]) {
			send_to_char(ch, libc.CString("%s is not a valid clan.\r\n"), &arg2[0])
			...
			send_to_char(ch, libc.CString("You are already a member of %s.\r\n"), &arg2[0])
		} else if ch.Clan != nil && checkCLAN(ch) == 1 && libc.StrStr(ch.Clan, libc.CString("Applying")) == nil {
		...
		return
	} else if libc.StrCmp(&arg1[0], libc.CString("leave")) == 0 {
		if arg2[0] == 0 {
			show_clan_info(ch)
		} else if !isClan(&arg2[0]) {
			send_to_char(ch, libc.CString("%s is not a valid clan.\r\n"), &arg2[0])
		...
		return
	} else if libc.StrCmp(&arg1[0], libc.CString("infow")) == 0 {
		if ch.Clan == nil || libc.StrCmp(ch.Clan, libc.CString("None.")) == 0 {
	...
	} else if libc.StrCmp(&arg1[0], libc.CString("deposit")) == 0 {
		var bank int = 0
		if ch.Clan == nil || libc.StrCmp(ch.Clan, libc.CString("None.")) == 0 {
			send_to_char(ch, libc.CString("You are not in a clan.\r\n"))
			return
			...
			if !clanIsMember(ch.Clan, ch) && !clanIsModerator(ch.Clan, ch) {
				send_to_char(ch, libc.CString("You are not in a clan.\r\n"))
				return
			} else if !ROOM_FLAGGED(ch.In_room, ROOM_CBANK) && !clanBANY(ch.Clan, ch) {
		...
		return
	} else if libc.StrCmp(&arg1[0], libc.CString("highrank")) == 0 {
		if ch.Clan == nil || libc.StrCmp(ch.Clan, libc.CString("None.")) == 0 {
			send_to_char(ch, libc.CString("You are not in a clan.\r\n"))
			return
		} else {
	...
	} else if libc.StrCmp(&arg1[0], libc.CString("midrank")) == 0 {
		if ch.Clan == nil || libc.StrCmp(ch.Clan, libc.CString("None.")) == 0 {
			send_to_char(ch, libc.CString("You are not in a clan.\r\n"))
			return
		} else {
	...
	} else if libc.StrCmp(&arg1[0], libc.CString("rank")) == 0 {
		var (
			vict *char_data = nil
			arg3 [100]byte
			name [100]byte
		...
		if ch.Clan == nil || libc.StrCmp(ch.Clan, libc.CString("None.")) == 0 {
			...
			if !clanIsModerator(ch.Clan, ch) {
				...
				return
			} else if vict.Clan == nil || libc.StrCmp(vict.Clan, libc.CString("None.")) == 0 {
	...
	} else if libc.StrCmp(&arg1[0], libc.CString("withdraw")) == 0 {
		var bank int = 0
		if ch.Clan == nil || libc.StrCmp(ch.Clan, libc.CString("None.")) == 0 {
			send_to_char(ch, libc.CString("You are not in a clan.\r\n"))
			return
		...
		return
	} else if libc.StrCmp(&arg1[0], libc.CString("bank")) == 0 {
		var bank int = 0
		if ch.Clan == nil || libc.StrCmp(ch.Clan, libc.CString("None.")) == 0 {
			send_to_char(ch, libc.CString("You are not in a clan.\r\n"))
			return
		...
		return
	} else if libc.StrCmp(&arg1[0], libc.CString("members")) == 0 {
		if ch.Clan == nil || libc.StrCmp(ch.Clan, libc.CString("None.")) == 0 {
	...
	} else if libc.StrCmp(&arg1[0], libc.CString("expel")) == 0 {
		var (
			is_file  int = 0
			player_i int = 0
			vict     *char_data
		...
		return
	} else if libc.StrCmp(&arg1[0], libc.CString("decline")) == 0 {
		var (
			vict *char_data
			arg3 [100]byte
			name [100]byte
		...
		if arg3[0] == 0 || name[0] == 0 {
			...
			send_to_char(ch, libc.CString("%s is not a valid clan.\r\n"), &arg3[0])
		} else if !clanIsModerator(&arg3[0], ch) && !clanIsMember(&arg3[0], ch) && ch.Admlevel < ADMLVL_IMPL || ch.Crank < 2 && ch.Admlevel < ADMLVL_IMPL {
		...
		return
	} else if libc.StrCmp(&arg1[0], libc.CString("enroll")) == 0 {
		var (
			vict *char_data
			arg3 [100]byte
			name [100]byte
		...
		return
	} else if libc.StrCmp(&arg1[0], libc.CString("makemod")) == 0 {
		var (
			vict *char_data
			arg3 [100]byte
			name [100]byte
		...
		return
	} else if libc.StrCmp(&arg1[0], libc.CString("setleave")) == 0 {
		var (
			name    [100]byte
			setting [100]byte
		)
...
func do_aid(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if libc.StrCaseCmp(&arg[0], libc.CString("adrenex")) == 0 {
		num = 380
		num2 = 381
	} else if libc.StrCaseCmp(&arg[0], libc.CString("antitoxin")) == 0 {
		num = 380
		num2 = 383
	} else if libc.StrCaseCmp(&arg[0], libc.CString("salve")) == 0 {
		num = 380
		num2 = 382
	} else if libc.StrCaseCmp(&arg[0], libc.CString("formula-82")) == 0 {
		num = 380
		num2 = 385
	}
	for obj = ch.Carrying; obj != nil; obj = next_obj {
		next_obj = obj.Next_content
		if found == 0 && GET_OBJ_VNUM(obj) == num && (!OBJ_FLAGGED(obj, ITEM_BROKEN) && !OBJ_FLAGGED(obj, ITEM_FORGED)) {
			found = 1
			aid_obj = obj
		}
	}
	if found == 0 || aid_obj == nil {
		if num == 47 {
	...
	if num == 47 {
		...
		if !AFF_FLAGGED(vict, AFF_SPIRIT) && !PLR_FLAGGED(vict, PLR_BANDAGED) {
			...
			act(libc.CString("$n attempts to bandage $N's wounds."), 1, ch, nil, unsafe.Pointer(vict), TO_ROOM)
			dc = axion_dice(0)
			if (GET_SKILL(ch, SKILL_FIRST_AID) + 1) > dc {
				send_to_char(ch, libc.CString("You bandage %s's wounds.\r\n"), GET_NAME(vict))
				var roll int64 = int64(float64((gear_pl(vict)/100)*int64(int(ch.Aff_abils.Wis)/4)) + float64(gear_pl(vict))*0.25)
				if (ch.Bonuses[BONUS_HEALER]) > 0 {
				...
				act(libc.CString("$n's wounds are stablized by $N!"), 1, vict, nil, unsafe.Pointer(ch), TO_NOTVICT)
				...
				if vict != ch {
					...
					act(libc.CString("$N fails to bandage $n's wounds properly, wasting an entire set of bandages..."), 1, vict, nil, unsafe.Pointer(ch), TO_NOTVICT)
					act(libc.CString("$N fails to bandage your wounds properly, wasting an entire set of bandages..."), 1, vict, nil, unsafe.Pointer(ch), TO_CHAR)
				} else {
					act(libc.CString("$N fails to bandage $s wounds properly, wasting an entire set of bandages..."), 1, vict, nil, unsafe.Pointer(ch), TO_NOTVICT)
					act(libc.CString("You fail to bandage your wounds properly, wasting an entire set of bandages..."), 1, vict, nil, unsafe.Pointer(ch), TO_VICT)
				}
				extract_obj(aid_obj)
			}
		...
		if GET_SKILL(ch, SKILL_FIRST_AID) < 65 {
			...
			if GET_SKILL(ch, SKILL_FIRST_AID) < axion_dice(15) {
				act(libc.CString("@WYou unlock and open the TCX-M.E.C.K. case. The case hisses as its lid opens. As you begin to construct an Andrenex Adreneline Injector you screw up and end up breaking the water tight seal. The adreneline leaks out and is wasted.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n@W holds a steel case up and opens it. The case hisses as its lid opens. @C$n@W wastes no time as $e reaches into the case and begins constructing something. A frown forms on $s face as it appears that $e has failed.@n"), 1, ch, nil, nil, TO_ROOM)
				extract_obj(aid_obj)
			} else {
				act(libc.CString("@WYou unlock and open the TCX-M.E.C.K. case. The case hisses as its lid opens. Your knowledge in basic medical devices and treatments helps you as you successfully construct an Adrenex Adreneline Injector@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n@W holds a steel case up and opens it. The case hisses as its lid opens. @C$n@W wastes no time as $e reaches into the case and begins constructing something. A moment later $e holds up a completed Adrenex Adreneline Injector!@n"), 1, ch, nil, nil, TO_ROOM)
		...
		if GET_SKILL(ch, SKILL_FIRST_AID) < 50 {
			...
			if GET_SKILL(ch, SKILL_FIRST_AID) < axion_dice(10) {
				act(libc.CString("@WYou unlock and open the TCX-M.E.C.K. case. The case hisses as its lid opens. As you go to put the salve ingredients into the kit's salve compartment and set the temperature you accidentally set it too high. The salve is burned and ruined. Yes you managed to burn a burn salve.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n@W holds a steel case up and opens it. The case hisses as its lid opens. @C$n@W wastes no time as $e reaches into the case and begins constructing something. A frown forms on $s face as it appears that $e has failed.@n"), 1, ch, nil, nil, TO_ROOM)
				extract_obj(aid_obj)
			} else {
				act(libc.CString("@WYou unlock and open the TCX-M.E.C.K. case. The case hisses as its lid opens. Your knowledge in basic medical devices and treatments helps you as you successfully boil a burn salve to perfection and it is automatically placed in a jar.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n@W holds a steel case up and opens it. The case hisses as its lid opens. @C$n@W wastes no time as $e reaches into the case and begins constructing something. A moment later $e holds up a jar of burn salve!@n"), 1, ch, nil, nil, TO_ROOM)
				aid_prod = read_object(num2, VIRTUAL)
				add_unique_id(aid_prod)
				obj_to_char(aid_prod, ch)
				extract_obj(aid_obj)
		...
		if GET_SKILL(ch, SKILL_FIRST_AID) < 40 {
			...
			if GET_SKILL(ch, SKILL_FIRST_AID) < axion_dice(5) {
				act(libc.CString("@WYou unlock and open the TCX-M.E.C.K. case. The case hisses as its lid opens. As you complete the Antitoxin Injector you notice that you didn't seal the syringe properly and it all leaks out.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n@W holds a steel case up and opens it. The case hisses as its lid opens. @C$n@W wastes no time as $e reaches into the case and begins constructing something. A frown forms on $s face as it appears that $e has failed.@n"), 1, ch, nil, nil, TO_ROOM)
				extract_obj(aid_obj)
			} else {
				act(libc.CString("@WYou unlock and open the TCX-M.E.C.K. case. The case hisses as its lid opens. Your knowledge in basic medical devices and treatments helps you as you successfully assemble the Antitoxin Injector.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n@W holds a steel case up and opens it. The case hisses as its lid opens. @C$n@W wastes no time as $e reaches into the case and begins constructing something. A moment later $e holds up a completed Antitoxin Injector!@n"), 1, ch, nil, nil, TO_ROOM)
				aid_prod = read_object(num2, VIRTUAL)
				add_unique_id(aid_prod)
				obj_to_char(aid_prod, ch)
				extract_obj(aid_obj)
		...
		if GET_SKILL(ch, SKILL_FIRST_AID) < 40 {
			...
			if GET_SKILL(ch, SKILL_FIRST_AID) < axion_dice(15) {
				act(libc.CString("@WYou unlock and open the TCX-M.E.C.K. case. The case hisses as its lid opens. As you complete a vial of Formula 82 you notice that you read the mixture measurements wrong. You dispose of the vile vial immediately.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n@W holds a steel case up and opens it. The case hisses as its lid opens. @C$n@W wastes no time as $e reaches into the case and begins constructing something. A frown forms on $s face as it appears that $e has failed.@n"), 1, ch, nil, nil, TO_ROOM)
				extract_obj(aid_obj)
			} else {
				act(libc.CString("@WYou unlock and open the TCX-M.E.C.K. case. The case hisses as its lid opens. Your knowledge in basic medical devices and treatments helps you as you successfully assemble a vial of Formula 82.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n@W holds a steel case up and opens it. The case hisses as its lid opens. @C$n@W wastes no time as $e reaches into the case and begins constructing something. A moment later $e holds up a completed Vial of Formula 82!@n"), 1, ch, nil, nil, TO_ROOM)
				aid_prod = read_object(num2, VIRTUAL)
				add_unique_id(aid_prod)
				obj_to_char(aid_prod, ch)
				extract_obj(aid_obj)
...
func do_aura(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if libc.StrCaseCmp(&arg[0], libc.CString("light")) == 0 {
		if GET_SKILL(ch, SKILL_FOCUS) != 0 && GET_SKILL(ch, SKILL_CONCENTRATION) < 75 {
			...
			if PLR_FLAGGED(ch, PLR_AURALIGHT) {
				...
				act(libc.CString("$n's aura fades as they stop shining light on the area."), 1, ch, nil, nil, TO_ROOM)
