...
func say_spell(ch *char_data, spellnum int, tch *char_data, tobj *obj_data) {
	...
	if tch != nil && tch != ch && tch.In_room == ch.In_room {
		...
		act(&buf1[0], 0, ch, nil, unsafe.Pointer(tch), TO_VICT)
	}
}
...
func find_skill_num(name *byte, sktype int) int {
	...
	for skindex = 1; skindex < SKILL_TABLE_SIZE; skindex++ {
		if is_abbrev(name, spell_info[skindex].Name) && (spell_info[skindex].Skilltype&sktype) != 0 {
			return skindex
		}
		ok = 1
...
func call_magic(caster *char_data, cvict *char_data, ovict *obj_data, spellnum int, level int, casttype int, arg *byte) bool {
	...
	if ROOM_FLAGGED(caster.In_room, ROOM_PEACEFUL) && caster.Admlevel < ADMLVL_IMPL && (int(spell_info[spellnum].Violent) != 0 || IS_SET(uint32(int32(spell_info[spellnum].Routines)), 1<<0)) {
		send_to_char(caster, libc.CString("A flash of white light fills the room, dispelling your violent magic!\r\n"))
		act(libc.CString("White light from no particular source suddenly fills the room, then vanishes."), 0, caster, nil, nil, TO_ROOM)
		return false
	}
	if IS_SET(uint32(int32(spell_info[spellnum].Routines)), 1<<15) && casttype != CAST_STRIKE {
		mag_nextstrike(level, caster, spellnum)
		return true
	}
	if IS_SET(uint32(int32(spell_info[spellnum].Routines)), 1<<0) {
		if mag_damage(level, caster, cvict, spellnum) == -1 {
			return true
		}
	}
	if IS_SET(uint32(int32(spell_info[spellnum].Routines)), 1<<1) {
		mag_affects(level, caster, cvict, spellnum)
	}
	if IS_SET(uint32(int32(spell_info[spellnum].Routines)), 1<<2) {
		mag_unaffects(level, caster, cvict, spellnum)
	}
	if IS_SET(uint32(int32(spell_info[spellnum].Routines)), 1<<3) {
		mag_points(level, caster, cvict, spellnum)
	}
	if IS_SET(uint32(int32(spell_info[spellnum].Routines)), 1<<4) {
		mag_alter_objs(level, caster, ovict, spellnum)
	}
	if IS_SET(uint32(int32(spell_info[spellnum].Routines)), 1<<5) {
		mag_groups(level, caster, spellnum)
	}
	if IS_SET(uint32(int32(spell_info[spellnum].Routines)), 1<<6) {
		mag_masses(level, caster, spellnum)
	}
	if IS_SET(uint32(int32(spell_info[spellnum].Routines)), 1<<7) {
		mag_areas(level, caster, spellnum)
	}
	if IS_SET(uint32(int32(spell_info[spellnum].Routines)), 1<<8) {
		mag_summons(level, caster, ovict, spellnum, arg)
	}
	if IS_SET(uint32(int32(spell_info[spellnum].Routines)), 1<<9) {
		mag_creations(level, caster, spellnum)
	}
	if IS_SET(uint32(int32(spell_info[spellnum].Routines)), 1<<10) {
		switch spellnum {
		case SPELL_CHARM:
	...
	if IS_SET(uint32(int32(spell_info[spellnum].Routines)), 1<<11) {
...
func mag_objectmagic(ch *char_data, obj *obj_data, argument *byte) {
	...
	case ITEM_STAFF:
		act(libc.CString("You tap $p three times on the ground."), 0, ch, obj, nil, TO_CHAR)
		if obj.Action_description != nil {
			act(obj.Action_description, 0, ch, obj, nil, TO_ROOM)
		} else {
			act(libc.CString("$n taps $p three times on the ground."), 0, ch, obj, nil, TO_ROOM)
		}
		if (obj.Value[VAL_STAFF_CHARGES]) <= 0 {
			send_to_char(ch, libc.CString("It seems powerless.\r\n"))
			act(libc.CString("Nothing seems to happen."), 0, ch, obj, nil, TO_ROOM)
		} else {
			...
			if IS_SET(uint32(int32(spell_info[obj.Value[VAL_STAFF_SPELL]].Routines)), (1<<6)|1<<7) {
				for func() *char_data {
					i = 0
	...
	case ITEM_WAND:
		if k == (1 << 0) {
			if tch == ch {
				act(libc.CString("You point $p at yourself."), 0, ch, obj, nil, TO_CHAR)
				act(libc.CString("$n points $p at $mself."), 0, ch, obj, nil, TO_ROOM)
			} else {
				act(libc.CString("You point $p at $N."), 0, ch, obj, unsafe.Pointer(tch), TO_CHAR)
				if obj.Action_description != nil {
					act(obj.Action_description, 0, ch, obj, unsafe.Pointer(tch), TO_ROOM)
				} else {
					act(libc.CString("$n points $p at $N."), 1, ch, obj, unsafe.Pointer(tch), TO_ROOM)
				}
			}
		} else if tobj != nil {
			act(libc.CString("You point $p at $P."), 0, ch, obj, unsafe.Pointer(tobj), TO_CHAR)
			if obj.Action_description != nil {
				act(obj.Action_description, 0, ch, obj, unsafe.Pointer(tobj), TO_ROOM)
			} else {
				act(libc.CString("$n points $p at $P."), 1, ch, obj, unsafe.Pointer(tobj), TO_ROOM)
			}
		} else if IS_SET(uint32(int32(spell_info[obj.Value[VAL_WAND_SPELL]].Routines)), (1<<7)|1<<6) {
			act(libc.CString("You point $p outward."), 0, ch, obj, nil, TO_CHAR)
			act(libc.CString("$n points $p outward."), 1, ch, obj, nil, TO_ROOM)
		} else {
			act(libc.CString("At what should $p be pointed?"), 0, ch, obj, nil, TO_CHAR)
			return
		}
	...
	case ITEM_SCROLL:
		if arg[0] != 0 {
			if k == 0 {
				act(libc.CString("There is nothing to here to affect with $p."), 0, ch, obj, nil, TO_CHAR)
				return
			}
		} else {
			tch = ch
		}
		act(libc.CString("You recite $p which dissolves."), 1, ch, obj, nil, TO_CHAR)
		if obj.Action_description != nil {
			act(obj.Action_description, 0, ch, obj, unsafe.Pointer(tch), TO_ROOM)
		} else {
			act(libc.CString("$n recites $p."), 0, ch, obj, nil, TO_ROOM)
		}
		SET_BIT_AR(ch.Affected_by[:], AFF_NEXTNOACTION)
		for i = 1; i <= 3; i++ {
			if int(libc.BoolToInt(call_magic(ch, tch, tobj, obj.Value[i], obj.Value[VAL_SCROLL_LEVEL], CAST_SCROLL, nil))) <= 0 {
				break
			}
		}
		if obj != nil {
	...
	case ITEM_POTION:
		tch = ch
		if !consume_otrigger(obj, ch, OCMD_QUAFF) {
			return
		}
		act(libc.CString("You swallow $p."), 0, ch, obj, nil, TO_CHAR)
		if obj.Action_description != nil {
			act(obj.Action_description, 0, ch, obj, nil, TO_ROOM)
		} else {
			act(libc.CString("$n swallows $p."), 1, ch, obj, nil, TO_ROOM)
		}
		SET_BIT_AR(ch.Affected_by[:], AFF_NEXTNOACTION)
		for i = 1; i <= 3; i++ {
			if int(libc.BoolToInt(call_magic(ch, ch, nil, obj.Value[i], obj.Value[VAL_POTION_LEVEL], CAST_POTION, nil))) <= 0 {
				break
			}
		}
		if obj != nil {
...
func cast_spell(ch *char_data, tch *char_data, tobj *obj_data, spellnum int, arg *byte) int {
	...
	if !IS_SET(uint32(int32(spell_info[spellnum].Skilltype)), (1<<0)|1<<4) {
	...
	if tch != ch && IS_SET(uint32(int32(spell_info[spellnum].Targets)), 1<<5) {
		send_to_char(ch, libc.CString("You can only cast this spell upon yourself!\r\n"))
		return 0
	}
	if tch == ch && IS_SET(uint32(int32(spell_info[spellnum].Targets)), 1<<6) {
		send_to_char(ch, libc.CString("You cannot cast this spell upon yourself!\r\n"))
		return 0
	}
	if IS_SET(uint32(int32(spell_info[spellnum].Routines)), 1<<5) && !AFF_FLAGGED(ch, AFF_GROUP) {
		send_to_char(ch, libc.CString("You can't cast this spell if you're not in a group!\r\n"))
		return 0
	}
	if IS_SET(uint32(int32(spell_info[spellnum].Skilltype)), 1<<0) {
		for i = 0; i < NUM_CLASSES; i++ {
	...
	} else if IS_SET(uint32(int32(spell_info[spellnum].Skilltype)), 1<<4) {
	...
	return int(libc.BoolToInt(call_magic(ch, tch, tobj, spellnum, lvl, CAST_SPELL, arg)))
}
func do_cast(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if IS_SET(uint32(int32(spell_info[spellnum].Targets)), 1<<0) {
		target = 1
	} else if t != nil && *t != 0 {
		if target == 0 && IS_SET(uint32(int32(spell_info[spellnum].Targets)), 1<<1) {
			if (func() *char_data {
		...
		if target == 0 && IS_SET(uint32(int32(spell_info[spellnum].Targets)), 1<<2) {
			if (func() *char_data {
		...
		if target == 0 && IS_SET(uint32(int32(spell_info[spellnum].Targets)), 1<<7) {
			if (func() *obj_data {
		...
		if target == 0 && IS_SET(uint32(int32(spell_info[spellnum].Targets)), 1<<10) {
			for i = 0; target == 0 && i < NUM_WEARS; i++ {
				if (ch.Equipment[i]) != nil && isname(t, (ch.Equipment[i]).Name) {
					tobj = ch.Equipment[i]
					target = 1
				}
			}
		}
		if target == 0 && IS_SET(uint32(int32(spell_info[spellnum].Targets)), 1<<8) {
			if (func() *obj_data {
		...
		if target == 0 && IS_SET(uint32(int32(spell_info[spellnum].Targets)), 1<<9) {
			if (func() *obj_data {
		...
		if target == 0 && IS_SET(uint32(int32(spell_info[spellnum].Targets)), 1<<3) {
			if ch.Fighting != nil {
				tch = ch
				target = 1
			}
		}
		if target == 0 && IS_SET(uint32(int32(spell_info[spellnum].Targets)), 1<<4) {
			if ch.Fighting != nil {
				tch = ch.Fighting
				target = 1
			}
		}
		if target == 0 && IS_SET(uint32(int32(spell_info[spellnum].Targets)), 1<<1) && int(spell_info[spellnum].Violent) == 0 {
			tch = ch
			target = 1
		}
		if target == 0 {
			send_to_char(ch, libc.CString("Upon %s should the spell be cast?\r\n"), func() string {
				if IS_SET(uint32(int32(spell_info[spellnum].Targets)), (1<<8)|1<<7|1<<9|1<<10) {
					return "what"
				}
				return "who"
			}())
	...
	if IS_SET(uint32(int32(spell_info[spellnum].Comp_flags)), 1<<4) && rand_number(1, 100) <= int(ch.Spellfail) {
		if IS_SET(uint32(int32(spell_info[spellnum].Routines)), (1<<14)|1<<13) {
			SET_BIT_AR(ch.Affected_by[:], AFF_NEXTPARTIAL)
		} else if IS_SET(uint32(int32(spell_info[spellnum].Routines)), (1<<14)|1<<14) {
			SET_BIT_AR(ch.Affected_by[:], AFF_NEXTNOACTION)
		}
...
func mag_assign_spells() {
	...
	spello(SPELL_ANIMATE_DEAD, libc.CString("animate dead"), 35, 10, 3, POS_STANDING, 1<<8, 0, (1<<14)|1<<8, 0, 0, nil, 3, SCHOOL_NECROMANCY, DOMAIN_DEATH)
	spello(SPELL_MAGE_ARMOR, libc.CString("mage armor"), 30, 15, 3, POS_FIGHTING, 1<<1, 0, (1<<14)|1<<1, 0, 0, libc.CString("You feel less protected."), 1, SCHOOL_CONJURATION, -1)
	spello(SPELL_BLESS, libc.CString("blessed"), 35, 5, 3, POS_STANDING, (1<<1)|1<<7, 0, (1<<14)|1<<1|1<<4, 0, 0, libc.CString("You feel less righteous."), 1, -1, DOMAIN_UNIVERSAL)
	spello(SKILL_SPIRITCONTROL, libc.CString("spirit control"), 35, 5, 3, POS_STANDING, (1<<1)|1<<7, 0, (1<<14)|1<<1|1<<4, 0, 0, libc.CString("You no longer have full control of your spirit."), 1, -1, DOMAIN_UNIVERSAL)
	spello(SPELL_BLINDNESS, libc.CString("blindness"), 35, 25, 1, POS_STANDING, (1<<1)|1<<6, 0, (1<<14)|1<<1, (1<<0)|1<<4, 0, libc.CString("You feel a cloak of blindness dissolve."), 2, SCHOOL_TRANSMUTATION, DOMAIN_UNIVERSAL)
	spello(SKILL_SOLARF, libc.CString("blind"), 25, 10, 1, POS_STANDING, (1<<1)|1<<6, 1, (1<<14)|1<<11, (1<<0)|1<<4, 0, libc.CString("You are no longer blind!"), 2, SCHOOL_TRANSMUTATION, -1)
	spello(SPELL_BURNING_HANDS, libc.CString("burning hands"), 30, 10, 3, POS_FIGHTING, (1<<1)|1<<4, 1, (1<<14)|1<<0, (1<<1)|1<<3, 0, nil, 1, SCHOOL_TRANSMUTATION, DOMAIN_FIRE)
	spello(SPELL_CALL_LIGHTNING, libc.CString("call lightning"), 40, 25, 3, POS_FIGHTING, (1<<1)|1<<4, 1, (1<<14)|1<<0, (1<<1)|1<<3, 0, nil, 3, -1, DOMAIN_UNIVERSAL)
	spello(SPELL_INFLICT_CRITIC, libc.CString("inflict critic"), 30, 10, 2, POS_FIGHTING, 1<<1, 1, (1<<14)|1<<0, (1<<2)|1<<3, 0, nil, 4, -1, DOMAIN_HEALING)
	spello(SPELL_INFLICT_LIGHT, libc.CString("inflict light"), 30, 10, 2, POS_FIGHTING, 1<<1, 1, (1<<14)|1<<0, (1<<2)|1<<3, 0, nil, 1, -1, DOMAIN_HEALING)
	spello(SPELL_CHARM, libc.CString("charm person"), 75, 50, 2, POS_FIGHTING, (1<<1)|1<<6, 1, (1<<14)|1<<10, (1<<2)|1<<4, 0, libc.CString("You feel more self-confident."), 1, SCHOOL_ENCHANTMENT, -1)
	spello(SPELL_CHILL_TOUCH, libc.CString("chill touch"), 30, 10, 3, POS_FIGHTING, (1<<1)|1<<4, 1, (1<<14)|1<<0|1<<1, (1<<0)|1<<5, 0, libc.CString("You feel your strength return."), 1, SCHOOL_NECROMANCY, -1)
	spello(SPELL_COLOR_SPRAY, libc.CString("color spray"), 30, 15, 3, POS_FIGHTING, (1<<1)|1<<4, 1, (1<<14)|1<<0, (1<<2)|1<<4, 0, nil, 1, SCHOOL_ILLUSION, -1)
	spello(SPELL_CONTROL_WEATHER, libc.CString("control weather"), 75, 25, 5, POS_STANDING, 1<<0, 0, (1<<14)|1<<10, 0, 0, nil, 7, SCHOOL_TRANSMUTATION, DOMAIN_AIR)
	spello(SPELL_CREATE_FOOD, libc.CString("create food"), 30, 5, 4, POS_STANDING, 1<<0, 0, (1<<14)|1<<9, 0, 0, nil, 3, -1, DOMAIN_UNIVERSAL)
	spello(SPELL_CREATE_WATER, libc.CString("create water"), 30, 5, 4, POS_STANDING, (1<<7)|1<<10, 0, (1<<14)|1<<10, 0, 0, nil, 0, -1, DOMAIN_UNIVERSAL)
	spello(SPELL_REMOVE_BLINDNESS, libc.CString("remove blindness"), 30, 5, 2, POS_STANDING, 1<<1, 0, (1<<14)|1<<2, 0, 0, nil, 3, -1, DOMAIN_UNIVERSAL)
	spello(SPELL_CURE_CRITIC, libc.CString("cure critic"), 30, 10, 2, POS_FIGHTING, 1<<1, 0, (1<<14)|1<<3, 0, 0, nil, 4, -1, DOMAIN_HEALING)
	spello(SPELL_CURE_LIGHT, libc.CString("cure light"), 30, 10, 2, POS_FIGHTING, 1<<1, 0, (1<<14)|1<<3, 0, 0, nil, 1, -1, DOMAIN_HEALING)
	spello(SPELL_BESTOW_CURSE, libc.CString("bestow curse"), 80, 50, 2, POS_FIGHTING, 1<<1, 1, 1<<1, (1<<2)|1<<4, 0, libc.CString("You feel more optimistic."), 8, SCHOOL_NECROMANCY, DOMAIN_DESTRUCTION)
	spello(SPELL_BANE, libc.CString("bane"), 80, 50, 2, POS_FIGHTING, 1<<1, 1, 1<<1, (1<<2)|1<<4, 0, libc.CString("You feel more optimistic."), 8, SCHOOL_ENCHANTMENT, DOMAIN_CHARM)
	spello(SPELL_DETECT_ALIGN, libc.CString("detect alignment"), 20, 10, 2, POS_STANDING, (1<<1)|1<<5, 0, (1<<14)|1<<1, 0, 0, libc.CString("You feel less aware."), 1, SCHOOL_DIVINATION, DOMAIN_UNIVERSAL)
	spello(SPELL_SEE_INVIS, libc.CString("see invisibility"), 20, 10, 2, POS_STANDING, (1<<1)|1<<5, 0, (1<<14)|1<<1, 0, 0, libc.CString("Your eyes stop tingling."), 2, SCHOOL_DIVINATION, -1)
	spello(SPELL_DETECT_MAGIC, libc.CString("detect magic"), 20, 10, 2, POS_STANDING, (1<<1)|1<<5, 0, (1<<14)|1<<1, 0, 0, libc.CString("The detect magic wears off."), 0, SCHOOL_UNIVERSAL, DOMAIN_UNIVERSAL)
	spello(SPELL_DETECT_POISON, libc.CString("detect poison"), 15, 5, 1, POS_STANDING, (1<<1)|1<<7|1<<8, 0, (1<<14)|1<<10, 0, 0, libc.CString("The detect poison wears off."), 0, SCHOOL_DIVINATION, DOMAIN_UNIVERSAL)
	spello(SPELL_DISPEL_EVIL, libc.CString("dispel evil"), 40, 25, 3, POS_FIGHTING, (1<<1)|1<<4, 1, (1<<14)|1<<0, (1<<2)|1<<4, 0, nil, 5, -1, DOMAIN_GOOD)
	spello(SPELL_DISPEL_GOOD, libc.CString("dispel good"), 40, 25, 3, POS_FIGHTING, (1<<1)|1<<4, 1, (1<<14)|1<<0, (1<<2)|1<<4, 0, nil, 5, -1, DOMAIN_EVIL)
	spello(SPELL_EARTHQUAKE, libc.CString("earthquake"), 40, 25, 3, POS_FIGHTING, 1<<0, 1, (1<<14)|1<<7, (1<<1)|1<<3, 0, nil, 8, -1, int(DOMAIN_DESTRUCTION|DOMAIN_EARTH))
	spello(SPELL_ENCHANT_WEAPON, libc.CString("enchant weapon"), 150, 100, 10, POS_STANDING, 1<<7, 0, (1<<14)|1<<10, 0, 0, nil, 9, SCHOOL_TRANSMUTATION, -1)
	spello(SPELL_ENERGY_DRAIN, libc.CString("energy drain"), 40, 25, 1, POS_FIGHTING, (1<<1)|1<<4, 1, (1<<0)|1<<10, (1<<0)|1<<4, 0, nil, 9, SCHOOL_NECROMANCY, DOMAIN_UNIVERSAL)
	spello(SPELL_GROUP_ARMOR, libc.CString("group armor"), 50, 30, 2, POS_STANDING, 1<<0, 0, (1<<14)|1<<5, 0, 0, nil, 5, SCHOOL_CONJURATION, -1)
	spello(SPELL_FAERIE_FIRE, libc.CString("faerie fire"), 20, 10, 2, POS_STANDING, (1<<2)|1<<6, 0, (1<<14)|1<<10, 0, 0, nil, 1, SCHOOL_EVOCATION, DOMAIN_UNIVERSAL)
	spello(SPELL_FIREBALL, libc.CString("fireball"), 40, 30, 2, POS_FIGHTING, (1<<1)|1<<4, 1, (1<<14)|1<<0, (1<<1)|1<<3, 0, nil, 3, SCHOOL_EVOCATION, -1)
	spello(SPELL_MASS_HEAL, libc.CString("mass heal"), 80, 60, 5, POS_STANDING, 1<<0, 0, (1<<14)|1<<5, 0, 0, nil, 6, SCHOOL_CONJURATION, DOMAIN_HEALING)
	spello(SPELL_HARM, libc.CString("harm"), 75, 45, 3, POS_FIGHTING, (1<<1)|1<<4, 1, (1<<14)|1<<0, (1<<0)|1<<4, 0, nil, 6, -1, DOMAIN_DESTRUCTION)
	spello(SPELL_HEAL, libc.CString("heal"), 60, 40, 3, POS_FIGHTING, 1<<1, 0, (1<<14)|1<<3|1<<2, 0, 0, nil, 6, -1, DOMAIN_HEALING)
	spello(SPELL_SENSU, libc.CString("sensu"), 1, 0, 3, POS_STANDING, 1<<1, 0, (1<<14)|1<<3, 0, 0, nil, 6, SCHOOL_DIVINATION, DOMAIN_HEALING)
	spello(SPELL_IDENTIFY, libc.CString("identify"), 50, 25, 5, POS_STANDING, (1<<7)|1<<8, 0, (1<<14)|1<<10, 0, 0, nil, 2, SCHOOL_DIVINATION, DOMAIN_MAGIC)
	spello(SPELL_DARKVISION, libc.CString("darkvision"), 25, 10, 1, POS_STANDING, (1<<1)|1<<5, 0, (1<<14)|1<<1, 0, 0, libc.CString("Your night vision seems to fade."), 2, -1, -1)
	spello(SPELL_INVISIBLE, libc.CString("invisibility"), 35, 25, 1, POS_STANDING, (1<<1)|1<<7|1<<8, 0, (1<<14)|1<<1|1<<4, 0, 0, libc.CString("You feel yourself exposed."), 2, SCHOOL_ILLUSION, DOMAIN_TRICKERY)
	spello(SPELL_LIGHTNING_BOLT, libc.CString("lightning bolt"), 30, 15, 1, POS_FIGHTING, (1<<1)|1<<4, 1, (1<<14)|1<<0, (1<<1)|1<<3, 0, nil, 3, SCHOOL_EVOCATION, -1)
	spello(SPELL_LOCATE_OBJECT, libc.CString("locate object"), 25, 20, 1, POS_STANDING, 1<<9, 0, (1<<14)|1<<10, 0, 0, nil, 3, SCHOOL_DIVINATION, DOMAIN_TRAVEL)
	spello(SPELL_MAGIC_MISSILE, libc.CString("magic missile"), 25, 10, 3, POS_FIGHTING, (1<<1)|1<<4, 1, (1<<14)|1<<0, 0, 0, nil, 1, SCHOOL_EVOCATION, -1)
	spello(SPELL_PARALYZE, libc.CString("stone"), 25, 10, 1, POS_STANDING, (1<<1)|1<<6, 1, (1<<14)|1<<11, (1<<0)|1<<4, 0, libc.CString("Your body is no longer petrified."), 2, SCHOOL_TRANSMUTATION, -1)
	spello(SKILL_HASSHUKEN, libc.CString("hasshuken"), 25, 10, 1, POS_STANDING, (1<<1)|1<<6, 1, (1<<14)|1<<11, (1<<0)|1<<4, 0, libc.CString("Your arms slow down."), 2, SCHOOL_TRANSMUTATION, -1)
	spello(SKILL_CURSE, libc.CString("curse"), 25, 10, 1, POS_STANDING, (1<<1)|1<<6, 1, (1<<14)|1<<11, (1<<0)|1<<4, 0, libc.CString("You are no longer cursed!"), 2, SCHOOL_TRANSMUTATION, -1)
	spello(SKILL_MIGHT, libc.CString("might"), 25, 10, 1, POS_STANDING, (1<<1)|1<<6, 1, (1<<14)|1<<11, (1<<0)|1<<4, 0, libc.CString("Your strength fades."), 2, SCHOOL_TRANSMUTATION, -1)
	spello(SKILL_PARALYZE, libc.CString("paralyze"), 25, 10, 1, POS_STANDING, (1<<1)|1<<6, 1, (1<<14)|1<<11, (1<<0)|1<<4, 0, libc.CString("Your feel like you are able to move again."), 2, SCHOOL_TRANSMUTATION, -1)
	spello(SKILL_POISON, libc.CString("poison"), 50, 20, 3, POS_STANDING, (1<<1)|1<<6|1<<7, 1, (1<<14)|1<<1|1<<4, (1<<0)|1<<4, 0, libc.CString("You feel like you got over something."), 4, SCHOOL_NECROMANCY, DOMAIN_UNIVERSAL)
	spello(SKILL_POISON, libc.CString("dark metamorphosis"), 50, 20, 3, POS_STANDING, (1<<1)|1<<6|1<<7, 1, (1<<14)|1<<1|1<<4, (1<<0)|1<<4, 0, libc.CString("Your dark metamorphosis fades."), 4, SCHOOL_NECROMANCY, DOMAIN_UNIVERSAL)
	spello(SKILL_POISON, libc.CString("healing glow"), 50, 20, 3, POS_STANDING, (1<<1)|1<<6|1<<7, 1, (1<<14)|1<<1|1<<4, (1<<0)|1<<4, 0, libc.CString("Your healing glow fades."), 4, SCHOOL_NECROMANCY, DOMAIN_UNIVERSAL)
	spello(SKILL_ENLIGHTEN, libc.CString("enlighten"), 25, 10, 1, POS_STANDING, (1<<1)|1<<6, 1, (1<<14)|1<<11, (1<<0)|1<<4, 0, libc.CString("You feel less wise."), 2, SCHOOL_TRANSMUTATION, -1)
	spello(SKILL_GENIUS, libc.CString("genius"), 25, 10, 1, POS_STANDING, (1<<1)|1<<6, 1, (1<<14)|1<<11, (1<<0)|1<<4, 0, libc.CString("You am dumb dumbner now."), 2, SCHOOL_TRANSMUTATION, -1)
	spello(SKILL_FLEX, libc.CString("flex"), 25, 10, 1, POS_STANDING, (1<<1)|1<<6, 1, (1<<14)|1<<11, (1<<0)|1<<4, 0, libc.CString("You feel less agile."), 2, SCHOOL_TRANSMUTATION, -1)
	spello(SPELL_PORTAL, libc.CString("portal"), 75, 75, 0, POS_STANDING, (1<<2)|1<<6, 0, (1<<14)|1<<10, 0, 0, nil, 7, SCHOOL_CONJURATION, -1)
	spello(SPELL_PROT_FROM_EVIL, libc.CString("protection from evil"), 40, 10, 3, POS_STANDING, (1<<1)|1<<5, 0, (1<<14)|1<<1, 0, 0, libc.CString("You feel less protected."), 1, SCHOOL_ABJURATION, DOMAIN_GOOD)
	spello(SPELL_REMOVE_CURSE, libc.CString("remove curse"), 45, 25, 5, POS_STANDING, (1<<1)|1<<7|1<<10, 0, (1<<14)|1<<2|1<<4, 0, 0, nil, 3, SCHOOL_ABJURATION, DOMAIN_UNIVERSAL)
	spello(SPELL_NEUTRALIZE_POISON, libc.CString("neutralize poison"), 40, 8, 4, POS_STANDING, (1<<1)|1<<7|1<<8, 0, (1<<14)|1<<2|1<<4, 0, 0, nil, 4, SCHOOL_CONJURATION, DOMAIN_UNIVERSAL)
	spello(SPELL_SANCTUARY, libc.CString("sanctuary"), 110, 85, 5, POS_STANDING, 1<<1, 0, (1<<14)|1<<1, 0, 0, libc.CString("The white aura around your body fades."), 9, -1, DOMAIN_PROTECTION)
	spello(SPELL_SENSE_LIFE, libc.CString("sense life"), 20, 10, 2, POS_STANDING, (1<<1)|1<<5, 0, (1<<14)|1<<1, 0, 0, libc.CString("You feel less aware of your surroundings."), 2, SCHOOL_DIVINATION, DOMAIN_UNIVERSAL)
	spello(SPELL_SHOCKING_GRASP, libc.CString("shocking grasp"), 30, 15, 3, POS_FIGHTING, (1<<1)|1<<4, 1, (1<<14)|1<<0, 0, 0, nil, 1, SCHOOL_TRANSMUTATION, -1)
	spello(SPELL_SLEEP, libc.CString("sleep"), 40, 25, 5, POS_FIGHTING, 1<<1, 1, (1<<14)|1<<1, (1<<2)|1<<4, 0, libc.CString("You feel like you can wake up again."), 1, SCHOOL_ENCHANTMENT, -1)
	spello(SPELL_HAYASA, libc.CString("hayasa"), 40, 25, 5, POS_FIGHTING, 1<<1, 1, (1<<14)|1<<1, (1<<2)|1<<4, 0, libc.CString("You feel your speed decrease as Hayasa fades."), 1, SCHOOL_ENCHANTMENT, -1)
	spello(SKILL_TSKIN, libc.CString("tough skin"), 25, 10, 1, POS_STANDING, (1<<1)|1<<6, 1, (1<<14)|1<<11, (1<<0)|1<<4, 0, libc.CString("Your skin isn't quite so thick anymore."), 2, SCHOOL_TRANSMUTATION, -1)
	spello(SPELL_BULL_STRENGTH, libc.CString("bull strength"), 35, 30, 1, POS_STANDING, 1<<1, 0, (1<<14)|1<<1, 0, 0, libc.CString("You feel weaker."), 2, SCHOOL_TRANSMUTATION, DOMAIN_STRENGTH)
	spello(SPELL_SUMMON, libc.CString("summon"), 75, 50, 3, POS_STANDING, (1<<2)|1<<6, 0, (1<<14)|1<<10, 0, 0, nil, 7, SCHOOL_CONJURATION, DOMAIN_UNIVERSAL)
	spello(SPELL_TELEPORT, libc.CString("teleport"), 75, 50, 3, POS_STANDING, 1<<1, 0, (1<<14)|1<<10, 0, 0, nil, 5, SCHOOL_TRANSMUTATION, DOMAIN_TRAVEL)
	spello(SPELL_WATERWALK, libc.CString("waterwalk"), 40, 20, 2, POS_STANDING, 1<<1, 0, (1<<14)|1<<1, 0, 0, libc.CString("Your feet seem less buoyant."), 3, -1, DOMAIN_UNIVERSAL)
	spello(SPELL_WORD_OF_RECALL, libc.CString("word of recall"), 20, 10, 2, POS_FIGHTING, 1<<1, 0, (1<<14)|1<<10, 0, 0, nil, 6, -1, DOMAIN_UNIVERSAL)
	spello(SPELL_RESISTANCE, libc.CString("resistance"), 40, 20, 0, POS_STANDING, 1<<1, 0, (1<<14)|1<<11, 0, (1<<3)|1<<4|1<<5|1<<0, nil, 0, SCHOOL_ABJURATION, -1)
	spello(SPELL_ACID_SPLASH, libc.CString("acid splash"), 40, 20, 0, POS_STANDING, 1<<1, 0, (1<<14)|1<<1, 0, (1<<4)|1<<5, nil, 0, SCHOOL_CONJURATION, -1)
	spello(SPELL_DAZE, libc.CString("daze"), 40, 20, 2, POS_STANDING, 1<<1, 0, (1<<14)|1<<11, (1<<2)|1<<4, (1<<3)|1<<4|1<<5, nil, 0, SCHOOL_ENCHANTMENT, -1)
	spello(SPELL_FLARE, libc.CString("flare"), 40, 20, 2, POS_STANDING, 1<<1, 0, (1<<14)|1<<11, (1<<0)|1<<4, 1<<5, nil, 0, SCHOOL_EVOCATION, -1)
	spello(SPELL_RAY_OF_FROST, libc.CString("ray of frost"), 40, 20, 2, POS_STANDING, 1<<1, 0, (1<<14)|1<<1, 0, (1<<4)|1<<5, nil, 0, SCHOOL_EVOCATION, -1)
	spello(SPELL_DISRUPT_UNDEAD, libc.CString("disrupt undead"), 40, 20, 2, POS_STANDING, 1<<1, 0, (1<<14)|1<<1, 0, (1<<4)|1<<5, nil, 0, SCHOOL_NECROMANCY, -1)
	spello(SPELL_SUMMON_MONSTER_I, libc.CString("summon monster i"), 40, 20, 2, POS_FIGHTING, 1<<0, 0, (1<<14)|1<<8, 0, 0, nil, 1, SCHOOL_CONJURATION, -1)
	spello(SPELL_SUMMON_MONSTER_II, libc.CString("summon monster ii"), 40, 20, 2, POS_FIGHTING, 1<<0, 0, (1<<14)|1<<8, 0, 0, nil, 2, SCHOOL_CONJURATION, -1)
	spello(SPELL_SUMMON_MONSTER_III, libc.CString("summon monster iii"), 40, 20, 2, POS_FIGHTING, 1<<0, 0, (1<<14)|1<<8, 0, 0, nil, 3, SCHOOL_CONJURATION, -1)
	spello(SPELL_SUMMON_MONSTER_IV, libc.CString("summon monster iv"), 40, 20, 2, POS_FIGHTING, 1<<0, 0, (1<<14)|1<<8, 0, 0, nil, 4, SCHOOL_CONJURATION, -1)
	spello(SPELL_SUMMON_MONSTER_V, libc.CString("summon monster v"), 40, 20, 2, POS_FIGHTING, 1<<0, 0, (1<<14)|1<<8, 0, 0, nil, 5, SCHOOL_CONJURATION, -1)
	spello(SPELL_SUMMON_MONSTER_VI, libc.CString("summon monster vi"), 40, 20, 2, POS_FIGHTING, 1<<0, 0, (1<<14)|1<<8, 0, 0, nil, 6, SCHOOL_CONJURATION, -1)
	spello(SPELL_SUMMON_MONSTER_VII, libc.CString("summon monster vii"), 40, 20, 2, POS_FIGHTING, 1<<0, 0, (1<<14)|1<<8, 0, 0, nil, 7, SCHOOL_CONJURATION, -1)
	spello(SPELL_SUMMON_MONSTER_VIII, libc.CString("summon monster viii"), 40, 20, 2, POS_FIGHTING, 1<<0, 0, (1<<14)|1<<8, 0, 0, nil, 8, SCHOOL_CONJURATION, -1)
	spello(SPELL_SUMMON_MONSTER_IX, libc.CString("summon monster ix"), 40, 20, 2, POS_FIGHTING, 1<<0, 0, (1<<14)|1<<8, 0, 0, nil, 9, SCHOOL_CONJURATION, -1)
	spello(SPELL_FIRE_SHIELD, libc.CString("fire shield"), 40, 20, 2, POS_FIGHTING, 1<<1, 0, (1<<14)|1<<11, 0, 0, nil, 4, SCHOOL_EVOCATION, DOMAIN_FIRE)
	spello(SPELL_ICE_STORM, libc.CString("ice storm"), 40, 20, 2, POS_STANDING, 1<<1, 0, (1<<14)|1<<1, 0, 0, nil, 0, -1, -1)
	spello(SPELL_SHOUT, libc.CString("shout"), 40, 20, 2, POS_STANDING, 1<<1, 0, (1<<14)|1<<1, 0, 0, nil, 0, -1, -1)
	spello(SPELL_FEAR, libc.CString("fear"), 40, 20, 2, POS_STANDING, 1<<1, 0, (1<<14)|1<<1, 0, 0, nil, 0, -1, -1)
	spello(SPELL_CLOUDKILL, libc.CString("cloudkill"), 40, 20, 2, POS_STANDING, 1<<1, 0, (1<<14)|1<<1, 0, 0, nil, 0, -1, -1)
	spello(SPELL_MAJOR_CREATION, libc.CString("major creation"), 40, 20, 2, POS_STANDING, 1<<1, 0, (1<<14)|1<<1, 0, 0, nil, 0, -1, -1)
	spello(SPELL_HOLD_MONSTER, libc.CString("hold monster"), 40, 20, 2, POS_STANDING, 1<<1, 0, (1<<14)|1<<1, 0, 0, nil, 0, -1, -1)
	spello(SPELL_CONE_OF_COLD, libc.CString("cone of cold"), 40, 20, 2, POS_STANDING, 1<<1, 0, (1<<14)|1<<1, 0, 0, nil, 0, -1, -1)
	spello(SPELL_ANIMAL_GROWTH, libc.CString("animal growth"), 40, 20, 2, POS_STANDING, 1<<1, 0, (1<<14)|1<<1, 0, 0, nil, 0, -1, -1)
	spello(SPELL_BALEFUL_POLYMORPH, libc.CString("baleful polymorph"), 40, 20, 2, POS_STANDING, 1<<1, 0, (1<<14)|1<<1, 0, 0, nil, 0, -1, -1)
	spello(SPELL_PASSWALL, libc.CString("passwall"), 40, 20, 2, POS_STANDING, 1<<1, 0, (1<<14)|1<<1, 0, 0, nil, 0, -1, -1)
	spello(SPELL_FIRE_BREATH, libc.CString("fire breath"), 0, 0, 0, POS_SITTING, 1<<0, 1, 0, 0, 0, nil, 0, 0, 0)
	spello(SPELL_GAS_BREATH, libc.CString("gas breath"), 0, 0, 0, POS_SITTING, 1<<0, 1, 0, 0, 0, nil, 0, 0, 0)
	spello(SPELL_FROST_BREATH, libc.CString("frost breath"), 0, 0, 0, POS_SITTING, 1<<0, 1, 0, 0, 0, nil, 0, 0, 0)
	spello(SPELL_ACID_BREATH, libc.CString("acid breath"), 0, 0, 0, POS_SITTING, 1<<0, 1, 0, 0, 0, nil, 0, 0, 0)
	spello(SPELL_LIGHTNING_BREATH, libc.CString("lightning breath"), 0, 0, 0, POS_SITTING, 1<<0, 1, 0, 0, 0, nil, 0, 0, 0)
	spello(SPELL_DG_AFFECT, libc.CString("Script-inflicted"), 0, 0, 0, POS_SITTING, 1<<0, 1, 0, 0, 0, nil, 0, 0, 0)
