...
func damage_weapon(ch *char_data, obj *obj_data, vict *char_data) {
	...
	if GET_SKILL(ch, SKILL_HANDLING) >= axion_dice(10) {
		act(libc.CString("@GYour superior handling prevents @C$p@G from being damaged.@n"), 1, ch, obj, nil, TO_CHAR)
		act(libc.CString("@g$n's@G superior handling prevents @C$p@G from being damaged.@n"), 1, ch, obj, nil, TO_ROOM)
		result = 0
	}
	if result > 0 {
		obj.Value[VAL_ALL_HEALTH] -= result
		if (obj.Value[VAL_ALL_HEALTH]) <= 0 {
			act(libc.CString("@RYour @C$p@R shatters on @r$N's@R body!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@r$n's@R @C$p@R shatters on YOUR body!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@r$n's@R @C$p@R shatters on @r$N's@R body!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_NOTVICT)
			SET_BIT_AR(obj.Extra_flags[:], ITEM_BROKEN)
			perform_remove(vict, 16)
			perform_remove(vict, 17)
		} else if result >= 8 {
			act(libc.CString("@RYour @C$p@R cracks loudly from striking @r$N's@R body!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@r$n's@R @C$p@R cracks loudly from striking YOUR body!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@r$n's@R @C$p@R cracks loudly from striking @r$N's@R body!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_NOTVICT)
		} else if result >= 6 {
			act(libc.CString("@RYour @C$p@R chips from striking @r$N's@R body!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@r$n's@R @C$p@R cracks from striking YOUR body!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@r$n's@R @C$p@R cracks from striking @r$N's@R body!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_NOTVICT)
		} else if result >= 3 {
			act(libc.CString("@RYour @C$p@R loses a piece from striking @r$N's@R body!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@r$n's@R @C$p@R loses a piece from striking YOUR body!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@r$n's@R @C$p@R loses a piece from striking @r$N's@R body!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_NOTVICT)
		} else if result >= 6 {
			act(libc.CString("@RYour @C$p@R has a nick in it from hitting @r$N's@R body!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@r$n's@R @C$p@R has a nick in it from hitting YOUR body!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@r$n's@R @C$p@R has a nick in it from hitting @r$N's@R body!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_NOTVICT)
		}
	}
}
func handle_multihit(ch *char_data, vict *char_data) {
	var (
		perc float64 = float64(int(ch.Aff_abils.Dex))
		prob int     = int(vict.Aff_abils.Dex)
	)
	prob += rand_number(1, 15)
	perc += float64(rand_number(-5, 5))
	if ch.Throws >= 3 {
		ch.Throws = 0
		return
	} else if ch.Throws == -1 {
	...
	if int(perc) >= prob {
		var buf [2048]byte
		act(libc.CString("@Y...in a lightning flash of speed you attack @y$N@Y again!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@Y...in a lightning flash of speed @y$n@Y attacks YOU again!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("@Y...in a lightning flash of speed @y$n@Y attacks @y$N@Y again!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		ch.Throws += 1
...
func handle_defender(vict *char_data, ch *char_data) int {
	...
	if vict.Defender != nil {
		...
		if GET_SPEEDI(def)+int(defnum) > GET_SPEEDI(ch)+int(chnum) && def.In_room == vict.In_room && int(def.Position) > POS_SITTING {
			act(libc.CString("@YYou move to and manage to intercept the attack aimed at @y$N@Y!@n"), 1, def, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@y$n@Y moves to and manages to intercept the attack aimed at YOU!@n"), 1, def, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@y$n@Y moves to and manages to intercept the attack aimed at @y$N@Y!@n"), 1, def, nil, unsafe.Pointer(vict), TO_NOTVICT)
			result = 1
		} else if def.In_room == vict.In_room && int(def.Position) > POS_SITTING {
			act(libc.CString("@YYou move to intercept the attack aimed at @y$N@Y, but just not fast enough!@n"), 1, def, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@y$n@Y moves to intercept the attack aimed at YOU, but $e wasn't fast enough!@n"), 1, def, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@y$n@Y moves to intercept the attack aimed at @y$N@Y, but $e wasn't fast enough!@n"), 1, def, nil, unsafe.Pointer(vict), TO_NOTVICT)
		}
	}
	return result
}
func handle_disarm(ch *char_data, vict *char_data) {
	...
	if roll1 < roll2 {
		var obj *obj_data
		if (ch.Equipment[WEAR_WIELD1]) != nil && handled == 0 {
			obj = ch.Equipment[WEAR_WIELD1]
			act(libc.CString("@y$N@Y manages to disarm you! The @w$p@Y falls from your grasp!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@y$N@Y manages to disarm @R$n@Y! The @w$p@Y falls from $s grasp!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_NOTVICT)
			act(libc.CString("@YYou manage to disarm @R$n@Y! The @w$p@Y falls from $s grasp!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_VICT)
			perform_remove(ch, 16)
			if GET_OBJ_VNUM(obj) != 0x4E82 {
				obj_from_char(obj)
				obj_to_room(obj, ch.In_room)
			}
		} else if (ch.Equipment[WEAR_WIELD1]) != nil && handled == 1 {
			obj = ch.Equipment[WEAR_WIELD1]
			act(libc.CString("@y$N@Y almosts disarms you, but your handling of @w$p@Y saves it from slipping from your grasp!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@y$N@Y almost disarms @R$n@Y, but $s handling of @w$p@Y saves it from slipping from $s grasp!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_NOTVICT)
			act(libc.CString("@YYou almost disarm @R$n@Y, but $s handling of @w$p@Y saves it from slipping from $s grasp!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_VICT)
		} else if (ch.Equipment[WEAR_WIELD2]) != nil && handled == 1 {
...
func combine_attacks(ch *char_data, vict *char_data) {
	...
	var totki int64 = 0
	act(&chbuf[0], 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
	act(&victbuf[0], 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
	act(&rmbuf[0], 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
	...
	for f = ch.Followers; f != nil; f = f.Next {
		if !AFF_FLAGGED(f.Follower, AFF_GROUP) {
			...
			act(&folbuf[0], 1, f.Follower, nil, nil, TO_ROOM)
			act(&folbuf2[0], 1, f.Follower, nil, nil, TO_CHAR)
		}
	}
	totki += bonus
	if same == 1 {
	...
	if ch.Combine != 5 {
		if attspd+attsk < GET_SKILL(vict, SKILL_DODGE)+int(ch.Aff_abils.Cha)/10 {
			act(libc.CString("@GYou manage to dodge nimbly through the combined attack of your enemies!@n"), 1, vict, nil, nil, TO_CHAR)
			act(libc.CString("@r$n@G manages to dodge nimbly through the combined attack!@n"), 1, vict, nil, nil, TO_ROOM)
			return
		} else if blockable == 1 && attspd+attsk < GET_SKILL(vict, SKILL_BLOCK)+int(ch.Aff_abils.Str)/10 {
			act(libc.CString("@GYou manage to effectivly block the combined attack of your enemies with the help of your great strength!@n"), 1, vict, nil, nil, TO_CHAR)
			act(libc.CString("@r$n@G manages to dodge nimbly through the combined attack!@n"), 1, vict, nil, nil, TO_ROOM)
			return
		}
	}
...
func check_ruby(ch *char_data) bool {
	var ruby *obj_data = find_obj_in_list_lambda(ch.Carrying, is_hot_ruby)
	if ruby != nil {
		act(libc.CString("@RYour $p@R flares up and disappears. Your fire attack has been aided!@n"), 1, ch, ruby, nil, TO_CHAR)
		act(libc.CString("@R$n's@R $p@R flares up and disappears!@n"), 1, ch, ruby, nil, TO_ROOM)
		extract_obj(ruby)
		return true
	} else {
		return false
...
func handle_knockdown(ch *char_data) {
	...
	if chance > axion_dice(0) {
		act(libc.CString("@mYou are @GALMOST@m knocked off your feet, but your great balance helps you keep your footing!@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@W$n@m is @GALMOST@m knocked off $s feet, but $s great balance helps $m keep $s footing!@n"), 1, ch, nil, nil, TO_ROOM)
	} else {
		act(libc.CString("@mYou are knocked off your feet!@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@W$n@m is knocked off $s feet!@n"), 1, ch, nil, nil, TO_ROOM)
		ch.Position = POS_SITTING
	}
}
func boom_headshot(ch *char_data) bool {
...
func backstab(ch *char_data, vict *char_data, wlvl int, dmg int64) bool {
	var (
		chance       int     = 0
	...
	if chance >= roll_to_beat {
		...
		if attacker_roll > defender_roll {
			act(libc.CString("@RYou manage to sneak behind @r$N@R and stab $M in the back!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@RYou feel @r$n's@R dagger thrust into your back unexpectantly!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@r$n@R sneaks up behind @r$N@R and stabs $M in the back!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
...
func cut_limb(ch *char_data, vict *char_data, wlvl int, hitspot int) {
	...
	if decapitate == 1 {
		act(libc.CString("@R$N's@r head is cut off in the attack!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@RYOUR head is cut off in the attack!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("@R$N's@rhead is cut off in the attack!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		remove_limb(vict, 0)
		die(vict, ch)
		...
		if !IS_NPC(vict) {
			if HAS_ARMS(vict) && rand_number(1, 2) == 2 {
				if (vict.Limb_condition[1]) > 0 {
					...
					act(libc.CString("@R$N@r loses $s left arm!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("@RYOU lose your left arm!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("@R$N@r loses $s left arm!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					remove_limb(vict, 2)
					...
					act(libc.CString("@R$N@r loses $s right arm!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("@RYOU lose your right arm!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("@R$N@r loses $s right arm!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					remove_limb(vict, 1)
				}
			} else {
				if (vict.Limb_condition[3]) > 0 {
					...
					act(libc.CString("@R$N@r loses $s left leg!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("@RYOU lose your left leg!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("@R$N@r loses $s left leg!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					remove_limb(vict, 4)
					...
					act(libc.CString("@R$N@r loses $s right leg!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("@RYOU lose your right leg!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("@R$N@r loses $s right leg!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					remove_limb(vict, 3)
				}
			}
		} else {
		...
		if rate > 0.0 {
			...
			if ch.Charge+add > ch.Max_mana {
				if ch.Charge < ch.Max_mana {
					...
					act(libc.CString("@MYou leech some of the energy away!@n"), 1, ch, nil, nil, TO_CHAR)
					act(libc.CString("@m$n@M leeches some of the energy away!@n"), 1, ch, nil, nil, TO_ROOM)
				} else {
	...
	if (ch.Bonuses[BONUS_INTOLERANT]) != 0 {
		...
		if rate > 0.0 {
			if ch.Charge > 0 && rand_number(1, 100) <= 10 {
				act(libc.CString("@MThe attack causes your weak control to slip and you are shocked by your own charged energy!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@m$n@M suffers shock from their own charged energy!@n"), 1, ch, nil, nil, TO_ROOM)
				ch.Hit -= ch.Charge / 4
				if ch.Hit <= 0 {
					ch.Hit = 1
				}
	...
	if !is_sparring(ch) {
		if area == 0 {
			if (vict.Limb_condition[1])-dmg <= 0 {
				act(libc.CString("@RYour attack @YDESTROYS @r$N's@R left arm!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@r$n's@R attack @YDESTROYS@R YOUR left arm!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@r$n's@R attack @YDESTROYS @r$N's@R left arm!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				vict.Limb_condition[1] = 0
				...
				act(libc.CString("@RYour attack hurts @r$N's@R left arm!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@r$n's@R attack hurts YOUR left arm!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@r$n's@R attack hurts @r$N's@R left arm!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			} else if (vict.Limb_condition[0])-dmg <= 0 {
				act(libc.CString("@RYour attack @YDESTROYS @r$N's@R right arm!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@r$n's@R attack @YDESTROYS@R YOUR right arm!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@r$n's@R attack @YDESTROYS @r$N's@R right arm!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				...
				act(libc.CString("@RYour attack hurts @r$N's@R right arm!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@r$n's@R attack hurts YOUR right arm!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@r$n's@R attack hurts @r$N's@R right arm!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			}
		} else if area == 1 {
			if (vict.Limb_condition[3])-dmg <= 0 {
				act(libc.CString("@RYour attack @YDESTROYS @r$N's@R left leg!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@r$n's@R attack @YDESTROYS@R YOUR left leg!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@r$n's@R attack @YDESTROYS @r$N's@R left leg!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				...
				act(libc.CString("@RYour attack hurts @r$N's@R left leg!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@r$n's@R attack hurts YOUR left leg!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@r$n's@R attack hurts @r$N's@R left leg!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			} else if (vict.Limb_condition[2])-dmg <= 0 {
				act(libc.CString("@RYour attack @YDESTROYS @r$N's@R right leg!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@r$n's@R attack @YDESTROYS@R YOUR right leg!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@r$n's@R attack @YDESTROYS @r$N's@R right leg!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				...
				act(libc.CString("@RYour attack hurts @r$N's@R right leg!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@r$n's@R attack hurts YOUR right leg!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@r$n's@R attack hurts @r$N's@R right leg!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			}
		}
	}
}
...
func damage_eq(vict *char_data, location int) {
	if (vict.Equipment[location]) != nil && rand_number(1, 20) >= 19 && !AFF_FLAGGED(vict, AFF_SANCTUARY) {
		...
		if (eq.Value[VAL_ALL_HEALTH]) <= 0 {
			...
			act(libc.CString("@WYour $p@W completely breaks!@n"), 0, nil, eq, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@C$N's@W $p@W completely breaks!@n"), 0, nil, eq, unsafe.Pointer(vict), TO_NOTVICT)
			...
			act(libc.CString("@WYour $p@W rips a little!@n"), 0, nil, eq, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@C$N's@W $p@W rips a little!@n"), 0, nil, eq, unsafe.Pointer(vict), TO_NOTVICT)
			if AFF_FLAGGED(vict, AFF_BLESS) {
				...
				act(libc.CString("@c...but @C$N's@c body glows blue for a moment and the damage mends a little.@n"), 1, nil, nil, unsafe.Pointer(vict), TO_NOTVICT)
				...
				act(libc.CString("@c...but @C$N's@c body glows red for a moment and the damage grow three times worse!@n"), 1, nil, nil, unsafe.Pointer(vict), TO_NOTVICT)
			}
		} else {
			act(libc.CString("@WYour $p@W cracks a little!@n"), 0, nil, eq, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@C$N's@W $p@W cracks a little!@n"), 0, nil, eq, unsafe.Pointer(vict), TO_NOTVICT)
			if AFF_FLAGGED(vict, AFF_BLESS) {
				send_to_char(vict, libc.CString("@c...But your blessing seems to have partly mended this damage.@n\r\n"))
				act(libc.CString("@c...but @C$N's@c body glows blue for a moment and the damage mends a little.@n"), 1, nil, nil, unsafe.Pointer(vict), TO_NOTVICT)
			} else if AFF_FLAGGED(vict, AFF_CURSE) {
...
func update_mob_absorb() {
	...
	for i = character_list; i != nil; i = i.Next {
		...
		if i.Absorbing == nil || i.Absorbing == nil {
			...
			if vict.Hit <= 0 {
				act(libc.CString("@R$n@r absorbs the last of YOUR energy and you die...@n"), 1, i, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$n@r absorbs the last of @R$N's@r energy and $E dies...@n"), 1, i, nil, unsafe.Pointer(vict), TO_NOTVICT)
				die(vict, i)
			} else if maxed >= 3 {
				act(libc.CString("@R$n@r absorbs some of YOUR energy...but $e seems to be full now and releases YOU!@n"), 1, i, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$n@r absorbs some of @R$N's@r energy...but $e seems to be full now and lets go.@n"), 1, i, nil, unsafe.Pointer(vict), TO_NOTVICT)
			} else {
				act(libc.CString("@R$n@r absorbs some of YOUR energy!@n"), 1, i, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$n@r absorbs some of @R$N's@r energy.@n"), 1, i, nil, unsafe.Pointer(vict), TO_NOTVICT)
			}
		}
	}
}
func huge_update() {
	...
	for k = object_list; k != nil; k = k.Next {
		...
		if GET_OBJ_VNUM(k) != 82 && GET_OBJ_VNUM(k) != 83 {
			...
			if k.Kitype == 497 {
				if k.Target.In_room == k.In_room {
					...
					act(libc.CString("@WThe large @cS@Cp@wi@cr@Ci@wt @cB@Co@wm@cb@W descends on YOU! It eclipses everything above you as it crushes down into you! You struggle against it with all your might!@n"), 1, k.Target, nil, nil, TO_CHAR)
					act(libc.CString("@WThe large @cS@Cp@wi@cr@Ci@wt @cB@Co@wm@cb@W descends on @C$n@W! It completely obscures $m from view as it crushes into $s body! It appears to be facing some resistance from $m!@n"), 1, k.Target, nil, nil, TO_ROOM)
					send_to_room(k.In_room, libc.CString("\r\n"))
					if k.Target.Hit*int64(bonus) < k.Kicharge*5 {
						act(libc.CString("@WYour strength is no match for the power of the attack! It slowly grinds into you before exploding into a massive blast!@n"), 1, k.Target, nil, nil, TO_CHAR)
						act(libc.CString("@C$n@W's strength is no match for the power of the attack! It slowly grinds into $m before exploding into a massive blast!@n"), 1, k.Target, nil, nil, TO_ROOM)
						...
						for vict = world[k.In_room].People; vict != nil; vict = next_v {
							...
							if (!IS_NPC(vict) && int(vict.Race) == RACE_ICER && rand_number(1, 30) >= 28 || AFF_FLAGGED(vict, AFF_ZANZOKEN)) && vict.Move >= 1 && int(vict.Position) != POS_SLEEPING {
								act(libc.CString("@C$N@c disappears, avoiding the explosion!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_CHAR)
								act(libc.CString("@cYou disappear, avoiding the explosion!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_VICT)
								act(libc.CString("@C$N@c disappears, avoiding the explosion!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
						...
						act(libc.CString("@WYou manage to overpower the attack! You lift up into the sky slowly with it and toss it up and away out of sight!@n"), 1, k.Target, nil, nil, TO_CHAR)
						act(libc.CString("@C$n@W manages to unbelievably overpower the attack! It is lifted up into the sky and tossed away dramaticly!@n"), 1, k.Target, nil, nil, TO_ROOM)
			...
			if k.Kitype == 498 {
				if k.Target.In_room == k.In_room {
					...
					act(libc.CString("@WThe large @mG@Me@wn@mo@Mc@wi@md@Me@W descends on YOU! It eclipses everything above you as it crushes down into you! You struggle against it with all your might!@n"), 1, k.Target, nil, nil, TO_CHAR)
					act(libc.CString("@WThe large @mG@Me@wn@mo@Mc@wi@md@Me@W descends on @C$n@W! It completely obscures $m from view as it crushes into $s body! It appears to be facing some resistance from $m!@n"), 1, k.Target, nil, nil, TO_ROOM)
					send_to_room(k.In_room, libc.CString("\r\n"))
		...
		act(libc.CString("$p@W descends slowly towards the ground!@n"), 1, nil, k, nil, TO_ROOM)
		k.Distance--
	}
}
func homing_update() {
		...
		if GET_OBJ_VNUM(k) != 80 && GET_OBJ_VNUM(k) != 81 && GET_OBJ_VNUM(k) != 84 {
			...
			if GET_OBJ_VNUM(k) == 80 {
				if k.In_room != vict.In_room {
					act(libc.CString("@wThe $p@w pursues after you!@n"), 1, vict, k, nil, TO_CHAR)
					act(libc.CString("@wThe $p@W pursues after @C$n@w!@n"), 1, vict, k, nil, TO_ROOM)
					obj_from_room(k)
					obj_to_room(k, vict.In_room)
					continue
				} else {
					act(libc.CString("@RThe $p@R makes a tight turn and rockets straight for you!@n"), 1, vict, k, nil, TO_CHAR)
					act(libc.CString("@RThe $p@R makes a tight turn and rockets straight for @r$n@n"), 1, vict, k, nil, TO_ROOM)
					if handle_parry(vict) < rand_number(1, 140) {
						act(libc.CString("@rThe $p@r slams into your body, exploding in a flash of bright light!@n"), 1, vict, k, nil, TO_CHAR)
						act(libc.CString("@rThe $p@r slams into @R$n's@r body, exploding in a flash of bright light!@n"), 1, vict, k, nil, TO_ROOM)
						var dmg int64 = k.Kicharge
						extract_obj(k)
						hurt(0, 0, ch, vict, nil, dmg, 1)
						continue
					} else if rand_number(1, 3) > 1 {
						act(libc.CString("@wYou manage to deflect the $p@W sending it flying away and depleting some of its energy.@n"), 1, vict, k, nil, TO_CHAR)
						act(libc.CString("@C$n @wmanages to deflect the $p@w sending it flying away and depleting some of its energy.@n"), 1, vict, k, nil, TO_ROOM)
						...
						act(libc.CString("@wYou manage to deflect the $p@w sending it flying away into the nearby surroundings!@n"), 1, vict, k, nil, TO_CHAR)
						act(libc.CString("@C$n @wmanages to deflect the $p@w sending it flying away into the nearby surroundings!@n"), 1, vict, k, nil, TO_ROOM)
				...
				if k.In_room != vict.In_room {
					act(libc.CString("@wYou lose sight of @C$N@W and let $p@W fly away!@n"), 1, ch, k, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("@wYou manage to escape @C$n's@W $p@W!@n"), 1, ch, k, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("@C$n@W loses sight of @c$N@W and lets $s $p@W fly away!@n"), 1, ch, k, unsafe.Pointer(vict), TO_NOTVICT)
					extract_obj(k)
					continue
				} else {
					act(libc.CString("@RYou move your hand and direct $p@R after @r$N@R!@n"), 1, ch, k, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("@r$n@R moves $s hand and directs $p@R after YOU!@n"), 1, ch, k, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("@r$n@R moves $s hand and directs $p@R after @r$N@R!@n"), 1, ch, k, unsafe.Pointer(vict), TO_NOTVICT)
					if handle_parry(vict) < rand_number(1, 140) {
...
func limb_ok(ch *char_data, type_ int) bool {
	if IS_NPC(ch) {
	...
	if type_ == 0 {
		...
		if AFF_FLAGGED(ch, AFF_ENSNARED) && rand_number(1, 100) <= 90 {
			...
			act(libc.CString("You manage to break the silk ensnaring your arms!"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("$n manages to break the silk ensnaring $s arms!"), 1, ch, nil, nil, TO_ROOM)
...
func handle_dodge(ch *char_data) int {
	...
	if !IS_NPC(ch) {
		if GET_SKILL(ch, SKILL_DODGE) == 0 {
			...
			if int(libc.BoolToInt(group_bonus(ch, 2))) == 8 {
...
func parry_ki(attperc float64, ch *char_data, vict *char_data, sname [1000]byte, prob int, perc int, skill int, type_ int) {
	...
	for tch = world[ch.In_room].People; tch != nil; tch = next_v {
		...
		if rand_number(1, 101) >= 90 && foundv == 0 {
			if handle_parry(tch) > rand_number(1, 140) {
				...
				act(&buf[0], 1, ch, nil, unsafe.Pointer(tch), TO_CHAR)
				act(&buf2[0], 1, ch, nil, unsafe.Pointer(tch), TO_VICT)
				act(&buf3[0], 1, ch, nil, unsafe.Pointer(tch), TO_NOTVICT)
				foundv = 0
			} else {
				foundv = 1
				...
				act(&buf[0], 1, vict, nil, unsafe.Pointer(tch), TO_CHAR)
				act(&buf2[0], 1, vict, nil, unsafe.Pointer(tch), TO_VICT)
				act(&buf3[0], 1, vict, nil, unsafe.Pointer(tch), TO_NOTVICT)
	...
	for tob = world[ch.In_room].Contents; tob != nil; tob = next_obj {
		...
		if rand_number(1, 101) >= 80 {
			...
			act(&buf[0], 1, vict, tob, nil, TO_CHAR)
			act(&buf2[0], 1, vict, tob, nil, TO_ROOM)
			hurt(0, 0, ch, nil, tob, 25, 1)
			return
		}
	}
	if (foundo == 0 || foundv == 0) && !ROOM_FLAGGED(vict.In_room, ROOM_SPACE) {
		...
		act(&buf[0], 1, vict, nil, nil, TO_CHAR)
		act(&buf2[0], 1, vict, nil, nil, TO_ROOM)
		if SECT(vict.In_room) != SECT_INSIDE && SECT(vict.In_room) != SECT_UNDERWATER && SECT(vict.In_room) != SECT_WATER_SWIM && SECT(vict.In_room) != SECT_WATER_NOSWIM && SECT(vict.In_room) != SECT_UNDERWATER && SECT(vict.In_room) != SECT_WATER_SWIM && SECT(vict.In_room) != SECT_WATER_NOSWIM {
			...
			case 2:
				if rand_number(1, 4) == 4 && world[vict.In_room].Geffect == 0 {
					...
					act(libc.CString("Lava spews up through cracks in the ground, roaring into the sky as a large column of molten rock!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("Lava spews up through cracks in the ground, roaring into the sky as a large column of molten rock!"), 1, ch, nil, unsafe.Pointer(vict), TO_ROOM)
				}
			case 3:
			...
			case 6:
				act(libc.CString("The explosion continues to burn spreading out and devouring some more of the ground before dying out."), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("The explosion continues to burn spreading out and devouring some more of the ground before dying out."), 1, ch, nil, unsafe.Pointer(vict), TO_ROOM)
			default:
			}
		}
		...
		if SECT(vict.In_room) == SECT_INSIDE {
			...
			case 1:
				...
				act(libc.CString("Debris is thrown into the air and showers down thunderously!"), 1, ch, nil, unsafe.Pointer(vict), TO_ROOM)
			case 2:
				act(libc.CString("The structure of the surrounding room cracks and quakes from the blast!"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("The structure of the surrounding room cracks and quakes from the blast!"), 1, ch, nil, unsafe.Pointer(vict), TO_ROOM)
			case 3:
...
func dodge_ki(ch *char_data, vict *char_data, type_ int, type2 int, skill int, skill2 int) {
	...
	if type_ == 1 {
		if rand_number(1, 3) != 2 {
			act(libc.CString("@RIt turns around at the last second and begins to pursue @r$N@R!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@RIt turns around at the last second and begins to pursue YOU!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@RIt turns around at the last second and begins to pursue @r$N@R!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			var obj *obj_data
			var num int = 0
			switch skill2 {
			case 461:
			...
			act(libc.CString("@RIt fails to follow after @r$N@R!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@RIt fails to follow after YOU!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@RIt fails to follow after @r$N@R!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		}
	}
	if type_ == 2 && (skill2 != 481 || int(ch.Chclass) == CLASS_FRIEZA) {
		...
		act(libc.CString("@RYou turn it around and send it back after @r$N@R!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@W$n @Rturns it around and sends it back after YOU!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("@W$n @Rturns it around and sends it back after @r$N@R!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		var obj *obj_data
		var num int = 0
		switch skill2 {
		case 496:
...
func damtype(ch *char_data, type_ int, skill int, percent float64) int64 {
	...
	if !IS_NPC(ch) {
		...
		case -1:
			...
			dam = float64(large_rand(cou1, cou2))
			dam += (float64(ch.Aff_abils.Str) * (dam * 0.005))
			if AFF_FLAGGED(ch, AFF_HASS) && !PLR_FLAGGED(ch, PLR_THANDW) {
				...
				dam += (dam / 100) * float64(GET_SKILL(ch, SKILL_INFUSE)/2)
				if int(ch.Chclass) == CLASS_JINTO {
					if GET_SKILL(ch, SKILL_INFUSE) >= 100 {
						dam += (((dam * 0.01) * float64(GET_SKILL(ch, SKILL_INFUSE)/2)) * 0.5)
					} else if GET_SKILL(ch, SKILL_INFUSE) >= 100 {
						dam += (((dam * 0.01) * float64(GET_SKILL(ch, SKILL_INFUSE)/2)) * 0.25)
					} else if GET_SKILL(ch, SKILL_INFUSE) >= 100 {
						dam += (((dam * 0.01) * float64(GET_SKILL(ch, SKILL_INFUSE)/2)) * 0.05)
					}
				}
			}
		...
		case 7:
			dam = (float64(ch.Max_mana) * percent)
			dam *= (1.25)
			if focus > 0 {
				dam += float64(focus) * (dam / 1000)
			}
		...
		case 13:
			dam = (float64(ch.Max_mana) * percent)
			dam += float64(GET_LEVEL(ch) * 800)
			dam += (dam * 0.25)
			if focus > 0 {
				dam += ((dam * 0.005) * float64(focus))
			}
			if int(ch.Race) == RACE_HUMAN {
				dam += (dam / 100) * 15
				if skill == 101 {
		...
		case 23:
			...
			dam += float64(GET_LEVEL(ch) * 500)
			dam *= (1.25)
			dam += (dam / 100) * float64(ch.Aff_abils.Str)
			if focus > 0 {
				dam += float64(focus) * (dam / 200)
			}
			if (ch.Bonuses[BONUS_BRAWLER]) > 0 {
		...
		case 27:
			...
			dam += float64(GET_LEVEL(ch) * 1200)
			dam *= (1.25)
			dam += (dam / 100) * float64(ch.Aff_abils.Intel)
			if focus > 0 {
				dam += float64(focus) * (dam / 200)
			}
			if int(ch.Race) == RACE_SAIYAN {
		...
		dam = float64((float64(ch.Hit) * 0.05) + float64(ch.Max_hit)*0.025)
		dam += ((dam * 0.005) * float64(ch.Aff_abils.Str))
		if GET_LEVEL(ch) >= 120 {
			dam *= (0.25)
	...
	if IS_NPC(ch) {
		if type_ == 0 || type_ == 1 || type_ == 2 || type_ == 3 || type_ == 4 || type_ == 5 || type_ == 6 || type_ == 8 || type_ == 51 || type_ == 52 || type_ == 56 {
			...
			dam += (float64(ch.Aff_abils.Intel) * (dam * 0.005))
		}
	...
	if ch.Kaioken > 0 {
		dam += (dam / 200) * float64(ch.Kaioken)
	}
	if PLR_FLAGGED(ch, PLR_FURY) && (type_ == 0 || type_ == 1 || type_ == 2 || type_ == 3 || type_ == 4 || type_ == 5 || type_ == 6 || type_ == 8 || type_ == 51 || type_ == 52) {
		dam *= (1.5)
		act(libc.CString("Your rage magnifies your attack power!"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("Swirling energy flows around $n as $e releases $s rage in the attack!"), 1, ch, nil, nil, TO_ROOM)
	...
	if type_ == -1 || type_ == 0 || type_ == 1 || type_ == 2 || type_ == 3 || type_ == 4 || type_ == 5 || type_ == 6 || type_ == 8 {
		...
		if !IS_NPC(ch) && dam > float64(ch.Max_hit)*0.1 {
			dam *= (0.6)
		}
	} else {
		dam += ((dam * 0.005) * float64(ch.Aff_abils.Intel))
...
func spar_gain(ch *char_data, vict *char_data, type_ int, dmg int64) {
	...
	if ch != nil && !IS_NPC(ch) {
		...
		gmult = float64(GET_LEVEL(ch) * ((gravity / 5) + 6))
		...
		if chance >= rand_number(60, 75) {
			...
			if vict != nil && IS_NPC(vict) {
				...
				gaincalc = (float64(num) * 1.5)
				type_ = 3
			} else if vict != nil && !IS_NPC(vict) {
				gaincalc = float64(large_rand(int64(float64(num)*0.7), num))
			...
			if vict != nil {
				if difference >= 51 {
					...
					gaincalc = (float64(gaincalc) * 0.05)
				} else if difference >= 30 {
					gaincalc = (float64(gaincalc) * 0.1)
				} else if difference >= 20 {
					gaincalc = (float64(gaincalc) * 0.25)
				} else if difference >= 10 {
					gaincalc = (float64(gaincalc) * 0.5)
				}
				if !IS_NPC(vict) {
			...
			if int(ch.Race) == RACE_SAIYAN {
				gaincalc = (float64(gaincalc) + float64(gaincalc)*0.5)
			}
			if int(ch.Race) == RACE_HALFBREED {
				gaincalc = (float64(gaincalc) + float64(gaincalc)*0.4)
			}
			if int(ch.Race) == RACE_ICER || int(ch.Race) == RACE_BIO && ((ch.Genome[0]) == 4 || (ch.Genome[1]) == 4) {
				gaincalc = (float64(gaincalc) - float64(gaincalc)*0.2)
			}
			if ROOM_FLAGGED(ch.In_room, ROOM_WORKOUT) || ROOM_FLAGGED(ch.In_room, ROOM_HBTC) {
				if int(libc.BoolToInt(GET_ROOM_VNUM(ch.In_room))) >= 19100 && int(libc.BoolToInt(GET_ROOM_VNUM(ch.In_room))) <= 0x4AFF {
					gaincalc *= (1.5)
			...
			gain = gear_exp(ch, int64(gaincalc))
	...
	if vict != nil && !IS_NPC(vict) && !IS_NPC(ch) {
		...
		gravity = world[ch.In_room].Gravity
		gmult = float64(GET_LEVEL(vict) * ((gravity / 5) + 6))
...
func can_grav(ch *char_data) bool {
...
func can_kill(ch *char_data, vict *char_data, obj *obj_data, num int) bool {
	...
	if vict != nil {
		...
		if ROOM_FLAGGED(ch.In_room, ROOM_PEACEFUL) {
			...
			act(libc.CString("@GYou stretch your limbs toward @g$N@G in an attempt to hit $M!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@g$n@G stretches $s limbs toward @RYOU@G in an attempt to land a hit!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@g$n@G stretches $s limbs toward @g$N@G in an attempt to hit $M!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			return true
...
func check_skill(ch *char_data, skill int) bool {
...
func check_points(ch *char_data, ki int64, st int64) bool {
...
func hurt(limb int, chance int, ch *char_data, vict *char_data, obj *obj_data, dmg int64, type_ int) {
	...
	if type_ <= 0 {
		...
		if int(libc.BoolToInt(group_bonus(ch, 2))) == 4 {
			dmg += int64(float64(dmg) * 0.1)
		} else if int(libc.BoolToInt(group_bonus(ch, 2))) == 12 {
			dmg -= int64(float64(dmg) * 0.1)
		}
	} else {
		dmg = int64(float64(dmg) * 0.6)
		if int(libc.BoolToInt(group_bonus(ch, 2))) == 9 {
			dmg -= int64(float64(dmg) * 0.1)
		}
	...
	if AFF_FLAGGED(ch, AFF_INFUSE) && !AFF_FLAGGED(ch, AFF_HASS) && type_ <= 0 {
		if float64(ch.Mana)-float64(ch.Max_mana)*0.005 > 0 && dmg > 1 {
			...
			act(libc.CString("@wYou can no longer infuse ki into your attacks!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@c$n@w can no longer infuse ki into $s attacks!@n"), 1, ch, nil, nil, TO_ROOM)
	...
	if vict != nil {
		...
		if int(vict.Race) == RACE_MUTANT && ((vict.Genome[0]) == 8 || (vict.Genome[1]) == 8) && type_ == 0 {
			...
			act(libc.CString("@Y$N's rubbery body makes hitting it tiring!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@Y$n's stamina is sapped a bit by hitting your rubbery body!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		}
		if !IS_NPC(ch) {
		...
		if AFF_FLAGGED(vict, AFF_SANCTUARY) {
			...
			if vict.Barrier-dmg > 0 {
				act(libc.CString("@c$N's@C barrier absorbs the damage!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				var barr [2048]byte
				stdio.Sprintf(&barr[0], "@CYour barrier absorbs the damage! @D[@B%s@D]@n", add_commas(dmg))
				act(&barr[0], 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@c$N's@C barrier absorbs the damage!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				vict.Barrier -= dmg
				dmg = 0
			} else if vict.Barrier-dmg <= 0 {
				dmg -= vict.Barrier
				vict.Barrier = 0
				act(libc.CString("@c$N's@C barrier bursts!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@CYour barrier bursts!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@c$N's@C barrier bursts!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		...
		if AFF_FLAGGED(vict, AFF_FIRESHIELD) && rand_number(1, 200) < GET_SKILL(vict, SKILL_FIRESHIELD) {
			act(libc.CString("@c$N's@C fireshield repels the damage!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@CYour fireshield repels the damage!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@c$N's@C fireshield repels the damage!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			if rand_number(1, 3) == 3 {
				act(libc.CString("@c$N's@C fireshield disappears...@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@CYour fireshield disappears...@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@c$N's@C fireshield disappears...@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		...
		if vict.Clan != nil && libc.StrCaseCmp(vict.Clan, libc.CString("Heavenly Kaios")) == 0 {
			if vict.Mana >= vict.Max_mana/2 {
				dmg -= (dmg / 100) * 20
				act(libc.CString("@wYou are covered in a pristine @Cglow@w.@n"), 1, vict, nil, nil, TO_CHAR)
				act(libc.CString("@w$n is covered in a pristine @Cglow@w!@n"), 1, vict, nil, nil, TO_ROOM)
			}
		}
		if !IS_NPC(vict) && GET_SKILL(vict, SKILL_ARMOR) != 0 {
			...
			if nanite >= perc {
				if PLR_FLAGGED(vict, PLR_TRANS6) {
					act(libc.CString("@WYour @gn@Ga@Wn@wite @Da@Wr@wm@Do@wr@W reacts in time to block MOST of the damage!@n"), 1, vict, nil, nil, TO_CHAR)
					act(libc.CString("@W$n's @gn@Ga@Wn@wite @Da@Wr@wm@Do@wr@W reacts in time to block MOST of the damage!@n"), 1, vict, nil, nil, TO_ROOM)
					dmg -= int64((float64(dmg) * 0.01) * 50)
				} else if PLR_FLAGGED(vict, PLR_TRANS5) {
					act(libc.CString("@WYour @gn@Ga@Wn@wite @Da@Wr@wm@Do@wr@W reacts in time to block some of the damage!@n"), 1, vict, nil, nil, TO_CHAR)
					act(libc.CString("@W$n's @gn@Ga@Wn@wite @Da@Wr@wm@Do@wr@W reacts in time to block some of the damage!@n"), 1, vict, nil, nil, TO_ROOM)
					dmg -= int64((float64(dmg) * 0.01) * 40)
				} else if PLR_FLAGGED(vict, PLR_TRANS4) {
					act(libc.CString("@WYour @gn@Ga@Wn@wite @Da@Wr@wm@Do@wr@W reacts in time to block a lot of the damage!@n"), 1, vict, nil, nil, TO_CHAR)
					act(libc.CString("@W$n's @gn@Ga@Wn@wite @Da@Wr@wm@Do@wr@W reacts in time to block a lot of the damage!@n"), 1, vict, nil, nil, TO_ROOM)
					dmg -= int64((float64(dmg) * 0.01) * 30)
				} else if PLR_FLAGGED(vict, PLR_TRANS3) {
					act(libc.CString("@WYour @gn@Ga@Wn@wite @Da@Wr@wm@Do@wr@W reacts in time to block a good deal of the damage!@n"), 1, vict, nil, nil, TO_CHAR)
					act(libc.CString("@W$n's @gn@Ga@Wn@wite @Da@Wr@wm@Do@wr@W reacts in time to block a good deal of the damage!@n"), 1, vict, nil, nil, TO_ROOM)
					...
					act(libc.CString("@WYour @gn@Ga@Wn@wite @Da@Wr@wm@Do@wr@W reacts in time to block a bit of the damage!@n"), 1, vict, nil, nil, TO_CHAR)
					act(libc.CString("@W$n's @gn@Ga@Wn@wite @Da@Wr@wm@Do@wr@W reacts in time to block a bit of the damage!@n"), 1, vict, nil, nil, TO_ROOM)
					dmg -= int64((float64(dmg) * 0.01) * 10)
				} else {
					act(libc.CString("@WYour @gn@Ga@Wn@wite @Da@Wr@wm@Do@wr@W reacts in time to block a tiny bit of the damage!@n"), 1, vict, nil, nil, TO_CHAR)
					act(libc.CString("@W$n's @gn@Ga@Wn@wite @Da@Wr@wm@Do@wr@W reacts in time to block a tiny bit of the damage!@n"), 1, vict, nil, nil, TO_ROOM)
		...
		if !AFF_FLAGGED(vict, AFF_KNOCKED) && (int(vict.Position) == POS_SITTING || int(vict.Position) == POS_RESTING) && GET_SKILL(vict, SKILL_ROLL) > axion_dice(0) {
			...
			if vict.Move >= rollcost {
				act(libc.CString("@GYou roll to your feet in an agile fashion!@n"), 1, vict, nil, nil, TO_CHAR)
				act(libc.CString("@G$n rolls to $s feet in an agile fashion!@n"), 1, vict, nil, nil, TO_ROOM)
		...
		if AFF_FLAGGED(vict, AFF_SLEEP) && rand_number(1, 2) == 2 {
			...
			act(libc.CString("@c$N@W seems to be more aware now.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@WYou are no longer so sleepy.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@c$N@W seems to be more aware now.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		}
		if AFF_FLAGGED(vict, AFF_KNOCKED) && rand_number(1, 12) >= 11 {
			act(libc.CString("@W$n@W is no longer senseless, and wakes up.@n"), 0, vict, nil, nil, TO_ROOM)
			...
			if IS_NPC(vict) && rand_number(1, 20) >= 12 {
				act(libc.CString("@W$n@W stands up.@n"), 0, vict, nil, nil, TO_ROOM)
		...
		if is_sparring(ch) && is_sparring(vict) && (vict.Suppressed+vict.Hit)-dmg <= 0 {
			if !IS_NPC(vict) {
				act(libc.CString("@c$N@w falls down unconscious, and you stop sparring with $M.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@C$n@w stops sparring with you as you fall unconscious.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@c$N@w falls down unconscious, and @C$n@w stops sparring with $M.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				vict.Hit = 1
				...
				act(libc.CString("@c$N@w admits defeat to you, stops sparring, and stumbles away.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@c$N@w admits defeat to $n, stops sparring, and stumbles away.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				solo_gain(ch, vict)
				var rew *obj_data
				...
				if founded == 1 {
					act(libc.CString("@c$N@w leaves a reward behind out of respect.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				}
				vict.Hit = 0
				extract_char(vict)
				return
			...
			act(libc.CString("@c$N@w falls down unconscious, and you spare $S life.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@C$n@w spares your life as you fall unconscious.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@c$N@w falls down unconscious, and @C$n@w spares $S life.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			vict.Hit = 1
			if vict.Fighting != nil {
				stop_fighting(vict)
			}
			...
			act(libc.CString("@w$n@w stops sparring!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_ROOM)
		...
		if PLR_FLAGGED(vict, PLR_IMMORTAL) && !is_sparring(ch) && vict.Hit-dmg <= 0 {
			if IN_ARENA(vict) {
				...
				act(libc.CString("@c$N@w disappears right before dying. $N appears to be immortal.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@CYou disappear right before death, having been saved by your immortality.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@c$N@w disappears right before dying. $N appears to be immortal.@n."), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				vict.Hit = 1
				vict.Mana = 1
				vict.Move = 1
				if vict.Fighting != nil {
		...
		if vict.Grappling != nil && vict.Grappling != ch && type_ == 1 {
			act(libc.CString("@YThe attack hurts YOU as well because you are grappling with $M!@n"), 1, vict, nil, unsafe.Pointer(vict.Grappling), TO_VICT)
			act(libc.CString("@YThe attack hurts @y$N@Y as well because $n is grappling with $m!@n"), 1, vict, nil, unsafe.Pointer(vict.Grappling), TO_NOTVICT)
			maindmg = maindmg / 2
			hurt(0, 0, ch, vict.Grappling, nil, maindmg, 3)
		}
		if vict.Grappled != nil && vict.Grappled != ch && type_ == 1 {
			act(libc.CString("@YThe attack hurts YOU as well because you are being grappled by $M!@n"), 1, vict, nil, unsafe.Pointer(vict.Grappled), TO_VICT)
			act(libc.CString("@YThe attack hurts @y$N@Y as well because $n is being grappled by $m!@n"), 1, vict, nil, unsafe.Pointer(vict.Grappled), TO_NOTVICT)
			maindmg = maindmg / 2
			hurt(0, 0, ch, vict.Grappled, nil, maindmg, 3)
		}
		if !is_sparring(ch) && !PLR_FLAGGED(vict, PLR_IMMORTAL) && vict.Hit-dmg <= 0 {
			if vict.Hit-dmg <= 0 && suppresso == 0 {
				vict.Hit = 0
				if !IS_NPC(vict) && vict.Lifeforce-(dmg-vict.Hit) >= 0 {
					act(libc.CString("@c$N@w barely clings to life!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("@CYou barely cling to life!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("@c$N@w barely clings to life!@n."), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				...
				if type_ <= 0 && (!IS_NPC(vict) || !MOB_FLAGGED(vict, MOB_DUMMY)) {
					...
					act(libc.CString("@R$N@w self destructs with a mild explosion!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("@R$N@w self destructs with a mild explosion!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_ROOM)
				}
				if dmg > 1 {
					...
					if AFF_FLAGGED(ch, AFF_METAMORPH) && ch.Hit <= ch.Max_hit {
						act(libc.CString("@RYour dark aura saps some of @r$N's@R life energy!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
						act(libc.CString("@r$n@R's dark aura saps some of your life energy!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
						ch.Hit += healhp
					}
					...
					if AFF_FLAGGED(vict, AFF_ECHAINS) {
						if IS_NPC(ch) && type_ == 0 {
							...
							if int(ch.Real_abils.Cha) < 5 {
								...
								act(libc.CString("@CEthereal chains burn into existence! They quickly latch onto @RYOUR@C body and begin temporarily hampering $s actions!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
								act(libc.CString("@CEthereal chains burn into existence! They quickly latch onto @c$n's@C body and begin temporarily hampering $s actions!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_ROOM)
							}
						} else if type_ == 0 {
		...
		} else if vict.Hit-dmg > 0 || suppresso == 1 {
			if suppresso == 0 {
				vict.Hit -= dmg
			}
			if ch.Fighting == nil {
		...
		if int(vict.Race) == RACE_ARLIAN && dead != 1 && !is_sparring(vict) && !is_sparring(ch) {
		...
		if OBJ_FLAGGED(obj, ITEM_UNBREAKABLE) {
			act(libc.CString("$p@w seems unaffected.@n"), 1, ch, obj, nil, TO_CHAR)
			act(libc.CString("$p@w seems unaffected.@n"), 1, ch, obj, nil, TO_ROOM)
			...
			if obj.Weight-dmg > 0 {
				if type_ <= 0 {
					...
					act(libc.CString("$p@w cracks some.@n"), 1, ch, obj, nil, TO_CHAR)
					act(libc.CString("$p@w cracks some.@n"), 1, ch, obj, nil, TO_ROOM)
					obj.Weight -= dmg
					if obj.Fellow_wall != nil {
						var wall *obj_data
						wall = obj.Fellow_wall
						wall.Weight -= dmg
						act(libc.CString("$p@w cracks some. A humanoid shadow can be seen moving on the other side.@n"), 1, nil, obj, nil, TO_ROOM)
					}
				} else {
					dmg *= 30
					act(libc.CString("$p@w melts some.@n"), 1, ch, obj, nil, TO_CHAR)
					act(libc.CString("$p@w melts some.@n"), 1, ch, obj, nil, TO_ROOM)
					obj.Weight -= dmg
					if obj.Fellow_wall != nil {
						var wall *obj_data
						wall = obj.Fellow_wall
				...
				if type_ <= 0 {
					act(libc.CString("$p@w breaks completely apart and then melts away.@n"), 1, ch, obj, nil, TO_CHAR)
					act(libc.CString("$p@w breaks completely apart and then melts away.@n"), 1, ch, obj, nil, TO_ROOM)
					extract_obj(obj)
				} else {
					act(libc.CString("$p@w is blown away into snow and water!@n"), 1, ch, obj, nil, TO_CHAR)
					act(libc.CString("$p@w is blown away into snow and water!@n"), 1, ch, obj, nil, TO_ROOM)
					extract_obj(obj)
				}
			}
			...
			if type_ <= 0 {
				act(libc.CString("$p@w shatters apart!@n"), 1, ch, obj, nil, TO_CHAR)
				act(libc.CString("$p@w shatters apart!@n"), 1, ch, obj, nil, TO_ROOM)
				...
				act(libc.CString("$p@w is disintegrated!@n"), 1, ch, obj, nil, TO_CHAR)
				act(libc.CString("$p@w is disintegrated!@n"), 1, ch, obj, nil, TO_ROOM)
				extract_obj(obj)
			}
		}
	} else {
...
func handle_spiral(ch *char_data, vict *char_data, skill int, first int) {
	...
	if vict == nil && ch.Fighting != nil {
		...
		act(libc.CString("@WHaving lost your target you slow down until your vortex disappears, and end your attack.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@C$n@W slows down until $s vortex disappears.@n"), 1, ch, nil, nil, TO_ROOM)
		REMOVE_BIT_AR(ch.Act[:], PLR_SPIRAL)
		return
	}
	if ch.Charge <= 0 {
		act(libc.CString("@WHaving no more charged ki you slow down until your vortex disappears, and end your attack.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@C$n@W slows down until $s vortex disappears.@n"), 1, ch, nil, nil, TO_ROOM)
		REMOVE_BIT_AR(ch.Act[:], PLR_SPIRAL)
		return
	}
	if vict != nil {
		...
		if (!IS_NPC(vict) && int(vict.Race) == RACE_ICER && rand_number(1, 30) >= 28 || AFF_FLAGGED(vict, AFF_ZANZOKEN)) && vict.Move >= 1 && int(vict.Position) != POS_SLEEPING {
			if !AFF_FLAGGED(ch, AFF_ZANZOKEN) || AFF_FLAGGED(ch, AFF_ZANZOKEN) && GET_SPEEDI(ch)+rand_number(1, 5) < GET_SPEEDI(vict)+rand_number(1, 5) {
				act(libc.CString("@C$N@c disappears, avoiding your Spiral Comet blast before reappearing!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@cYou disappear, avoiding @C$n's@c Spiral Comet blast before reappearing!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@C$N@c disappears, avoiding @C$n's@c Spiral Comet blast before reappearing!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		...
		if prob < perc {
			if vict.Move > 0 {
				if blk > rand_number(1, 130) {
					act(libc.CString("@C$N@W moves quickly and blocks your Spiral Comet blast!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("@WYou move quickly and block @C$n's@W Spiral Comet blast!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("@C$N@W moves quickly and blocks @c$n's@W Spiral Comet blast!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					pcost(ch, amount, 0)
					...
					act(libc.CString("@C$N@W manages to dodge your Spiral Comet blast, letting it slam into the surroundings!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("@WYou dodge @C$n's@W Spiral Comet blast, letting it slam into the surroundings!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("@C$N@W manages to dodge @c$n's@W Spiral Comet blast, letting it slam into the surroundings!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					...
					act(libc.CString("@WYou can't believe it but your Spiral Comet blast misses, flying through the air harmlessly!@n"), 0, ch, nil, unsafe.Pointer(vict), TO_CHAR)
					act(libc.CString("@C$n@W fires a Spiral Comet blast at you, but misses!@n "), 0, ch, nil, unsafe.Pointer(vict), TO_VICT)
					act(libc.CString("@c$n@W fires a Spiral Comet blast at @C$N@W, but somehow misses!@n "), 0, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
					pcost(ch, amount, 0)
					hurt(0, 0, ch, vict, nil, 0, 1)
					return
				}
			...
			case 1:
				act(libc.CString("@WYou launch a bright @mp@Mu@mr@Mp@ml@Me@W ball of energy down at @c$N@W! It slams into $S chest and explodes!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@C$n@W launches a bright @mp@Mu@mr@Mp@ml@Me@W ball of energy down at YOU! It slams into YOUR chest and explodes!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@C$n@W launches a bright @mp@Mu@mr@Mp@ml@Me@W ball of energy down at @c$N@W! It slams into $S chest and explodes!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				hurt(0, 0, ch, vict, nil, dmg, 1)
				dam_eq_loc(vict, 4)
			case 2:
				act(libc.CString("@WYou launch a bright @mp@Mu@mr@Mp@ml@Me@W ball of energy down at @c$N@W! It slams into $S head and explodes!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@C$n@W launches a bright @mp@Mu@mr@Mp@ml@Me@W ball of energy down at YOU! It slams into YOUR head and explodes!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@C$n@W launches a bright @mp@Mu@mr@Mp@ml@Me@W ball of energy down at @c$N@W! It slams into $S head and explodes!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				dmg *= 2
				hurt(0, 0, ch, vict, nil, dmg, 1)
				dam_eq_loc(vict, 3)
			case 3:
				act(libc.CString("@WYou launch a bright @mp@Mu@mr@Mp@ml@Me@W ball of energy down at @c$N@W! It slams into $S body and explodes!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@C$n@W launches a bright @mp@Mu@mr@Mp@ml@Me@W ball of energy down at YOU! It slams into YOUR body and explodes!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@C$n@W launches a bright @mp@Mu@mr@Mp@ml@Me@W ball of energy down at @c$N@W! It slams into $S body and explodes!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				hurt(0, 0, ch, vict, nil, dmg, 1)
				dam_eq_loc(vict, 4)
			case 4:
				act(libc.CString("@WYou launch a bright @mp@Mu@mr@Mp@ml@Me@W ball of energy down at @c$N@W! It slams into $S arm and explodes!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@C$n@W launches a bright @mp@Mu@mr@Mp@ml@Me@W ball of energy down at YOU! It slams into YOUR arm and explodes!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@C$n@W launches a bright @mp@Mu@mr@Mp@ml@Me@W ball of energy down at @c$N@W! It slams into $S arm and explodes!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				dmg /= 2
				dam_eq_loc(vict, 1)
				hurt(0, 190, ch, vict, nil, dmg, 1)
			case 5:
				act(libc.CString("@WYou launch a bright @mp@Mu@mr@Mp@ml@Me@W ball of energy down at @c$N@W! It slams into $S leg and explodes!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@C$n@W launches a bright @mp@Mu@mr@Mp@ml@Me@W ball of energy down at YOU! It slams into YOUR leg and explodes!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@C$n@W launches a bright @mp@Mu@mr@Mp@ml@Me@W ball of energy down at @c$N@W! It slams into $S leg and explodes!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				dmg /= 2
				dam_eq_loc(vict, 2)
...
func handle_death_msg(ch *char_data, vict *char_data, type_ int) {
	if type_ == 0 {
		if !SUNKEN(vict.In_room) && SECT(vict.In_room) != SECT_WATER_SWIM && SECT(vict.In_room) != SECT_WATER_NOSWIM && !ROOM_FLAGGED(vict.In_room, ROOM_SPACE) && SECT(vict.In_room) != SECT_FLYING {
			switch rand_number(1, 5) {
			case 1:
				act(libc.CString("@R$N@r coughs up blood before falling to the ground dead.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou cough up blood before falling to the ground dead.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r coughs up blood before falling down dead.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			case 2:
				act(libc.CString("@R$N@r crumples to the ground dead.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou crumple to the ground dead.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r crumples to the ground dead.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			case 3:
				act(libc.CString("@R$N@r cries out $S last breath before dying.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou cry out your last breath before dying.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r cries out $S last breath before dying.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			case 4:
				act(libc.CString("@R$N@r writhes on the ground screaming in pain before finally dying!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou writhe on the ground screaming in pain before finally dying!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r writhes on the ground screaming in pain before finally dying!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				if vict.Death_type != DTYPE_HEAD {
					vict.Death_type = DTYPE_PULP
				}
			case 5:
				act(libc.CString("@R$N@r hits the ground dead with such force that blood flies into the air briefly!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou hit the ground dead with such force that blood flies into the air briefly!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r hits the ground dead with such force that blood flies into the air briefly!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				if vict.Death_type != DTYPE_HEAD {
					vict.Death_type = DTYPE_PULP
				}
			}
			...
			switch rand_number(1, 5) {
			case 1:
				act(libc.CString("@R$N@r coughs up blood and dies before falling down to the water. A large splash accompanies $S body hitting the water!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou cough up blood and die before falling down to the water. A large splash accompanies your body hitting the water!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r coughs up blood and dies before falling down to the water. A large splash accompanies $S body hitting the water!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			case 2:
				act(libc.CString("@R$N@r crumples down to the water, with the signs of life leaving $S eyes as $E floats in the water.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou crumple down to the water and die.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r crumples down to the water, with the signs of life leaving $S eyes as $E floats in the water.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			case 3:
				act(libc.CString("@R$N@r cries out $S last breath before dying and leaving a floating corpse behind.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou cry out your last breath before dying.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r cries out $S last breath before dying and leaving a floating corpse behind.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			case 4:
				act(libc.CString("@R$N@r writhes in the water screaming in pain before finally dying!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou writhe in the water screaming in pain before finally dying!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r writhes in the water screaming in pain before finally dying!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				if vict.Death_type != DTYPE_HEAD {
					vict.Death_type = DTYPE_PULP
				}
			case 5:
				act(libc.CString("@R$N@r hits the water dead with such force that blood mixed with water flies into the air briefly!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou hit the water dead with such force that blood mixed with water flies into the air briefly!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r hits the water dead with such force that blood mixed with water flies into the air briefly!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				if vict.Death_type != DTYPE_HEAD {
					vict.Death_type = DTYPE_PULP
				}
			}
			...
			switch rand_number(1, 5) {
			case 1:
				act(libc.CString("@R$N@r coughs up blood and dies. The blood freezes and floats freely through space...@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou cough up blood and die. The blood freezes and floats freely through space...@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r coughs up blood and dies. The blood freezes and floats freely through space...@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			case 2:
				act(libc.CString("@R$N@r dies and leaves $S corpse floating freely in space.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou die and leave your corpse floating freely in space.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r dies and leaves $S corpse floating freely in space.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			case 3:
				act(libc.CString("@R$N@r cries out $S last breath before dying.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou cry out your last breath before dying.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r cries out $S last breath before dying.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			case 4:
				act(libc.CString("@R$N@r writhes in space trying to scream in pain before $e finally dies!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou writhe in space trying to scream in pain before you finally die!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r writhes in space trying to scream in pain before $e finally dies!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				if vict.Death_type != DTYPE_HEAD {
					vict.Death_type = DTYPE_PULP
				}
			case 5:
				act(libc.CString("@R$N@r dies suddenly leaving behind a badly damaged corpse floating in space!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou die suddenly leaving behind a badly damaged corpse floating in space!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r dies suddenly leaving behind a badly damaged corpse floating in space!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				if vict.Death_type != DTYPE_HEAD {
					vict.Death_type = DTYPE_PULP
				}
			}
			...
			switch rand_number(1, 5) {
			case 1:
				act(libc.CString("@R$N@r coughs up blood before $s corpse starts to fall to the ground far below.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou coughs up blood before your corpse starts to fall to the ground far below.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r coughs up blood before $s corpse starts to fall to the ground far below.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			case 2:
				act(libc.CString("@R$N@r dies and $S corpse begins to fall to the ground below.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou die and your corpse begins to fall to the ground below.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r dies and $S corpse begins to fall to the ground below.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			case 3:
				act(libc.CString("@R$N@r cries out $S last breath before dying.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou cry out your last breath before dying.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r cries out $S last breath before dying.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			case 4:
				act(libc.CString("@R$N@r writhes in midair screaming in pain before finally dying!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou writhe in midair screaming in pain before finally dying!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r writhes in midair screaming in pain before finally dying!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				if vict.Death_type != DTYPE_HEAD {
					vict.Death_type = DTYPE_PULP
				}
			case 5:
				act(libc.CString("@R$N@r snaps back and dies with such force that blood flies into the air briefly!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou snap back and die with such force that blood flies into the air briefly!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r hits the ground dead with such force that blood flies into the air briefly!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				if vict.Death_type != DTYPE_HEAD {
					vict.Death_type = DTYPE_PULP
				}
			...
			switch rand_number(1, 5) {
			case 1:
				act(libc.CString("@R$N@r coughs up blood before $s corpse starts to float limply in the water.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou coughs up blood before your corpse starts to float limply in the water.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r coughs up blood before $s corpse starts to float limply in the water.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			case 2:
				act(libc.CString("@R$N@r dies and $S corpse begins to float limply in the water.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou die and your corpse begins to float limply in the water.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r dies and $S corpse begins to float limply in the water.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			case 3:
				act(libc.CString("@R$N@r cries out $S last breath before dying.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou cry out your last breath before dying.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r cries out $S last breath before dying.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			case 4:
				act(libc.CString("@R$N@r writhes and thrases in the water trying to scream before finally dying!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou writhe and thrash in the water trying to scream before finally dying!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r writhes and thrashes in the water trying to scream before finally dying!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				if vict.Death_type != DTYPE_HEAD {
					vict.Death_type = DTYPE_PULP
				}
			case 5:
				act(libc.CString("@R$N@r snaps back and dies with such force that blood floods out of $S body into the water!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou snap back and die with such force that blood floods out of your body into the water!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r hits the ground dead with such force that blood floods out of $S body into the water!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				if vict.Death_type != DTYPE_HEAD {
					vict.Death_type = DTYPE_PULP
				}
			}
		...
		if !SUNKEN(vict.In_room) && SECT(vict.In_room) != SECT_WATER_SWIM && SECT(vict.In_room) != SECT_WATER_NOSWIM && !ROOM_FLAGGED(vict.In_room, ROOM_SPACE) && SECT(vict.In_room) != SECT_FLYING {
			switch rand_number(1, 5) {
			case 1:
				act(libc.CString("@R$N@r explodes and chunks of $M shower to the ground.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou explode leaving only chunks behind.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r explodes and chunks of $M shower to the ground.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				vict.Death_type = DTYPE_VAPOR
			case 2:
				act(libc.CString("@rThe bottom half of @R$N@r is all that remains as $E dies.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rThe bottom half of your body is all that remains as you die.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@rThe bottom half of @R$N@r is all that remains as $E dies.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				vict.Death_type = DTYPE_HALF
			case 3:
				act(libc.CString("@R$N@r is completely disintegrated in the attack!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYour body completely disintegrates in the attack!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r is completely disintegrated in the attack!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				vict.Death_type = DTYPE_VAPOR
			case 4:
				act(libc.CString("@R$N@r falls down as a smoldering corpse!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYour body falls down as a smoldering corpse!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r falls down as a smoldering corpse!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			case 5:
				act(libc.CString("@rWhat's left of @R$N@r's body slams into the ground as $E dies!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rWhat's left of your body slams into the ground as you die!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@rWhat's left of @R$N@r's body slams into the ground as $E dies!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			}
		} else if SECT(vict.In_room) == SECT_WATER_SWIM || SECT(vict.In_room) == SECT_WATER_NOSWIM {
			switch rand_number(1, 5) {
			case 1:
			...
			switch rand_number(1, 5) {
			case 1:
				act(libc.CString("@R$N@r explodes and chunks of $M shower out into every direction of space.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou explode leaving only chunks behind.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r explodes and chunks of $M shower out into every direction of space.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				vict.Death_type = DTYPE_VAPOR
			case 2:
				act(libc.CString("@rThe bottom half of @R$N@r is all that remains as $E dies.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rThe bottom half of your body is all that remains as you die.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			...
			case 3:
				...
				vict.Death_type = DTYPE_VAPOR
			case 4:
				act(libc.CString("@R$N@r floats away as a smoldering corpse!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYour body floats away as a smoldering corpse!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r floats away as a smoldering corpse!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			case 5:
				act(libc.CString("@rWhat's left of @R$N@r's body floats away through space!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rWhat's left of your body floats away through space!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@rWhat's left of @R$N@r's body floats away through space!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			}
		} else if SECT(vict.In_room) == SECT_FLYING {
			switch rand_number(1, 5) {
			case 1:
				act(libc.CString("@R$N@r explodes and chunks of $M shower towards the ground far below.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou explode leaving only chunks behind.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r explodes and chunks of $M shower toward the ground far below.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				vict.Death_type = DTYPE_VAPOR
			case 2:
				act(libc.CString("@rThe bottom half of @R$N@r is all that remains as $E dies.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rThe bottom half of your body is all that remains as you die.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			...
			case 3:
				...
				vict.Death_type = DTYPE_VAPOR
			case 4:
				act(libc.CString("@R$N@r falls down toward the ground as a smoldering corpse!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYour body falls down toward the ground as a smoldering corpse!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r falls down toward the ground as a smoldering corpse!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			case 5:
				act(libc.CString("@rWhat's left of @R$N@r's body falls toward the ground as $E dies!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rWhat's left of yor body falls toward the ground as you die!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@rWhat's left of @R$N@r's body falls toward the ground as $E dies!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			}
		} else {
			switch rand_number(1, 5) {
			case 1:
				act(libc.CString("@R$N@r explodes and chunks of $M float freely through the water.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYou explode leaving only chunks behind.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r explodes and chunks of $M float freely through the water.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				vict.Death_type = DTYPE_VAPOR
			case 2:
				act(libc.CString("@rThe bottom half of @R$N@r is all that remains as $E dies.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rThe bottom half of your body is all that remains as you die.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			...
			case 3:
				...
				vict.Death_type = DTYPE_VAPOR
			case 4:
				act(libc.CString("@R$N@r falls back as a smoldering corpse!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rYour body falls back as a smoldering corpse!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@R$N@r falls back as a smoldering corpse!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			case 5:
				act(libc.CString("@rWhat's left of @R$N@r's body floats limply as $E dies!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@rWhat's left of yor body floats limply as you die!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@rWhat's left of @R$N@r's body floats limply as $E dies!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			}
		}
	}
}
