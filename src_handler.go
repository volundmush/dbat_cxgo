...
func is_name(str *byte, namelist *byte) bool {
	var (
		curname *byte
		curstr  *byte
	)
...
func isname(str *byte, namelist *byte) bool {
...
func aff_apply_modify(ch *char_data, loc int, mod int, spec int, msg *byte) {
	...
	default:
		basic_mud_log(libc.CString("SYSERR: Unknown apply adjust %d attempt (%s, affect_modify)."), loc, "__THISFILE__")
	}
}
func affect_modify(ch *char_data, loc int, mod int, spec int, bitv int, add bool) {
	if add {
		if bitv != AFF_INFRAVISION || int(ch.Race) != RACE_ANDROID {
			SET_BIT_AR(ch.Affected_by[:], uint32(int32(bitv)))
		}
	} else {
		if bitv != AFF_INFRAVISION || int(ch.Race) != RACE_ANDROID {
			REMOVE_BIT_AR(ch.Affected_by[:], uint32(int32(bitv)))
			mod = -mod
		}
	}
	aff_apply_modify(ch, loc, mod, spec, libc.CString("affect_modify"))
}
func affect_modify_ar(ch *char_data, loc int, mod int, spec int, bitv []uint32, add bool) {
	var (
		i int
		j int
	)
	if add {
		for i = 0; i < AF_ARRAY_MAX; i++ {
			for j = 0; j < 32; j++ {
				if IS_SET_AR(bitv, uint32(int32((i*32)+j))) {
					if (i*32)+j != AFF_INFRAVISION || int(ch.Race) != RACE_ANDROID {
						SET_BIT_AR(ch.Affected_by[:], uint32(int32((i*32)+j)))
					}
				}
			}
		}
		...
		for i = 0; i < AF_ARRAY_MAX; i++ {
			for j = 0; j < 32; j++ {
				if IS_SET_AR(bitv, uint32(int32((i*32)+j))) {
					if (i*32)+j != AFF_INFRAVISION || int(ch.Race) != RACE_ANDROID {
						REMOVE_BIT_AR(ch.Affected_by[:], uint32(int32((i*32)+j)))
					}
				}
			}
		}
...
func affect_total(ch *char_data) {
	...
	for af = ch.Affected; af != nil; af = af.Next {
		affect_modify(ch, af.Location, af.Modifier, af.Specific, int(af.Bitvector), false)
	}
	...
	for i = 0; i < NUM_WEARS; i++ {
		if (ch.Equipment[i]) != nil {
			if int((ch.Equipment[i]).Type_flag) == ITEM_ARMOR {
				...
				if !is_proficient_with_armor(ch, (ch.Equipment[i]).Value[VAL_ARMOR_SKILL]) {
					ch.Armorcheck += int16((ch.Equipment[i]).Value[VAL_ARMOR_CHECK])
				}
			}
			for j = 0; j < MAX_OBJ_AFFECT; j++ {
				affect_modify_ar(ch, (ch.Equipment[i]).Affected[j].Location, (ch.Equipment[i]).Affected[j].Modifier, (ch.Equipment[i]).Affected[j].Specific, (ch.Equipment[i]).Bitvector[:], true)
			}
		}
	}
	for af = ch.Affected; af != nil; af = af.Next {
		affect_modify(ch, af.Location, af.Modifier, af.Specific, int(af.Bitvector), true)
	}
...
func char_from_room(ch *char_data) {
	...
	if ch == nil || ch.In_room == int(-1) {
		basic_mud_log(libc.CString("SYSERR: NULL character or NOWHERE in %s, char_from_room"), "__THISFILE__")
		return
	}
...
func char_to_room(ch *char_data, room int) {
	var i int
	if ch == nil || room == int(-1) || room > top_of_world {
...
func invalid_align(ch *char_data, obj *obj_data) bool {
...
func equip_char(ch *char_data, obj *obj_data, pos int) {
	...
	if invalid_align(ch, obj) || invalid_class(ch, obj) || invalid_race(ch, obj) {
		act(libc.CString("You stop wearing $p as something prevents you."), 0, ch, obj, nil, TO_CHAR)
		act(libc.CString("$n stops wearing $p as something prevents $m."), 0, ch, obj, nil, TO_ROOM)
		obj_to_char(obj, ch)
		return
	}
	...
	for j = 0; j < MAX_OBJ_AFFECT; j++ {
		affect_modify_ar(ch, obj.Affected[j].Location, obj.Affected[j].Modifier, obj.Affected[j].Specific, obj.Bitvector[:], true)
	}
	affect_total(ch)
}
func unequip_char(ch *char_data, pos int) *obj_data {
	...
	for j = 0; j < MAX_OBJ_AFFECT; j++ {
		affect_modify_ar(ch, obj.Affected[j].Location, obj.Affected[j].Modifier, obj.Affected[j].Specific, obj.Bitvector[:], false)
	}
	affect_total(ch)
	return obj
}
...
func get_obj_num(nr int) *obj_data {
	var i *obj_data
...
func get_char_room(name *byte, number *int, room int) *char_data {
	var (
		i   *char_data
		num int
	)
...
func get_char_num(nr int) *char_data {
	var i *char_data
...
func obj_to_room(object *obj_data, room int) {
	var vehicle *obj_data = nil
	if object == nil || room == int(-1) || room > top_of_world {
		...
		if int(object.Type_flag) == ITEM_HATCH && GET_OBJ_VNUM(object) <= 0x4AFF {
			if GET_OBJ_VNUM(object) <= 0x4A37 && GET_OBJ_VNUM(object) >= 18800 || GET_OBJ_VNUM(object) <= 0x4AFF && GET_OBJ_VNUM(object) >= 19100 {
				var (
					...
					house *obj_data = read_object(hnum, VIRTUAL)
				)
				obj_to_room(house, real_room(object.Value[6]))
		...
		if int(object.Type_flag) == ITEM_HATCH && (object.Value[0]) > 1 && GET_OBJ_VNUM(object) > 0x4AFF {
			if (func() *obj_data {
				...
				if real_room(object.Value[3]) != int(-1) {
					vehicle = read_object(object.Value[0], VIRTUAL)
					obj_to_room(vehicle, real_room(object.Value[3]))
		...
		if (world[object.In_room].Dir_option[5]) != nil && (SECT(object.In_room) == SECT_UNDERWATER || SECT(object.In_room) == SECT_WATER_NOSWIM) {
			act(libc.CString("$p @Bsinks to deeper waters.@n"), 1, nil, object, nil, TO_ROOM)
		...
		if (world[object.In_room].Dir_option[5]) != nil && SECT(object.In_room) == SECT_FLYING && (GET_OBJ_VNUM(object) < 80 || GET_OBJ_VNUM(object) > 83) {
			act(libc.CString("$p @Cfalls down.@n"), 1, nil, object, nil, TO_ROOM)
			var numb int = int(libc.BoolToInt(GET_ROOM_VNUM((world[object.In_room].Dir_option[5]).To_room)))
			obj_from_room(object)
			obj_to_room(object, real_room(numb))
			if SECT(object.In_room) != SECT_FLYING {
				act(libc.CString("$p @Cfalls down and smacks the ground.@n"), 1, nil, object, nil, TO_ROOM)
			}
		}
		if (object.Value[0]) != 0 {
			if GET_OBJ_VNUM(object) == 0x4141 || GET_OBJ_VNUM(object) == 0x4142 || GET_OBJ_VNUM(object) == 0x4143 {
...
func obj_from_room(object *obj_data) {
	var temp *obj_data
	if object == nil || object.In_room == int(-1) {
...
func obj_to_obj(obj *obj_data, obj_to *obj_data) {
	...
	if obj == nil || obj_to == nil || obj == obj_to {
		basic_mud_log(libc.CString("SYSERR: NULL object (%p) or same source (%p) and target (%p VNUM: %d) obj passed to obj_to_obj."), obj, obj, obj_to, func() int {
			if obj_to != nil {
	...
	if obj_to.In_room != int(-1) && ROOM_FLAGGED(obj_to.In_room, ROOM_HOUSE) {
...
func obj_from_obj(obj *obj_data) {
	...
	if obj.In_obj == nil {
		basic_mud_log(libc.CString("SYSERR: (%s): trying to illegally extract obj from obj."), "__THISFILE__")
		return
	}
	obj_from = obj.In_obj
	temp = obj.In_obj
	...
	if obj_from.In_room != int(-1) && ROOM_FLAGGED(obj_from.In_room, ROOM_HOUSE) {
...
func extract_obj(obj *obj_data) {
	...
	if GET_OBJ_VNUM(obj) != 80 && GET_OBJ_VNUM(obj) != 81 {
		if obj.Item_number == int(-1) || obj.Proto_script != obj_proto[obj.Item_number].Proto_script {
...
func update_object(obj *obj_data, use int) {
	...
	if (obj.Script == nil || !IS_SET(uint32(int32(obj.Script.Types)), 1<<5)) && obj.Timer > 0 {
		obj.Timer -= use
	}
	if obj.Contains != nil {
		...
		if (ch.Equipment[i]) != nil {
			if int((ch.Equipment[i]).Type_flag) == ITEM_LIGHT && ((ch.Equipment[i]).Value[VAL_LIGHT_HOURS]) > 0 && ((ch.Equipment[i]).Value[VAL_LIGHT_TIME]) <= 0 {
				...
				if j == 1 {
					...
					act(libc.CString("$n's light begins to flicker and fade."), 0, ch, nil, nil, TO_ROOM)
				} else if j == 0 {
					send_to_char(ch, libc.CString("Your light sputters out and dies.\r\n"))
					act(libc.CString("$n's light sputters out and dies."), 0, ch, nil, nil, TO_ROOM)
					world[ch.In_room].Light--
				}
...
func extract_char_final(ch *char_data) {
	...
	if ch.In_room == int(-1) {
		basic_mud_log(libc.CString("SYSERR: NOWHERE extracting char %s. (%s, extract_char_final)"), GET_NAME(ch), "__THISFILE__")
		os.Exit(1)
	}
	...
	if ch.Grappling != nil {
		act(libc.CString("@WYou stop grappling with @C$N@W!@n"), 1, ch, nil, unsafe.Pointer(ch.Grappling), TO_CHAR)
		act(libc.CString("@C$n@W stops grappling with @c$N@W!@n"), 1, ch, nil, unsafe.Pointer(ch.Grappling), TO_ROOM)
		ch.Grappling.Grap = -1
	...
	if ch.Grappled != nil {
		act(libc.CString("@WYou stop being grappled with by @C$N@W!@n"), 1, ch, nil, unsafe.Pointer(ch.Grappled), TO_CHAR)
		act(libc.CString("@C$n@W stops being grappled with by @c$N@W!@n"), 1, ch, nil, unsafe.Pointer(ch.Grappled), TO_ROOM)
		ch.Grappled.Grap = -1
	...
	if ch.Dragged != nil {
		act(libc.CString("@WYou stop being dragged by @C$N@W!@n"), 1, ch, nil, unsafe.Pointer(ch.Dragged), TO_CHAR)
		act(libc.CString("@C$n@W stops being dragged by @c$N@W!@n"), 1, ch, nil, unsafe.Pointer(ch.Dragged), TO_ROOM)
		ch.Dragged.Drag = nil
		ch.Dragged = nil
	}
	if ch.Defender != nil {
...
func get_char_room_vis(ch *char_data, name *byte, number *int) *char_data {
	...
	for i = world[ch.In_room].People; i != nil && *number != 0; i = i.Next_in_room {
		...
		} else if isname(name, i.Name) && (IS_NPC(i) || IS_NPC(ch) || i.Admlevel > 0 || ch.Admlevel > 0) && i != ch {
			if CAN_SEE(ch, i) {
				if func() int {
					p := number
					*p--
...
func get_obj_in_equip_vis(ch *char_data, arg *byte, number *int, equipment []*obj_data) *obj_data {
	...
	for j = 0; j < NUM_WEARS; j++ {
		if equipment[j] != nil && CAN_SEE_OBJ(ch, equipment[j]) && isname(arg, equipment[j].Name) {
			if func() int {
				p := number
				*p--
				return *p
...
func generic_find(arg *byte, bitvector uint32, ch *char_data, tar_ch **char_data, tar_obj **obj_data) int {
	var (
		i        int
		found    int
		number   int
	...
	*tar_obj = nil
	...
	if IS_SET(bitvector, 1<<5) {
		for func() int {
			...
			if (ch.Equipment[i]) != nil && isname(name, (ch.Equipment[i]).Name) && func() int {
				p := &number
				*p--
				return *p
			}() == 0 {
...
func is_better(object *obj_data, object2 *obj_data) bool {
	var (
		value1 int = 0
		value2 int = 0
	)
	...
	if IS_HUMANOID(ch) && libc.FuncAddr(mob_index[ch.Nr].Func) != libc.FuncAddr(shop_keeper) {
		...
		case ITEM_WEAPON:
			if (ch.Equipment[WEAR_WIELD1]) == nil {
				...
				if is_better(object, ch.Equipment[WEAR_WIELD1]) {
		...
		case ITEM_WORN:
			...
			if (ch.Equipment[where]) == nil {
				...
				if is_better(object, ch.Equipment[where]) {
