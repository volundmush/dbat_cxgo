...
func real_zone_by_thing(vznum int) int {
	var (
		bot      int
		top      int
		mid      int
...
func create_new_zone(vzone_num int, bottom int, top int, error **byte) int {
	var (
		fp    *stdio.File
		zone  *zone_data
		i     int
	...
	if (func() *stdio.File {
		...
		mudlog(BRF, ADMLVL_IMPL, 1, libc.CString("SYSERR: OLC: Can't write new zone file."))
	...
	if (func() *stdio.File {
		...
		mudlog(BRF, ADMLVL_IMPL, 1, libc.CString("SYSERR: OLC: Can't write new world file."))
	...
	if (func() *stdio.File {
		...
		mudlog(BRF, ADMLVL_IMPL, 1, libc.CString("SYSERR: OLC: Can't write new mob file."))
	...
	if (func() *stdio.File {
		...
		mudlog(BRF, ADMLVL_IMPL, 1, libc.CString("SYSERR: OLC: Can't write new obj file."))
	...
	if (func() *stdio.File {
		...
		mudlog(BRF, ADMLVL_IMPL, 1, libc.CString("SYSERR: OLC: Can't write new shop file."))
	...
	if (func() *stdio.File {
		...
		mudlog(BRF, ADMLVL_IMPL, 1, libc.CString("SYSERR: OLC: Can't write new trigger file"))
	...
	if (func() *stdio.File {
		...
		mudlog(BRF, ADMLVL_IMPL, 1, libc.CString("SYSERR: OLC: Can't write new guild file"))
		*error = libc.CString("Could not write guild file.\r\n")
		return -1
	}
	stdio.Fprintf(fp, "$~\n")
	fp.Close()
	create_world_index(vzone_num, libc.CString("zon"))
	create_world_index(vzone_num, libc.CString("wld"))
	create_world_index(vzone_num, libc.CString("mob"))
	create_world_index(vzone_num, libc.CString("obj"))
	create_world_index(vzone_num, libc.CString("shp"))
	create_world_index(vzone_num, libc.CString("trg"))
	create_world_index(vzone_num, libc.CString("gld"))
	// todo: fix this
	//zone_table = []zone_data((*zone_data)(libc.Realloc(unsafe.Pointer(&zone_table[0]), top_of_zone_table*int(unsafe.Sizeof(zone_data{}))+2)))
	...
	for i = top_of_world; i > 0; i-- {
		if world[i].Zone < real_zone(rznum) {
			...
			world[i].Zone = real_zone_by_thing(int(libc.BoolToInt(GET_ROOM_VNUM(i))))
		}
	}
...
func create_world_index(znum int, type_ *byte) {
	...
	if (func() *stdio.File {
		...
		mudlog(BRF, ADMLVL_IMPL, 1, libc.CString("SYSERR: OLC: Failed to open %s."), &old_name[0])
		return
	} else if (func() *stdio.File {
		newfile = stdio.FOpen(libc.GoString(&new_name[0]), "w")
		return newfile
	}()) == nil {
		mudlog(BRF, ADMLVL_IMPL, 1, libc.CString("SYSERR: OLC: Failed to open %s."), &new_name[0])
		oldfile.Close()
		return
	}
...
func remove_room_zone_commands(zone int, room_num int) {
	var (
		subcmd   int = 0
		cmd_room int = -2
	)
...
func save_zone(zone_num int) bool {
	var (
		subcmd  int
		arg1    int = -1
		arg2    int = -1
	...
	if (func() *stdio.File {
		...
		mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("SYSERR: OLC: save_zones:  Can't write zone %d."), zone_table[zone_num].Number)
		return false
	}
	...
	for subcmd = 0; int(zone_table[zone_num].Cmd[subcmd].Command) != 'S'; subcmd++ {
		...
		case 'M':
			arg1 = mob_index[zone_table[zone_num].Cmd[subcmd].Arg1].Vnum
			arg2 = zone_table[zone_num].Cmd[subcmd].Arg2
			arg3 = world[zone_table[zone_num].Cmd[subcmd].Arg3].Number
			arg4 = zone_table[zone_num].Cmd[subcmd].Arg4
			arg5 = zone_table[zone_num].Cmd[subcmd].Arg5
			comment = mob_proto[zone_table[zone_num].Cmd[subcmd].Arg1].Short_descr
		case 'O':
			arg1 = obj_index[zone_table[zone_num].Cmd[subcmd].Arg1].Vnum
			arg2 = zone_table[zone_num].Cmd[subcmd].Arg2
			arg3 = world[zone_table[zone_num].Cmd[subcmd].Arg3].Number
			arg4 = zone_table[zone_num].Cmd[subcmd].Arg4
			arg5 = zone_table[zone_num].Cmd[subcmd].Arg5
		...
		case 'E':
			arg1 = obj_index[zone_table[zone_num].Cmd[subcmd].Arg1].Vnum
			arg2 = zone_table[zone_num].Cmd[subcmd].Arg2
			arg3 = zone_table[zone_num].Cmd[subcmd].Arg3
			arg4 = -1
			arg5 = zone_table[zone_num].Cmd[subcmd].Arg5
			comment = obj_proto[zone_table[zone_num].Cmd[subcmd].Arg1].Short_description
		case 'P':
			arg1 = obj_index[zone_table[zone_num].Cmd[subcmd].Arg1].Vnum
			arg2 = zone_table[zone_num].Cmd[subcmd].Arg2
			arg3 = obj_index[zone_table[zone_num].Cmd[subcmd].Arg3].Vnum
			arg4 = -1
			arg5 = zone_table[zone_num].Cmd[subcmd].Arg5
			comment = obj_proto[zone_table[zone_num].Cmd[subcmd].Arg1].Short_description
		case 'D':
			arg1 = world[zone_table[zone_num].Cmd[subcmd].Arg1].Number
			arg2 = zone_table[zone_num].Cmd[subcmd].Arg2
			arg3 = zone_table[zone_num].Cmd[subcmd].Arg3
			comment = world[zone_table[zone_num].Cmd[subcmd].Arg1].Name
		case 'R':
			arg1 = world[zone_table[zone_num].Cmd[subcmd].Arg1].Number
			arg2 = obj_index[zone_table[zone_num].Cmd[subcmd].Arg2].Vnum
			comment = obj_proto[zone_table[zone_num].Cmd[subcmd].Arg2].Short_description
			arg3 = -1
		case 'T':
			arg1 = zone_table[zone_num].Cmd[subcmd].Arg1
			arg2 = trig_index[zone_table[zone_num].Cmd[subcmd].Arg2].Vnum
			arg3 = world[zone_table[zone_num].Cmd[subcmd].Arg3].Number
			arg4 = -1
			arg5 = zone_table[zone_num].Cmd[subcmd].Arg5
			comment = trig_index[real_trigger(arg2)].Proto.Name
		case 'V':
			arg1 = zone_table[zone_num].Cmd[subcmd].Arg1
			arg2 = zone_table[zone_num].Cmd[subcmd].Arg2
			arg3 = world[zone_table[zone_num].Cmd[subcmd].Arg3].Number
		...
		case '*':
			continue
		default:
			mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("SYSERR: OLC: z_save_to_disk(): Unknown cmd '%c' - NOT saving"), zone_table[zone_num].Cmd[subcmd].Command)
			continue
		}
	...
	if in_save_list(zone_table[zone_num].Number, SL_ZON) {
		...
		create_world_index(zone_table[zone_num].Number, libc.CString("zon"))
...
func new_command(zone *zone_data, pos int) bool {
	var (
		subcmd  int = 0
		new_com reset_com
	)
