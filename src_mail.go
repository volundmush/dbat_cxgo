...
var mail_index *mail_index_type_d = nil
var free_list *position_list_type_d = nil
var file_end_pos int = 0

func free_mail_index() {
	var tmp *mail_index_type_d
	for mail_index != nil {
		tmp = mail_index.Next
		if mail_index.List_start != nil {
			var (
				i *position_list_type_d
				j *position_list_type_d
			)
...
func mail_recip_ok(name *byte) int {
	...
	if (func() int {
		...
		if !IS_SET(uint32(int32(player_table[player_i].Flags)), 1<<0) {
			ret = 1
		}
	}
	return ret
...
func push_free_list(pos int) {
	var new_pos *position_list_type_d
	new_pos = new(position_list_type_d)
...
func pop_free_list() int {
	var (
		old_pos      *position_list_type_d
		return_value int
	)
	if (func() *position_list_type_d {
		old_pos = free_list
		return old_pos
	}()) == nil {
		return file_end_pos
...
func find_char_in_index(searchee int) *mail_index_type_d {
	var tmp *mail_index_type_d
	if searchee < 0 {
		basic_mud_log(libc.CString("SYSERR: Mail system -- non fatal error #1 (searchee == %ld)."), searchee)
		return nil
	}
	for tmp = mail_index; tmp != nil && tmp.Recipient != searchee; tmp = tmp.Next {
	}
	return tmp
}
...
func scan_file() bool {
	var (
		...
		next_block     header_block_type_d
	...
	for int(mail_file.ReadN((*byte)(unsafe.Pointer(&next_block)), int(unsafe.Sizeof(header_block_type_d{})), 1)) != 0 {
...
func store_mail(to int, from int, message_pointer *byte) {
	var (
		header         header_block_type_d
		data           data_block_type_d
		last_address   int
		target_address int
	...
	if unsafe.Sizeof(header_block_type_d{}) != unsafe.Sizeof(data_block_type_d{}) || BLOCK_SIZE != unsafe.Sizeof(header_block_type_d{}) {
		panic("OOPS!")
		return
	}
	...
	*(*header_block_type_d)(unsafe.Pointer((*byte)(unsafe.Pointer(&header)))) = header_block_type_d{}
	header.Block_type = -1
	...
	*(*data_block_type_d)(unsafe.Pointer((*byte)(unsafe.Pointer(&data)))) = data_block_type_d{}
	data.Block_type = -2
...
func read_delete(recipient int, from **byte) *byte {
	var (
		header           header_block_type_d
		data             data_block_type_d
		mail_pointer     *mail_index_type_d
		prev_mail        *mail_index_type_d
		position_pointer *position_list_type_d
		mail_address     int
		following_block  int
		tmstr            *byte
		buf              [4352]byte
...
func postmaster(ch *char_data, me unsafe.Pointer, cmd int, argument *byte) bool {
...
func postmaster_send_mail(ch *char_data, mailman *char_data, cmd int, arg *byte) {
	...
	if GET_LEVEL(ch) < MIN_MAIL_LEVEL && ch.Admlevel < ADMLVL_IMMORT {
		...
		act(&buf[0], 0, mailman, nil, unsafe.Pointer(ch), TO_VICT)
		return
	}
	one_argument(arg, &buf[0])
	if buf[0] == 0 {
		act(libc.CString("$n tells you, 'You need to specify an addressee!'"), 0, mailman, nil, unsafe.Pointer(ch), TO_VICT)
		return
	}
	...
	if (func() int {
		...
		act(libc.CString("$n tells you, 'No one by that name is registered here!'"), 0, mailman, nil, unsafe.Pointer(ch), TO_VICT)
		return
	}
	act(libc.CString("$n starts to write some mail."), 1, ch, nil, nil, TO_ROOM)
	...
	act(&buf[0], 0, mailman, nil, unsafe.Pointer(ch), TO_VICT)
	act(libc.CString("@C$n@w starts writing a letter.@n"), 1, ch, nil, nil, TO_ROOM)
	ch.Gold -= STAMP_PRICE
...
func postmaster_check_mail(ch *char_data, mailman *char_data, cmd int, arg *byte) {
	if has_mail(int(ch.Idnum)) != 0 {
		act(libc.CString("$n tells you, 'You have mail waiting.'"), 0, mailman, nil, unsafe.Pointer(ch), TO_VICT)
	} else {
		act(libc.CString("$n tells you, 'Sorry, you don't have any mail waiting.'"), 0, mailman, nil, unsafe.Pointer(ch), TO_VICT)
	}
}
func postmaster_receive_mail(ch *char_data, mailman *char_data, cmd int, arg *byte) {
	...
	for has_mail(int(ch.Idnum)) != 0 {
		...
		if IS_PLAYING(ch.Desc) && mailman != nil {
			obj_to_char(obj, ch)
			act(libc.CString("$n gives you a piece of mail."), 0, mailman, nil, unsafe.Pointer(ch), TO_VICT)
			act(libc.CString("$N gives $n a piece of mail."), 0, ch, nil, unsafe.Pointer(mailman), TO_ROOM)
		} else {
			extract_obj(obj)
		}
	}
