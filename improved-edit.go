...
const CONFIG_IMPROVED_EDITOR = 1
const using_improved_editor = 1
const PARSE_FORMAT = 0
const PARSE_REPLACE = 1
const PARSE_HELP = 2
const PARSE_DELETE = 3
const PARSE_INSERT = 4
const PARSE_LIST_NORM = 5
const PARSE_LIST_NUM = 6
const PARSE_EDIT = 7
const STRINGADD_OK = 0
const STRINGADD_SAVE = 1
const STRINGADD_ABORT = 2
const STRINGADD_ACTION = 4
...
func parse_edit_action(command int, string_ *byte, d *descriptor_data) {
	...
	default:
		write_to_output(d, libc.CString("Invalid option.\r\n"))
		mudlog(BRF, ADMLVL_IMPL, TRUE, libc.CString("SYSERR: invalid command passed to parse_edit_action"))
		return
	}
}
func format_text(ptr_string **byte, mode int, d *descriptor_data, maxlen uint, low int, high int) int {
	var (
		line_chars    int
		cap_next      int = TRUE
		cap_next_next int = FALSE
		color_chars   int = 0
		i             int
		pass_line     int = 0
		flow          *byte
