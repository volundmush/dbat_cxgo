...
func House_get_filename(vnum int, filename *byte, maxlen uint64) bool {
	if vnum == int(-1) {
		return false
	}
...
func House_save(obj *obj_data, fp *stdio.File, location int) bool {
	var (
		tmp    *obj_data
		result int
	)
...
func House_crashsave(vnum int) {
	...
	if !House_get_filename(vnum, &buf[0], uint64(64936)) {
		return
	}
	if (func() *stdio.File {
...
func House_delete_file(vnum int) {
	var (
		filename [2048]byte
		fl       *stdio.File
	)
	if !House_get_filename(vnum, &filename[0], uint64(2048)) {
		return
	}
	if (func() *stdio.File {
...
func hcontrol_build_house(ch *char_data, arg *byte) {
	...
	virt_house = libc.Atoi(libc.GoString(&arg1[0]))
	if (func() int {
	...
	if int(func() int16 {
		exit_num = int16(search_block(&arg1[0], &dirs[0], 0))
		return exit_num
	}()) < 0 && int(func() int16 {
		exit_num = int16(search_block(&arg1[0], &abbr_dirs[0], 0))
		return exit_num
	}()) < 0 {
...
func hcontrol_destroy_house(ch *char_data, arg *byte) {
	...
	if (func() int {
		i = find_house(libc.Atoi(libc.GoString(arg)))
		return i
	}()) == int(-1) {
...
func hcontrol_pay_house(ch *char_data, arg *byte) {
	...
	if *arg == 0 {
		...
		mudlog(NRM, int(MAX(ADMLVL_IMMORT, int64(ch.Player_specials.Invis_level))), 1, libc.CString("Payment for house %s collected by %s."), arg, GET_NAME(ch))
...
func do_hcontrol(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if is_abbrev(&arg1[0], libc.CString("build")) {
		hcontrol_build_house(ch, &arg2[0])
	} else if is_abbrev(&arg1[0], libc.CString("destroy")) {
		hcontrol_destroy_house(ch, &arg2[0])
	} else if is_abbrev(&arg1[0], libc.CString("pay")) {
		hcontrol_pay_house(ch, &arg2[0])
	} else if is_abbrev(&arg1[0], libc.CString("show")) {
...
func do_house(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if !ROOM_FLAGGED(ch.In_room, ROOM_HOUSE) {
		...
		i = find_house(int(libc.BoolToInt(GET_ROOM_VNUM(ch.In_room))))
		return i
	}()) == int(-1) {
...
func House_can_enter(ch *char_data, house int) bool {
	var (
		i int
		j int
	)
...
func House_load(rvnum int) bool {
	var (
		fl      *stdio.File
		f1      [256]byte
		f2      [256]byte
	...
	if !House_get_filename(rvnum, &cmfname[0], uint64(64936)) {
		return false
	}
	if (func() *stdio.File {
