...
func do_oasis_zedit(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if buf1[0] == 0 {
		...
		if libc.StrCaseCmp(libc.CString("save"), &buf1[0]) == 0 {
			...
			if libc.StrCaseCmp(libc.CString("new"), &buf1[0]) != 0 || buf3 == nil || *buf3 == 0 {
				...
				bottom = libc.Atoi(libc.GoString(&sbot[0]))
				top = libc.Atoi(libc.GoString(&stop[0]))
	...
	d = ch.Desc
	if d.Olc != nil {
		mudlog(BRF, ADMLVL_IMMORT, 1, libc.CString("SYSERR: do_oasis_zedit: Player already had olc structure."))
	...
	if save != 0 {
		...
		mudlog(CMP, int(MAX(ADMLVL_BUILDER, int64(ch.Player_specials.Invis_level))), 1, libc.CString("OLC: %s saves zone information for zone %d."), GET_NAME(ch), zone_table[d.Olc.Zone_num].Number)
	...
	mudlog(CMP, ADMLVL_IMMORT, 1, libc.CString("OLC: %s starts editing zone %d allowed zone %d"), GET_NAME(ch), zone_table[d.Olc.Zone_num].Number, ch.Player_specials.Olc_zone)
}
func zedit_setup(d *descriptor_data, room_num int) {
	...
	for int(zone_table[d.Olc.Zone_num].Cmd[subcmd].Command) != 'S' {
		...
		case 'T':
			fallthrough
		case 'V':
			cmd_room = zone_table[d.Olc.Zone_num].Cmd[subcmd].Arg3
		case 'D':
			fallthrough
		case 'R':
			cmd_room = zone_table[d.Olc.Zone_num].Cmd[subcmd].Arg1
		default:
		}
...
func zedit_new_zone(ch *char_data, vzone_num int, bottom int, top int) {
	var (
		result int
		error  *byte
		dsc    *descriptor_data
	...
	if (func() int {
		result = create_new_zone(vzone_num, bottom, top, &error)
		return result
	}()) == int(-1) {
	...
	for dsc = descriptor_list; dsc != nil; dsc = dsc.Next {
		...
		case CON_REDIT:
			dsc.Olc.Room.Zone += int(libc.BoolToInt(dsc.Olc.Zone_num >= result))
			fallthrough
		case CON_ZEDIT:
			fallthrough
		case CON_MEDIT:
		...
		case CON_TRIGEDIT:
			fallthrough
		case CON_GEDIT:
			dsc.Olc.Zone_num += int(libc.BoolToInt(dsc.Olc.Zone_num >= result))
		default:
		}
	}
	zedit_save_to_disk(result)
	mudlog(BRF, int(MAX(ADMLVL_BUILDER, int64(ch.Player_specials.Invis_level))), 1, libc.CString("OLC: %s creates new zone #%d"), GET_NAME(ch), vzone_num)
...
func zedit_save_internally(d *descriptor_data) {
	var (
		...
		objloaded int = 0
		subcmd    int
		room_num  int = real_room(d.Olc.Number)
	)
	if room_num == int(-1) {
...
func start_change_command(d *descriptor_data, pos int) bool {
...
func zedit_disp_arg1(d *descriptor_data) {
	...
	case 'D':
		fallthrough
	case 'R':
		(d.Olc.Zone.Cmd[d.Olc.Value]).Arg1 = real_room(d.Olc.Number)
		zedit_disp_arg2(d)
	case 'T':
		fallthrough
	case 'V':
	...
	default:
		...
		mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("SYSERR: OLC: zedit_disp_arg1(): Help!"))
...
func zedit_disp_arg2(d *descriptor_data) {
	...
	default:
		cleanup_olc(d, CLEANUP_ALL)
		mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("SYSERR: OLC: zedit_disp_arg2(): Help!"))
		write_to_output(d, libc.CString("Oops...\r\n"))
		return
	}
...
func zedit_disp_arg3(d *descriptor_data) {
	...
	default:
		cleanup_olc(d, CLEANUP_ALL)
		mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("SYSERR: OLC: zedit_disp_arg3(): Help!"))
		write_to_output(d, libc.CString("Oops...\r\n"))
		return
	}
...
func zedit_disp_arg4(d *descriptor_data) {
	...
	default:
		cleanup_olc(d, CLEANUP_ALL)
		mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("SYSERR: OLC: zedit_disp_arg4(): Help!"))
		write_to_output(d, libc.CString("Oops...\r\n"))
		return
	}
...
func zedit_disp_arg5(d *descriptor_data) {
	...
	default:
		cleanup_olc(d, CLEANUP_ALL)
		mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("SYSERR: OLC: zedit_disp_arg5(): Help!"))
		write_to_output(d, libc.CString("Oops...\r\n"))
		return
	}
...
func zedit_parse(d *descriptor_data, arg *byte) {
	...
	case ZEDIT_CONFIRM_SAVESTRING:
		...
		case 'Y':
			...
			if config_info.Operation.Auto_save_olc != 0 {
				...
				zedit_save_to_disk(d.Olc.Zone_num)
			} else {
				write_to_output(d, libc.CString("Saving zone info in memory.\r\n"))
			}
			mudlog(CMP, int(MAX(ADMLVL_BUILDER, int64(d.Character.Player_specials.Invis_level))), 1, libc.CString("OLC: %s edits zone info for room %d."), GET_NAME(d.Character), d.Olc.Number)
			fallthrough
		case 'n':
			fallthrough
		case 'N':
	...
	case ZEDIT_MAIN_MENU:
		...
		case 'N':
			if int(d.Olc.Zone.Cmd[0].Command) == 'S' {
				if new_command(d.Olc.Zone, 0) && start_change_command(d, 0) {
	...
	case ZEDIT_NEW_ENTRY:
		...
		if unicode.IsDigit(rune(*arg)) && new_command(d.Olc.Zone, pos) {
	...
	case ZEDIT_CHANGE_ENTRY:
		...
		pos = libc.Atoi(libc.GoString(arg))
		if unicode.IsDigit(rune(*arg)) && start_change_command(d, pos) {
			zedit_disp_comtype(d)
			d.Olc.Zone.Age = 1
		} else {
			zedit_disp_menu(d)
	...
	case ZEDIT_ARG1:
		...
		case 'M':
			if (func() int {
				pos = real_mobile(libc.Atoi(libc.GoString(arg)))
				return pos
			}()) != int(-1) {
		...
		case 'G':
			if (func() int {
				pos = real_object(libc.Atoi(libc.GoString(arg)))
				return pos
			}()) != int(-1) {
				(d.Olc.Zone.Cmd[d.Olc.Value]).Arg1 = pos
				zedit_disp_arg2(d)
		...
		case 'V':
			if libc.Atoi(libc.GoString(arg)) < MOB_TRIGGER || libc.Atoi(libc.GoString(arg)) > WLD_TRIGGER {
				...
				(d.Olc.Zone.Cmd[d.Olc.Value]).Arg1 = libc.Atoi(libc.GoString(arg))
				zedit_disp_arg2(d)
			}
		case 'D':
			fallthrough
		...
		default:
			cleanup_olc(d, CLEANUP_ALL)
			mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("SYSERR: OLC: zedit_parse(): case ARG1: Ack!"))
			write_to_output(d, libc.CString("Oops...\r\n"))
		}
	case ZEDIT_ARG2:
		if !unicode.IsDigit(rune(*arg)) {
		...
		case 'M':
			fallthrough
		case 'O':
			(d.Olc.Zone.Cmd[d.Olc.Value]).Arg2 = int(MIN(MAX_DUPLICATES, int64(libc.Atoi(libc.GoString(arg)))))
			(d.Olc.Zone.Cmd[d.Olc.Value]).Arg3 = real_room(d.Olc.Number)
			zedit_disp_arg4(d)
		case 'G':
			(d.Olc.Zone.Cmd[d.Olc.Value]).Arg2 = int(MIN(MAX_DUPLICATES, int64(libc.Atoi(libc.GoString(arg)))))
			zedit_disp_arg5(d)
		...
		case 'E':
			...
			zedit_disp_arg3(d)
		case 'V':
			(d.Olc.Zone.Cmd[d.Olc.Value]).Arg2 = libc.Atoi(libc.GoString(arg))
			(d.Olc.Zone.Cmd[d.Olc.Value]).Arg3 = real_room(d.Olc.Number)
			write_to_output(d, libc.CString("Enter the global name : "))
			d.Olc.Mode = ZEDIT_SARG1
		case 'T':
			if real_trigger(libc.Atoi(libc.GoString(arg))) != int(-1) {
				(d.Olc.Zone.Cmd[d.Olc.Value]).Arg2 = real_trigger(libc.Atoi(libc.GoString(arg)))
				(d.Olc.Zone.Cmd[d.Olc.Value]).Arg3 = real_room(d.Olc.Number)
				zedit_disp_menu(d)
			} else {
		...
		default:
			cleanup_olc(d, CLEANUP_ALL)
			mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("SYSERR: OLC: zedit_parse(): case ARG2: Ack!"))
			write_to_output(d, libc.CString("Oops...\r\n"))
		}
	case ZEDIT_ARG3:
		if !unicode.IsDigit(rune(*arg)) {
		...
		default:
			cleanup_olc(d, CLEANUP_ALL)
			mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("SYSERR: OLC: zedit_parse(): case ARG3: Ack!"))
			write_to_output(d, libc.CString("Oops...\r\n"))
		}
	case ZEDIT_ARG4:
		if *arg == 0 {
		...
		case 'M':
			fallthrough
		case 'O':
			(d.Olc.Zone.Cmd[d.Olc.Value]).Arg4 = int(MIN(MAX_FROM_ROOM, int64(libc.Atoi(libc.GoString(arg)))))
			zedit_disp_arg5(d)
		case 'G':
			fallthrough
		case 'P':
		...
		default:
			cleanup_olc(d, CLEANUP_ALL)
			mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("SYSERR: OLC: zedit_parse(): case ARG4: Ack!"))
			write_to_output(d, libc.CString("Oops...\r\n"))
		}
	case ZEDIT_ARG5:
		if !unicode.IsDigit(rune(*arg)) {
		...
		case 'P':
			fallthrough
		case 'E':
			(d.Olc.Zone.Cmd[d.Olc.Value]).Arg5 = libc.Atoi(libc.GoString(arg))
			zedit_disp_menu(d)
		case 'V':
			fallthrough
		case 'T':
		...
		default:
			cleanup_olc(d, CLEANUP_ALL)
			mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("SYSERR: OLC: zedit_parse(): case ARG5: Ack!"))
			write_to_output(d, libc.CString("Oops...\r\n"))
		}
	case ZEDIT_SARG1:
	...
	case ZEDIT_ZONE_BOT:
		if d.Olc.Zone_num == 0 {
			d.Olc.Zone.Bot = int(MIN(int64(d.Olc.Zone.Top), MAX(int64(libc.Atoi(libc.GoString(arg))), 0)))
		} else {
			d.Olc.Zone.Bot = int(MIN(int64(d.Olc.Zone.Top), MAX(int64(libc.Atoi(libc.GoString(arg))), int64(zone_table[d.Olc.Zone_num-1].Top+1))))
		}
		d.Olc.Zone.Number = 1
		zedit_disp_menu(d)
	case ZEDIT_ZONE_TOP:
		if d.Olc.Zone_num == top_of_zone_table {
			d.Olc.Zone.Top = int(MIN(65000, MAX(int64(libc.Atoi(libc.GoString(arg))), int64(genolc_zonep_bottom(d.Olc.Zone)))))
		} else {
			d.Olc.Zone.Top = int(MIN(int64(genolc_zone_bottom(d.Olc.Zone_num+1)-1), MAX(int64(libc.Atoi(libc.GoString(arg))), int64(genolc_zonep_bottom(d.Olc.Zone)))))
		}
		d.Olc.Zone.Number = 1
		zedit_disp_menu(d)
	case ZEDIT_ZONE_FLAGS:
		...
		if number < 0 || number > NUM_ZONE_FLAGS {
			...
			TOGGLE_BIT_AR(d.Olc.Zone.Zone_flags[:], uint32(int32(number-1)))
			d.Olc.Zone.Number = 1
			zedit_disp_flag_menu(d)
		}
		return
	...
	default:
		cleanup_olc(d, CLEANUP_ALL)
		mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("SYSERR: OLC: zedit_parse(): Reached default case!"))
		write_to_output(d, libc.CString("Oops...\r\n"))
	}
}
