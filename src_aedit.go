...
func do_oasis_aedit(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if libc.StrCaseCmp(libc.CString("save"), &arg[0]) == 0 {
		mudlog(CMP, int(MAX(ADMLVL_BUILDER, int64(ch.Player_specials.Invis_level))), 1, libc.CString("OLC: %s saves socials."), GET_NAME(ch))
	...
	for d.Olc.Zone_num = 0; d.Olc.Zone_num <= top_of_socialt; d.Olc.Zone_num++ {
		if is_abbrev(d.Olc.Storage, soc_mess_list[d.Olc.Zone_num].Command) {
			break
		}
	}
	if d.Olc.Zone_num > top_of_socialt {
		if (func() int {
	...
	mudlog(CMP, ADMLVL_IMMORT, 1, libc.CString("OLC: %s starts editing actions."), GET_NAME(ch))
}
...
func aedit_save_internally(d *descriptor_data) {
	...
	if d.Olc.Zone_num > top_of_socialt {
		...
		for i = 0; i <= top_of_socialt; i++ {
			*(*social_messg)(unsafe.Add(unsafe.Pointer(new_soc_mess_list), unsafe.Sizeof(social_messg{})*uintptr(i))) = soc_mess_list[i]
		}
		*(*social_messg)(unsafe.Add(unsafe.Pointer(new_soc_mess_list), unsafe.Sizeof(social_messg{})*uintptr(func() int {
		...
		// todo: fix this
		//soc_mess_list = []social_messg(new_soc_mess_list)
	} else {
		i = aedit_find_command(d.Olc.Action.Command)
		d.Olc.Action.Act_nr = soc_mess_list[d.Olc.Zone_num].Act_nr
		free_action(&soc_mess_list[d.Olc.Zone_num])
		soc_mess_list[d.Olc.Zone_num] = *d.Olc.Action
	}
	create_command_list()
	sort_commands()
...
func aedit_parse(d *descriptor_data, arg *byte) {
	...
	case AEDIT_CONFIRM_SAVESTRING:
		...
		case 'Y':
			...
			mudlog(CMP, ADMLVL_IMPL, 1, libc.CString("OLC: %s edits action %s"), GET_NAME(d.Character), d.Olc.Action.Command)
	...
	case AEDIT_CONFIRM_EDIT:
		...
		case 'y':
			fallthrough
		case 'Y':
			aedit_setup_existing(d, d.Olc.Zone_num)
		case 'q':
			fallthrough
		case 'Q':
			cleanup_olc(d, CLEANUP_ALL)
		...
		case 'N':
			d.Olc.Zone_num++
			for ; d.Olc.Zone_num <= top_of_socialt; d.Olc.Zone_num++ {
				if is_abbrev(d.Olc.Storage, soc_mess_list[d.Olc.Zone_num].Command) {
					break
				}
			}
...
func do_astat(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	for i = 0; i <= top_of_socialt; i++ {
		if is_abbrev(&arg[0], soc_mess_list[i].Command) {
			real = 1
			break
		}
	}
