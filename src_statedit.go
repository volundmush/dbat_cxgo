...
func parse_stats(d *descriptor_data, arg *byte) bool {
	var ch *char_data
	ch = d.Character
	switch d.Olc.Mode {
	case STAT_QUIT:
...
func parse_stat_menu(d *descriptor_data, arg *byte) bool {
...
func stats_assign_stat(abil int, arg *byte, d *descriptor_data) int {
	...
	if d.Olc.Value <= 0 {
		temp = 0
		d.Olc.Value = 0
		mudlog(NRM, ADMLVL_IMMORT, 1, libc.CString("Stat total below 0: possible code error"))
	}
	abil = temp
	d.Olc.Value -= temp
	return abil
