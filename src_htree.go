...
func htree_add(root *htree_node, htindex int, content int) {
	var (
		tmp   *htree_node
		i     int
		depth int
	...
	for htindex != 0 {
		depth++
		i = htindex & ((int(1 << HTREE_NODE_BITS)) - 1)
...
func htree_find_node(root *htree_node, htindex int) *htree_node {
	var (
		tmp *htree_node
		i   int
	)
...
func htree_del(root *htree_node, htindex int) {
	var tmp *htree_node
	tmp = htree_find_node(root, htindex)
	tmp.Content = -1
}
func htree_find(root *htree_node, htindex int) int {
	var tmp *htree_node
	tmp = htree_find_node(root, htindex)
	return tmp.Content
}
func real_room_old(vnum int) int {
	var (
		bot int
		top int
		mid int
...
func htree_test() {
	basic_mud_log(libc.CString("htree stats (global): %d nodes, %lld bytes (depth %d/%lld used/possible)"), htree_total_nodes, htree_total_nodes*int(unsafe.Sizeof(htree_node{})), htree_depth_used, ((unsafe.Sizeof(int(0))*8)/HTREE_NODE_BITS)+1)
}
