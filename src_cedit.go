...
func do_oasis_cedit(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if buf1[0] == 0 {
		...
		mudlog(BRF, ADMLVL_IMMORT, 1, libc.CString("OLC: %s starts editing the game configuration."), GET_NAME(ch))
		return
	...
	mudlog(CMP, int(MAX(ADMLVL_BUILDER, int64(ch.Player_specials.Invis_level))), 1, libc.CString("OLC: %s saves the game configuration."), GET_NAME(ch))
	cedit_save_to_disk()
}
...
func cedit_save_internally(d *descriptor_data) {
	var copyover_needed bool = false
	_ = copyover_needed
...
func save_config(nowhere int) bool {
	var (
		fl  *stdio.File
		buf [64936]byte
	)
	if (func() *stdio.File {
		...
		fmt.Println(libc.CString("SYSERR: save_config"))
		return false
	}
	...
	stdio.Fprintf(fl, "* The virtual numbers of the donation rooms.  Note: Add donation rooms\n* sequentially (1 & 2 before 3). If you don't, you might not be able to\n* donate. Use -1 for 'no such room'.\ndonation_room_1 = %d\ndonation_room_2 = %d\ndonation_room_3 = %d\n\n", func() int {
		if config_info.Room_nums.Donation_room_1 != int(-1) {
			return config_info.Room_nums.Donation_room_1
		}
		return -1
	}(), func() int {
		if config_info.Room_nums.Donation_room_2 != int(-1) {
			return config_info.Room_nums.Donation_room_2
		}
		return -1
	}(), func() int {
		if config_info.Room_nums.Donation_room_3 != int(-1) {
	...
	fl.Close()
	if in_save_list(int(-1), SL_CFG) {
		remove_from_save_list(int(-1), SL_CFG)
	}
	return true
}
...
func cedit_parse(d *descriptor_data, arg *byte) {
	...
	case CEDIT_CONFIRM_SAVESTRING:
		...
		case 'Y':
			...
			mudlog(CMP, int(MAX(ADMLVL_BUILDER, int64(d.Character.Player_specials.Invis_level))), 1, libc.CString("OLC: %s modifies the game configuration."), GET_NAME(d.Character))
	...
	case CEDIT_MORTAL_START_ROOM:
		...
		} else if real_room(libc.Atoi(libc.GoString(arg))) == int(-1) {
			write_to_output(d, libc.CString("That room doesn't exist!\r\nEnter the room's vnum where mortals should load into : "))
		} else {
			d.Olc.Config.Room_nums.Mortal_start_room = libc.Atoi(libc.GoString(arg))
			cedit_disp_room_numbers(d)
		}
	case CEDIT_IMMORT_START_ROOM:
		if *arg == 0 {
			...
			d.Olc.Config.Room_nums.Immort_start_room = libc.Atoi(libc.GoString(arg))
			cedit_disp_room_numbers(d)
		}
	case CEDIT_FROZEN_START_ROOM:
		if *arg == 0 {
			...
			d.Olc.Config.Room_nums.Frozen_start_room = libc.Atoi(libc.GoString(arg))
			cedit_disp_room_numbers(d)
		}
	case CEDIT_DONATION_ROOM_1:
		if *arg == 0 {
			...
			d.Olc.Config.Room_nums.Donation_room_1 = libc.Atoi(libc.GoString(arg))
			cedit_disp_room_numbers(d)
		}
	case CEDIT_DONATION_ROOM_2:
		if *arg == 0 {
			...
			d.Olc.Config.Room_nums.Donation_room_2 = libc.Atoi(libc.GoString(arg))
			cedit_disp_room_numbers(d)
		}
	case CEDIT_DONATION_ROOM_3:
		if *arg == 0 {
			...
			d.Olc.Config.Room_nums.Donation_room_3 = libc.Atoi(libc.GoString(arg))
			cedit_disp_room_numbers(d)
		}
	...
	default:
		cleanup_olc(d, CLEANUP_CONFIG)
		mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("SYSERR: OLC: cedit_parse(): Reached default case!"))
