...
func do_action(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if int(vict.Position) < action.Min_victim_position {
		act(libc.CString("$N is not in a proper position for that."), 0, ch, nil, unsafe.Pointer(vict), int(TO_CHAR|2<<7))
	} else {
		if part[0] != 0 {
				...
				case 0:
					if int(ch.Sex) == SEX_MALE {
						if int(victim.Sex) == SEX_MALE {
							act(libc.CString("$n accuses you of fighting like a woman!"), 0, ch, nil, unsafe.Pointer(victim), TO_VICT)
						} else {
							act(libc.CString("$n says that women can't fight."), 0, ch, nil, unsafe.Pointer(victim), TO_VICT)
						}
					} else {
						if int(victim.Sex) == SEX_MALE {
							act(libc.CString("$n accuses you of having the smallest... (brain?)"), 0, ch, nil, unsafe.Pointer(victim), TO_VICT)
						} else {
							act(libc.CString("$n tells you that you'd lose a beauty contest against a troll."), 0, ch, nil, unsafe.Pointer(victim), TO_VICT)
						}
					}
				case 1:
					act(libc.CString("$n calls your mother a bitch!"), 0, ch, nil, unsafe.Pointer(victim), TO_VICT)
				default:
					act(libc.CString("$n tells you to get lost!"), 0, ch, nil, unsafe.Pointer(victim), TO_VICT)
				}
				act(libc.CString("$n insults $N."), 1, ch, nil, unsafe.Pointer(victim), TO_NOTVICT)
			} else {
...
func boot_social_messages() {
	...
	if config_info.Operation.Use_new_socials == 1 {
		if (func() *stdio.File {
	...
	for {
		...
		if config_info.Operation.Use_new_socials == 0 && soc_mess_list[curr_soc].Char_found == nil {
			continue
		}
...
func do_gmote(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if (func() int {
		...
		act(&buf[0], 0, ch, nil, unsafe.Pointer(vict), TO_GMOTE)
		return
	}
