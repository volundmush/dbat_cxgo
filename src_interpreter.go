...
func command_interpreter(ch *char_data, argument *byte) {
	...
	{
		var cont int
		cont = int(libc.BoolToInt(command_wtrigger(ch, &arg[0], line)))
		if cont == 0 {
			cont = int(libc.BoolToInt(command_mtrigger(ch, &arg[0], line)))
		}
		if cont == 0 {
			cont = int(libc.BoolToInt(command_otrigger(ch, &arg[0], line)))
		}
		if cont != 0 {
			return
		}
	...
	if *complete_cmd_info[cmd].Command == '\n' {
		...
		return
	} else if !command_pass(&blah[0], ch) && ch.Admlevel < 1 {
		send_to_char(ch, libc.CString("It's unfortunate...\r\n"))
	} else if check_disabled(&complete_cmd_info[cmd]) {
	...
	} else if no_specials != 0 || !special(ch, cmd, line) {
		if skip_ld == 0 {
...
func perform_alias(d *descriptor_data, orig *byte, maxlen uint64) bool {
	var (
		first_arg [2048]byte
		ptr       *byte
		a         *alias_data
...
func fill_word(argument *byte) int {
	return int(libc.BoolToInt(search_block(argument, &fill[0], 1) >= 0))
}
func topLoad() {
	var (
		file   *stdio.File
...
func reserved_word(argument *byte) int {
	return int(libc.BoolToInt(search_block(argument, &reserved[0], 1) >= 0))
}
...
func is_abbrev(arg1 *byte, arg2 *byte) bool {
	if *arg1 == 0 {
		return false
	}
...
func special(ch *char_data, cmd int, arg *byte) bool {
	var (
		i *obj_data
		k *char_data
		j int
	...
	if GET_ROOM_SPEC(ch.In_room) != nil {
		if GET_ROOM_SPEC(ch.In_room)(ch, unsafe.Pointer(&world[ch.In_room]), cmd, arg) {
			return true
		}
	}
	for j = 0; j < NUM_WEARS; j++ {
		if (ch.Equipment[j]) != nil && GET_OBJ_SPEC(ch.Equipment[j]) != nil {
			if GET_OBJ_SPEC(ch.Equipment[j])(ch, unsafe.Pointer(ch.Equipment[j]), cmd, arg) {
				return true
			}
		}
	}
	for i = ch.Carrying; i != nil; i = i.Next_content {
		if GET_OBJ_SPEC(i) != nil {
			if GET_OBJ_SPEC(i)(ch, unsafe.Pointer(i), cmd, arg) {
				return true
			}
		}
	}
	for k = world[ch.In_room].People; k != nil; k = k.Next_in_room {
		if !MOB_FLAGGED(k, MOB_NOTDEADYET) {
			if GET_MOB_SPEC(k) != nil && GET_MOB_SPEC(k)(ch, unsafe.Pointer(k), cmd, arg) {
				return true
			}
		}
	}
...
func _parse_name(arg *byte, name *byte) bool {
	var i int
	skip_spaces(&arg)
	for i = 0; (func() byte {
		p := name
...
func perform_dupe_check(d *descriptor_data) int {
	...
	case RECON:
		...
		act(libc.CString("$n has reconnected."), 1, d.Character, nil, nil, TO_ROOM)
		mudlog(NRM, int(MAX(ADMLVL_NONE, int64(d.Character.Player_specials.Invis_level))), 1, libc.CString("%s [%s] has reconnected."), GET_NAME(d.Character), &d.Host[0])
	...
	case USURP:
		...
		act(libc.CString("$n suddenly keels over in pain, surrounded by a white aura...\r\n$n's body has been taken over by a new spirit!"), 1, d.Character, nil, nil, TO_ROOM)
		d.Character.Rp = d.Rpp
		mudlog(NRM, int(MAX(ADMLVL_IMMORT, int64(d.Character.Player_specials.Invis_level))), 1, libc.CString("%s has re-logged in ... disconnecting old socket."), GET_NAME(d.Character))
	case UNSWITCH:
		write_to_output(d, libc.CString("Reconnecting to unswitched char."))
		mudlog(NRM, int(MAX(ADMLVL_IMMORT, int64(d.Character.Player_specials.Invis_level))), 1, libc.CString("%s [%s] has reconnected."), GET_NAME(d.Character), &d.Host[0])
	}
	return 1
}
func enter_player_game(d *descriptor_data) int {
	...
	if (func() int {
		load_room = d.Character.Player_specials.Load_room
		return load_room
	}()) != int(-1) {
	...
	if load_room == int(-1) {
		if d.Character.Admlevel != 0 {
			load_room = real_room(config_info.Room_nums.Immort_start_room)
		} else {
			load_room = real_room(config_info.Room_nums.Mortal_start_room)
		}
	}
	if PLR_FLAGGED(d.Character, PLR_FROZEN) {
		load_room = real_room(config_info.Room_nums.Frozen_start_room)
	}
	d.Character.Next = character_list
	character_list = d.Character
	char_to_room(d.Character, load_room)
	load_result = int(libc.BoolToInt(Crash_load(d.Character)))
	...
	if MOON_UP == 0 && (int(d.Character.Race) == RACE_SAIYAN || int(d.Character.Race) == RACE_HALFBREED) {
		oozaru_drop(d.Character)
	}
	if MOON_UP == 1 && (int(d.Character.Race) == RACE_SAIYAN || int(d.Character.Race) == RACE_HALFBREED) {
...
func readUserIndex(name *byte) bool {
	var (
		fname [40]byte
		fl    *stdio.File
	)
	if !get_filename(&fname[0], uint64(40), USER_FILE, name) {
		return false
	} else if (func() *stdio.File {
...
func command_pass(cmd *byte, ch *char_data) bool {
...
func userCreate(d *descriptor_data) {
	...
	if !get_filename(&fname[0], uint64(40), USER_FILE, d.User) {
		return
	}
	if (func() *stdio.File {
		fl = stdio.FOpen(libc.GoString(&fname[0]), "w")
...
func userWrite(d *descriptor_data, setTot int, setRpp int, setRBank int, name *byte) {
	...
	if libc.StrCaseCmp(name, libc.CString("index")) == 0 {
		...
		if !get_filename(&filename[0], uint64(40), USER_FILE, name) {
			return
		} else if (func() *stdio.File {
...
func nanny(d *descriptor_data, arg *byte) {
	...
	case CON_GET_NAME:
		...
		if _parse_name(d.Loadplay, &tmp_name[0]) || libc.StrLen(&tmp_name[0]) < 2 || libc.StrLen(&tmp_name[0]) > MAX_NAME_LENGTH || !Valid_Name(&tmp_name[0]) || fill_word(libc.StrCpy(&buf[0], &tmp_name[0])) != 0 || reserved_word(&buf[0]) != 0 {
		...
		if d.Writenew > 0 && (func() int {
			...
			if (func() int {
				...
				if PLR_FLAGGED(d.Character, PLR_DELETED) {
					...
					REMOVE_BIT_AR(d.Character.Affected_by[:], AFF_GROUP)
					if int(libc.BoolToInt(isbanned(&d.Host[0]))) == BAN_SELECT && !PLR_FLAGGED(d.Character, PLR_SITEOK) {
						write_to_output(d, libc.CString("Sorry, this char has not been cleared for login from your site!\r\n"))
						mudlog(NRM, ADMLVL_GOD, 1, libc.CString("Connection attempt for %s denied from %s"), GET_NAME(d.Character), &d.Host[0])
						...
						d.Connected = CON_UMENU
						mudlog(NRM, ADMLVL_GOD, 1, libc.CString("Request for character load denied for %s [%s] (wizlock)"), GET_NAME(d.Character), &d.Host[0])
						return
	...
	case CON_NAME_CNFRM:
		if unicode.ToUpper(rune(*arg)) == 'Y' {
			if int(libc.BoolToInt(isbanned(&d.Host[0]))) >= BAN_NEW {
				mudlog(NRM, ADMLVL_GOD, 1, libc.CString("Request for new char %s denied from [%s] (siteban)"), d.Character.Name, &d.Host[0])
			...
			if circle_restrict != 0 {
				...
				mudlog(NRM, ADMLVL_GOD, 1, libc.CString("Request for new char %s denied from [%s] (wizlock)"), d.Character.Name, &d.Host[0])
				d.Connected = CON_CLOSE
				return
			}
			d.Idle_tics = 0
	...
	case CON_GET_EMAIL:
		if readUserIndex(d.User) && *arg == 0 {
			write_to_output(d, libc.CString("Email, or M for menu?\r\n"))
			return
		} else if *arg == 0 {
			write_to_output(d, libc.CString("Email?\r\n"))
			return
		} else if libc.StrCaseCmp(arg, libc.CString("M")) == 0 && readUserIndex(d.User) {
			userRead(d)
			d.Connected = CON_UMENU
	...
	case CON_NEWPASSWD:
		if readUserIndex(d.User) && *arg == 0 {
			...
			userRead(d)
			d.Connected = CON_UMENU
		} else if libc.StrLen(arg) > MAX_PWD_LENGTH && readUserIndex(d.User) {
	...
	case CON_QROLLSTATS:
		...
		total -= 30
		mudlog(CMP, ADMLVL_GOD, 1, libc.CString("New player: %s [%s %s]"), GET_NAME(d.Character), pc_race_types[d.Character.Race], pc_class_types[d.Character.Chclass])
	case CON_QSTATS:
		if parse_stats(d, arg) {
			...
			total -= 30
			mudlog(CMP, ADMLVL_GOD, 1, libc.CString("New player: %s [%s %s]"), GET_NAME(d.Character), pc_race_types[d.Character.Race], pc_class_types[d.Character.Chclass])
			mudlog(CMP, ADMLVL_GOD, 1, libc.CString("Str: %2d Dex: %2d Con: %2d Int: %2d Wis:  %2d Cha: %2d mod total: %2d"), d.Character.Aff_abils.Str, d.Character.Aff_abils.Dex, d.Character.Aff_abils.Con, d.Character.Aff_abils.Intel, d.Character.Aff_abils.Wis, d.Character.Aff_abils.Cha, total)
		}
	case CON_RMOTD:
	...
	case CON_MENU:
		...
		case '1':
			if lockRead(GET_NAME(d.Character)) && d.Level <= 0 {
				...
				act(libc.CString("$n has entered the game."), 1, d.Character, nil, nil, TO_ROOM)
			...
			if int(libc.BoolToInt(GET_ROOM_VNUM(d.Character.In_room))) <= 1 && d.Character.Player_specials.Load_room != int(-1) {
				char_from_room(d.Character)
				char_to_room(d.Character, real_room(real_room(d.Character.Player_specials.Load_room)))
	...
	case CON_DELCNF2:
		if libc.StrCmp(arg, libc.CString("yes")) == 0 || libc.StrCmp(arg, libc.CString("YES")) == 0 {
			...
			mudlog(NRM, ADMLVL_GOD, 1, libc.CString("User %s has deleted character %s (lev %d)."), d.User, GET_NAME(d.Character), GET_LEVEL(d.Character))
...
func do_disable(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if p != nil {
		...
		mudlog(BRF, ADMLVL_IMMORT, 1, libc.CString("(GC) %s has enabled the command '%s'."), GET_NAME(ch), p.Command.Command)
		...
		mudlog(BRF, ADMLVL_IMMORT, 1, libc.CString("(GC) %s has disabled the command '%s'."), GET_NAME(ch), p.Command.Command)
		save_disabled()
	}
}
func check_disabled(command *command_info) bool {
	var p *disabled_data
	for p = disabled_first; p != nil; p = p.Next {
