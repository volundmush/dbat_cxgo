...
func do_dg_cast(gohere unsafe.Pointer, sc *script_data, trig *trig_data, type_ int, cmd *byte) {
	...
	if spellnum < 1 || (spellnum >= SKILL_TABLE_SIZE || !IS_SET(uint32(int32(skill_type(spellnum))), 1<<0)) {
	...
	if IS_SET(uint32(int32(spell_info[spellnum].Targets)), 1<<0) {
		...
		if target == 0 && (IS_SET(uint32(int32(spell_info[spellnum].Targets)), 1<<1) || IS_SET(uint32(int32(spell_info[spellnum].Targets)), 1<<2)) {
			if (func() *char_data {
				tch = get_char(t)
				return tch
			}()) != nil {
		...
		if target == 0 && (IS_SET(uint32(int32(spell_info[spellnum].Targets)), 1<<7) || IS_SET(uint32(int32(spell_info[spellnum].Targets)), 1<<10) || IS_SET(uint32(int32(spell_info[spellnum].Targets)), 1<<8) || IS_SET(uint32(int32(spell_info[spellnum].Targets)), 1<<9)) {
			if (func() *obj_data {
				tobj = get_obj(t)
				return tobj
			}()) != nil {
...
func send_char_pos(ch *char_data, dam int) {
	switch ch.Position {
	case POS_MORTALLYW:
		act(libc.CString("$n is mortally wounded, and will die soon, if not aided."), 1, ch, nil, nil, TO_ROOM)
		send_to_char(ch, libc.CString("You are mortally wounded, and will die soon, if not aided.\r\n"))
	case POS_INCAP:
		act(libc.CString("$n is incapacitated and will slowly die, if not aided."), 1, ch, nil, nil, TO_ROOM)
		send_to_char(ch, libc.CString("You are incapacitated and will slowly die, if not aided.\r\n"))
	case POS_STUNNED:
		act(libc.CString("$n is stunned, but will probably regain consciousness again."), 1, ch, nil, nil, TO_ROOM)
		send_to_char(ch, libc.CString("You're stunned, but will probably regain consciousness again.\r\n"))
	case POS_DEAD:
		act(libc.CString("$n is dead!  R.I.P."), 0, ch, nil, nil, TO_ROOM)
		send_to_char(ch, libc.CString("You are dead!  Sorry...\r\n"))
	default:
		if dam > int(ch.Max_hit>>2) {
			act(libc.CString("That really did HURT!"), 0, ch, nil, nil, TO_CHAR)
		}
...
func valid_dg_target(ch *char_data, bitvector int) bool {
	if IS_NPC(ch) {
		return true
	} else if ch.Admlevel < ADMLVL_IMMORT {
		return true
	} else if !IS_SET(uint32(int32(bitvector)), 1<<0) && (ch.Admlevel >= 2 && !PRF_FLAGGED(ch, PRF_TEST)) {
		return false
...
func script_damage(vict *char_data, dam int) {
	...
	if int(vict.Position) == POS_DEAD {
		if !IS_NPC(vict) {
			mudlog(BRF, 0, 1, libc.CString("%s killed by script at %s"), GET_NAME(vict), world[vict.In_room].Name)
		}
		die(vict, nil)
	}
}
