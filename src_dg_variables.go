...
func item_in_list(item *byte, list *obj_data) int {
	...
	} else if int8(libc.BoolToInt(is_number(item))) > -1 {
		var ovnum int = libc.Atoi(libc.GoString(item))
		for i = list; i != nil; i = i.Next_content {
...
func char_has_item(item *byte, ch *char_data) bool {
	if get_object_in_equip(ch, item) != nil {
		return true
	}
	return item_in_list(item, ch.Carrying) != 0
}
func text_processed(field *byte, subfield *byte, vd *trig_var_data, str *byte, slen uint64) bool {
	var (
		p      *byte
		p2     *byte
		tmpvar [64936]byte
	...
	if *field == 0 {
		...
		if vd != nil {
			...
			case MOB_TRIGGER:
				...
				} else if ch.In_room != int(-1) && (func() *char_data {
			...
			if libc.StrCaseCmp(var_, libc.CString("self")) == 0 {
				...
				*str = '\x00'
				...
				if field == nil || *field == 0 || subfield == nil || *subfield == 0 {
					...
					var (
						rrnum int = real_room(libc.Atoi(libc.GoString(field)))
						mvnum int = libc.Atoi(libc.GoString(subfield))
					)
					if rrnum == int(-1) {
				...
				if field == nil || *field == 0 || subfield == nil || *subfield == 0 {
					...
					var rrnum int = real_room(libc.Atoi(libc.GoString(field)))
					if rrnum == int(-1) {
				...
				if libc.StrCaseCmp(field, libc.CString("char")) == 0 {
					...
					if type_ == MOB_TRIGGER {
						...
						for c = world[ch.In_room].People; c != nil; c = c.Next_in_room {
							if c != ch && valid_dg_target(c, 1<<0) && CAN_SEE(ch, c) {
			...
			*str = '\x01'
			...
			case 'a':
				if libc.StrCaseCmp(field, libc.CString("aaaaa")) == 0 {
					...
					if subfield != nil && *subfield != 0 {
						...
						if affect != int(-1) && AFF_FLAGGED(c, uint32(int32(affect))) {
							libc.StrCpy(str, libc.CString("1"))
						} else {
							libc.StrCpy(str, libc.CString("0"))
						}
				...
				if libc.StrCaseCmp(field, libc.CString("id")) == 0 {
					...
					if subfield != nil && *subfield != 0 {
						for obj = c.Carrying; obj != nil; obj = obj.Next_content {
							if GET_OBJ_VNUM(obj) == libc.Atoi(libc.GoString(subfield)) {
			...
			case 'p':
				if libc.StrCaseCmp(field, libc.CString("pos")) == 0 {
					...
					if subfield != nil && *subfield != 0 {
						...
						if plr != int(-1) && PLR_FLAGGED(c, uint32(int32(plr))) {
							libc.StrCpy(str, libc.CString("1"))
						} else {
							libc.StrCpy(str, libc.CString("0"))
						}
					...
					if subfield != nil && *subfield != 0 {
						...
						if pref != int(-1) && PRF_FLAGGED(c, uint32(int32(pref))) {
							libc.StrCpy(str, libc.CString("1"))
						} else {
							libc.StrCpy(str, libc.CString("0"))
						}
			...
			case 'r':
				if libc.StrCaseCmp(field, libc.CString("room")) == 0 {
					stdio.Snprintf(str, int(slen), "%c%d", UID_CHAR, func() int {
						if c.In_room != int(-1) {
			...
			case 's':
				if libc.StrCaseCmp(field, libc.CString("sex")) == 0 {
					...
					if subfield != nil && *subfield != 0 {
						var ns int
						if (func() int {
							ns = search_block(subfield, &size_names[0], 0)
							return ns
						}()) > -1 {
							c.Size = ns
						}
					...
					if !IS_NPC(c) && subfield != nil && *subfield != 0 {
						...
						skip_spaces(&amount)
						if amount != nil && *amount != 0 && is_number(amount) {
			...
			case 'v':
				if libc.StrCaseCmp(field, libc.CString("vnum")) == 0 {
					if subfield != nil && *subfield != 0 {
						stdio.Snprintf(str, int(slen), "%d", func() int {
							if IS_NPC(c) {
								return int(libc.BoolToInt(GET_MOB_VNUM(c) == libc.Atoi(libc.GoString(subfield))))
							}
							return -1
						}())
					} else {
			...
			*str = '\x01'
			...
			case 'a':
				if libc.StrCaseCmp(field, libc.CString("affects")) == 0 {
					if subfield != nil && *subfield != 0 {
						if int(libc.BoolToInt(check_flags_by_name_ar(o.Bitvector[:], NUM_AFF_FLAGS, subfield, affected_bits[:]))) > 0 {
			...
			case 'e':
				if libc.StrCaseCmp(field, libc.CString("extra")) == 0 {
					if subfield != nil && *subfield != 0 {
						if int(libc.BoolToInt(check_flags_by_name_ar(o.Extra_flags[:], NUM_ITEM_FLAGS, subfield, extra_bits[:]))) > 0 {
							stdio.Snprintf(str, int(slen), "1")
						} else {
							stdio.Snprintf(str, int(slen), "0")
						}
			...
			case 'i':
				if libc.StrCaseCmp(field, libc.CString("id")) == 0 {
					...
					if subfield != nil && *subfield != 0 {
						...
						if item != int(-1) && OBJ_FLAGGED(o, uint32(int32(item))) {
							libc.StrCpy(str, libc.CString("1"))
						} else {
							libc.StrCpy(str, libc.CString("0"))
						}
			...
			case 's':
				if libc.StrCaseCmp(field, libc.CString("shortdesc")) == 0 {
					...
					if subfield != nil && *subfield != 0 {
						var ns int
						if (func() int {
							ns = int(libc.BoolToInt(check_flags_by_name_ar(o.Bitvector[:], NUM_AFF_FLAGS, subfield, affected_bits[:])))
							return ns
						}()) > 0 {
							TOGGLE_BIT_AR(o.Bitvector[:], uint32(int32(ns)))
							stdio.Snprintf(str, int(slen), "1")
						}
					}
					...
					if subfield != nil && *subfield != 0 {
						var ns int
						if (func() int {
							ns = int(libc.BoolToInt(check_flags_by_name_ar(o.Extra_flags[:], NUM_ITEM_FLAGS, subfield, extra_bits[:])))
							return ns
						}()) > 0 {
							TOGGLE_BIT_AR(o.Extra_flags[:], uint32(int32(ns)))
							stdio.Snprintf(str, int(slen), "1")
						}
					}
				} else if libc.StrCaseCmp(field, libc.CString("size")) == 0 {
			...
			case 'v':
				if libc.StrCaseCmp(field, libc.CString("vnum")) == 0 {
					if subfield != nil && *subfield != 0 {
						stdio.Snprintf(str, int(slen), "%d", int(libc.BoolToInt(GET_OBJ_VNUM(o) == libc.Atoi(libc.GoString(subfield)))))
					} else {
			...
			if r.Number == 0 {
				...
				if subfield != nil && *subfield != 0 {
					stdio.Snprintf(str, int(slen), "%d", int(libc.BoolToInt(r.Number == libc.Atoi(libc.GoString(subfield)))))
				} else {
				...
				var rnum int = real_room(r.Number)
				if rnum != int(-1) {
				...
				var thisroom int = real_room(r.Number)
				...
				if subfield != nil && *subfield != 0 {
					var thisroom int = real_room(r.Number)
					if int(libc.BoolToInt(check_flags_by_name_ar(world[thisroom].Room_flags[:], NUM_ROOM_FLAGS, subfield, room_bits[:]))) > 0 {
						stdio.Snprintf(str, int(slen), "1")
					} else {
						stdio.Snprintf(str, int(slen), "0")
					}
				...
				if (r.Dir_option[NORTH]) != nil {
					if subfield != nil && *subfield != 0 {
						if libc.StrCaseCmp(subfield, libc.CString("vnum")) == 0 {
							...
							if (r.Dir_option[NORTH]).To_room != int(-1) {
				...
				if (r.Dir_option[EAST]) != nil {
					if subfield != nil && *subfield != 0 {
						if libc.StrCaseCmp(subfield, libc.CString("vnum")) == 0 {
							...
							if (r.Dir_option[EAST]).To_room != int(-1) {
				...
				if (r.Dir_option[SOUTH]) != nil {
					if subfield != nil && *subfield != 0 {
						if libc.StrCaseCmp(subfield, libc.CString("vnum")) == 0 {
							...
							if (r.Dir_option[SOUTH]).To_room != int(-1) {
				...
				if (r.Dir_option[WEST]) != nil {
					if subfield != nil && *subfield != 0 {
						if libc.StrCaseCmp(subfield, libc.CString("vnum")) == 0 {
							...
							if (r.Dir_option[WEST]).To_room != int(-1) {
				...
				if (r.Dir_option[UP]) != nil {
					if subfield != nil && *subfield != 0 {
						if libc.StrCaseCmp(subfield, libc.CString("vnum")) == 0 {
							...
							if (r.Dir_option[UP]).To_room != int(-1) {
				...
				if (r.Dir_option[DOWN]) != nil {
					if subfield != nil && *subfield != 0 {
						if libc.StrCaseCmp(subfield, libc.CString("vnum")) == 0 {
							...
							if (r.Dir_option[DOWN]).To_room != int(-1) {
				...
				if (r.Dir_option[NORTHWEST]) != nil {
					if subfield != nil && *subfield != 0 {
						if libc.StrCaseCmp(subfield, libc.CString("vnum")) == 0 {
							...
							if (r.Dir_option[NORTHWEST]).To_room != int(-1) {
				...
				if (r.Dir_option[NORTHEAST]) != nil {
					if subfield != nil && *subfield != 0 {
						if libc.StrCaseCmp(subfield, libc.CString("vnum")) == 0 {
							...
							if (r.Dir_option[NORTHEAST]).To_room != int(-1) {
				...
				if (r.Dir_option[SOUTHWEST]) != nil {
					if subfield != nil && *subfield != 0 {
						if libc.StrCaseCmp(subfield, libc.CString("vnum")) == 0 {
							...
							if (r.Dir_option[SOUTHWEST]).To_room != int(-1) {
				...
				if (r.Dir_option[SOUTHEAST]) != nil {
					if subfield != nil && *subfield != 0 {
						if libc.StrCaseCmp(subfield, libc.CString("vnum")) == 0 {
							...
							if (r.Dir_option[SOUTHEAST]).To_room != int(-1) {
				...
				if (r.Dir_option[INDIR]) != nil {
					if subfield != nil && *subfield != 0 {
						if libc.StrCaseCmp(subfield, libc.CString("vnum")) == 0 {
							...
							if (r.Dir_option[INDIR]).To_room != int(-1) {
				...
				if (r.Dir_option[OUTDIR]) != nil {
					if subfield != nil && *subfield != 0 {
						if libc.StrCaseCmp(subfield, libc.CString("vnum")) == 0 {
							...
							if (r.Dir_option[OUTDIR]).To_room != int(-1) {
