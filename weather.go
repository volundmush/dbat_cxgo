	...
	if tch == nil {
		for d = descriptor_list; d != nil; d = d.Next {
			...
			if MOON_OK(d.Character) && !PLR_FLAGGED(d.Character, PLR_OOZARU) {
				act(libc.CString("@rLooking up at the moon your heart begins to beat loudly. Sudden rage begins to fill your mind while your body begins to grow. Hair sprouts  all over your body and your teeth become sharp as your body takes on the Oozaru form!@n"), TRUE, d.Character, nil, nil, TO_CHAR)
				act(libc.CString("@R$n@r looks up at the moon as $s eyes turn red and $s heart starts to beat loudly. Hair starts to grow all over $s body as $e starts screaming. The scream turns into a roar as $s body begins to grow into a giant ape!@n"), TRUE, d.Character, nil, nil, TO_ROOM)
	...
	if ch == nil {
		for d = descriptor_list; d != nil; d = d.Next {
			...
			if int(d.Character.Race) == RACE_HOSHIJIN {
				...
				case 0:
					if ch.Starphase > 0 {
						act(libc.CString("@WYour eyes and the glyphs on your skin slowly start to lose their glow. You feel the power received from the @GE@gl@Dd@wri@Dt@gc@Gh @YS@yta@Yr@W drain away from your body. It has apparently entered the @rDeath Phase@W of its cycle...@n"), TRUE, ch, nil, nil, TO_CHAR)
						act(libc.CString("@c$n@W's eyes and the glyphs on $s skin slowly start to lose their glow. You notice that $e seems weaker now for some reason.@n"), TRUE, ch, nil, nil, TO_ROOM)
						phase_powerup(ch, 0, ch.Starphase)
					}
				case 1:
					if ch.Starphase != 1 {
						act(libc.CString("@WYou suddenly feel a @RSURGE@W of power through your body. You feel the @GE@gl@Dd@wri@Dt@gc@Gh @YS@yta@Yr@W come into its @CBirth Phase@W and its power is flowing into your body! Finally your eyes and the glyphs on your skin begin to glow an electric @bb@Bl@Cue@W!@n"), TRUE, ch, nil, nil, TO_CHAR)
						act(libc.CString("@c$n@W suddenly seems to grow stronger for some reason. You notice $s eyes begin to glow an electric @bb@Bl@Cue@W. Suddenly glyphs start to appear all over $s skin and glow with the same light!@n"), TRUE, ch, nil, nil, TO_ROOM)
						phase_powerup(ch, 0, ch.Starphase)
						phase_powerup(ch, 1, 1)
					}
				case 2:
					if ch.Starphase != 2 {
						act(libc.CString("@WYou suddenly feel a @RSURGE@W of power through your body. You feel the @GE@gl@Dd@wri@Dt@gc@Gh @YS@yta@Yr@W come into its @GLife Phase@W and its power is flowing into your body! Finally your eyes and the glyphs on your skin begin to glow an fiery @Rr@re@Rd@W!@n"), TRUE, ch, nil, nil, TO_CHAR)
						act(libc.CString("@c$n@W suddenly seems to grow stronger for some reason. You notice $s eyes begin to glow a fiery @rR@Re@rd@W. Suddenly glyphs start to appear all over $s skin and glow with the same light!@n"), TRUE, ch, nil, nil, TO_ROOM)
						phase_powerup(ch, 0, ch.Starphase)
