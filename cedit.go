...
func do_oasis_cedit(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	mudlog(CMP, int(MAX(ADMLVL_BUILDER, int64(ch.Player_specials.Invis_level))), TRUE, libc.CString("OLC: %s saves the game configuration."), GET_NAME(ch))
	cedit_save_to_disk()
}
...
func cedit_parse(d *descriptor_data, arg *byte) {
	...
	case CEDIT_CONFIRM_SAVESTRING:
		...
		case 'Y':
			...
			mudlog(CMP, int(MAX(ADMLVL_BUILDER, int64(d.Character.Player_specials.Invis_level))), TRUE, libc.CString("OLC: %s modifies the game configuration."), GET_NAME(d.Character))
