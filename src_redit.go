...
func do_oasis_redit(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if remove == nil && ch.Admlevel < 1 {
		...
		act(libc.CString("@GYou open up the computer panel on the droid and begin to program its remodeling routine.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@g$n@G opens up a computer panel on some kind of small spherical droid and begins to program it.@n"), 1, ch, nil, nil, TO_ROOM)
		extract_obj(remove)
		remodeling = 1
	}
	...
	d = ch.Desc
	if d.Olc != nil {
		mudlog(BRF, ADMLVL_IMMORT, 1, libc.CString("SYSERR: do_oasis_redit: Player already had olc structure."))
	...
	if !can_edit_zone(ch, d.Olc.Zone_num) && remodeling == 0 {
	...
	if save != 0 {
		...
		mudlog(CMP, int(MAX(ADMLVL_BUILDER, int64(ch.Player_specials.Invis_level))), 1, libc.CString("OLC: %s saves room info for zone %d."), GET_NAME(ch), zone_table[d.Olc.Zone_num].Number)
		...
		if dsc.Connected == CON_ZEDIT {
			for j = 0; int(dsc.Olc.Zone.Cmd[j].Command) != 'S'; j++ {
				...
				case 'T':
					fallthrough
				case 'V':
					dsc.Olc.Zone.Cmd[j].Arg3 += int(libc.BoolToInt(dsc.Olc.Zone.Cmd[j].Arg3 >= room_num))
				case 'D':
					dsc.Olc.Zone.Cmd[j].Arg2 += int(libc.BoolToInt(dsc.Olc.Zone.Cmd[j].Arg2 >= room_num))
					fallthrough
				case 'R':
					dsc.Olc.Zone.Cmd[j].Arg1 += int(libc.BoolToInt(dsc.Olc.Zone.Cmd[j].Arg1 >= room_num))
				}
			}
		} else if dsc.Connected == CON_REDIT {
			for j = 0; j < NUM_OF_DIRS; j++ {
				if dsc.Olc.Room.Dir_option[j] != nil {
					if dsc.Olc.Room.Dir_option[j].To_room >= room_num {
...
func redit_save_to_disk(zone_num int) {
	save_rooms(zone_num)
	update_space()
}
...
func redit_disp_exit_menu(d *descriptor_data) {
	...
	write_to_output(d, libc.CString("@g1@n) Exit to     \t\t: @c%d\r\n@g2@n) Description \t\t:-\r\n@y%s\r\n@g3@n) Door name   \t\t: @y%s\r\n@g4@n) Key         \t\t: @c%d\r\n@g5@n) Door flags  \t\t: @c%s\r\n@g6@n) Purge exit.\r\n@g7@n) DC Lock\t\t: @c%d\r\n@g8@n) DC Hide     \t\t: @c%d\r\n@g9@n) DC Skill\t\t: @c%s\r\n@gA@n) DC Move\t\t: @c%d\r\n@gB@n) Skill Fail Save Type\t\t: @c%d\r\n@gC@n) DC Skill Save\t\t: @c%d\r\n@gD@n) Minor Fail Dest. Room\t: @c%d\r\n@gE@n) Major Fail Dest. Room\t: @c%d@n\r\nEnter choice, 0 to quit : "), func() int {
		...
		if (d.Olc.Room.Dir_option[d.Olc.Value]).Key != int(-1) {
...
func redit_parse(d *descriptor_data, arg *byte) {
	...
	case REDIT_CONFIRM_SAVESTRING:
		...
		case 'Y':
			...
			mudlog(CMP, int(MAX(ADMLVL_BUILDER, int64(d.Character.Player_specials.Invis_level))), 1, libc.CString("OLC: %s edits room %d."), GET_NAME(d.Character), d.Olc.Number)
			if config_info.Operation.Auto_save_olc != 0 {
				redit_save_to_disk(real_zone_by_thing(d.Olc.Number))
				if d.Character.Admlevel < 1 {
					...
					act(libc.CString("@GThe remodeling droid quickly launches about with remodeling the room as to @g$n's@G specifications. It seems to finish in no time at all...@n"), 1, d.Character, nil, nil, TO_ROOM)
				} else {
	...
	case REDIT_DESC:
		mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("SYSERR: Reached REDIT_DESC case in parse_redit()."))
	...
	case REDIT_FLAGS:
		...
		if number < 0 || number > NUM_ROOM_FLAGS {
			...
			TOGGLE_BIT_AR(d.Olc.Room.Room_flags[:], uint32(int32(number-1)))
			redit_disp_flag_menu(d)
		}
		return
	case REDIT_SECTOR:
	...
	case REDIT_EXIT_NUMBER:
		...
		(d.Olc.Room.Dir_option[d.Olc.Value]).To_room = number
		redit_disp_exit_menu(d)
		return
	case REDIT_EXIT_DESCRIPTION:
		mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("SYSERR: Reached REDIT_EXIT_DESC case in parse_redit"))
	...
	case REDIT_EXIT_KEY:
		...
		if number < 0 {
			...
			(d.Olc.Room.Dir_option[d.Olc.Value]).Key = number
		}
		redit_disp_exit_menu(d)
		return
	case REDIT_EXIT_DOORFLAGS:
		...
		if number < 0 || number > NUM_EXIT_FLAGS {
			...
			(d.Olc.Room.Dir_option[d.Olc.Value]).Exit_info ^= uint32(int32(1 << (number - 1)))
			redit_disp_exit_flag_menu(d)
		}
		return
	...
	case REDIT_COPY:
		if (func() int {
			number = real_room(libc.Atoi(libc.GoString(arg)))
			return number
		}()) != int(-1) {
	...
	case REDIT_DELETE:
		if *arg == 'y' || *arg == 'Y' {
			if delete_room(real_room(d.Olc.Room.Number)) {
	...
	case REDIT_EXIT_TOTALFAILROOM:
		...
		return
	default:
		mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("SYSERR: Reached default case in parse_redit"))
	}
	d.Olc.Value = 1
	redit_disp_menu(d)
}
