...
func save_player_index() {
	...
	for i = 0; i <= top_of_p_table; i++ {
		if *player_table[i].Name != 0 {
			sprintascii(&bits[0], uint32(int32(player_table[i].Flags)))
...
func load_char(name *byte, ch *char_data) int {
	...
	if (func() int {
		...
		if !get_filename(&fname[0], uint64(256), PLR_FILE, player_table[id].Name) {
			return -1
		}
		if (func() *stdio.File {
			...
			mudlog(NRM, ADMLVL_GOD, 1, libc.CString("SYSERR: Couldn't open player file %s"), &fname[0])
			return -1
		}
		ch.Affected = nil
		ch.Affectedv = nil
		...
		for get_line(fl, &line[0]) != 0 {
			...
			case 'D':
				if libc.StrCmp(&tag[0], libc.CString("Deat")) == 0 {
					...
					ch.Droom = libc.Atoi(libc.GoString(&line[0]))
				}
			case 'E':
			...
			case 'H':
				if libc.StrCmp(&tag[0], libc.CString("Hit ")) == 0 {
					...
					ch.Hometown = libc.Atoi(libc.GoString(&line[0]))
			...
			case 'R':
				if libc.StrCmp(&tag[0], libc.CString("Race")) == 0 {
					...
					ch.Radar1 = libc.Atoi(libc.GoString(&line[0]))
				} else if libc.StrCmp(&tag[0], libc.CString("Rad2")) == 0 {
					ch.Radar2 = libc.Atoi(libc.GoString(&line[0]))
				} else if libc.StrCmp(&tag[0], libc.CString("Rad3")) == 0 {
					ch.Radar3 = libc.Atoi(libc.GoString(&line[0]))
				} else if libc.StrCmp(&tag[0], libc.CString("Room")) == 0 {
					ch.Player_specials.Load_room = libc.Atoi(libc.GoString(&line[0]))
			...
			case 'S':
				if libc.StrCmp(&tag[0], libc.CString("Sex ")) == 0 {
					...
					ch.Shipr = libc.Atoi(libc.GoString(&line[0]))
...
func save_char_pets(ch *char_data) {
	...
	if !get_filename(&fname[0], uint64(40), PET_FILE, GET_NAME(ch)) {
		return
	}
	if (func() *stdio.File {
		...
		mudlog(NRM, ADMLVL_GOD, 1, libc.CString("SYSERR: Couldn't open pet file %s for write"), &fname[0])
		return
	}
...
func load_char_pets(ch *char_data) {
	...
	for foll = ch.Followers; foll != nil; foll = foll.Next {
		...
		if load_room == int(-1) {
			load_room = real_room(ch.In_room)
		}
		char_to_room(foll.Follower, load_room)
		act(libc.CString("You are joined by $N."), 0, ch, nil, unsafe.Pointer(foll.Follower), TO_CHAR)
	}
}
func save_char(ch *char_data) {
	...
	if !get_filename(&fname[0], uint64(40), PLR_FILE, GET_NAME(ch)) {
		return
	}
	if (func() *stdio.File {
		fl = stdio.FOpen(libc.GoString(&fname[0]), "w")
		...
		mudlog(NRM, ADMLVL_GOD, 1, libc.CString("SYSERR: Couldn't open player file %s for write"), &fname[0])
		return
	}
	if ch.Trp < ch.Rp {
		ch.Trp = ch.Rp
	...
	if player_table[id].Shiproom != ch.Shipr {
		save_index = 1
		player_table[id].Shiproom = ch.Shipr
	}
...
func remove_player(pfilepos int) {
	...
	for i = 0; i < MAX_FILES; i++ {
		if get_filename(&fname[0], uint64(40), i, player_table[pfilepos].Name) {
			stdio.Unlink(&fname[0])
		}
		if get_filename(&fname[0], uint64(40), i, CAP(player_table[pfilepos].Name)) {
			stdio.Unlink(&fname[0])
		}
	}
...
func clean_pfiles() {
	...
	for i = 0; i <= top_of_p_table; i++ {
		if !IS_SET(uint32(int32(player_table[i].Flags)), 1<<1) && *player_table[i].Name != 0 {
