...
func free_strings(data unsafe.Pointer, type_ int) bool {
	...
	case OASIS_CFG:
		...
		return true
	default:
		mudlog(BRF, ADMLVL_GOD, 1, libc.CString("SYSERR: oasis_delete.c: free_strings: Invalid type handled (Type %d)."), type_)
		return false
	}
}
