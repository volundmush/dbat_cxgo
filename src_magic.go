...
func mag_materials(ch *char_data, item0 int, item1 int, item2 int, extract int, verbose int) bool {
	var (
		tobj *obj_data
		obj0 *obj_data = nil
		obj1 *obj_data = nil
	...
	if verbose != 0 {
		...
		act(libc.CString("A puff of smoke rises from $n's pack."), 1, ch, nil, nil, TO_ROOM)
	}
	return true
}
func mag_newsaves(ch *char_data, victim *char_data, spellnum int, level int, cast_stat int) bool {
	var (
		stype int
		dc    int
		total int
	)
	if IS_SET(uint32(int32(spell_info[spellnum].Save_flags)), 1<<0) {
		stype = SAVING_FORTITUDE
	} else if IS_SET(uint32(int32(spell_info[spellnum].Save_flags)), 1<<1) {
		stype = SAVING_REFLEX
	} else if IS_SET(uint32(int32(spell_info[spellnum].Save_flags)), 1<<2) {
		stype = SAVING_WILL
	} else {
		return false
	}
	...
	if ch != nil {
		if IS_SET(uint32(int32(ch.School_feats[CFEAT_SPELL_FOCUS])), uint32(int32(spell_info[spellnum].School))) {
			dc++
		}
		if IS_SET(uint32(int32(ch.School_feats[CFEAT_GREATER_SPELL_FOCUS])), uint32(int32(spell_info[spellnum].School))) {
			dc++
		}
	}
	return total >= dc
...
func mag_damage(level int, ch *char_data, victim *char_data, spellnum int) int {
	...
	case SPELL_DISPEL_EVIL:
		...
		if IS_EVIL(ch) {
			...
			act(libc.CString("The gods protect $N."), 0, ch, nil, unsafe.Pointer(victim), TO_CHAR)
			return 0
		}
	...
	if mag_newsaves(ch, victim, spellnum, level, int(ch.Aff_abils.Intel)) {
		if IS_SET(uint32(int32(spell_info[spellnum].Save_flags)), 1<<4) {
			send_to_char(victim, libc.CString("@g*save*@y You avoid any injury.@n\r\n"))
			dam = 0
		} else if IS_SET(uint32(int32(spell_info[spellnum].Save_flags)), 1<<3) {
			if IS_SET(uint32(int32(spell_info[spellnum].Save_flags)), 1<<1) && int(victim.Feats[FEAT_EVASION]) != 0 && ((victim.Equipment[WEAR_BODY]) == nil || int((victim.Equipment[WEAR_BODY]).Type_flag) != ITEM_ARMOR || ((victim.Equipment[WEAR_BODY]).Value[VAL_ARMOR_SKILL]) < ARMOR_TYPE_MEDIUM) {
...
func mag_affects(level int, ch *char_data, victim *char_data, spellnum int) {
	var (
		af             [5]affected_type
		accum_affect   bool  = false
		accum_duration bool  = false
		to_vict        *byte = nil
		to_room        *byte = nil
		i              int
	)
	...
	if mag_newsaves(ch, victim, spellnum, level, int(ch.Aff_abils.Intel)) {
		if IS_SET(uint32(int32(spell_info[spellnum].Save_flags)), (1<<5)|1<<4) {
	...
	case SPELL_SLEEP:
		...
		if int(victim.Position) > POS_SLEEPING {
			...
			act(libc.CString("$n goes to sleep."), 1, victim, nil, nil, TO_ROOM)
	...
	for i = 0; i < MAX_SPELL_AFFECTS; i++ {
		if int(af[i].Bitvector) != 0 || af[i].Location != APPLY_NONE {
			affect_join(victim, &af[i], accum_duration, false, accum_affect, false)
		}
	}
	if to_vict != nil {
		act(to_vict, 0, victim, nil, unsafe.Pointer(ch), TO_CHAR)
	}
	if to_room != nil {
		act(to_room, 1, victim, nil, unsafe.Pointer(ch), TO_ROOM)
	}
}
...
func mag_areas(level int, ch *char_data, spellnum int) {
	...
	if to_char != nil {
		act(to_char, 0, ch, nil, nil, TO_CHAR)
	}
	if to_room != nil {
		act(to_room, 0, ch, nil, nil, TO_ROOM)
	}
...
var monsum_list_lg_1 [4]int = [4]int{300, 301, 302, int(-1)}
var monsum_list_ng_1 [6]int = [6]int{300, 301, 302, 303, 304, int(-1)}
var monsum_list_cg_1 [4]int = [4]int{302, 303, 304, int(-1)}
var monsum_list_ln_1 [5]int = [5]int{300, 301, 305, 306, int(-1)}
var monsum_list_nn_1 [4]int = [4]int{302, 307, 308, int(-1)}
var monsum_list_cn_1 [6]int = [6]int{303, 304, 309, 310, 311, int(-1)}
var monsum_list_le_1 [5]int = [5]int{305, 306, 307, 308, int(-1)}
var monsum_list_ne_1 [8]int = [8]int{305, 306, 307, 308, 309, 310, 311, int(-1)}
var monsum_list_ce_1 [6]int = [6]int{307, 308, 309, 310, 311, int(-1)}
var monsum_list_lg_2 [4]int = [4]int{312, 313, 314, int(-1)}
var monsum_list_ng_2 [4]int = [4]int{312, 313, 314, int(-1)}
var monsum_list_cg_2 [5]int = [5]int{312, 313, 314, 315, int(-1)}
var monsum_list_ln_2 [3]int = [3]int{312, 317, int(-1)}
var monsum_list_nn_2 [4]int = [4]int{315, 316, 317, int(-1)}
var monsum_list_cn_2 [4]int = [4]int{313, 316, 318, int(-1)}
var monsum_list_le_2 [3]int = [3]int{316, 317, int(-1)}
var monsum_list_ne_2 [3]int = [3]int{318, 319, int(-1)}
var monsum_list_ce_2 [3]int = [3]int{320, 321, int(-1)}
var monsum_list_lg_3 [1]int = [1]int{int(-1)}
var monsum_list_ng_3 [1]int = [1]int{int(-1)}
var monsum_list_cg_3 [1]int = [1]int{int(-1)}
var monsum_list_ln_3 [1]int = [1]int{int(-1)}
var monsum_list_nn_3 [1]int = [1]int{int(-1)}
var monsum_list_cn_3 [1]int = [1]int{int(-1)}
var monsum_list_le_3 [1]int = [1]int{int(-1)}
var monsum_list_ne_3 [1]int = [1]int{int(-1)}
var monsum_list_ce_3 [1]int = [1]int{int(-1)}
var monsum_list_lg_4 [1]int = [1]int{int(-1)}
var monsum_list_ng_4 [1]int = [1]int{int(-1)}
var monsum_list_cg_4 [1]int = [1]int{int(-1)}
var monsum_list_ln_4 [1]int = [1]int{int(-1)}
var monsum_list_nn_4 [1]int = [1]int{int(-1)}
var monsum_list_cn_4 [1]int = [1]int{int(-1)}
var monsum_list_le_4 [1]int = [1]int{int(-1)}
var monsum_list_ne_4 [1]int = [1]int{int(-1)}
var monsum_list_ce_4 [1]int = [1]int{int(-1)}
var monsum_list_lg_5 [1]int = [1]int{int(-1)}
var monsum_list_ng_5 [1]int = [1]int{int(-1)}
var monsum_list_cg_5 [1]int = [1]int{int(-1)}
var monsum_list_ln_5 [1]int = [1]int{int(-1)}
var monsum_list_nn_5 [1]int = [1]int{int(-1)}
var monsum_list_cn_5 [1]int = [1]int{int(-1)}
var monsum_list_le_5 [1]int = [1]int{int(-1)}
var monsum_list_ne_5 [1]int = [1]int{int(-1)}
var monsum_list_ce_5 [1]int = [1]int{int(-1)}
var monsum_list_lg_6 [1]int = [1]int{int(-1)}
var monsum_list_ng_6 [1]int = [1]int{int(-1)}
var monsum_list_cg_6 [1]int = [1]int{int(-1)}
var monsum_list_ln_6 [1]int = [1]int{int(-1)}
var monsum_list_nn_6 [1]int = [1]int{int(-1)}
var monsum_list_cn_6 [1]int = [1]int{int(-1)}
var monsum_list_le_6 [1]int = [1]int{int(-1)}
var monsum_list_ne_6 [1]int = [1]int{int(-1)}
var monsum_list_ce_6 [1]int = [1]int{int(-1)}
var monsum_list_lg_7 [1]int = [1]int{int(-1)}
var monsum_list_ng_7 [1]int = [1]int{int(-1)}
var monsum_list_cg_7 [1]int = [1]int{int(-1)}
var monsum_list_ln_7 [1]int = [1]int{int(-1)}
var monsum_list_nn_7 [1]int = [1]int{int(-1)}
var monsum_list_cn_7 [1]int = [1]int{int(-1)}
var monsum_list_le_7 [1]int = [1]int{int(-1)}
var monsum_list_ne_7 [1]int = [1]int{int(-1)}
var monsum_list_ce_7 [1]int = [1]int{int(-1)}
var monsum_list_lg_8 [1]int = [1]int{int(-1)}
var monsum_list_ng_8 [1]int = [1]int{int(-1)}
var monsum_list_cg_8 [1]int = [1]int{int(-1)}
var monsum_list_ln_8 [1]int = [1]int{int(-1)}
var monsum_list_nn_8 [1]int = [1]int{int(-1)}
var monsum_list_cn_8 [1]int = [1]int{int(-1)}
var monsum_list_le_8 [1]int = [1]int{int(-1)}
var monsum_list_ne_8 [1]int = [1]int{int(-1)}
var monsum_list_ce_8 [1]int = [1]int{int(-1)}
var monsum_list_lg_9 [1]int = [1]int{int(-1)}
var monsum_list_ng_9 [1]int = [1]int{int(-1)}
var monsum_list_cg_9 [1]int = [1]int{int(-1)}
var monsum_list_ln_9 [1]int = [1]int{int(-1)}
var monsum_list_nn_9 [1]int = [1]int{int(-1)}
var monsum_list_cn_9 [1]int = [1]int{int(-1)}
var monsum_list_le_9 [1]int = [1]int{int(-1)}
var monsum_list_ne_9 [1]int = [1]int{int(-1)}
var monsum_list_ce_9 [1]int = [1]int{int(-1)}
var monsum_list [9][9]*int = [9][9]*int{{&monsum_list_lg_1[0], &monsum_list_ng_1[0], &monsum_list_cg_1[0], &monsum_list_ln_1[0], &monsum_list_nn_1[0], &monsum_list_cn_1[0], &monsum_list_le_1[0], &monsum_list_ne_1[0], &monsum_list_ce_1[0]}, {&monsum_list_lg_2[0], &monsum_list_ng_2[0], &monsum_list_cg_2[0], &monsum_list_ln_2[0], &monsum_list_nn_2[0], &monsum_list_cn_2[0], &monsum_list_le_2[0], &monsum_list_ne_2[0], &monsum_list_ce_2[0]}, {&monsum_list_lg_3[0], &monsum_list_ng_3[0], &monsum_list_cg_3[0], &monsum_list_ln_3[0], &monsum_list_nn_3[0], &monsum_list_cn_3[0], &monsum_list_le_3[0], &monsum_list_ne_3[0], &monsum_list_ce_3[0]}, {&monsum_list_lg_4[0], &monsum_list_ng_4[0], &monsum_list_cg_4[0], &monsum_list_ln_4[0], &monsum_list_nn_4[0], &monsum_list_cn_4[0], &monsum_list_le_4[0], &monsum_list_ne_4[0], &monsum_list_ce_4[0]}, {&monsum_list_lg_5[0], &monsum_list_ng_5[0], &monsum_list_cg_5[0], &monsum_list_ln_5[0], &monsum_list_nn_5[0], &monsum_list_cn_5[0], &monsum_list_le_5[0], &monsum_list_ne_5[0], &monsum_list_ce_5[0]}, {&monsum_list_lg_6[0], &monsum_list_ng_6[0], &monsum_list_cg_6[0], &monsum_list_ln_6[0], &monsum_list_nn_6[0], &monsum_list_cn_6[0], &monsum_list_le_6[0], &monsum_list_ne_6[0], &monsum_list_ce_6[0]}, {&monsum_list_lg_7[0], &monsum_list_ng_7[0], &monsum_list_cg_7[0], &monsum_list_ln_7[0], &monsum_list_nn_7[0], &monsum_list_cn_7[0], &monsum_list_le_7[0], &monsum_list_ne_7[0], &monsum_list_ce_7[0]}, {&monsum_list_lg_8[0], &monsum_list_ng_8[0], &monsum_list_cg_8[0], &monsum_list_ln_8[0], &monsum_list_nn_8[0], &monsum_list_cn_8[0], &monsum_list_le_8[0], &monsum_list_ne_8[0], &monsum_list_ce_8[0]}, {&monsum_list_lg_9[0], &monsum_list_ng_9[0], &monsum_list_cg_9[0], &monsum_list_ln_9[0], &monsum_list_nn_9[0], &monsum_list_cn_9[0], &monsum_list_le_9[0], &monsum_list_ne_9[0], &monsum_list_ce_9[0]}}
...
func mag_summons(level int, ch *char_data, obj *obj_data, spellnum int, arg *byte) {
	...
	case SPELL_SUMMON_MONSTER_IX:
		...
		if buf != nil {
			...
			for i = lev - 1; i >= 0; i-- {
				for count = 0; *(*int)(unsafe.Add(unsafe.Pointer(monsum_list[i][j]), unsafe.Sizeof(int(0))*uintptr(count))) != int(-1); count++ {
					mob_num = *(*int)(unsafe.Add(unsafe.Pointer(monsum_list[i][j]), unsafe.Sizeof(int(0))*uintptr(count)))
					if real_mobile(mob_num) == int(-1) {
						mob_num = -1
					} else if !is_name(&buf2[0], mob_proto[real_mobile(mob_num)].Name) {
						mob_num = -1
					} else {
						break
					}
		...
		if mob_num == int(-1) {
			num = 1
			for count = 0; *(*int)(unsafe.Add(unsafe.Pointer(monsum_list[lev-1][j]), unsafe.Sizeof(int(0))*uintptr(count))) != int(-1); count++ {
			}
			if count == 0 {
			...
			count--
			mob_num = *(*int)(unsafe.Add(unsafe.Pointer(monsum_list[lev-1][j]), unsafe.Sizeof(int(0))*uintptr(rand_number(0, count))))
		}
	default:
		return
	}
	...
	for i = 0; i < num; i++ {
		...
		act(mag_summon_msgs[msg], 0, ch, nil, unsafe.Pointer(mob), TO_ROOM)
		load_mtrigger(mob)
		add_follower(mob, ch)
...
func mag_points(level int, ch *char_data, victim *char_data, spellnum int) {
	...
	case SPELL_SENSU:
		...
		act(libc.CString("@C$n@W suddenly looks a lot better!@b"), 0, victim, nil, nil, TO_NOTVICT)
		affect_from_char(victim, SPELL_POISON)
		if AFF_FLAGGED(victim, AFF_BURNED) {
			send_to_char(victim, libc.CString("Your burns are healed now.\r\n"))
			act(libc.CString("$n@w's burns are now healed.@n"), 1, victim, nil, nil, TO_ROOM)
...
func mag_alter_objs(level int, ch *char_data, obj *obj_data, spellnum int) {
	...
	case SPELL_INVISIBLE:
		if !OBJ_FLAGGED(obj, uint32(int32(int(ITEM_NOINVIS|ITEM_INVISIBLE)))) {
	...
	if to_char == nil {
		...
		act(to_char, 1, ch, obj, nil, TO_CHAR)
	}
	if to_room != nil {
		act(to_room, 1, ch, obj, nil, TO_ROOM)
	} else if to_char != nil {
		act(to_char, 1, ch, obj, nil, TO_ROOM)
	}
}
func mag_creations(level int, ch *char_data, spellnum int) {
	...
	add_unique_id(tobj)
	obj_to_char(tobj, ch)
	act(libc.CString("$n creates $p."), 0, ch, tobj, nil, TO_ROOM)
	act(libc.CString("You create $p."), 0, ch, tobj, nil, TO_CHAR)
	load_otrigger(tobj)
}
	...
	for i = affectv_list; i != nil; i = i.Next_affectv {
		for af = i.Affectedv; af != nil; af = next {
			...
			if int(af.Duration) == 0 {
				...
				if int(af.Bitvector) == AFF_SUMMONED {
					stop_follower(i)
