...
func load_inv_backup(ch *char_data) int {
	...
	stdio.Sprintf(&source_file[0], "/home/m053car2/dbat/lib/plrobjs/%s/%s.copy", &alpha[0], ch.Name)
	if !get_filename(&buf2[0], uint64(20480), NEW_OBJ_FILES, GET_NAME(ch)) {
		return -1
	}
...
func Obj_to_store(obj *obj_data, fl *stdio.File, location int) bool {
	...
	if location > 0 {
		...
		if location > 0 {
			if (ch.Equipment[j]) == nil {
				...
				mudlog(BRF, ADMLVL_IMMORT, 1, libc.CString("SYSERR: autoeq: '%s' already equipped in position %d."), GET_NAME(ch), location)
				location = LOC_INVENTORY
			}
		}
	}
...
func Crash_delete_file(name *byte) bool {
	var (
		filename [50]byte
		fl       *stdio.File
	)
	if !get_filename(&filename[0], uint64(50), NEW_OBJ_FILES, name) {
		return false
	}
	if (func() *stdio.File {
...
func Crash_delete_crashfile(ch *char_data) bool {
	var (
		filename [2048]byte
		fl       *stdio.File
		rentcode int
	...
	if !get_filename(&filename[0], uint64(2048), NEW_OBJ_FILES, GET_NAME(ch)) {
		return false
	}
	if (func() *stdio.File {
		fl = stdio.FOpen(libc.GoString(&filename[0]), "rb")
...
func Crash_clean_file(name *byte) bool {
	var (
		filename [64936]byte
		fl       *stdio.File
		rentcode int
	...
	if !get_filename(&filename[0], uint64(64936), NEW_OBJ_FILES, name) {
		return false
	}
	if (func() *stdio.File {
...
func Crash_save(obj *obj_data, fp *stdio.File, location int) bool {
	var (
		tmp    *obj_data
		result int
	)
	if obj != nil {
		...
		result = int(libc.BoolToInt(Obj_to_store(obj, fp, location)))
...
func Crash_extract_norent_eq(ch *char_data) {
	...
	for j = 0; j < NUM_WEARS; j++ {
		...
		if Crash_is_unrentable(ch.Equipment[j]) {
...
func Crash_is_unrentable(obj *obj_data) bool {
	if obj == nil {
		return false
	}
	if OBJ_FLAGGED(obj, ITEM_NORENT) || obj.Cost_per_day < 0 || obj.Item_number <= int(-1) && !IS_SET_AR(obj.Extra_flags[:], ITEM_UNIQUE_SAVE) {
		return true
	}
	return false
}
...
func Crash_crashsave(ch *char_data) {
	...
	if !get_filename(&buf[0], uint64(2048), NEW_OBJ_FILES, GET_NAME(ch)) {
		return
	}
	if (func() *stdio.File {
	...
	for j = 0; j < NUM_WEARS; j++ {
		if (ch.Equipment[j]) != nil {
			if !Crash_save(ch.Equipment[j], fp, j+1) {
				fp.Close()
				return
			}
...
func Crash_cryosave(ch *char_data, cost int) {
	...
	if !get_filename(&buf[0], uint64(2048), CRASH_FILE, GET_NAME(ch)) {
		return
	}
	if (func() *stdio.File {
		fp = stdio.FOpen(libc.GoString(&buf[0]), "wb")
...
func Crash_rent_deadline(ch *char_data, recep *char_data, cost int) {
	...
	act(&buf[0], 0, recep, nil, unsafe.Pointer(ch), TO_VICT)
}
...
func Crash_offer_rent(ch *char_data, recep *char_data, display int, factor int) int {
	...
	if numitems == 0 {
		act(libc.CString("$n tells you, 'But you are not carrying anything!  Just quit!'"), 0, recep, nil, unsafe.Pointer(ch), TO_VICT)
		return 0
	}
	...
	if display != 0 {
		...
		if totalcost > ch.Gold+ch.Bank_gold {
			act(libc.CString("$n tells you, '...which I see you can't afford.'"), 0, recep, nil, unsafe.Pointer(ch), TO_VICT)
			return 0
...
func gen_receptionist(ch *char_data, recep *char_data, cmd int, arg *byte, mode int) bool {
	var (
		cost         int
	...
	if !CAN_SEE(recep, ch) {
		act(libc.CString("$n says, 'I don't deal with people I can't see!'"), 0, recep, nil, nil, TO_ROOM)
		return true
	}
	if config_info.Csd.Free_rent != 0 {
		act(libc.CString("$n tells you, 'Rent is free here.  Just quit, and your objects will be saved!'"), 0, recep, nil, unsafe.Pointer(ch), TO_VICT)
		return true
	}
	if libc.StrCmp(libc.CString("rent"), complete_cmd_info[cmd].Command) == 0 {
		var buf [128]byte
		if (func() int {
			cost = Crash_offer_rent(ch, recep, 0, mode)
			return cost
		}()) == 0 {
			return true
		}
		...
		if mode == RENT_FACTOR {
			act(libc.CString("$n stores your belongings and helps you into your private chamber."), 0, recep, nil, unsafe.Pointer(ch), TO_VICT)
			Crash_rentsave(ch, cost)
			mudlog(NRM, int(MAX(ADMLVL_IMMORT, int64(ch.Player_specials.Invis_level))), 1, libc.CString("%s has rented (%d/day, %d tot.)"), GET_NAME(ch), cost, ch.Gold+ch.Bank_gold)
		} else {
			act(libc.CString("$n stores your belongings and helps you into your private chamber.\r\nA white mist appears in the room, chilling you to the bone...\r\nYou begin to lose consciousness..."), 0, recep, nil, unsafe.Pointer(ch), TO_VICT)
			Crash_cryosave(ch, cost)
			mudlog(NRM, int(MAX(ADMLVL_IMMORT, int64(ch.Player_specials.Invis_level))), 1, libc.CString("%s has cryo-rented."), GET_NAME(ch))
			SET_BIT_AR(ch.Act[:], PLR_CRYO)
		}
		act(libc.CString("$n helps $N into $S private chamber."), 0, recep, nil, unsafe.Pointer(ch), TO_NOTVICT)
		extract_char(ch)
	} else {
		Crash_offer_rent(ch, recep, 1, mode)
		act(libc.CString("$N gives $n an offer."), 0, ch, nil, unsafe.Pointer(recep), TO_ROOM)
	}
	return true
}
func receptionist(ch *char_data, me unsafe.Pointer, cmd int, argument *byte) bool {
	return gen_receptionist(ch, (*char_data)(me), cmd, argument, RENT_FACTOR)
}
func cryogenicist(ch *char_data, me unsafe.Pointer, cmd int, argument *byte) bool {
...
func Crash_load(ch *char_data) bool {
	var (
		fl      *stdio.File
		cmfname [64936]byte
		buf1    [64936]byte
	...
	var f3 [256]byte
	var f4 [256]byte
	if !get_filename(&cmfname[0], uint64(64936), NEW_OBJ_FILES, GET_NAME(ch)) {
		return true
	}
	if (func() *stdio.File {
		...
		if GET_LEVEL(ch) > 1 {
			mudlog(NRM, int(MAX(ADMLVL_IMMORT, int64(ch.Player_specials.Invis_level))), 1, libc.CString("%s entering game with no equipment. Loading backup."), GET_NAME(ch))
		}
		if inv_backup(ch) == 0 {
			return true
		} else {
	...
	case RENT_RENTED:
		mudlog(NRM, int(MAX(ADMLVL_IMMORT, int64(ch.Player_specials.Invis_level))), 1, libc.CString("%s un-renting and entering game."), GET_NAME(ch))
	case RENT_CRASH:
		mudlog(NRM, int(MAX(ADMLVL_IMMORT, int64(ch.Player_specials.Invis_level))), 1, libc.CString("%s retrieving crash-saved items and entering game."), GET_NAME(ch))
	case RENT_CRYO:
		mudlog(NRM, int(MAX(ADMLVL_IMMORT, int64(ch.Player_specials.Invis_level))), 1, libc.CString("%s un-cryo'ing and entering game."), GET_NAME(ch))
	case RENT_FORCED:
		fallthrough
	case RENT_TIMEDOUT:
		mudlog(NRM, int(MAX(ADMLVL_IMMORT, int64(ch.Player_specials.Invis_level))), 1, libc.CString("%s retrieving force-saved items and entering game."), GET_NAME(ch))
	default:
		mudlog(BRF, int(MAX(ADMLVL_IMMORT, int64(ch.Player_specials.Invis_level))), 1, libc.CString("WARNING: %s entering game with undefined rent code."), GET_NAME(ch))
	}
	...
	for int(fl.IsEOF()) == 0 {
		...
		if line[0] == '#' {
			...
			if libc.StrCmp(libc.CString("XAP"), &line[0]) == 0 {
				...
				temp.Wear_flags[0] = uint32(int32(t[1]))
				temp.Wear_flags[1] = uint32(int32(t[2]))
				temp.Wear_flags[2] = uint32(int32(t[3]))
				temp.Wear_flags[3] = uint32(int32(t[4]))
	...
	mudlog(NRM, int(MAX(ADMLVL_IMMORT, int64(ch.Player_specials.Invis_level))), 1, libc.CString("%s (level %d) has %d objects (max %d)."), GET_NAME(ch), GET_LEVEL(ch), num_objs, config_info.Csd.Max_obj_save)
	fl.Close()
	Crash_crashsave(ch)
