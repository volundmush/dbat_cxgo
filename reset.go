...
const PERFORM_NORMAL_RESET = 0
const BLOCK_NORMAL_RESET = 1
const RESET_GAUNTLET = 328
const NUM_GAUNTLET_ROOMS = 20

type gauntlet_mob struct {
	Vroom room_vnum
	Vmob  mob_vnum
}

func pre_reset(znum zone_vnum) bool {
...
func post_reset(znum zone_vnum) {
	switch znum {
	default:
	}
}
func prereset_gauntlet_zone() bool {
	...
	for i = 0; i < NUM_GAUNTLET_ROOMS; i++ {
		if int(libc.BoolToInt(check_mob_in_room(gauntlet_mobs[i].Vmob, gauntlet_mobs[i].Vroom))) == FALSE {
