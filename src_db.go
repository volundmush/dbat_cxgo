...
const FASTBOOT_FILE = "../.fastboot"     /* autorun: boot without sleep  */
const KILLSCRIPT_FILE = "../.killscript" /* autorun: shut mud down       */
const PAUSE_FILE = "../pause"            /* autorun: don't restart mud   */

/* names of various files and directories */
const INDEX_FILE = "index"       /* index of world files		*/
const MINDEX_FILE = "index.mini" /* ... and for mini-mud-mode	*/
...
var r_mortal_start_room int
var r_immort_start_room int
var r_frozen_start_room int
var xap_objs int = 0
var converting int = 0
...
func boot_db() {
	...
	file_to_string_alloc(libc.CString(NEWS_FILE), &news)
	file_to_string_alloc(libc.CString(CREDITS_FILE), &credits)
	file_to_string_alloc(libc.CString(MOTD_FILE), &motd)
	file_to_string_alloc(libc.CString(IMOTD_FILE), &imotd)
	file_to_string_alloc(libc.CString(INFO_FILE), &info)
	file_to_string_alloc(libc.CString(WIZLIST_FILE), &wizlist)
	file_to_string_alloc(libc.CString(IMMLIST_FILE), &immlist)
	file_to_string_alloc(libc.CString(POLICIES_FILE), &policies)
	file_to_string_alloc(libc.CString(HANDBOOK_FILE), &handbook)
	file_to_string_alloc(libc.CString(BACKGROUND_FILE), &background)
	file_to_string_alloc(libc.CString(HELP_PAGE_FILE), &help)
	file_to_string_alloc(libc.CString(IHELP_PAGE_FILE), &ihelp)
	if file_to_string_alloc(libc.CString(GREETINGS_FILE), &GREETINGS) == 0 {
		prune_crlf(GREETINGS)
	}
	if file_to_string_alloc(libc.CString(GREETANSI_FILE), &GREETANSI) == 0 {
		prune_crlf(GREETANSI)
	}
	...
	insure_directory(libc.CString(LIB_PLROBJS+"CRASH"), 0)
...
func save_mud_time(when *time_info_data) {
	...
	if (func() *stdio.File {
		bgtime = stdio.FOpen(TIME_FILE, "w")
		return bgtime
	}()) == nil {
		basic_mud_log(libc.CString("SYSERR: Can't write to '%s' time file."), TIME_FILE)
	} else {
...
func index_boot(mode int) {
	...
	stdio.Snprintf(&buf2[0], 260, "%s%s", prefix, index_filename)
	if (func() *stdio.File {
	...
	stdio.Fscanf(db_index, "%s", &buf1[0])
	for buf1[0] != '$' {
...
func asciiflag_conv(flag *byte) uint32 {
	var (
		flags  uint32 = 0
		is_num int    = 1
		p      *byte
	...
	if is_num != 0 {
		flags = uint32(int32(libc.Atoi(libc.GoString(flag))))
	}
	return flags
}
func asciiflag_conv_aff(flag *byte) uint32 {
	var (
		flags  uint32 = 0
		is_num int    = 1
		p      *byte
...
func parse_room(fl *stdio.File, virtual_nr int) {
	...
	for virtual_nr > zone_table[zone].Top {
		if func() int {
			p := &zone
			*p++
			return *p
...
func setup_dir(fl *stdio.File, room int, dir int) {
	...
	stdio.Snprintf(&buf2[0], int(128), "room #%d, direction D%d", int(libc.BoolToInt(GET_ROOM_VNUM(room)))+1, dir)
	...
	for room = 0; room <= top_of_world; room++ {
		for door = 0; door < NUM_OF_DIRS; door++ {
			if world[room].Dir_option[door] != nil {
				if world[room].Dir_option[door].To_room != int(-1) {
...
func renum_zone_table() {
	...
	for zone = 0; zone <= top_of_zone_table; zone++ {
		for cmd_no = 0; int(zone_table[zone].Cmd[cmd_no].Command) != 'S'; cmd_no++ {
			...
			olda = zone_table[zone].Cmd[cmd_no].Arg1
			oldb = zone_table[zone].Cmd[cmd_no].Arg2
			oldc = zone_table[zone].Cmd[cmd_no].Arg3
			switch zone_table[zone].Cmd[cmd_no].Command {
			case 'M':
				a = func() int {
					p := &zone_table[zone].Cmd[cmd_no].Arg1
					zone_table[zone].Cmd[cmd_no].Arg1 = real_mobile(zone_table[zone].Cmd[cmd_no].Arg1)
					return *p
				}()
				c = func() int {
					p := &zone_table[zone].Cmd[cmd_no].Arg3
					zone_table[zone].Cmd[cmd_no].Arg3 = real_room(zone_table[zone].Cmd[cmd_no].Arg3)
					return *p
				}()
			case 'O':
				a = func() int {
					p := &zone_table[zone].Cmd[cmd_no].Arg1
					zone_table[zone].Cmd[cmd_no].Arg1 = real_object(zone_table[zone].Cmd[cmd_no].Arg1)
					return *p
				}()
				if zone_table[zone].Cmd[cmd_no].Arg3 != int(-1) {
					c = func() int {
						p := &zone_table[zone].Cmd[cmd_no].Arg3
						zone_table[zone].Cmd[cmd_no].Arg3 = real_room(zone_table[zone].Cmd[cmd_no].Arg3)
						return *p
			...
			case 'P':
				...
				c = func() int {
					p := &zone_table[zone].Cmd[cmd_no].Arg3
					zone_table[zone].Cmd[cmd_no].Arg3 = real_object(zone_table[zone].Cmd[cmd_no].Arg3)
					return *p
				}()
			case 'D':
				a = func() int {
					p := &zone_table[zone].Cmd[cmd_no].Arg1
					zone_table[zone].Cmd[cmd_no].Arg1 = real_room(zone_table[zone].Cmd[cmd_no].Arg1)
					return *p
				}()
			case 'R':
				a = func() int {
				...
				b = func() int {
					...
					zone_table[zone].Cmd[cmd_no].Arg2 = real_object(zone_table[zone].Cmd[cmd_no].Arg2)
					return *p
				}()
			case 'T':
				b = func() int {
					p := &zone_table[zone].Cmd[cmd_no].Arg2
					zone_table[zone].Cmd[cmd_no].Arg2 = real_trigger(zone_table[zone].Cmd[cmd_no].Arg2)
					return *p
				}()
				c = func() int {
					p := &zone_table[zone].Cmd[cmd_no].Arg3
			...
			if a == int(-1) || b == int(-1) || c == int(-1) {
				if mini_mud == 0 {
					stdio.Snprintf(&buf[0], int(128), "Invalid vnum %lld, cmd disabled", func() int {
						if a == int(-1) {
							return olda
						}
						if b == int(-1) {
...
func parse_simple_mob(mob_f *stdio.File, ch *char_data, nr int) bool {
	var (
		j    int
		t    [10]int
		line [256]byte
...
func interpret_espec(keyword *byte, value *byte, ch *char_data, nr int) {
	...
	if value != nil && matched == 0 && libc.StrCaseCmp(keyword, libc.CString("Affect")) == 0 && (func() int {
		...
		if num > 0 {
			...
			af.Modifier = num3
			af.Location = num4
			af.Bitvector = uint32(int32(num5))
			af.Specific = num6
...
func parse_enhanced_mob(mob_f *stdio.File, ch *char_data, nr int) bool {
	var line [256]byte
...
func parse_mobile_from_file(mob_f *stdio.File, ch *char_data) bool {
	var (
		j      int
		t      [10]int
		retval int
...
func load_zones(fl *stdio.File, zonename *byte) {
	...
	if stdio.Sscanf(&buf[0], "#%d", &zone_table[zone].Number) != 1 {
	...
	if version >= 2 {
		...
		if stdio.Sscanf(&buf[0], "%d %d %d %d %s %s %s %s %d %d", &zone_table[zone].Bot, &zone_table[zone].Top, &zone_table[zone].Lifespan, &zone_table[zone].Reset_mode, &zbuf1[0], &zbuf2[0], &zbuf3[0], &zbuf4[0], &zone_table[zone].Min_level, &zone_table[zone].Max_level) != 10 {
	...
	} else if stdio.Sscanf(&buf[0], "%d %d %d %d", &zone_table[zone].Bot, &zone_table[zone].Top, &zone_table[zone].Lifespan, &zone_table[zone].Reset_mode) != 4 {
		basic_mud_log(libc.CString("SYSERR: Format error in numeric constant line of %s, attempting to fix."), &zname[0])
		if stdio.Sscanf(zone_table[zone].Name, " %d %hd %d %d", &zone_table[zone].Bot, &zone_table[zone].Top, &zone_table[zone].Lifespan, &zone_table[zone].Reset_mode) != 4 {
...
func read_mobile(nr int, type_ int) *char_data {
	var (
		i   int
		mob *char_data
	)
	...
	if mob.Exp <= 0 && !MOB_FLAGGED(mob, MOB_DUMMY) {
		...
		if int(mob.Chclass) == CLASS_NPC_COMMONER && IS_HUMANOID(mob) && int(mob.Race) != RACE_LIZARDFOLK {
			if int(mob.Race) != RACE_ANDROID && int(mob.Race) != RACE_SAIYAN && int(mob.Race) != RACE_BIO && int(mob.Race) != RACE_MAJIN {
				mob.Exp = int64(float64(mob.Exp) * .75)
			}
		}
		if GET_LEVEL(mob) > 90 {
...
func log_dupe_objects(obj1 *obj_data, obj2 *obj_data) {
	mudlog(BRF, ADMLVL_GOD, 1, libc.CString("DUPE: Dupe object found: %s [%d] [%ld:%lld]"), func() *byte {
	...
	mudlog(BRF, ADMLVL_GOD, 1, libc.CString("DUPE: First: In room: %d (%s), In object: %s, Carried by: %s, Worn by: %s"), GET_ROOM_VNUM(obj1.In_room), func() string {
		if obj1.In_room == int(-1) {
			return "Nowhere"
		}
	...
	mudlog(BRF, ADMLVL_GOD, 1, libc.CString("DUPE: Newer: In room: %d (%s), In object: %s, Carried by: %s, Worn by: %s"), GET_ROOM_VNUM(obj2.In_room), func() string {
		if obj2.In_room == int(-1) {
			return "Nowhere"
		}
...
func read_object(nr int, type_ int) *obj_data {
	var (
		obj *obj_data
		i   int
	)
...
func zone_update() {
	...
	for update_u = reset_q.Head; update_u != nil; update_u = update_u.Next {
		if zone_table[update_u.Zone_to_reset].Reset_mode == 2 || is_empty(update_u.Zone_to_reset) {
			...
			mudlog(CMP, ADMLVL_GOD, 0, libc.CString("Auto zone reset: %s (Zone %d)"), zone_table[update_u.Zone_to_reset].Name, zone_table[update_u.Zone_to_reset].Number)
...
func log_zone_error(zone int, cmd_no int, message *byte) {
	mudlog(NRM, ADMLVL_GOD, 1, libc.CString("SYSERR: zone file: %s"), message)
	mudlog(NRM, ADMLVL_GOD, 1, libc.CString("SYSERR: ...offending cmd: '%c' cmd in zone #%d, line %d"), zone_table[zone].Cmd[cmd_no].Command, zone_table[zone].Number, zone_table[zone].Cmd[cmd_no].Line)
}
func reset_zone(zone int) {
	var (
		cmd_no   int
		last_cmd int        = 0
		mob      *char_data = nil
	...
	if !pre_reset(zone_table[zone].Number) {
		for cmd_no = 0; int(zone_table[zone].Cmd[cmd_no].Command) != 'S'; cmd_no++ {
			...
			case '*':
				last_cmd = 0
			case 'M':
				if mob_index[zone_table[zone].Cmd[cmd_no].Arg1].Number < zone_table[zone].Cmd[cmd_no].Arg2 && rand_number(1, 100) >= zone_table[zone].Cmd[cmd_no].Arg5 {
					var (
						room_max int = 0
						i        *char_data
					)
					mob = read_mobile(zone_table[zone].Cmd[cmd_no].Arg1, REAL)
					if zone_table[zone].Cmd[cmd_no].Arg4 > 0 {
						for i = character_list; i != nil; i = i.Next {
							if i.Hometown == int(libc.BoolToInt(GET_ROOM_VNUM(zone_table[zone].Cmd[cmd_no].Arg3))) && GET_MOB_VNUM(i) == GET_MOB_VNUM(mob) {
								room_max++
							}
						}
					}
					char_to_room(mob, zone_table[zone].Cmd[cmd_no].Arg3)
					if room_max != 0 && room_max >= zone_table[zone].Cmd[cmd_no].Arg4 {
						extract_char(mob)
						extract_pending_chars()
						break
					}
					mob.Hometown = int(libc.BoolToInt(GET_ROOM_VNUM(zone_table[zone].Cmd[cmd_no].Arg3)))
					load_mtrigger(mob)
					tmob = mob
					last_cmd = 1
					mob_load = 1
				...
				tobj = nil
			case 'O':
				if obj_index[zone_table[zone].Cmd[cmd_no].Arg1].Number < zone_table[zone].Cmd[cmd_no].Arg2 && rand_number(1, 100) >= zone_table[zone].Cmd[cmd_no].Arg5 {
					if zone_table[zone].Cmd[cmd_no].Arg3 != int(-1) {
						var (
							room_max int = 0
							k        *obj_data
						)
						obj = read_object(zone_table[zone].Cmd[cmd_no].Arg1, REAL)
						if zone_table[zone].Cmd[cmd_no].Arg4 > 0 {
							for k = object_list; k != nil; k = k.Next {
								if k.Room_loaded == int(libc.BoolToInt(GET_ROOM_VNUM(zone_table[zone].Cmd[cmd_no].Arg3))) && GET_OBJ_VNUM(k) == GET_OBJ_VNUM(obj) || GET_OBJ_VNUM(k) == GET_OBJ_VNUM(obj) && GET_ROOM_VNUM(zone_table[zone].Cmd[cmd_no].Arg3) == GET_ROOM_VNUM(k.In_room) {
									if k.In_room == int(-1) || GET_ROOM_VNUM(k.In_room) != GET_ROOM_VNUM(zone_table[zone].Cmd[cmd_no].Arg3) {
										continue
									}
									room_max++
								}
						...
						add_unique_id(obj)
						obj_to_room(obj, zone_table[zone].Cmd[cmd_no].Arg3)
						if room_max != 0 && room_max >= zone_table[zone].Cmd[cmd_no].Arg4 {
							extract_obj(obj)
							break
						}
						obj.Room_loaded = int(libc.BoolToInt(GET_ROOM_VNUM(zone_table[zone].Cmd[cmd_no].Arg3)))
						last_cmd = 1
						load_otrigger(obj)
						tobj = obj
						obj_load = 1
				...
				tmob = nil
			case 'P':
				if obj_index[zone_table[zone].Cmd[cmd_no].Arg1].Number < zone_table[zone].Cmd[cmd_no].Arg2 && obj_load != 0 && rand_number(1, 100) >= zone_table[zone].Cmd[cmd_no].Arg5 {
					obj = read_object(zone_table[zone].Cmd[cmd_no].Arg1, REAL)
					if (func() *obj_data {
						obj_to = get_obj_num(zone_table[zone].Cmd[cmd_no].Arg3)
						return obj_to
					}()) == nil {
			...
			case 'G':
				...
				if obj_index[zone_table[zone].Cmd[cmd_no].Arg1].Number < zone_table[zone].Cmd[cmd_no].Arg2 && mob_load != 0 && rand_number(1, 100) >= zone_table[zone].Cmd[cmd_no].Arg5 {
					obj = read_object(zone_table[zone].Cmd[cmd_no].Arg1, REAL)
					add_unique_id(obj)
					obj_to_char(obj, mob)
			...
			case 'E':
				...
				if obj_index[zone_table[zone].Cmd[cmd_no].Arg1].Number < zone_table[zone].Cmd[cmd_no].Arg2 && mob_load != 0 && rand_number(1, 100) >= zone_table[zone].Cmd[cmd_no].Arg5 {
					if zone_table[zone].Cmd[cmd_no].Arg3 < 0 || zone_table[zone].Cmd[cmd_no].Arg3 >= NUM_WEARS {
						...
						load_otrigger(obj)
						if wear_otrigger(obj, mob, zone_table[zone].Cmd[cmd_no].Arg3) {
							obj.In_room = -1
							equip_char(mob, obj, zone_table[zone].Cmd[cmd_no].Arg3)
						} else {
							obj_to_char(obj, mob)
						}
						tobj = obj
			...
			case 'R':
				if (func() *obj_data {
					obj = get_obj_in_list_num(zone_table[zone].Cmd[cmd_no].Arg2, world[zone_table[zone].Cmd[cmd_no].Arg1].Contents)
					return obj
				}()) != nil {
					extract_obj(obj)
				}
			...
			case 'D':
				if zone_table[zone].Cmd[cmd_no].Arg2 < 0 || zone_table[zone].Cmd[cmd_no].Arg2 >= NUM_OF_DIRS || world[zone_table[zone].Cmd[cmd_no].Arg1].Dir_option[zone_table[zone].Cmd[cmd_no].Arg2] == nil {
					...
					case 0:
						world[zone_table[zone].Cmd[cmd_no].Arg1].Dir_option[zone_table[zone].Cmd[cmd_no].Arg2].Exit_info &= ^(uint32(1) << 2)
						world[zone_table[zone].Cmd[cmd_no].Arg1].Dir_option[zone_table[zone].Cmd[cmd_no].Arg2].Exit_info &= ^(uint32(1) << 1)
					case 1:
			...
			case 'T':
				if zone_table[zone].Cmd[cmd_no].Arg1 == MOB_TRIGGER && tmob != nil {
					...
					add_trigger(tmob.Script, read_trigger(zone_table[zone].Cmd[cmd_no].Arg2), -1)
					last_cmd = 1
				} else if zone_table[zone].Cmd[cmd_no].Arg1 == OBJ_TRIGGER && tobj != nil {
					if tobj.Script == nil {
						tobj.Script = new(script_data)
					}
					add_trigger(tobj.Script, read_trigger(zone_table[zone].Cmd[cmd_no].Arg2), -1)
					last_cmd = 1
				} else if zone_table[zone].Cmd[cmd_no].Arg1 == WLD_TRIGGER {
					if zone_table[zone].Cmd[cmd_no].Arg3 == int(-1) || zone_table[zone].Cmd[cmd_no].Arg3 > top_of_world {
					...
					add_trigger(world[zone_table[zone].Cmd[cmd_no].Arg3].Script, read_trigger(zone_table[zone].Cmd[cmd_no].Arg2), -1)
					last_cmd = 1
				}
			case 'V':
				if zone_table[zone].Cmd[cmd_no].Arg1 == MOB_TRIGGER && tmob != nil {
					if tmob.Script == nil {
						...
						add_var(&tmob.Script.Global_vars, zone_table[zone].Cmd[cmd_no].Sarg1, zone_table[zone].Cmd[cmd_no].Sarg2, zone_table[zone].Cmd[cmd_no].Arg3)
					}
					last_cmd = 1
				} else if zone_table[zone].Cmd[cmd_no].Arg1 == OBJ_TRIGGER && tobj != nil {
					if tobj.Script == nil {
						...
						add_var(&tobj.Script.Global_vars, zone_table[zone].Cmd[cmd_no].Sarg1, zone_table[zone].Cmd[cmd_no].Sarg2, zone_table[zone].Cmd[cmd_no].Arg3)
					}
					last_cmd = 1
				} else if zone_table[zone].Cmd[cmd_no].Arg1 == WLD_TRIGGER {
					if zone_table[zone].Cmd[cmd_no].Arg3 == int(-1) || zone_table[zone].Cmd[cmd_no].Arg3 > top_of_world {
						...
						if world[zone_table[zone].Cmd[cmd_no].Arg3].Script == nil {
							...
							add_var(&world[zone_table[zone].Cmd[cmd_no].Arg3].Script.Global_vars, zone_table[zone].Cmd[cmd_no].Sarg1, zone_table[zone].Cmd[cmd_no].Sarg2, zone_table[zone].Cmd[cmd_no].Arg2)
						}
						last_cmd = 1
					}
				}
...
func free_char(ch *char_data) {
	...
	if !IS_NPC(ch) || IS_NPC(ch) && ch.Nr == int(-1) {
		if GET_NAME(ch) != nil {
...
func real_room(vnum int) int {
	...
	if i != int(-1) && world[i].Number == vnum {
		return i
	} else {
		bot = 0
		top = top_of_world
...
func real_mobile(vnum int) int {
	...
	if i != int(-1) && mob_index[i].Vnum == vnum {
		return i
	} else {
		bot = 0
		top = top_of_mobt
...
func real_object(vnum int) int {
	...
	if i != int(-1) && obj_index[i].Vnum == vnum {
		return i
	} else {
		bot = 0
		top = top_of_objt
...
func check_object(obj *obj_data) int {
	...
	case ITEM_DRINKCON:
		...
		if search_block(&onealias[0], &drinknames[0], 1) < 0 && (func() int {
			error = 1
			return error
		}()) != 0 {
		}
...
func check_bitvector_names(bits uint32, namecount uint64, whatami *byte, whatbits *byte) int {
	var (
		flagnum uint
		error   bool = false
	)
	if uintptr(bits) <= (uintptr(^uint32(0)) >> (unsafe.Sizeof(uint32(0))*8 - uintptr(namecount))) {
		return 0
	}
	for flagnum = uint(namecount); flagnum < uint(unsafe.Sizeof(uint32(0))*8); flagnum++ {
...
func my_obj_save_to_disk(fp *stdio.File, obj *obj_data, locate int) bool {
	var (
		counter2 int
		i        int
