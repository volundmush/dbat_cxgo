...
func sub_write(arg *byte, ch *char_data, find_invis int8, targets int) {
	...
	*s = '\x00'
	...
	if IS_SET(uint32(int32(targets)), TO_CHAR) && SENDOK(ch) {
		sub_write_to_char(ch, tokens[:], otokens[:], type_[:])
	}
	if IS_SET(uint32(int32(targets)), TO_ROOM) {
...
func send_to_zone(messg *byte, zone int) {
	var i *descriptor_data
	if messg == nil || *messg == 0 {
		return
	}
	for i = descriptor_list; i != nil; i = i.Next {
		if i.Connected == 0 && i.Character != nil && AWAKE(i.Character) && i.Character.In_room != int(-1) && world[i.Character.In_room].Zone == zone {
			write_to_output(i, libc.CString("%s"), messg)
		}
	}
}
func fly_zone(zone int, messg *byte, ch *char_data) {
	var i *descriptor_data
	if messg == nil || *messg == 0 {
		return
	}
	for i = descriptor_list; i != nil; i = i.Next {
		if i.Connected == 0 && i.Character != nil && AWAKE(i.Character) && OUTSIDE(i.Character) && i.Character.In_room != int(-1) && world[i.Character.In_room].Zone == zone && i.Character != ch {
