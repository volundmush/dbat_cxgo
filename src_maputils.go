...
func checkship(rnum int, vnum int) int {
	...
	for i = world[rnum].Contents; i != nil; i = i.Next_content {
		if !ROOM_FLAGGED(rnum, ROOM_NEBULA) {
			if int(i.Type_flag) == ITEM_VEHICLE && there != 1 {
				there = 1
...
func getmapchar(rnum int, ch *char_data, start int, vnum int) *byte {
	...
	if rnum == real_room(ch.Radar1) || rnum == real_room(ch.Radar2) || rnum == real_room(ch.Radar3) {
		if there != 0 {
	...
	} else if int(libc.BoolToInt(GET_ROOM_VNUM(rnum))) == 0xC654 {
		if there != 0 {
	...
	} else if int(libc.BoolToInt(GET_ROOM_VNUM(rnum))) == 0x948C {
		if there != 0 {
