...
func delete_mobile(refpt mob_rnum) int {
	...
	m_vnum = mob_index[refpt].Vnum
	extract_mobile_all(m_vnum)
	for counter = int(refpt); counter < int(top_of_mobt); counter++ {
		...
		mob_proto[counter].Nr = mob_rnum(counter)
	}
	top_of_mobt--
	// todo: figure this out
	//mob_index = []index_data((*index_data)(libc.Realloc(unsafe.Pointer(&mob_index[0]), int(top_of_mobt*mob_rnum(unsafe.Sizeof(index_data{}))+1))))
	//mob_proto = []char_data((*char_data)(libc.Realloc(unsafe.Pointer(&mob_proto[0]), int(top_of_mobt*mob_rnum(unsafe.Sizeof(char_data{}))+1))))
	...
	for zone = 0; zone <= top_of_zone_table; zone++ {
		for cmd_no = 0; int(zone_table[zone].Cmd[cmd_no].Command) != 'S'; cmd_no++ {
			if int(zone_table[zone].Cmd[cmd_no].Command) == 'M' && zone_table[zone].Cmd[cmd_no].Arg1 == vnum(refpt) {
	...
	save_mobiles(real_zone_by_thing(room_vnum(m_vnum)))
	return int(refpt)
}
...
func save_mobiles(zone_num zone_rnum) int {
	...
	for i = genolc_zone_bottom(zone_num); i <= zone_table[zone_num].Top; i++ {
		...
		if write_mobile_record(mob_vnum(i), &mob_proto[rmob], mobfd) < 0 {
	...
	if in_save_list(zone_table[zone_num].Number, SL_MOB) != 0 {
		remove_from_save_list(zone_table[zone_num].Number, SL_MOB)
		create_world_index(int(zone_table[zone_num].Number), libc.CString("mob"))
...
func check_mobile_strings(mob *char_data) {
	var mvnum mob_vnum = mob_index[mob.Nr].Vnum
