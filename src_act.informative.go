...
func search_room(ch *char_data) {
	...
	reveal_hiding(ch, 0)
	act(libc.CString("@y$n@Y begins searching the room carefully.@n"), 1, ch, nil, nil, TO_ROOM)
	...
	for vict = world[ch.In_room].People; vict != nil; vict = next_v {
		...
		if AFF_FLAGGED(vict, AFF_HIDE) && vict != ch {
			...
			if float64(perc)*bonus >= float64(prob)*terrain {
				act(libc.CString("@YYou find @y$N@Y hiding nearby!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@y$n@Y has found your hiding spot!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@y$n@Y has found @y$N's@Y hiding spot!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
	...
	for obj = world[ch.In_room].Contents; obj != nil; obj = obj.Next_content {
		if OBJ_FLAGGED(obj, ITEM_BURIED) && float64(perc)*bonus > float64(rand_number(50, 200)) {
			act(libc.CString("@YYou uncover @y$p@Y, which had been burried here.@n"), 1, ch, obj, nil, TO_CHAR)
			act(libc.CString("@y$n@Y uncovers @y$p@Y, which had burried here.@n"), 1, ch, obj, nil, TO_ROOM)
...
func do_mimic(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if israce == 1 {
		...
		ch.Mana -= cost
		act(libc.CString("@mYou concentrate and attempt to create an illusion to obscure your racial features. However you frown as you realize you have failed.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@M$n@m concentrates and the light around them seems to shift and blur. It stops a moment later and $e frowns.@n"), 1, ch, nil, nil, TO_ROOM)
		return
	} else if change > -1 {
		var buf [64936]byte
		ch.Mimic = change
		...
		act(libc.CString("@mYou concentrate for a moment and release the illusion that was mimicing another race.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@M$n@m concentrates for a moment and SUDDENLY $s appearance changes some what!@n"), 1, ch, nil, nil, TO_ROOM)
		ch.Mimic = 0
	} else {
...
func do_kyodaika(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if (ch.Genome[0]) == 0 {
		act(libc.CString("@GYou growl as your body grows to ten times its normal size!@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@g$n@G growls as $s body grows to ten times its normal size!@n"), 1, ch, nil, nil, TO_ROOM)
		...
		act(libc.CString("@GYou growl as your body shrinks to its normal size!@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@g$n@G growls as $s body shrinks to its normal size!@n"), 1, ch, nil, nil, TO_ROOM)
		send_to_char(ch, libc.CString("@cStrength@D: @C-5\r\n@cSpeed@D: @c+2@n\r\n"))
		ch.Real_abils.Str -= 5
		ch.Real_abils.Cha += 2
		ch.Genome[0] = 0
...
func do_shuffle(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	act(libc.CString("$n shuffles their deck."), 1, ch, nil, nil, TO_ROOM)
...
func do_hand(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if libc.StrCaseCmp(libc.CString("look"), &arg[0]) == 0 {
		...
		act(libc.CString("$n looks at $s hand."), 1, ch, nil, nil, TO_ROOM)
		if count == 0 {
			send_to_char(ch, libc.CString("No cards."))
			act(libc.CString("There were no cards."), 1, ch, nil, nil, TO_ROOM)
		} else if count > 7 {
			act(libc.CString("You have more than seven cards in your hand."), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("$n has more than seven cards in $s hand."), 1, ch, nil, nil, TO_ROOM)
		} else {
			var buf [200]byte
		...
		for obj = ch.Carrying; obj != nil; obj = obj.Next_content {
			...
			if obj != nil {
				count += 1
				act(libc.CString("$p"), 1, ch, obj, nil, TO_ROOM)
			}
		}
		if count == 0 {
			act(libc.CString("No cards."), 1, ch, nil, nil, TO_ROOM)
		}
		if count > 7 {
			act(libc.CString("You have more than seven cards in your hand."), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("$n has more than seven cards in $s hand."), 1, ch, nil, nil, TO_ROOM)
...
func do_post(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if arg2[0] == 0 {
		...
		act(libc.CString("@WYou post $p@W on a nearby structure.@n"), 1, ch, obj, nil, TO_CHAR)
		act(libc.CString("@C$n@W posts $p@W on a nearby structure.@n"), 1, ch, obj, nil, TO_ROOM)
		obj_from_char(obj)
...
func do_play(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	act(libc.CString("You play $p on your table."), 1, ch, obj, nil, TO_CHAR)
	act(libc.CString("$n plays $p on $s table."), 1, ch, obj, nil, TO_ROOM)
	obj_from_char(obj)
	obj_to_obj(obj, obj2)
}
...
func do_showoff(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if (func() *obj_data {
		...
		act(libc.CString("@WYou hold up $p@W for @C$N@W to see:@n"), 1, ch, obj, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@C$n@W holds up $p@W for you to see:@n"), 1, ch, obj, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("@C$n@W holds up $p@W for @c$N@W to see.@n"), 1, ch, obj, unsafe.Pointer(vict), TO_NOTVICT)
...
func introCreate(ch *char_data) {
	...
	if !get_filename(&fname[0], uint64(40), INTRO_FILE, GET_NAME(ch)) {
		return
	}
	if (func() *stdio.File {
...
func do_intro(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if readIntro(vict, ch) == 2 {
		...
		act(libc.CString("You decide to call $M, $N."), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("$n seems to decide something about you."), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("$n seems to decide something about $N."), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		return
	}
}
...
func map_draw_room(map_ [9][10]byte, x int, y int, rnum int, ch *char_data) {
	var door int
	for door = 0; door < NUM_OF_DIRS; door++ {
		if world[rnum].Dir_option[door] != nil && world[rnum].Dir_option[door].To_room != int(-1) && (EXIT_FLAGGED(world[rnum].Dir_option[door], 1<<1) && !EXIT_FLAGGED(world[rnum].Dir_option[door], 1<<4)) {
			switch door {
			case NORTH:
				map_[y-1][x] = '8'
			case EAST:
		...
		} else if world[rnum].Dir_option[door] != nil && world[rnum].Dir_option[door].To_room != int(-1) && !EXIT_FLAGGED(world[rnum].Dir_option[door], 1<<1) {
			switch door {
			case NORTH:
...
func gen_map(ch *char_data, num int) {
	...
	for door = 0; door < NUM_OF_DIRS; door++ {
		if (world[ch.In_room].Dir_option[door]) != nil && (world[ch.In_room].Dir_option[door]).To_room != int(-1) && !EXIT_FLAGGED(world[ch.In_room].Dir_option[door], 1<<1) {
			switch door {
			case NORTH:
...
func show_obj_to_char(obj *obj_data, ch *char_data, mode int) {
	...
	case SHOW_OBJ_LONG:
		...
		if OBJ_FLAGGED(obj, ITEM_BURIED) {
			...
			if spotted == 1 && SECT(obj.In_room) != SECT_DESERT {
				send_to_char(ch, libc.CString("@yA %s soft dirt is here.@n\r\n"), &bury[0])
				return
			} else if spotted == 1 && SECT(obj.In_room) == SECT_DESERT {
	...
	case SHOW_OBJ_ACTION:
		...
		case ITEM_NOTE:
			if obj.Action_description != nil {
				...
				page_string(ch.Desc, &notebuf[0], 1)
			} else {
...
func look_at_char(i *char_data, ch *char_data) {
	...
	send_to_char(ch, libc.CString("\r\n"))
	if i.Clan != nil && libc.StrStr(i.Clan, libc.CString("None")) == nil {
	...
	if found != 0 && (!IS_NPC(ch) && !PRF_FLAGGED(ch, PRF_NOEQSEE)) {
		...
		if !PLR_FLAGGED(i, PLR_DISGUISED) {
			act(libc.CString("$n is using:"), 0, i, nil, unsafe.Pointer(ch), TO_VICT)
		} else {
			act(libc.CString("The disguised person is using:"), 0, i, nil, unsafe.Pointer(ch), TO_VICT)
		}
	...
	if ch != i && (GET_SKILL(ch, SKILL_KEEN) != 0 && AFF_FLAGGED(ch, AFF_SNEAK) || ch.Admlevel != 0) {
		found = 0
		act(libc.CString("\r\nYou attempt to peek at $s inventory:"), 0, i, nil, unsafe.Pointer(ch), TO_VICT)
		if CAN_SEE(i, ch) {
			act(libc.CString("$n tries to evaluate what you have in your inventory."), 1, ch, nil, unsafe.Pointer(i), TO_VICT)
		}
		if GET_SKILL(ch, SKILL_KEEN) > axion_dice(0) && (!IS_NPC(i) || ch.Admlevel > 1) {
			...
			act(libc.CString("You are unsure about $s inventory."), 0, i, nil, unsafe.Pointer(ch), TO_VICT)
			if CAN_SEE(i, ch) {
				act(libc.CString("$n didn't seem to get a good enough look."), 1, ch, nil, unsafe.Pointer(i), TO_VICT)
			}
			improve_skill(ch, SKILL_KEEN, 1)
			return
		}
...
func list_one_char(i *char_data, ch *char_data) {
	...
	if IS_NPC(i) && i.Long_descr != nil && int(i.Position) == int(i.Mob_specials.Default_pos) && i.Fighting == nil {
		...
		if IS_NPC(i) && float64(i.Hit) >= float64(gear_pl(i))*0.9 && i.Hit != gear_pl(i) {
			act(libc.CString("@R...Some slight wounds on $s body.@w"), 1, i, nil, unsafe.Pointer(ch), TO_VICT)
		} else if IS_NPC(i) && float64(i.Hit) >= float64(gear_pl(i))*0.8 && float64(i.Hit) < float64(gear_pl(i))*0.9 {
			act(libc.CString("@R...A few wounds on $s body.@w"), 1, i, nil, unsafe.Pointer(ch), TO_VICT)
		} else if IS_NPC(i) && float64(i.Hit) >= float64(gear_pl(i))*0.7 && float64(i.Hit) < float64(gear_pl(i))*0.8 {
			act(libc.CString("@R...Many wounds on $s body.@w"), 1, i, nil, unsafe.Pointer(ch), TO_VICT)
		} else if IS_NPC(i) && float64(i.Hit) >= float64(gear_pl(i))*0.6 && float64(i.Hit) < float64(gear_pl(i))*0.7 {
			act(libc.CString("@R...Quite a few wounds on $s body.@w"), 1, i, nil, unsafe.Pointer(ch), TO_VICT)
		} else if IS_NPC(i) && float64(i.Hit) >= float64(gear_pl(i))*0.5 && float64(i.Hit) < float64(gear_pl(i))*0.6 {
			act(libc.CString("@R...Horrible wounds on $s body.@w"), 1, i, nil, unsafe.Pointer(ch), TO_VICT)
		} else if IS_NPC(i) && float64(i.Hit) >= float64(gear_pl(i))*0.4 && float64(i.Hit) < float64(gear_pl(i))*0.5 {
			act(libc.CString("@R...Blood is seeping from the wounds on $s body.@w"), 1, i, nil, unsafe.Pointer(ch), TO_VICT)
		} else if IS_NPC(i) && float64(i.Hit) >= float64(gear_pl(i))*0.3 && float64(i.Hit) < float64(gear_pl(i))*0.4 {
			act(libc.CString("@R...$s body is in terrible shape.@w"), 1, i, nil, unsafe.Pointer(ch), TO_VICT)
		} else if IS_NPC(i) && float64(i.Hit) >= float64(gear_pl(i))*0.2 && float64(i.Hit) < float64(gear_pl(i))*0.3 {
			act(libc.CString("@R...Is absolutely covered in wounds.@w"), 1, i, nil, unsafe.Pointer(ch), TO_VICT)
		} else if IS_NPC(i) && float64(i.Hit) >= float64(gear_pl(i))*0.1 && float64(i.Hit) < float64(gear_pl(i))*0.2 {
			act(libc.CString("@R...Is on $s last leg.@w"), 1, i, nil, unsafe.Pointer(ch), TO_VICT)
		} else if IS_NPC(i) && float64(i.Hit) < float64(gear_pl(i))*0.1 {
			act(libc.CString("@R...Should be DEAD soon.@w"), 1, i, nil, unsafe.Pointer(ch), TO_VICT)
		}
		if i.Listenroom > 0 {
			var eaves [300]byte
			stdio.Sprintf(&eaves[0], "@w...$e is spying on everything to the @c%s@w.", dirs[i.Eavesdir])
			act(&eaves[0], 1, i, nil, unsafe.Pointer(ch), TO_VICT)
		}
		if AFF_FLAGGED(i, AFF_FLYING) && i.Altitude == 1 {
			act(libc.CString("...$e is in the air!"), 0, i, nil, unsafe.Pointer(ch), TO_VICT)
		}
		if AFF_FLAGGED(i, AFF_FLYING) && i.Altitude == 2 {
			act(libc.CString("...$e is high in the air!"), 0, i, nil, unsafe.Pointer(ch), TO_VICT)
		}
		if AFF_FLAGGED(i, AFF_SANCTUARY) && GET_SKILL(i, SKILL_AQUA_BARRIER) == 0 {
			act(libc.CString("...$e has a barrier around $s body!"), 0, i, nil, unsafe.Pointer(ch), TO_VICT)
		}
		if AFF_FLAGGED(i, AFF_FIRESHIELD) {
			act(libc.CString("...$e has @rf@Rl@Ya@rm@Re@Ys@w around $s body!"), 0, i, nil, unsafe.Pointer(ch), TO_VICT)
		}
		if AFF_FLAGGED(i, AFF_SANCTUARY) && GET_SKILL(i, SKILL_AQUA_BARRIER) != 0 {
			act(libc.CString("...$e has a @Gbarrier@w of @cwater@w and @Cki@w around $s body!"), 0, i, nil, unsafe.Pointer(ch), TO_VICT)
		}
		if !IS_NPC(i) && PLR_FLAGGED(i, PLR_SPIRAL) {
			act(libc.CString("...$e is spinning in a vortex!"), 0, i, nil, unsafe.Pointer(ch), TO_VICT)
		}
		if i.Charge != 0 {
			act(libc.CString("...$e has a bright %s aura around $s body!"), 0, i, nil, unsafe.Pointer(ch), TO_VICT)
		}
		if AFF_FLAGGED(i, AFF_METAMORPH) {
			act(libc.CString("@w...$e has a dark, @rred@w aura and menacing presence."), 0, i, nil, unsafe.Pointer(ch), TO_VICT)
		}
		if AFF_FLAGGED(i, AFF_HAYASA) {
			act(libc.CString("@w...$e has a soft @cblue@w glow around $s body!"), 0, i, nil, unsafe.Pointer(ch), TO_VICT)
		}
		if AFF_FLAGGED(i, AFF_BLIND) {
			act(libc.CString("...$e is groping around blindly!"), 0, i, nil, unsafe.Pointer(ch), TO_VICT)
		}
		if affected_by_spell(i, SPELL_FAERIE_FIRE) {
			act(libc.CString("@m...$e @mis outlined with purple fire!@m"), 0, i, nil, unsafe.Pointer(ch), TO_VICT)
		}
		if i.Feature != nil {
			var woo [64936]byte
			stdio.Sprintf(&woo[0], "@C%s@n", i.Feature)
			act(&woo[0], 0, i, nil, unsafe.Pointer(ch), TO_VICT)
		}
		return
	}
	...
	if !IS_NPC(i) {
		if PLR_FLAGGED(i, PLR_FISHING) {
			act(libc.CString("@w...$e is @Cfishing@w.@n"), 1, i, nil, unsafe.Pointer(ch), TO_VICT)
		}
	}
	if PLR_FLAGGED(i, PLR_AURALIGHT) {
		var bloom [2048]byte
		stdio.Sprintf(&bloom[0], "...is surrounded by a bright %s aura.@n", aura_types[i.Aura])
		act(&bloom[0], 1, i, nil, unsafe.Pointer(ch), TO_VICT)
	}
	if AFF_FLAGGED(i, AFF_SANCTUARY) && GET_SKILL(i, SKILL_AQUA_BARRIER) == 0 {
		act(libc.CString("@w...$e has a @bbarrier@w around $s body!"), 1, i, nil, unsafe.Pointer(ch), TO_VICT)
	}
	if AFF_FLAGGED(i, AFF_FIRESHIELD) {
		act(libc.CString("@w...$e has @rf@Rl@Ya@rm@Re@Ys@w around $s body!"), 0, i, nil, unsafe.Pointer(ch), TO_VICT)
	}
	if AFF_FLAGGED(i, AFF_HEALGLOW) {
		act(libc.CString("@w...$e has a serene @Cblue@Y glow@w around $s body."), 1, i, nil, unsafe.Pointer(ch), TO_VICT)
	}
	if AFF_FLAGGED(i, AFF_EARMOR) {
		act(libc.CString("@w...$e has ghostly @Ggreen@w ethereal armor around $s body."), 1, i, nil, unsafe.Pointer(ch), TO_VICT)
	}
	if AFF_FLAGGED(i, AFF_SANCTUARY) && GET_SKILL(i, SKILL_AQUA_BARRIER) != 0 {
		act(libc.CString("@w...$e has a @bbarrier@w of @cwater@w and @CKi@w around $s body!"), 1, i, nil, unsafe.Pointer(ch), TO_VICT)
	}
	if AFF_FLAGGED(i, AFF_FLYING) && i.Altitude == 1 {
		act(libc.CString("@w...$e is in the air!"), 1, i, nil, unsafe.Pointer(ch), TO_VICT)
	}
	if AFF_FLAGGED(i, AFF_FLYING) && i.Altitude == 2 {
		act(libc.CString("@w...$e is high in the air!"), 1, i, nil, unsafe.Pointer(ch), TO_VICT)
	}
	if i.Kaioken > 0 {
		act(libc.CString("@w...@r$e has a red aura around $s body!"), 1, i, nil, unsafe.Pointer(ch), TO_VICT)
	}
	if !IS_NPC(i) && PLR_FLAGGED(i, PLR_SPIRAL) {
		act(libc.CString("@w...$e is spinning in a vortex!"), 0, i, nil, unsafe.Pointer(ch), TO_VICT)
	}
	if IS_TRANSFORMED(i) && int(i.Race) != RACE_ANDROID && int(i.Race) != RACE_SAIYAN && int(i.Race) != RACE_HALFBREED {
		act(libc.CString("@w...$e has energy crackling around $s body!"), 1, i, nil, unsafe.Pointer(ch), TO_VICT)
	}
	if i.Charge != 0 && int(i.Race) != RACE_SAIYAN && int(i.Race) != RACE_HALFBREED {
		...
		act(&aura[0], 1, i, nil, unsafe.Pointer(ch), TO_VICT)
	}
	if !PLR_FLAGGED(i, PLR_OOZARU) && i.Charge != 0 && IS_TRANSFORMED(i) && (int(i.Race) == RACE_SAIYAN || int(i.Race) == RACE_HALFBREED) {
		act(libc.CString("@w...$e has a @Ybright @Yg@yo@Yl@yd@Ye@yn@w aura around $s body!"), 1, i, nil, unsafe.Pointer(ch), TO_VICT)
	}
	if !PLR_FLAGGED(i, PLR_OOZARU) && i.Charge != 0 && !IS_TRANSFORMED(i) && (int(i.Race) == RACE_SAIYAN || int(i.Race) == RACE_HALFBREED) {
		var aura [2048]byte
		stdio.Sprintf(&aura[0], "@w...$e has a @Ybright@w %s aura around $s body!", aura_types[i.Aura])
	...
	if PLR_FLAGGED(i, PLR_OOZARU) && i.Charge != 0 && (int(i.Race) == RACE_SAIYAN || int(i.Race) == RACE_HALFBREED) {
		act(libc.CString("@w...$e is in the form of a @rgreat ape@w!"), 1, i, nil, unsafe.Pointer(ch), TO_VICT)
	}
	if (i.Genome[0]) == 11 {
		act(libc.CString("@w...$e has expanded $s body size@w!"), 1, i, nil, unsafe.Pointer(ch), TO_VICT)
	}
	if AFF_FLAGGED(i, AFF_HAYASA) {
		act(libc.CString("@w...$e has a soft @cblue@w glow around $s body!"), 0, i, nil, unsafe.Pointer(ch), TO_VICT)
	}
	if PLR_FLAGGED(i, PLR_OOZARU) && i.Charge == 0 && (int(i.Race) == RACE_SAIYAN || int(i.Race) == RACE_HALFBREED) {
		act(libc.CString("@w...$e has energy crackling around $s @rgreat ape@w body!"), 1, i, nil, unsafe.Pointer(ch), TO_VICT)
	}
	if i.Feature != nil {
		var woo [64936]byte
		stdio.Sprintf(&woo[0], "@C%s@n", i.Feature)
	...
	if i.Rdisplay != nil {
		if i.Rdisplay != libc.CString("Empty") {
			...
			act(&rdis[0], 0, i, nil, unsafe.Pointer(ch), TO_VICT)
		}
	}
}
		...
		if CAN_SEE(ch, i) {
			...
			if config_info.Play.Stack_mobs != 0 {
				for j = list; j != i; j = j.Next_in_room {
					if i.Nr == j.Nr && int(i.Position) == int(j.Position) && int(i.Affected_by[0]) == int(j.Affected_by[0]) && int(i.Affected_by[1]) == int(j.Affected_by[1]) && int(i.Affected_by[2]) == int(j.Affected_by[2]) && int(i.Affected_by[3]) == int(j.Affected_by[3]) && (i.Fighting == nil && j.Fighting == nil) && (i.Hit == gear_pl(i) && j.Hit == gear_pl(j)) && libc.StrCmp(GET_NAME(i), GET_NAME(j)) == 0 {
						for tmphide = hideinfo; tmphide != nil; tmphide = tmphide.Next {
							if tmphide.Hidden == j {
								break
							}
				...
				for j = i; j != nil; j = j.Next_in_room {
					if i.Nr == j.Nr && int(i.Position) == int(j.Position) && int(i.Affected_by[0]) == int(j.Affected_by[0]) && int(i.Affected_by[1]) == int(j.Affected_by[1]) && int(i.Affected_by[2]) == int(j.Affected_by[2]) && int(i.Affected_by[3]) == int(j.Affected_by[3]) && (i.Fighting == nil && j.Fighting == nil) && (i.Hit == i.Max_hit && j.Hit == j.Max_hit) && libc.StrCmp(GET_NAME(i), GET_NAME(j)) == 0 {
						for tmphide = hideinfo; tmphide != nil; tmphide = tmphide.Next {
							if tmphide.Hidden == j {
								break
							}
			...
			list_one_char(i, ch)
			send_to_char(ch, libc.CString("@n"))
		} else if room_is_dark(ch.In_room) && !CAN_SEE_IN_DARK(ch) && AFF_FLAGGED(i, AFF_INFRAVISION) {
			send_to_char(ch, libc.CString("@wYou see a pair of glowing red eyes looking your way.@n\r\n"))
		}
	}
}
func do_auto_exits(target_room int, ch *char_data, exit_mode int) {
	var (
		door       int
		door_found int = 0
		has_light  int = 0
	...
	if exit_mode == EXIT_NORMAL && space == 0 && ch.In_room == target_room {
		send_to_char(ch, libc.CString("@D------------------------------------------------------------------------@n\r\n"))
	...
	if exit_mode == EXIT_COMPLETE || exit_mode == EXIT_NORMAL && space == 0 && ch.In_room != target_room {
		...
		if room_is_dark(ch.In_room) && !CAN_SEE_IN_DARK(ch) && !PLR_FLAGGED(ch, PLR_AURALIGHT) {
		...
		for door = 0; door < NUM_OF_DIRS; door++ {
			if (world[target_room].Dir_option[door]) != nil && (world[target_room].Dir_option[door]).To_room != int(-1) {
					...
					if !IS_SET((world[target_room].Dir_option[door]).Exit_info, 1<<1) {
						...
						if door == 6 {
							stdio.Sprintf(&dlist1[0], "@c%-9s @D-@w %s\r\n", &blam[0], func() string {
								if room_is_dark((world[target_room].Dir_option[door]).To_room) && !CAN_SEE_IN_DARK(ch) && has_light == 0 {
		...
		if ch.Radar1 == int(libc.BoolToInt(GET_ROOM_VNUM(target_room))) && ch.Radar2 == int(libc.BoolToInt(GET_ROOM_VNUM(target_room))) && ch.Radar3 != int(libc.BoolToInt(GET_ROOM_VNUM(target_room))) {
			send_to_char(ch, libc.CString("@CTwo of your buoys are floating here.@n\r\n"))
		} else if ch.Radar1 == int(libc.BoolToInt(GET_ROOM_VNUM(target_room))) && ch.Radar2 != int(libc.BoolToInt(GET_ROOM_VNUM(target_room))) && ch.Radar3 == int(libc.BoolToInt(GET_ROOM_VNUM(target_room))) {
			send_to_char(ch, libc.CString("@CTwo of your buoys are floating here.@n\r\n"))
		} else if ch.Radar1 != int(libc.BoolToInt(GET_ROOM_VNUM(target_room))) && ch.Radar2 == int(libc.BoolToInt(GET_ROOM_VNUM(target_room))) && ch.Radar3 == int(libc.BoolToInt(GET_ROOM_VNUM(target_room))) {
			send_to_char(ch, libc.CString("@CTwo of your buoys are floating here.@n\r\n"))
		} else if ch.Radar1 == int(libc.BoolToInt(GET_ROOM_VNUM(target_room))) && ch.Radar2 == int(libc.BoolToInt(GET_ROOM_VNUM(target_room))) && ch.Radar3 == int(libc.BoolToInt(GET_ROOM_VNUM(target_room))) && target_room != 0 {
			send_to_char(ch, libc.CString("@CAll three of your buoys are floating here. Why?@n\r\n"))
		} else if ch.Radar1 == int(libc.BoolToInt(GET_ROOM_VNUM(target_room))) {
			send_to_char(ch, libc.CString("@CYour @cBuoy #1@C is floating here.@n\r\n"))
		} else if ch.Radar2 == int(libc.BoolToInt(GET_ROOM_VNUM(target_room))) {
			send_to_char(ch, libc.CString("@CYour @cBuoy #2@C is floating here.@n\r\n"))
		} else if ch.Radar3 == int(libc.BoolToInt(GET_ROOM_VNUM(target_room))) {
			send_to_char(ch, libc.CString("@CYour @cBuoy #3@C is floating here.@n\r\n"))
		}
	}
}
func do_auto_exits2(target_room int, ch *char_data) {
	var (
		door int
		slen int = 0
	)
...
func do_autoexit(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if (func() int {
		tp = search_block(&arg[0], &exitlevels[0], 0)
		return tp
	}()) == -1 {
...
func look_at_room(target_room int, ch *char_data, ignore_brief int) {
	var (
	...
	if room_is_dark(target_room) && !CAN_SEE_IN_DARK(ch) && !PLR_FLAGGED(ch, PLR_AURALIGHT) {
		send_to_char(ch, libc.CString("It is pitch black...\r\n"))
		return
...
func look_in_obj(ch *char_data, arg *byte) {
	...
	if *arg == 0 {
		...
		if (obj.Value[VAL_PORTAL_APPEAR]) < 0 {
			var portal_dest int = real_room(obj.Value[VAL_PORTAL_DEST])
			if portal_dest == int(-1) {
				send_to_char(ch, libc.CString("You see nothing but infinite darkness...\r\n"))
			} else if room_is_dark(portal_dest) && !CAN_SEE_IN_DARK(ch) && !PLR_FLAGGED(ch, PLR_AURALIGHT) {
				send_to_char(ch, libc.CString("You see nothing but infinite darkness...\r\n"))
			} else {
		...
		if OBJVAL_FLAGGED(obj, 1<<2) {
			...
			var vehicle_inside int = real_room(obj.Value[VAL_VEHICLE_ROOM])
			if vehicle_inside == int(-1) {
				send_to_char(ch, libc.CString("You cannot see inside that.\r\n"))
			} else if room_is_dark(vehicle_inside) && !CAN_SEE_IN_DARK(ch) && !PLR_FLAGGED(ch, PLR_AURALIGHT) {
				send_to_char(ch, libc.CString("It is pitch black...\r\n"))
			} else {
		...
		if OBJVAL_FLAGGED(obj, 1<<2) {
			...
			if int(obj.Type_flag) == ITEM_CONTAINER && (GET_OBJ_VNUM(obj) == 697 || GET_OBJ_VNUM(obj) == 698 || GET_OBJ_VNUM(obj) == 682 || GET_OBJ_VNUM(obj) == 683 || GET_OBJ_VNUM(obj) == 684) {
				act(libc.CString("$n looks in $p."), 1, ch, obj, nil, TO_ROOM)
			}
			switch bits {
			case (1 << 2):
		...
		if found_char != nil {
			...
			if ch != found_char {
				if !AFF_FLAGGED(ch, AFF_HIDE) {
					act(libc.CString("$n looks at you."), 1, ch, nil, unsafe.Pointer(found_char), TO_VICT)
					act(libc.CString("$n looks at $N."), 1, ch, nil, unsafe.Pointer(found_char), TO_NOTVICT)
				}
			}
			return
		}
...
func look_out_window(ch *char_data, arg *byte) {
	...
	if *arg != 0 {
		...
		for i = world[ch.In_room].Contents; i != nil; i = i.Next_content {
			if int(i.Type_flag) == ITEM_WINDOW && isname(libc.CString("window"), i.Name) {
				viewport = i
				continue
			}
		}
	...
	if viewport == nil {
		...
		if (viewport.Value[VAL_WINDOW_UNUSED1]) < 0 {
			if (viewport.Value[VAL_WINDOW_UNUSED4]) < 0 {
				...
				target_room = real_room(viewport.Value[VAL_WINDOW_UNUSED4])
			}
		} else {
			if (func() *obj_data {
		...
		if target_room == int(-1) {
			send_to_char(ch, libc.CString("You don't seem to be able to see outside.\r\n"))
		} else {
			if viewport.Action_description != nil {
				act(viewport.Action_description, 1, ch, viewport, nil, TO_CHAR)
			} else {
				act(libc.CString("$n looks out the window."), 1, ch, nil, nil, TO_ROOM)
			}
...
func do_rptrans(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	mudlog(NRM, int(MAX(ADMLVL_IMPL, int64(ch.Player_specials.Invis_level))), 1, libc.CString("EXCHANGE: %s gave %d RPP to user %s"), GET_NAME(ch), amt, &arg[0])
	save_char(ch)
}
func do_rpbank(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	mudlog(NRM, int(MAX(ADMLVL_IMMORT, int64(ch.Player_specials.Invis_level))), 1, libc.CString("RPP Bank: %s has put %d RPP into their bank"), GET_NAME(ch), amt)
}
func do_rbanktrans(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	mudlog(NRM, int(MAX(ADMLVL_IMPL, int64(ch.Player_specials.Invis_level))), 1, libc.CString("EXCHANGE: %s gave %d Banked RPP to user %s"), GET_NAME(ch), amt, &arg[0])
	save_char(ch)
}
...
func do_look(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if int(ch.Position) < POS_SLEEPING {
		...
		send_to_char(ch, libc.CString("You can't see a damned thing, your eyes are closed!\r\n"))
	} else if room_is_dark(ch.In_room) && !CAN_SEE_IN_DARK(ch) && !PLR_FLAGGED(ch, PLR_AURALIGHT) {
		send_to_char(ch, libc.CString("It is pitch black...\r\n"))
		...
		if arg[0] == 0 {
			if subcmd == SCMD_SEARCH {
				...
				if ch.Admlevel < 1 && !AFF_FLAGGED(ch, AFF_HIDE) {
					act(libc.CString("@w$n@w looks around the room.@n"), 1, ch, nil, nil, TO_ROOM)
				}
			}
		} else if is_abbrev(&arg[0], libc.CString("inside")) && (world[ch.In_room].Dir_option[INDIR]) != nil && arg2[0] == 0 {
			if subcmd == SCMD_SEARCH {
				search_in_direction(ch, INDIR)
			} else {
				look_in_direction(ch, INDIR)
			}
		} else if is_abbrev(&arg[0], libc.CString("inside")) && subcmd == SCMD_SEARCH && arg2[0] == 0 {
			search_in_direction(ch, INDIR)
		} else if is_abbrev(&arg[0], libc.CString("inside")) || is_abbrev(&arg[0], libc.CString("into")) || is_abbrev(&arg[0], libc.CString("onto")) {
			look_in_obj(ch, &arg2[0])
		} else if (is_abbrev(&arg[0], libc.CString("outside")) || is_abbrev(&arg[0], libc.CString("through")) || is_abbrev(&arg[0], libc.CString("thru"))) && subcmd == SCMD_LOOK && arg2[0] != 0 {
			look_out_window(ch, &arg2[0])
		} else if is_abbrev(&arg[0], libc.CString("outside")) && subcmd == SCMD_LOOK && (world[ch.In_room].Dir_option[OUTDIR]) == nil {
			look_out_window(ch, &arg2[0])
		} else if (func() int {
			look_type = search_block(&arg[0], &dirs[0], 0)
			return look_type
		}()) >= 0 || (func() int {
			look_type = search_block(&arg[0], &abbr_dirs[0], 0)
			return look_type
		}()) >= 0 {
			if subcmd == SCMD_SEARCH {
		...
		} else if is_abbrev(&arg[0], libc.CString("towards")) && ((func() int {
			look_type = search_block(&arg2[0], &dirs[0], 0)
			return look_type
		}()) >= 0 || (func() int {
			look_type = search_block(&arg2[0], &abbr_dirs[0], 0)
			return look_type
		}()) >= 0) {
			if subcmd == SCMD_SEARCH {
				search_in_direction(ch, look_type)
		...
		} else if is_abbrev(&arg[0], libc.CString("at")) {
			if subcmd == SCMD_SEARCH {
				send_to_char(ch, libc.CString("That is not a direction!\r\n"))
			} else {
				look_at_target(ch, &arg2[0], 0)
			}
		} else if is_abbrev(&arg[0], libc.CString("around")) {
			var (
				i     *extra_descr_data
				found int = 0
			)
...
func do_equipment(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	for i = 1; i < NUM_WEARS; i++ {
		if (ch.Equipment[i]) != nil {
			...
			if BODY_FLAGGED(ch, uint32(int32(i))) && i != WEAR_WIELD2 {
				send_to_char(ch, libc.CString("%s@wNothing.@n\r\n"), wear_where[i])
			} else if BODY_FLAGGED(ch, uint32(int32(i))) && (i == WEAR_WIELD2 && !PLR_FLAGGED(ch, PLR_THANDW)) {
				send_to_char(ch, libc.CString("%s@wNothing.@n\r\n"), wear_where[i])
			}
		}
	}
...
func do_help(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if (func() int {
		...
		if ch.Admlevel < 3 {
			mudlog(NRM, int(MAX(ADMLVL_IMPL, int64(ch.Player_specials.Invis_level))), 1, libc.CString("%s tried to get help on %s"), GET_NAME(ch), argument)
		}
...
func perform_mortal_where(ch *char_data, arg *byte) {
	...
	if *arg == 0 {
		...
		for d = descriptor_list; d != nil; d = d.Next {
			...
			if i.In_room == int(-1) || !CAN_SEE(ch, i) {
				continue
			}
...
func perform_immort_where(ch *char_data, arg *byte) {
	...
	if *arg == 0 {
		mudlog(NRM, int(MAX(ADMLVL_GRGOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("GODCMD: %s has checked where to check player locations"), GET_NAME(ch))
		send_to_char(ch, libc.CString("Players                  Vnum    Planet        Location\r\n-------                 ------   ----------    ----------------\r\n"))
		for d = descriptor_list; d != nil; d = d.Next {
			if IS_PLAYING(d) {
				if d.Character.In_room != int(-1) {
				...
				if i != nil && CAN_SEE(ch, i) && i.In_room != int(-1) {
					if d.Original != nil {
		...
		mudlog(NRM, int(MAX(ADMLVL_GRGOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("GODCMD: %s has checked where for the location of %s"), GET_NAME(ch), arg)
		for i = character_list; i != nil; i = i.Next {
			if CAN_SEE(ch, i) && i.In_room != int(-1) && isname(arg, i.Name) {
				found = 1
...
func do_color(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if (func() int {
		tp = search_block(&arg[0], &ctypes[0], 0)
		return tp
	}()) == -1 {
		send_to_char(ch, libc.CString("Usage: color [ off | on ]\r\n"))
		return
...
func do_history(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	one_argument(argument, &arg[0])
	type_ = search_block(&arg[0], &history_types[0], 0)
	if arg[0] == 0 || type_ < 0 {
		var i int
...
func do_scan(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	for i = 0; i < 10; i++ {
		if (world[ch.In_room].Dir_option[i]) != nil {
			if room_is_dark(ch.In_room) && ch.Admlevel < ADMLVL_IMMORT && !AFF_FLAGGED(ch, AFF_INFRAVISION) {
			...
			if CAN_GO(ch, i) {
				...
				if (world[(world[ch.In_room].Dir_option[i]).To_room].Dir_option[i]) != nil && (world[(world[ch.In_room].Dir_option[i]).To_room].Dir_option[i]).To_room != 0 {
					newroom = (world[(world[ch.In_room].Dir_option[i]).To_room].Dir_option[i]).To_room
					if newroom != int(-1) && !IS_SET((world[(world[ch.In_room].Dir_option[i]).To_room].Dir_option[i]).Exit_info, 1<<1) {
						if !room_is_dark((world[(world[ch.In_room].Dir_option[i]).To_room].Dir_option[i]).To_room) {
							send_to_char(ch, libc.CString("@w-----------------------------------------@n\r\n"))
...
func do_toplist(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	var count int = 0
	var x int = 0
	if !get_filename(&fname[0], uint64(40), INTRO_FILE, libc.CString("toplist")) {
	...
	if *argument == 0 {
		...
		if load_char(argument, victim) >= 0 {
			...
			if victim.Admlevel >= ADMLVL_IMMORT {
				...
				if clan == 1 && libc.StrStr(victim.Clan, libc.CString("Applying")) == nil {
...
func search_in_direction(ch *char_data, dir int) {
	...
	act(libc.CString("$n searches the area intently."), 1, ch, nil, nil, TO_ROOM)
	...
	if (world[ch.In_room].Dir_option[dir]) != nil {
		...
		} else if EXIT_FLAGGED(world[ch.In_room].Dir_option[dir], 1<<0) && EXIT_FLAGGED(world[ch.In_room].Dir_option[dir], 1<<4) && (world[ch.In_room].Dir_option[dir]).Keyword != nil && check == 1 {
