...
func delete_room(rnum room_rnum) int {
	...
	for {
		...
		for j = 0; j < NUM_OF_DIRS; j++ {
			if (world[i].Dir_option[j]) == nil {
				...
				(world[i].Dir_option[j]).To_room -= room_rnum(libc.BoolToInt((world[i].Dir_option[j]).To_room != room_rnum(-1)))
			} else if (world[i].Dir_option[j]).To_room == rnum {
	...
	for i = 0; i <= room_rnum(top_of_zone_table); i++ {
		for j = 0; int(zone_table[i].Cmd[j].Command) != 'S'; j++ {
			...
			case 'T':
				fallthrough
			case 'V':
				if zone_table[i].Cmd[j].Arg3 == vnum(rnum) {
					zone_table[i].Cmd[j].Command = '*'
				} else if zone_table[i].Cmd[j].Arg3 > vnum(rnum) {
					zone_table[i].Cmd[j].Arg3 -= vnum(libc.BoolToInt(zone_table[i].Cmd[j].Arg3 != vnum(-1)))
				}
			case 'D':
				fallthrough
			case 'R':
				if zone_table[i].Cmd[j].Arg1 == vnum(rnum) {
					zone_table[i].Cmd[j].Command = '*'
				} else if zone_table[i].Cmd[j].Arg1 > vnum(rnum) {
					zone_table[i].Cmd[j].Arg1 -= vnum(libc.BoolToInt(zone_table[i].Cmd[j].Arg1 != vnum(-1)))
				}
				fallthrough
			case 'G':
				fallthrough
	...
	top_of_world--
	// todo: figure this out
	//world = []room_data((*room_data)(libc.Realloc(unsafe.Pointer(&world[0]), int(top_of_world*room_rnum(unsafe.Sizeof(room_data{}))+1))))
	return TRUE
}
func save_rooms(zone_num zone_rnum) int {
	...
	if in_save_list(zone_table[zone_num].Number, SL_WLD) != 0 {
		remove_from_save_list(zone_table[zone_num].Number, SL_WLD)
		create_world_index(int(zone_table[zone_num].Number), libc.CString("wld"))
