...
func do_oasis_medit(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	d = ch.Desc
	if d.Olc != nil {
		mudlog(BRF, ADMLVL_IMMORT, 1, libc.CString("SYSERR: do_oasis_medit: Player already had olc structure."))
	...
	if save != 0 {
		...
		mudlog(CMP, int(MAX(ADMLVL_BUILDER, int64(ch.Player_specials.Invis_level))), 1, libc.CString("OLC: %s saves mobile info for zone %d."), GET_NAME(ch), zone_table[d.Olc.Zone_num].Number)
...
func medit_save_to_disk(foo int) {
...
func medit_save_internally(d *descriptor_data) {
	...
	i = int(libc.BoolToInt(real_mobile(d.Olc.Number) == int(-1)))
	if (func() int {
		new_rnum = add_mobile(d.Olc.Mob, d.Olc.Number)
		return new_rnum
	}()) == int(-1) {
	...
	for dsc = descriptor_list; dsc != nil; dsc = dsc.Next {
		if dsc.Connected == CON_SEDIT {
			dsc.Olc.Shop.Keeper += int(libc.BoolToInt(dsc.Olc.Shop.Keeper != int(-1) && dsc.Olc.Shop.Keeper >= new_rnum))
		} else if dsc.Connected == CON_MEDIT {
			dsc.Olc.Mob.Nr += int(libc.BoolToInt(dsc.Olc.Mob.Nr != int(-1) && dsc.Olc.Mob.Nr >= new_rnum))
		}
	}
	for dsc = descriptor_list; dsc != nil; dsc = dsc.Next {
		if dsc.Connected == CON_ZEDIT {
			for i = 0; int(dsc.Olc.Zone.Cmd[i].Command) != 'S'; i++ {
				if int(dsc.Olc.Zone.Cmd[i].Command) == 'M' {
					if dsc.Olc.Zone.Cmd[i].Arg1 >= new_rnum {
...
func medit_parse(d *descriptor_data, arg *byte) {
	...
	case MEDIT_CONFIRM_SAVESTRING:
		...
		case 'Y':
			...
			mudlog(CMP, int(MAX(ADMLVL_BUILDER, int64(d.Character.Player_specials.Invis_level))), 1, libc.CString("OLC: %s edits mob %d"), GET_NAME(d.Character), d.Olc.Number)
	...
	case MEDIT_D_DESC:
		cleanup_olc(d, CLEANUP_ALL)
		mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("SYSERR: OLC: medit_parse(): Reached D_DESC case!"))
		write_to_output(d, libc.CString("Oops...\r\n"))
	case MEDIT_NPC_FLAGS:
		if (func() int {
			...
			TOGGLE_BIT_AR(d.Olc.Mob.Act[:], uint32(int32(i-1)))
		}
		medit_disp_mob_flags(d)
		return
	...
	case MEDIT_AFF_FLAGS:
		if (func() int {
			...
			TOGGLE_BIT_AR(d.Olc.Mob.Affected_by[:], uint32(int32(i)))
		}
		REMOVE_BIT_AR(d.Olc.Mob.Affected_by[:], uint32(int32(int(AFF_CHARM|AFF_POISON)|AFF_GROUP|AFF_SLEEP)))
		medit_disp_aff_flags(d)
		return
	case MEDIT_SEX:
	...
	case MEDIT_COPY:
		if (func() int {
			i = real_mobile(libc.Atoi(libc.GoString(arg)))
			return i
		}()) != int(-1) {
	...
	default:
		cleanup_olc(d, CLEANUP_ALL)
		mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("SYSERR: OLC: medit_parse(): Reached default case!"))
		write_to_output(d, libc.CString("Oops...\r\n"))
	}
	d.Olc.Value = 1
	medit_disp_menu(d)
