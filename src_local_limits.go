...
func barrier_shed(ch *char_data) {
	...
	if ch.Barrier <= 0 {
		ch.Barrier = 0
		act(libc.CString("@cYour barrier disappears.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@c$n@c's barrier disappears.@n"), 1, ch, nil, nil, TO_ROOM)
	} else {
		act(libc.CString("@cYour barrier loses some energy.@n"), 1, ch, nil, nil, TO_CHAR)
		send_to_char(ch, libc.CString("@D[@C%s@D]@n\r\n"), add_commas(loss))
		act(libc.CString("@c$n@c's barrier sends some sparks into the air as it seems to get a bit weaker.@n"), 1, ch, nil, nil, TO_ROOM)
	}
...
func wearing_stardust(ch *char_data) bool {
	var (
		count int = 0
		i     int
	)
...
func update_flags(ch *char_data) {
	...
	if (ch.Bonuses[BONUS_LATE]) != 0 && int(ch.Position) == POS_SLEEPING && rand_number(1, 3) == 3 {
		if ch.Hit >= gear_pl(ch) && ch.Move >= ch.Max_move && ch.Mana >= ch.Max_mana {
			...
			act(libc.CString("$n wakes up."), 1, ch, nil, nil, TO_ROOM)
			ch.Position = POS_SITTING
		}
	}
	if AFF_FLAGGED(ch, AFF_KNOCKED) && ch.Fighting == nil {
		act(libc.CString("@W$n is no longer senseless, and wakes up.@n"), 0, ch, nil, nil, TO_ROOM)
	...
	if !IS_NPC(ch) && !PLR_FLAGGED(ch, PLR_STAIL) && !PLR_FLAGGED(ch, PLR_NOGROW) && (int(ch.Race) == RACE_SAIYAN || int(ch.Race) == RACE_HALFBREED) {
		...
		if ch.Tail_growth == 10 {
			...
			act(libc.CString("$n@w's tail grows back.@n"), 1, ch, nil, nil, TO_ROOM)
	...
	if AFF_FLAGGED(ch, AFF_FROZEN) && rand_number(1, 2) == 2 {
		...
		act(libc.CString("$n@W breaks out of the ice holding $m prisoner!"), 1, ch, nil, nil, TO_ROOM)
	...
	if AFF_FLAGGED(ch, AFF_WITHER) && rand_number(1, int(sick_fail+6)) == 2 {
		...
		act(libc.CString("$n@W's looks more fit now."), 1, ch, nil, nil, TO_ROOM)
		ch.Real_abils.Str += 3
		ch.Real_abils.Cha += 3
		save_char(ch)
		REMOVE_BIT_AR(ch.Affected_by[:], AFF_WITHER)
	}
	if wearing_stardust(ch) {
...
func gain_level(ch *char_data, whichclass int) {
	...
	if GET_LEVEL(ch) < 100 && ch.Exp >= int64(level_exp(ch, GET_LEVEL(ch)+1)) {
		...
		mudlog(BRF, int(MAX(ADMLVL_IMMORT, int64(ch.Player_specials.Invis_level))), 1, libc.CString("%s advanced level to level %d."), GET_NAME(ch), GET_LEVEL(ch))
		...
		if GET_LEVEL(ch) < 100 {
			if ch.Mindlink != nil && gain > 0 && ch.Linker == 0 {
				if GET_LEVEL(ch)+20 < GET_LEVEL(ch.Mindlink) || GET_LEVEL(ch)-20 > GET_LEVEL(ch.Mindlink) {
					...
					act(libc.CString("@GYou've absorbed some new experiences from @W$n@G!@n"), 0, ch, nil, unsafe.Pointer(ch.Mindlink), TO_VICT)
					...
					act(libc.CString("@RYou sense that @W$N@R has stolen some of your experiences with $S mind!@n"), 0, ch, nil, unsafe.Pointer(ch.Mindlink), TO_CHAR)
				}
			}
...
func gain_exp_regardless(ch *char_data, gain int) {
	...
	if !IS_NPC(ch) {
		...
		if is_altered != 0 {
			mudlog(BRF, int(MAX(ADMLVL_IMMORT, int64(ch.Player_specials.Invis_level))), 1, libc.CString("%s advanced %d level%s to level %d."), GET_NAME(ch), num_levels, func() string {
				if num_levels == 1 {
					return ""
				}
				return "s"
	...
	if PLR_FLAGGED(ch, PLR_WRITING) {
				...
				if int(ch.Player_specials.Conditions[condition])+value > 48 {
					...
					if condition != DRUNK && prior >= 48 && int(ch.Race) != RACE_MAJIN {
						...
						if pukeroll > int(ch.Aff_abils.Con)+19 {
							act(libc.CString("@r@6You retch violently until your stomach is empty! Your constitution couldn't handle being that stuffed!@n"), 1, ch, nil, nil, TO_CHAR)
							act(libc.CString("@m@6$n@r@6 retches violently! It seems $e stuffed $mself too much!@n"), 1, ch, nil, nil, TO_ROOM)
							...
							act(libc.CString("@r@6You puke violently! Your constitution couldn't handle being that stuffed!@n"), 1, ch, nil, nil, TO_CHAR)
							act(libc.CString("@m@6$n@r@6 pukes violently! It seems $e stuffed $mself too much!@n"), 1, ch, nil, nil, TO_ROOM)
							SET_BIT_AR(ch.Affected_by[:], AFF_PUKED)
							if int(ch.Race) != RACE_NAMEK {
							...
							act(libc.CString("@r@6You puke a little! Your constitution couldn't handle being that stuffed!@n"), 1, ch, nil, nil, TO_CHAR)
							act(libc.CString("@m@6$n@r@6 pukes a little! It seems $e stuffed $mself too much!@n"), 1, ch, nil, nil, TO_ROOM)
							SET_BIT_AR(ch.Affected_by[:], AFF_PUKED)
							if int(ch.Race) != RACE_NAMEK {
		...
		if !AFF_FLAGGED(ch, AFF_SPIRIT) && (GET_SKILL(ch, SKILL_SURVIVAL) == 0 || GET_SKILL(ch, SKILL_SURVIVAL) < rand_number(1, 140)) {
			...
			if ch.Hit <= 0 && int(ch.Player_specials.Conditions[HUNGER]) == 0 {
				...
				ch.Move = 0
				act(libc.CString("@W$n@W falls down dead before you...@n"), 0, ch, nil, nil, TO_ROOM)
				die(ch, nil)
...
func check_idling(ch *char_data) {
	...
	if func() int {
		...
		*p++
		...
		if ch.Was_in_room == int(-1) && ch.In_room != int(-1) {
			...
			if int(libc.BoolToInt(GET_ROOM_VNUM(ch.In_room))) >= 2002 && int(libc.BoolToInt(GET_ROOM_VNUM(ch.In_room))) <= 2011 {
				ch.Player_specials.Load_room = int(libc.BoolToInt(GET_ROOM_VNUM(real_room(1960))))
			}
			if int(libc.BoolToInt(GET_ROOM_VNUM(ch.In_room))) == 2069 {
				ch.Player_specials.Load_room = int(libc.BoolToInt(GET_ROOM_VNUM(real_room(2017))))
			}
			if int(libc.BoolToInt(GET_ROOM_VNUM(ch.In_room))) == 2070 {
				ch.Player_specials.Load_room = int(libc.BoolToInt(GET_ROOM_VNUM(real_room(2046))))
			}
			if int(libc.BoolToInt(GET_ROOM_VNUM(ch.In_room))) >= 101 && int(libc.BoolToInt(GET_ROOM_VNUM(ch.In_room))) <= 139 {
				if GET_LEVEL(ch) == 1 {
					ch.Player_specials.Load_room = int(libc.BoolToInt(GET_ROOM_VNUM(real_room(100))))
					ch.Exp = 0
				} else {
					if int(ch.Chclass) == CLASS_ROSHI {
						ch.Player_specials.Load_room = int(libc.BoolToInt(GET_ROOM_VNUM(real_room(1130))))
					}
					if int(ch.Chclass) == CLASS_KABITO {
						ch.Player_specials.Load_room = int(libc.BoolToInt(GET_ROOM_VNUM(real_room(0x2F42))))
					}
					if int(ch.Chclass) == CLASS_NAIL {
						ch.Player_specials.Load_room = int(libc.BoolToInt(GET_ROOM_VNUM(real_room(0x2DA3))))
					}
					if int(ch.Chclass) == CLASS_BARDOCK {
						ch.Player_specials.Load_room = int(libc.BoolToInt(GET_ROOM_VNUM(real_room(2268))))
					}
					if int(ch.Chclass) == CLASS_KRANE {
						ch.Player_specials.Load_room = int(libc.BoolToInt(GET_ROOM_VNUM(real_room(0x32D1))))
					}
					if int(ch.Chclass) == CLASS_TAPION {
						ch.Player_specials.Load_room = int(libc.BoolToInt(GET_ROOM_VNUM(real_room(8231))))
					}
					if int(ch.Chclass) == CLASS_PICCOLO {
						ch.Player_specials.Load_room = int(libc.BoolToInt(GET_ROOM_VNUM(real_room(1659))))
					}
					if int(ch.Chclass) == CLASS_ANDSIX {
						ch.Player_specials.Load_room = int(libc.BoolToInt(GET_ROOM_VNUM(real_room(1713))))
					}
					if int(ch.Chclass) == CLASS_DABURA {
						ch.Player_specials.Load_room = int(libc.BoolToInt(GET_ROOM_VNUM(real_room(6486))))
					}
					if int(ch.Chclass) == CLASS_FRIEZA {
						ch.Player_specials.Load_room = int(libc.BoolToInt(GET_ROOM_VNUM(real_room(4282))))
					}
					if int(ch.Chclass) == CLASS_GINYU {
						ch.Player_specials.Load_room = int(libc.BoolToInt(GET_ROOM_VNUM(real_room(4289))))
					}
				}
			}
			act(libc.CString("$n disappears into the void."), 1, ch, nil, nil, TO_ROOM)
			...
			Crash_rentsave(ch, 0)
			cp(ch)
			mudlog(CMP, ADMLVL_GOD, 1, libc.CString("%s force-rented and extracted (idle)."), GET_NAME(ch))
			extract_char(ch)
		}
	}
}
	...
	if healrate > 0 {
		if (ch.Limb_condition[0]) > 0 && (ch.Limb_condition[0]) < 50 {
			if (ch.Limb_condition[0])+healrate >= 50 {
				act(libc.CString("You realize your right arm is no longer broken."), 1, ch, nil, nil, TO_CHAR)
		...
		if (ch.Limb_condition[1]) > 0 && (ch.Limb_condition[1]) < 50 {
			if (ch.Limb_condition[1])+healrate >= 50 {
				act(libc.CString("You realize your left arm is no longer broken."), 1, ch, nil, nil, TO_CHAR)
		...
		if (ch.Limb_condition[2]) > 0 && (ch.Limb_condition[2]) < 50 {
			if (ch.Limb_condition[2])+healrate >= 50 {
				act(libc.CString("You realize your right leg is no longer broken."), 1, ch, nil, nil, TO_CHAR)
		...
		if (ch.Limb_condition[3]) > 0 && (ch.Limb_condition[3]) < 50 {
			if (ch.Limb_condition[3])+healrate >= 50 {
				act(libc.CString("You realize your left leg is no longer broken."), 1, ch, nil, nil, TO_CHAR)
	...
	for i = character_list; i != nil; i = next_char {
		...
		if int(i.Position) >= POS_STUNNED {
			...
			if i.Kaioken > 0 && (GET_SKILL(i, SKILL_KAIOKEN) < rand_number(1, x) || i.Move <= i.Max_move/10) {
				...
				act(libc.CString("$n loses focus and $s kaioken aura disappears."), 1, i, nil, nil, TO_ROOM)
			...
			if AFF_FLAGGED(i, AFF_BURNED) {
				if rand_number(1, 5) >= 4 {
					...
					act(libc.CString("$n@w's burns are now healed.@n"), 1, i, nil, nil, TO_ROOM)
			...
			if SECT(i.In_room) == SECT_WATER_NOSWIM && i.Player_specials.Carried_by == nil && int(i.Race) != RACE_KANASSAN {
				if i.Move >= int64(gear_weight(i)) {
					act(libc.CString("@bYou swim in place.@n"), 1, i, nil, nil, TO_CHAR)
					act(libc.CString("@C$n@b swims in place.@n"), 1, i, nil, nil, TO_ROOM)
					...
					act(libc.CString("@RYou are drowning!@n"), 1, i, nil, nil, TO_CHAR)
					act(libc.CString("@C$n@b gulps water as $e struggles to stay above the water line.@n"), 1, i, nil, nil, TO_ROOM)
					if i.Hit-gear_pl(i)/3 <= 0 {
						act(libc.CString("@rYou drown!@n"), 1, i, nil, nil, TO_CHAR)
						act(libc.CString("@R$n@r drowns!@n"), 1, i, nil, nil, TO_ROOM)
						die(i, nil)
						i.Hit = 1
					} else {
						i.Hit -= gear_pl(i) / 3
			...
			if !has_o2(i) && SUNKEN(i.In_room) && !ROOM_FLAGGED(i.In_room, ROOM_SPACE) {
				if (i.Mana - mana_gain(i)) > i.Max_mana/200 {
					...
					if i.Suppressed > 0 && float64(i.Suppressed) > float64(gear_pl(i))*0.05 {
						...
						i.Hit = 1
						act(libc.CString("@W$n@W drowns right in front of you.@n"), 0, i, nil, nil, TO_ROOM)
						die(i, nil)
					}
				}
			}
			if !has_o2(i) && ROOM_FLAGGED(i.In_room, ROOM_SPACE) {
			...
			if !AFF_FLAGGED(i, AFF_FLYING) && world[i.In_room].Geffect == 6 && !MOB_FLAGGED(i, MOB_NOKILL) && int(i.Race) != RACE_DEMON {
				act(libc.CString("@rYour legs are burned by the lava!@n"), 1, i, nil, nil, TO_CHAR)
				act(libc.CString("@R$n@r's legs are burned by the lava!@n"), 1, i, nil, nil, TO_ROOM)
				...
				if float64(i.Suppressed) > float64(gear_pl(i))*0.05 {
					...
					if i.Hit < 0 {
						act(libc.CString("@rYou have burned to death!@n"), 1, i, nil, nil, TO_CHAR)
						act(libc.CString("@R$n@r has burned to death!@n"), 1, i, nil, nil, TO_ROOM)
						die(i, nil)
					}
				}
			}
			if change == 1 && !AFF_FLAGGED(i, AFF_POISON) {
				if PLR_FLAGGED(i, PLR_HEALT) && i.Sits != nil {
					...
					if i.Sits.Healcharge == 0 {
						...
						act(libc.CString("You step out of the now empty healing tank."), 1, i, nil, nil, TO_CHAR)
						act(libc.CString("@C$n@w steps out of the now empty healing tank.@n"), 1, i, nil, nil, TO_ROOM)
			...
			if AFF_FLAGGED(i, AFF_POISON) {
				...
				if float64(i.Hit)-float64(i.Max_hit)*cost > 0 {
					...
					act(libc.CString("$n shivers and then pukes."), 1, i, nil, nil, TO_ROOM)
					i.Hit -= int64(float64(i.Max_hit) * cost)
				} else {
					send_to_char(i, libc.CString("The poison claims your life!\r\n"))
					act(libc.CString("$n pukes up blood and falls down dead!"), 1, i, nil, nil, TO_ROOM)
					if i.Poisonby != nil {
	...
	for j = object_list; j != nil; j = next_thing {
		...
		if IS_CORPSE(j) {
			...
			if libc.StrStr(j.Name, libc.CString("android")) == nil && libc.StrStr(j.Name, libc.CString("Android")) == nil && !OBJ_FLAGGED(j, ITEM_BURIED) {
				if j.Timer == 5 {
					if j.In_room != int(-1) && world[j.In_room].People != nil {
						act(libc.CString("@DFlies start to gather around $p@D.@n"), 1, world[j.In_room].People, j, nil, TO_CHAR)
						act(libc.CString("@DFlies start to gather around $p@D.@n"), 1, world[j.In_room].People, j, nil, TO_ROOM)
					}
				}
				if j.Timer == 3 {
					if j.In_room != int(-1) && world[j.In_room].People != nil {
						act(libc.CString("@DA cloud of flies has formed over $p@D.@n"), 1, world[j.In_room].People, j, nil, TO_CHAR)
						act(libc.CString("@DA cloud of flies has formed over $p@D.@n"), 1, world[j.In_room].People, j, nil, TO_ROOM)
					}
				}
				if j.Timer == 2 {
					if j.In_room != int(-1) && world[j.In_room].People != nil {
						act(libc.CString("@DMaggots can be seen crawling all over $p@D.@n"), 1, world[j.In_room].People, j, nil, TO_CHAR)
						act(libc.CString("@DMaggots can be seen crawling all over $p@D.@n"), 1, world[j.In_room].People, j, nil, TO_ROOM)
					}
				}
				if j.Timer == 1 {
					if j.In_room != int(-1) && world[j.In_room].People != nil {
						act(libc.CString("@DMaggots have nearly stripped $p of all its flesh@D.@n"), 1, world[j.In_room].People, j, nil, TO_CHAR)
						act(libc.CString("@DMaggots have nearly stripped $p of all its flesh@D.@n"), 1, world[j.In_room].People, j, nil, TO_ROOM)
					}
				}
			}
			if j.Timer == 0 {
				if j.Carried_by != nil {
					if libc.StrStr(j.Name, libc.CString("android")) == nil {
						act(libc.CString("$p decays in your hands."), 0, j.Carried_by, j, nil, TO_CHAR)
						if j.In_room != int(-1) && world[j.In_room].People != nil {
							act(libc.CString("A quivering horde of maggots consumes $p."), 1, world[j.In_room].People, j, nil, TO_ROOM)
							act(libc.CString("A quivering horde of maggots consumes $p."), 1, world[j.In_room].People, j, nil, TO_CHAR)
						}
					} else {
						act(libc.CString("$p decays in your hands."), 0, j.Carried_by, j, nil, TO_CHAR)
						if j.In_room != int(-1) && world[j.In_room].People != nil {
							act(libc.CString("$p breaks down completely into a pile of junk."), 1, world[j.In_room].People, j, nil, TO_ROOM)
							act(libc.CString("$p breaks down completely into a pile of junk."), 1, world[j.In_room].People, j, nil, TO_CHAR)
						}
					}
				}
		...
		if int(j.Type_flag) == ITEM_PORTAL {
			...
			if j.Timer == 0 {
				act(libc.CString("A glowing portal fades from existence."), 1, world[j.In_room].People, j, nil, TO_ROOM)
				act(libc.CString("A glowing portal fades from existence."), 1, world[j.In_room].People, j, nil, TO_CHAR)
				extract_obj(j)
			}
			...
			if j.Timer == 0 {
				act(libc.CString("The $p@n settles to the ground and goes out."), 1, world[j.In_room].People, j, nil, TO_ROOM)
				act(libc.CString("A $p@n settles to the ground and goes out."), 1, world[j.In_room].People, j, nil, TO_CHAR)
				extract_obj(j)
			}
