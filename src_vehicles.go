...
func drive_into_vehicle(ch *char_data, vehicle *obj_data, arg *byte) {
	...
	if *arg == 0 {
		...
		is_going_to = real_room(vehicle_in_out.Value[0])
...
func drive_outof_vehicle(ch *char_data, vehicle *obj_data) {
	...
	if (func() *obj_data {
		...
		if ch.Desc != nil {
			act(libc.CString("@wThe @De@Wn@wg@Di@wn@We@Ds@w of the ship @rr@Ro@ra@Rr@w as it moves."), 1, ch, nil, nil, TO_ROOM)
		}
		send_to_char(ch, libc.CString("@wThe ship flies onward:\r\n"))
		look_at_room(vehicle.In_room, ch, 0)
		var door int
...
func drive_in_direction(ch *char_data, vehicle *obj_data, dir int) {
	var buf [2048]byte
	if (world[vehicle.In_room].Dir_option[dir]) == nil || (world[vehicle.In_room].Dir_option[dir]).To_room == int(-1) {
		...
		for hatch = world[real_room(vehicle.Value[0])].Contents; hatch != nil; hatch = hatch.Next_content {
...
func do_warp(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if (func() *obj_data {
		...
		send_to_char(ch, libc.CString("Syntax: shipwarp [ earth | vegeta | namek | konack | aether | frigid | buoy1 | buoy2 | buoy3 ]\r\n"))
		return
	} else if ROOM_FLAGGED(int(libc.BoolToInt(vehicle.In_room == 0)), ROOM_SPACE) {
		...
		if libc.StrCaseCmp(&arg[0], libc.CString("earth")) == 0 {
			if int(libc.BoolToInt(GET_ROOM_VNUM(vehicle.In_room))) == 0xA013 || int(libc.BoolToInt(GET_ROOM_VNUM(vehicle.In_room))) == 50 {
				...
				act(libc.CString("@BA glow of blue light floods in through the window for an instant. You feel a strange shift as the light disappears and you find your ship in a new location!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@BA glow of blue light floods in through the window for an instant. You feel a strange shift as the light disappears and you find the ship in a new location!@n"), 1, ch, nil, nil, TO_ROOM)
			...
			if ch.Radar1 <= 0 {
				...
				return
			} else if int(libc.BoolToInt(GET_ROOM_VNUM(vehicle.In_room))) == ch.Radar1 {
				send_to_char(ch, libc.CString("Your ship is already there!\r\n"))
				return
			} else {
				act(libc.CString("@BA glow of blue light floods in through the window for an instant. You feel a strange shift as the light disappears and you find your ship in a new location!@n"), 1, ch, nil, nil, TO_CHAR)
			...
			if ch.Radar2 <= 0 {
				send_to_char(ch, libc.CString("You have not launched that buoy!\r\n"))
				return
			} else if int(libc.BoolToInt(GET_ROOM_VNUM(vehicle.In_room))) == ch.Radar2 {
				send_to_char(ch, libc.CString("Your ship is already there!\r\n"))
				return
			} else {
				act(libc.CString("@BA glow of blue light floods in through the window for an instant. You feel a strange shift as the light disappears and you find your ship in a new location!@n"), 1, ch, nil, nil, TO_CHAR)
			...
			if ch.Radar3 <= 0 {
				send_to_char(ch, libc.CString("You have not launched that buoy!\r\n"))
				return
			} else if int(libc.BoolToInt(GET_ROOM_VNUM(vehicle.In_room))) == ch.Radar3 {
				send_to_char(ch, libc.CString("Your ship is already there!\r\n"))
				return
			} else {
				act(libc.CString("@BA glow of blue light floods in through the window for an instant. You feel a strange shift as the light disappears and you find your ship in a new location!@n"), 1, ch, nil, nil, TO_CHAR)
...
func do_drive(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if libc.StrCaseCmp(&arg3[0], libc.CString("unready")) == 0 && !IS_NPC(ch) {
		if !PLR_FLAGGED(ch, PLR_PILOTING) {
			...
			act(libc.CString("@w$n stands up and stops piloting the ship."), 1, ch, nil, nil, TO_ROOM)
	...
	if confirmed == 1 {
		...
		act(libc.CString("@w$n sits down and begins piloting the ship."), 1, ch, nil, nil, TO_ROOM)
	...
	} else if room_is_dark(ch.In_room) && !CAN_SEE_IN_DARK(ch) {
		...
		act(libc.CString("@wYou are zapped by $p@w and instantly step away from it."), 0, ch, controls, nil, TO_CHAR)
		act(libc.CString("@w$n@w is zapped by $p@w and instantly steps away from it."), 0, ch, controls, nil, TO_ROOM)
	} else if (func() *obj_data {
		vehicle = find_vehicle_by_vnum(controls.Value[0])
		return vehicle
	}()) == nil {
		...
		if arg[0] == 0 {
		...
		} else if is_abbrev(&arg[0], libc.CString("into")) || is_abbrev(&arg[0], libc.CString("onto")) {
			drive_into_vehicle(ch, vehicle, &arg2[0])
		} else if is_abbrev(&arg[0], libc.CString("out")) && (world[vehicle.In_room].Dir_option[OUTDIR]) == nil {
			...
			if libc.StrCaseCmp(&arg[0], libc.CString("north")) == 0 || libc.StrCaseCmp(&arg[0], libc.CString("n")) == 0 {
				...
				var buf3 [2048]byte
				buf3[0] = '\x00'
				act(libc.CString("@wYou set the controls to descend.@n"), 0, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n @wmanipulates the ship controls.@n"), 0, ch, nil, nil, TO_ROOM)
				act(libc.CString("@RThe ship rocks and shakes as it descends through the atmosphere!@n"), 0, ch, nil, nil, TO_CHAR)
				act(libc.CString("@RThe ship rocks and shakes as it descends through the atmosphere!@n"), 0, ch, nil, nil, TO_ROOM)
				if land_location <= 50 {
					act(libc.CString("@wThe ship has landed.@n"), 0, ch, nil, nil, TO_CHAR)
					act(libc.CString("@wThe ship has landed.@n"), 0, ch, nil, nil, TO_ROOM)
				}
				if land_location > 50 {
					act(libc.CString("@wThe ship slams into the ground and forms a small crater!@n"), 0, ch, nil, nil, TO_CHAR)
					act(libc.CString("@wThe ship slams into the ground and forms a small crater!@n"), 0, ch, nil, nil, TO_ROOM)
					obj_from_room(vehicle)
					...
					if libc.StrCaseCmp(&arg2[0], libc.CString("1")) == 0 {
						...
						act(libc.CString("@wLanding sequence aborted, improper coordinates.@n"), 0, ch, nil, nil, TO_CHAR)
						act(libc.CString("@wLanding sequence aborted, improper coordinates.@n"), 0, ch, nil, nil, TO_ROOM)
					}
				...
				act(libc.CString("@wYou set the controls to launch.@n"), 0, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n @wmanipulates the ship controls.@n"), 0, ch, nil, nil, TO_ROOM)
				act(libc.CString("@RThe ship shudders as it launches up into the sky!@n"), 0, ch, nil, nil, TO_CHAR)
				act(libc.CString("@RThe ship shudders as it launches up into the sky!@n"), 0, ch, nil, nil, TO_ROOM)
				act(libc.CString("@wThe ship has reached low orbit.@n"), 0, ch, nil, nil, TO_CHAR)
				act(libc.CString("@wThe ship has reached low orbit.@n"), 0, ch, nil, nil, TO_ROOM)
				...
				if ch.Radar2 > 0 && libc.StrCaseCmp(&arg2[0], libc.CString("2")) == 0 {
					...
					act(libc.CString("@wYou enter a unique code and launch a marker buoy.@n\r\n"), 0, ch, nil, nil, TO_CHAR)
					act(libc.CString("@C$n@w manipulates the ship controls.@n\r\n"), 0, ch, nil, nil, TO_ROOM)
					ch.Radar2 = int(libc.BoolToInt(GET_ROOM_VNUM(vehicle.In_room)))
				}
				if ch.Radar3 > 0 && libc.StrCaseCmp(&arg2[0], libc.CString("3")) == 0 {
					...
					act(libc.CString("@wYou enter a unique code and launch a marker buoy.@n\r\n"), 0, ch, nil, nil, TO_CHAR)
					act(libc.CString("@C$n@w manipulates the ship controls.@n\r\n"), 0, ch, nil, nil, TO_ROOM)
					ch.Radar3 = int(libc.BoolToInt(GET_ROOM_VNUM(vehicle.In_room)))
				}
