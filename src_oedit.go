...
func do_oasis_oedit(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	d = ch.Desc
	if d.Olc != nil {
		mudlog(BRF, ADMLVL_IMMORT, 1, libc.CString("SYSERR: do_oasis: Player already had olc structure."))
	...
	if save != 0 {
		...
		mudlog(CMP, int(MAX(ADMLVL_BUILDER, int64(ch.Player_specials.Invis_level))), 1, libc.CString("OLC: %s saves object info for zone %d."), GET_NAME(ch), zone_table[d.Olc.Zone_num].Number)
...
func oedit_save_internally(d *descriptor_data) {
	...
	i = int(libc.BoolToInt(real_object(d.Olc.Number) == int(-1)))
	if (func() int {
		robj_num = add_object(d.Olc.Obj, d.Olc.Number)
		return robj_num
	}()) == int(-1) {
	...
	for dsc = descriptor_list; dsc != nil; dsc = dsc.Next {
		if dsc.Connected == CON_SEDIT {
			for i = 0; (dsc.Olc.Shop.Producing[i]) != int(-1); i++ {
				if (dsc.Olc.Shop.Producing[i]) >= robj_num {
	...
	for dsc = descriptor_list; dsc != nil; dsc = dsc.Next {
		if dsc.Connected == CON_ZEDIT {
			for i = 0; int(dsc.Olc.Zone.Cmd[i].Command) != 'S'; i++ {
				...
				case 'P':
					dsc.Olc.Zone.Cmd[i].Arg3 += int(libc.BoolToInt(dsc.Olc.Zone.Cmd[i].Arg3 >= robj_num))
					fallthrough
				case 'E':
					fallthrough
				case 'G':
					fallthrough
				case 'O':
					dsc.Olc.Zone.Cmd[i].Arg1 += int(libc.BoolToInt(dsc.Olc.Zone.Cmd[i].Arg1 >= robj_num))
				case 'R':
					dsc.Olc.Zone.Cmd[i].Arg2 += int(libc.BoolToInt(dsc.Olc.Zone.Cmd[i].Arg2 >= robj_num))
				default:
				}
			}
		}
...
func oedit_disp_container_flags_menu(d *descriptor_data) {
	var bits [64936]byte
	clear_screen(d)
	sprintbit(uint32(int32(d.Olc.Obj.Value[1])), container_bits[:], &bits[0], uint64(64936))
...
func oedit_disp_spells_menu(d *descriptor_data) {
	...
	clear_screen(d)
	for counter = 0; counter < SKILL_TABLE_SIZE; counter++ {
		if IS_SET(uint32(int32(skill_type(counter))), 1<<0) {
...
func oedit_disp_menu(d *descriptor_data) {
	...
	write_to_output(d, libc.CString("@g7@n) Wear flags  : @c%s@n\r\n@g8@n) Weight      : @c%-4lld@n, \t@g9@n) Cost        : @c%-4d@n\r\n@gA@n) Cost/Day    : @c%-4d@n, \t@gB@n) Timer       : @c%-4d@n\r\n@gC@n) Values      : @c%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d@n\r\n@gD@n) Applies menu@n\r\n@gE@n) Extra descriptions menu %s\r\n@gM@n) Min Level   : @c%d@n\r\n@gN@n) Material    : @c%s@n\r\n@gP@n) Perm Affects: @c%s@n\r\n@gS@n) Script      : @c%s@n\r\n@gT@n) Spellbook menu\r\n@gW@n) Copy object        ,\t@gX@n) Delete object\r\n@gY@n) Size        : @c%s@n\r\n@gZ@n) Wiznet      :\r\n@gQ@n) Quit\r\nEnter choice : "), &tbitbuf[0], obj.Weight, obj.Cost, obj.Cost_per_day, obj.Timer, obj.Value[0], obj.Value[1], obj.Value[2], obj.Value[3], obj.Value[4], obj.Value[5], obj.Value[6], obj.Value[7], obj.Value[8], obj.Value[9], obj.Value[10], obj.Value[11], obj.Value[12], obj.Value[13], obj.Value[14], obj.Value[15], func() string {
		...
		if obj.Extra_flags[0] > 0 || obj.Extra_flags[1] > 0 || obj.Extra_flags[2] > 0 || obj.Extra_flags[3] > 0 {
			return "Set."
		}
		return "Not Set."
...
func oedit_parse(d *descriptor_data, arg *byte) {
	...
	case OEDIT_CONFIRM_SAVESTRING:
		...
		case 'Y':
			...
			mudlog(CMP, int(MAX(ADMLVL_BUILDER, int64(d.Character.Player_specials.Invis_level))), 1, libc.CString("OLC: %s edits obj %d"), GET_NAME(d.Character), d.Olc.Number)
			if config_info.Operation.Auto_save_olc != 0 {
				oedit_save_to_disk(real_zone_by_thing(d.Olc.Number))
	...
	case OEDIT_COPY:
		if (func() int {
			number = real_object(libc.Atoi(libc.GoString(arg)))
			return number
		}()) != int(-1) {
	...
	case OEDIT_DELETE:
		if *arg == 'y' || *arg == 'Y' {
			if delete_object(d.Olc.Obj.Item_number) != int(-1) {
		...
		case 'Q':
			if d.Connected != CON_IEDIT {
				...
				*obj = *d.Olc.Obj
				...
				mudlog(CMP, int(MAX(ADMLVL_BUILDER, int64(d.Character.Player_specials.Invis_level))), 1, libc.CString("OLC: %s iedit a unique #%d"), GET_NAME(d.Character), GET_OBJ_VNUM(obj))
				if d.Character != nil {
	...
	case OEDIT_EXTRAS:
		...
		if number < 0 || number > NUM_ITEM_FLAGS {
			...
			TOGGLE_BIT_AR(d.Olc.Obj.Extra_flags[:], uint32(int32(number-1)))
			oedit_disp_extra_menu(d)
			return
		}
		fallthrough
	case OEDIT_WEAR:
		...
		if number < 0 || number > NUM_ITEM_WEARS {
			...
			TOGGLE_BIT_AR(d.Olc.Obj.Wear_flags[:], uint32(int32(number-1)))
			oedit_disp_wear_menu(d)
			return
		}
		fallthrough
	...
	case OEDIT_PERM:
		...
		if number > 0 && number <= NUM_AFF_FLAGS {
			if number != AFF_CHARM {
				TOGGLE_BIT_AR(d.Olc.Obj.Bitvector[:], uint32(int32(number)))
			}
		}
		oedit_disp_perm_menu(d)
		return
	...
	case OEDIT_SPELLBOOK:
		...
		return
	default:
		mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("SYSERR: OLC: Reached default case in oedit_parse()!"))
...
func do_iedit(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	ch.Desc.Olc.Value = 0
	act(libc.CString("$n starts using OLC."), 1, ch, nil, nil, TO_ROOM)
