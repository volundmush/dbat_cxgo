...
func do_oasis_hedit(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if libc.StrCaseCmp(libc.CString("save"), &arg[0]) == 0 {
		mudlog(CMP, int(MAX(ADMLVL_BUILDER, int64(ch.Player_specials.Invis_level))), 1, libc.CString("OLC: %s saves help files."), GET_NAME(ch))
	...
	d.Olc.Zone_num = search_help(d.Olc.Storage, ADMLVL_IMPL)
	...
	mudlog(CMP, ADMLVL_IMMORT, 1, libc.CString("OLC: %s starts editing help files."), GET_NAME(ch))
}
...
func hedit_save_internally(d *descriptor_data) {
	...
	if d.Olc.Zone_num == int(-1) {
		...
		for i = 0; i < top_of_helpt; i++ {
			*(*help_index_element)(unsafe.Add(unsafe.Pointer(new_help_table), unsafe.Sizeof(help_index_element{})*uintptr(i))) = help_table[i]
		}
		*(*help_index_element)(unsafe.Add(unsafe.Pointer(new_help_table), unsafe.Sizeof(help_index_element{})*uintptr(func() int {
		...
		// todo: fix this
		//help_table = []help_index_element(new_help_table)
	} else {
...
func hedit_parse(d *descriptor_data, arg *byte) {
	...
	case HEDIT_CONFIRM_EDIT:
		...
		case 'y':
			fallthrough
		case 'Y':
			hedit_setup_existing(d, d.Olc.Zone_num)
		case 'q':
			fallthrough
		case 'Q':
			cleanup_olc(d, CLEANUP_ALL)
		...
		case 'N':
			d.Olc.Zone_num++
			for ; d.Olc.Zone_num < top_of_helpt; d.Olc.Zone_num++ {
				if is_abbrev(d.Olc.Storage, help_table[d.Olc.Zone_num].Keywords) {
					break
				} else {
					d.Olc.Zone_num = top_of_helpt + 1
				}
			}
			if d.Olc.Zone_num > top_of_helpt {
	...
	case HEDIT_ENTRY:
		mudlog(1, ADMLVL_BUILDER, BRF, libc.CString("SYSERR: Reached HEDIT_ENTRY case in parse_hedit"))
	case HEDIT_MIN_LEVEL:
		...
		return
	default:
		mudlog(1, ADMLVL_BUILDER, BRF, libc.CString("SYSERR: Reached default case in parse_hedit"))
	}
	if change == 1 {
		d.Olc.Value = 1
		hedit_disp_menu(d)
...
func do_hindex(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	for i = 0; i < top_of_helpt; i++ {
		num++
		if is_abbrev(argument, help_table[i].Keywords) && ch.Admlevel >= help_table[i].Min_level {
