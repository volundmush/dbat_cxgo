...
func news_string_cleanup(d *descriptor_data, terminator int) {
	...
	case STRINGADD_SAVE:
		if (func() *stdio.File {
			...
			mudlog(CMP, ADMLVL_IMPL, 1, libc.CString("SYSERR: Can't write file '%s'."), storage)
		}
		if *d.Str == nil {
			mudlog(CMP, ADMLVL_IMPL, 1, libc.CString("SYSERR: Can't write file '%s'."), storage)
		} else {
	...
	case STRINGADD_ABORT:
		...
		act(libc.CString("$n stops editing the news."), 1, d.Character, nil, nil, TO_ROOM)
	default:
...
func tedit_string_cleanup(d *descriptor_data, terminator int) {
	...
	case STRINGADD_SAVE:
		if (func() *stdio.File {
			...
			fl.Close()
			mudlog(CMP, ADMLVL_GOD, 1, libc.CString("OLC: %s saves '%s'."), GET_NAME(d.Character), storage)
	...
	case STRINGADD_ABORT:
		write_to_output(d, libc.CString("Edit aborted.\r\n"))
		act(libc.CString("$n stops editing some scrolls."), 1, d.Character, nil, nil, TO_ROOM)
	default:
...
func do_tedit(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if ch.Desc.Olc != nil {
		mudlog(BRF, ADMLVL_IMMORT, 1, libc.CString("SYSERR: do_tedit: Player already had olc structure."))
	...
	act(libc.CString("$n begins editing a text file."), 1, ch, nil, nil, TO_ROOM)
