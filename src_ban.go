...
func isbanned(hostname *byte) bool {
	var (
		i           int
...
func do_ban(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	ban_list = ban_node
	mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("%s has banned %s for %s players."), GET_NAME(ch), &site[0], ban_types[ban_node.Type])
...
func do_unban(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("%s removed the %s-player ban on %s."), GET_NAME(ch), ban_types[ban_node.Type], &ban_node.Site[0])
...
func Valid_Name(newname *byte) bool {
	var (
		i        int
		wovels   int = 0
		dt       *descriptor_data
