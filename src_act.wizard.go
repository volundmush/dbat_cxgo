...
func do_newsedit(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	*((*byte)(unsafe.Add(unsafe.Pointer((*byte)(unsafe.Add(unsafe.Pointer(tmstr), libc.StrLen(tmstr)))), -1))) = '\x00'
	...
	act(libc.CString("$n begins to edit the news."), 1, ch, nil, nil, TO_ROOM)
...
func print_lockout(ch *char_data) {
	...
	var count int = 0
	var first int = 1
	if !get_filename(&fname[0], uint64(40), INTRO_FILE, libc.CString("lockout")) {
	...
	for int(file.IsEOF()) == 0 {
		...
		if first != 1 && libc.StrStr(&buf[0], &filler[0]) == nil {
			if count == 0 {
...
func do_transobj(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if (func() *obj_data {
		...
		var (
			num  int       = GET_OBJ_VNUM(obj)
			obj2 *obj_data = nil
		)
		act(libc.CString("You send $p to everyone in the game."), 1, ch, obj, nil, TO_CHAR)
		for d = descriptor_list; d != nil; d = d.Next {
			if IS_NPC(d.Character) {
				...
				act(libc.CString("$N sends $p across the universe to you."), 1, d.Character, obj, unsafe.Pointer(ch), TO_CHAR)
		...
		act(libc.CString("You send $p to $N."), 1, ch, obj, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("$n sends $p across the universe to you."), 1, ch, obj, unsafe.Pointer(vict), TO_VICT)
		obj_from_char(obj)
	...
	if vnum != int(-1) {
		...
		for i = 0; i <= top_of_world; i++ {
			for d = 0; d < NUM_OF_DIRS; d++ {
				if world[i].Dir_option[d] != nil && world[i].Dir_option[d].Key != 0 && world[i].Dir_option[d].Key == vnum {
...
func do_recall(ch *char_data, argument *byte, cmd int, subcmd int) {
	if ch.Admlevel < 1 {
		...
		act(libc.CString("$n disappears in a burst of light!"), 0, ch, nil, nil, TO_ROOM)
		if real_room(2) != int(-1) {
			char_from_room(ch)
...
func do_echo(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if *argument == 0 {
		...
		if NoName {
			...
			act(&blom[0], 0, ch, nil, nil, TO_ROOM)
		}
		if found == 0 {
			act(&buf[0], 0, ch, nil, nil, TO_CHAR)
			act(&buf[0], 0, ch, nil, nil, TO_ROOM)
		} else {
			act(&buf[0], 0, ch, nil, unsafe.Pointer(tch), TO_CHAR)
			act(&buf[0], 0, ch, nil, unsafe.Pointer(tch), TO_NOTVICT)
			search_replace(&buf[0], libc.CString("$N"), libc.CString("you"))
			act(&buf[0], 0, ch, nil, unsafe.Pointer(tch), TO_VICT)
		}
	}
}
...
func find_target_room(ch *char_data, rawroomstr *byte) int {
	var (
		location int = int(-1)
		roomstr  [2048]byte
		rm       *room_data
	...
	if unicode.IsDigit(rune(roomstr[0])) && libc.StrChr(&roomstr[0], '.') == nil {
		if (func() int {
			location = real_room(libc.Atoi(libc.GoString(&roomstr[0])))
			return location
		}()) == int(-1) {
		...
		if (func() *char_data {
			...
			} else if target_obj.Carried_by != nil && target_obj.Carried_by.In_room != int(-1) {
				location = target_obj.Carried_by.In_room
			} else if target_obj.Worn_by != nil && target_obj.Worn_by.In_room != int(-1) {
	...
	rm = &world[location]
	if !can_edit_zone(ch, rm.Zone) && ch.Admlevel < ADMLVL_GOD && ZONE_FLAGGED(rm.Zone, ZONE_QUEST) {
...
func do_trans(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if buf[0] == 0 {
		...
		if (func() *char_data {
			...
			act(libc.CString("$n disappears in a mushroom cloud."), 0, victim, nil, nil, TO_ROOM)
			char_from_room(victim)
			char_to_room(victim, ch.In_room)
			act(libc.CString("$n arrives from a puff of smoke."), 0, victim, nil, nil, TO_ROOM)
			act(libc.CString("$n has transferred you!"), 0, ch, nil, unsafe.Pointer(victim), TO_VICT)
...
func do_teleport(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if buf[0] == 0 {
		...
		send_to_char(ch, libc.CString("%s"), config_info.Play.OK)
		act(libc.CString("$n disappears in a puff of smoke."), 0, victim, nil, nil, TO_ROOM)
		char_from_room(victim)
		char_to_room(victim, target)
		act(libc.CString("$n arrives from a puff of smoke."), 0, victim, nil, nil, TO_ROOM)
		act(libc.CString("$n has teleported you!"), 0, ch, nil, unsafe.Pointer((*byte)(unsafe.Pointer(victim))), TO_VICT)
		look_at_room(victim.In_room, victim, 0)
		enter_wtrigger(&world[victim.In_room], victim, -1)
	}
}
func do_vnum(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if buf[0] == 0 || buf2[0] == 0 || !is_abbrev(&buf[0], libc.CString("mob")) && !is_abbrev(&buf[0], libc.CString("obj")) && !is_abbrev(&buf[0], libc.CString("mat")) && !is_abbrev(&buf[0], libc.CString("wtype")) && !is_abbrev(&buf[0], libc.CString("atype")) {
		send_to_char(ch, libc.CString("Usage: vnum { atype | material | mob | obj | wtype } <name>\r\n"))
		return
	}
	if is_abbrev(&buf[0], libc.CString("mob")) {
		if vnum_mobile(&buf2[0], ch) == 0 {
			send_to_char(ch, libc.CString("No mobiles by that name.\r\n"))
		}
	}
	if is_abbrev(&buf[0], libc.CString("obj")) {
		if vnum_object(&buf2[0], ch) == 0 {
			send_to_char(ch, libc.CString("No objects by that name.\r\n"))
		}
	}
	if is_abbrev(&buf[0], libc.CString("mat")) {
		if vnum_material(&buf2[0], ch) == 0 {
			send_to_char(ch, libc.CString("No materials by that name.\r\n"))
		}
	}
	if is_abbrev(&buf[0], libc.CString("wtype")) {
		if vnum_weapontype(&buf2[0], ch) == 0 {
			send_to_char(ch, libc.CString("No weapon types by that name.\r\n"))
		}
	}
	if is_abbrev(&buf[0], libc.CString("atype")) {
		if vnum_armortype(&buf2[0], ch) == 0 {
			send_to_char(ch, libc.CString("No armor types by that name.\r\n"))
		}
	}
}
func list_zone_commands_room(ch *char_data, rvnum int) {
	var (
		zrnum    int = real_zone_by_thing(rvnum)
		rrnum    int = real_room(rvnum)
		cmd_room int = int(-1)
		subcmd   int = 0
		count    int = 0
	)
	if zrnum == int(-1) || rrnum == int(-1) {
	...
	for int(zone_table[zrnum].Cmd[subcmd].Command) != 'S' {
		...
		case 'T':
			fallthrough
		case 'V':
			cmd_room = zone_table[zrnum].Cmd[subcmd].Arg3
		case 'D':
			fallthrough
		case 'R':
			cmd_room = zone_table[zrnum].Cmd[subcmd].Arg1
		default:
		}
		if cmd_room == rrnum {
			count++
...
func do_stat_room(ch *char_data) {
	...
	send_to_char(ch, libc.CString("Zone: [%3d], VNum: [@g%5d@n], RNum: [%5d], IDNum: [%5ld], Type: %s\r\n"), zone_table[rm.Zone].Number, rm.Number, ch.In_room, rm.Number+ROOM_ID_BASE, &buf2[0])
	...
	for i = 0; i < NUM_OF_DIRS; i++ {
		...
		if rm.Dir_option[i].To_room == int(-1) {
...
func do_stat_object(ch *char_data, j *obj_data) {
	...
	case ITEM_CONTAINER:
		sprintbit(uint32(int32(j.Value[VAL_CONTAINER_FLAGS])), container_bits[:], &buf[0], uint64(64936))
...
func do_stat_character(ch *char_data, k *char_data) {
	...
	send_to_char(ch, libc.CString("%s %s '%s'  IDNum: [%5d], In room [%5d], Loadroom : [%5d]\r\n"), &buf[0], func() string {
		...
		return int(k.Idnum)
	}(), GET_ROOM_VNUM(k.In_room), func() int {
		if IS_NPC(k) {
			return k.Hometown
		}
...
func do_stat(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if buf1[0] == 0 {
		...
		return
	} else if is_abbrev(&buf1[0], libc.CString("room")) {
		do_stat_room(ch)
	} else if is_abbrev(&buf1[0], libc.CString("mob")) {
		if buf2[0] == 0 {
	...
	} else if is_abbrev(&buf1[0], libc.CString("player")) {
		if buf2[0] == 0 {
	...
	} else if is_abbrev(&buf1[0], libc.CString("file")) {
		if buf2[0] == 0 {
			send_to_char(ch, libc.CString("Stats on which player?\r\n"))
		} else if (func() *char_data {
			victim = get_player_vis(ch, &buf2[0], nil, 1<<1)
	...
	} else if is_abbrev(&buf1[0], libc.CString("object")) {
		if buf2[0] == 0 {
	...
	} else if is_abbrev(&buf1[0], libc.CString("zone")) {
		if buf2[0] == 0 {
			send_to_char(ch, libc.CString("Stats on which zone?\r\n"))
			return
		} else {
			print_zone(ch, libc.Atoi(libc.GoString(&buf2[0])))
			return
		}
	} else {
		var (
...
func do_load(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if !is_number(&buf2[0]) || !is_number(&buf3[0]) {
	...
	if is_abbrev(&buf[0], libc.CString("mob")) {
		...
		if (func() int {
			r_num = real_mobile(libc.Atoi(libc.GoString(&buf2[0])))
			return r_num
		}()) == int(-1) {
		...
		for i = 0; i < n; i++ {
			...
			act(libc.CString("$n makes a quaint, magical gesture with one hand."), 1, ch, nil, nil, TO_ROOM)
			act(libc.CString("$n has created $N!"), 0, ch, nil, unsafe.Pointer(mob), TO_ROOM)
			act(libc.CString("You create $N."), 0, ch, nil, unsafe.Pointer(mob), TO_CHAR)
			load_mtrigger(mob)
		}
	} else if is_abbrev(&buf[0], libc.CString("obj")) {
		var (
			obj   *obj_data
			r_num int
		)
		if (func() int {
			r_num = real_object(libc.Atoi(libc.GoString(&buf2[0])))
			return r_num
		}()) == int(-1) {
		...
		for i = 0; i < n; i++ {
			...
			act(libc.CString("$n makes a strange magical gesture."), 1, ch, nil, nil, TO_ROOM)
			act(libc.CString("$n has created $p!"), 0, ch, obj, nil, TO_ROOM)
			act(libc.CString("You create $p."), 0, ch, obj, nil, TO_CHAR)
			load_otrigger(obj)
		}
	} else {
...
func do_purge(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if buf[0] != 0 {
		if (func() *char_data {
			...
			act(libc.CString("$n disintegrates $N."), 0, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			if !IS_NPC(vict) {
				send_to_all(libc.CString("@R%s@r purges @R%s's@r sorry ass right off the MUD!@n\r\n"), GET_NAME(ch), GET_NAME(vict))
			}
			if !IS_NPC(vict) {
				mudlog(BRF, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("(GC) %s has purged %s."), GET_NAME(ch), GET_NAME(vict))
			...
			act(libc.CString("$n destroys $p."), 0, ch, obj, nil, TO_ROOM)
			extract_obj(obj)
		} else {
		...
		var i int
		act(libc.CString("$n gestures... You are surrounded by scorching flames!"), 0, ch, nil, nil, TO_ROOM)
...
func do_syslog(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if (func() int {
		tp = search_block(&arg[0], &logtypes[0], 0)
		return tp
	}()) == -1 {
...
func do_copyover(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if arg[0] == 0 {
		execute_copyover()
	} else if is_abbrev(&arg[0], libc.CString("cancel")) || is_abbrev(&arg[0], libc.CString("stop")) {
		if copyover_timer == 0 {
	...
	} else if is_abbrev(&arg[0], libc.CString("help")) {
...
func do_restore(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if buf[0] == 0 {
	...
	} else if is_abbrev(&buf[0], libc.CString("all")) {
		...
		for j = descriptor_list; j != nil; j = j.Next {
			...
			update_pos(vict)
			act(libc.CString("You have been fully healed by $N!"), 0, vict, nil, unsafe.Pointer(ch), int(TO_CHAR|2<<7))
		...
		act(libc.CString("You have been fully healed by $N!"), 0, vict, nil, unsafe.Pointer(ch), TO_CHAR)
		if vict.Suppression > 0 && vict.Hit > ((gear_pl(vict)/100)*vict.Suppression) {
			vict.Hit = (gear_pl(vict) / 100) * vict.Suppression
			send_to_char(vict, libc.CString("@mYou are healed to your suppression limit.@n\r\n"))
		}
...
func perform_immort_invis(ch *char_data, level int) {
	...
	for tch = world[ch.In_room].People; tch != nil; tch = tch.Next_in_room {
		...
		if tch.Admlevel >= int(ch.Player_specials.Invis_level) && tch.Admlevel < level {
			act(libc.CString("You blink and suddenly realize that $n is gone."), 0, ch, nil, unsafe.Pointer(tch), TO_VICT)
		}
		if tch.Admlevel < int(ch.Player_specials.Invis_level) && tch.Admlevel >= level {
			act(libc.CString("You suddenly realize that $n is standing beside you."), 0, ch, nil, unsafe.Pointer(tch), TO_VICT)
		}
	}
...
func do_force(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if arg[0] == 0 || to_force[0] == 0 {
		...
		if (func() *char_data {
			...
			send_to_char(ch, libc.CString("%s"), config_info.Play.OK)
			act(&buf1[0], 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("(GC) %s forced %s to %s"), GET_NAME(ch), GET_NAME(vict), &to_force[0])
			command_interpreter(vict, &to_force[0])
		}
	} else if libc.StrCaseCmp(libc.CString("room"), &arg[0]) == 0 {
		send_to_char(ch, libc.CString("%s"), config_info.Play.OK)
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("(GC) %s forced room %d to %s"), GET_NAME(ch), GET_ROOM_VNUM(ch.In_room), &to_force[0])
		...
		send_to_char(ch, libc.CString("%s"), config_info.Play.OK)
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("(GC) %s forced all to %s"), GET_NAME(ch), &to_force[0])
...
func do_wiznet(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	for d = descriptor_list; d != nil; d = d.Next {
		if IS_PLAYING(d) && d.Character.Admlevel >= level && !PRF_FLAGGED(d.Character, PRF_NOWIZ) && !PLR_FLAGGED(d.Character, uint32(int32(int(PLR_WRITING|PLR_MAILING)))) && (d != ch.Desc || !PRF_FLAGGED(d.Character, PRF_NOREPEAT)) {
...
func do_zreset(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if arg[0] == '*' {
		if ch.Admlevel < ADMLVL_VICE {
			...
			mudlog(NRM, int(MAX(ADMLVL_GRGOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("(GC) %s reset all MUD zones."), GET_NAME(ch))
		...
		j = libc.Atoi(libc.GoString(&arg[0]))
		for i = 0; i <= top_of_zone_table; i++ {
	...
	if i <= top_of_zone_table && (can_edit_zone(ch, i) || ch.Admlevel > ADMLVL_IMMORT) {
		reset_zone(i)
		send_to_char(ch, libc.CString("Reset zone #%d: %s.\r\n"), zone_table[i].Number, zone_table[i].Name)
		mudlog(NRM, int(MAX(ADMLVL_GRGOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("(GC) %s reset zone %d (%s)"), GET_NAME(ch), zone_table[i].Number, zone_table[i].Name)
...
func do_wizutil(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if arg[0] == 0 {
		...
		case SCMD_PARDON:
			...
			mudlog(BRF, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("(GC) %s pardoned by %s"), GET_NAME(vict), GET_NAME(ch))
		case SCMD_NOTITLE:
			result = int(PLR_TOG_CHK(vict, PLR_NOTITLE))
			mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("(GC) Notitle %s for %s by %s."), func() string {
				if result != 0 {
					return "ON"
				}
				return "OFF"
		...
		case SCMD_SQUELCH:
			...
			mudlog(BRF, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("(GC) Squelch %s for %s by %s."), func() string {
				if result != 0 {
					return "ON"
				}
				return "OFF"
		...
		case SCMD_FREEZE:
			...
			act(libc.CString("A sudden cold wind conjured from nowhere freezes $n!"), 0, vict, nil, nil, TO_ROOM)
			mudlog(BRF, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("(GC) %s frozen by %s."), GET_NAME(vict), GET_NAME(ch))
		case SCMD_THAW:
			if !PLR_FLAGGED(vict, PLR_FROZEN) {
			...
			mudlog(BRF, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("(GC) %s un-frozen by %s."), GET_NAME(vict), GET_NAME(ch))
			...
			act(libc.CString("A sudden fireball conjured from nowhere thaws $n!"), 0, vict, nil, nil, TO_ROOM)
		case SCMD_UNAFFECT:
		...
		default:
			basic_mud_log(libc.CString("SYSERR: Unknown subcmd %d passed to do_wizutil (%s)"), subcmd, "__THISFILE__")
		}
		save_char(vict)
	}
}
func print_zone_to_buf(bufptr *byte, left uint64, zone int, listall int) uint64 {
	var tmp uint64
	if listall != 0 {
		var (
			i int
...
func do_show(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	case 1:
		...
		} else if value[0] != 0 && is_number(&value[0]) {
			for func() int {
				zvn = libc.Atoi(libc.GoString(&value[0]))
				return func() int {
					zrn = 0
					return zrn
				}()
	...
	case 2:
		...
		// todo: fix the 0
		send_to_char(ch, libc.CString("Started: %-20.16s  "), 0)
		send_to_char(ch, libc.CString("Last: %-20.16s  Played: %3dh %2dm\r\n"), 0, int(vict.Time.Played/3600), int(vict.Time.Played/60%60))
		free_char(vict)
	case 3:
		if value[0] == 0 {
			send_to_char(ch, libc.CString("A name would help.\r\n"))
			...
			for j = 0; j < NUM_OF_DIRS; j++ {
				...
				if (world[i].Dir_option[j]).To_room == 0 {
					nlen = uint64(stdio.Snprintf(&buf[len_], int(64936-uintptr(len_)), "%2d: (void   ) [%5d] %-*s%s (%s)\r\n", func() int {
						...
						*p++
						return *p
					}(), GET_ROOM_VNUM(i), count_color_chars(world[i].Name)+40, world[i].Name, func() string {
						if (func() int {
							if !IS_NPC(ch) {
				...
				if (world[i].Dir_option[j]).To_room == int(-1) && (world[i].Dir_option[j]).General_description == nil {
	...
	case 10:
		...
		for d = descriptor_list; d != nil; d = d.Next {
			...
			if !CAN_SEE(ch, d.Character) || d.Character.In_room == int(-1) {
				continue
			}
			i++
...
func perform_set(ch *char_data, vict *char_data, mode int, val_arg *byte) bool {
	var (
		i     int
		on    int   = 0
		off   int   = 0
	...
	case 4:
		vict.Max_hit = value
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set maxpl for %s."), GET_NAME(ch), GET_NAME(vict))
		log_imm_action(libc.CString("SET: %s has set maxpl for %s."), GET_NAME(ch), GET_NAME(vict))
	case 5:
		vict.Max_mana = value
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set maxki for %s."), GET_NAME(ch), GET_NAME(vict))
		log_imm_action(libc.CString("SET: %s has set maxki for %s."), GET_NAME(ch), GET_NAME(vict))
	case 6:
		vict.Max_move = value
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set maxsta for %s."), GET_NAME(ch), GET_NAME(vict))
		log_imm_action(libc.CString("SET: %s has set maxsta for %s."), GET_NAME(ch), GET_NAME(vict))
	case 7:
		vict.Hit = value
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set pl for %s."), GET_NAME(ch), GET_NAME(vict))
		log_imm_action(libc.CString("SET: %s has set pl for %s."), GET_NAME(ch), GET_NAME(vict))
	case 8:
		vict.Mana = value
		affect_total(vict)
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set ki for %s."), GET_NAME(ch), GET_NAME(vict))
		log_imm_action(libc.CString("SET: %s has set ki for %s."), GET_NAME(ch), GET_NAME(vict))
	case 9:
		vict.Move = value
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set st for %s."), GET_NAME(ch), GET_NAME(vict))
	...
	case 10:
		...
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set align for %s."), GET_NAME(ch), GET_NAME(vict))
		log_imm_action(libc.CString("SET: %s has set align for %s."), GET_NAME(ch), GET_NAME(vict))
		affect_total(vict)
	case 11:
		value = MAX(0, MIN(100, value))
		vict.Real_abils.Str = int8(value)
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set str for %s."), GET_NAME(ch), GET_NAME(vict))
	...
	case 13:
		...
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set intel for %s."), GET_NAME(ch), GET_NAME(vict))
		log_imm_action(libc.CString("SET: %s has set intel for %s."), GET_NAME(ch), GET_NAME(vict))
		affect_total(vict)
	case 14:
		value = MAX(0, MIN(100, value))
		vict.Real_abils.Wis = int8(value)
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set wis for %s."), GET_NAME(ch), GET_NAME(vict))
		log_imm_action(libc.CString("SET: %s has set wis for %s."), GET_NAME(ch), GET_NAME(vict))
		affect_total(vict)
	case 15:
		value = MAX(0, MIN(100, value))
		vict.Real_abils.Dex = int8(value)
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set dex for %s."), GET_NAME(ch), GET_NAME(vict))
		log_imm_action(libc.CString("SET: %s has set dex for %s."), GET_NAME(ch), GET_NAME(vict))
		affect_total(vict)
	case 16:
		value = MAX(0, MIN(100, value))
		vict.Real_abils.Con = int8(value)
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set con for %s."), GET_NAME(ch), GET_NAME(vict))
		log_imm_action(libc.CString("SET: %s has set con for %s."), GET_NAME(ch), GET_NAME(vict))
		affect_total(vict)
	case 17:
		value = MAX(0, MIN(100, value))
		vict.Real_abils.Cha = int8(value)
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set speed for %s."), GET_NAME(ch), GET_NAME(vict))
	...
	case 18:
		...
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set armor index for %s."), GET_NAME(ch), GET_NAME(vict))
	...
	case 19:
		...
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set zenni for %s."), GET_NAME(ch), GET_NAME(vict))
	...
	case 20:
		...
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set bank for %s."), GET_NAME(ch), GET_NAME(vict))
	...
	case 21:
		...
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set exp for %s."), GET_NAME(ch), GET_NAME(vict))
	...
	case 28:
		if vict.Level != 0 {
			...
			mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set PS for %s."), GET_NAME(ch), GET_NAME(vict))
	...
	case 35:
		if (func() int {
			rnum = real_room(int(value))
			return rnum
		}()) == int(-1) {
			send_to_char(ch, libc.CString("No room exists with that number.\r\n"))
			return false
		}
		if vict.In_room != int(-1) {
			char_from_room(vict)
		}
	...
	case 39:
		if (func() int {
			i = search_block(val_arg, &class_names[0], 0)
			return i
		}()) < 0 {
	...
	case 42:
		if libc.StrCaseCmp(val_arg, libc.CString("off")) == 0 {
			...
			rvnum = libc.Atoi(libc.GoString(val_arg))
			if real_room(rvnum) != int(-1) {
	...
	case 47:
		if (func() int {
			i = search_block(val_arg, &genders[0], 0)
			return i
		}()) < 0 {
	...
	case 50:
		...
		affect_total(vict)
	case 51:
		if is_abbrev(val_arg, libc.CString("socials")) || is_abbrev(val_arg, libc.CString("actions")) {
			vict.Player_specials.Olc_zone = AEDIT_PERMISSION
		} else if is_abbrev(val_arg, libc.CString("hedit")) {
			vict.Player_specials.Olc_zone = HEDIT_PERMISSION
		} else if is_abbrev(val_arg, libc.CString("off")) {
	...
	case 52:
		if (func() int {
			i = search_block(val_arg, &race_names[0], 0)
			return i
		}()) < 0 {
	...
	case 64:
		vict.Basepl = value
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set basepl for %s."), GET_NAME(ch), GET_NAME(vict))
		log_imm_action(libc.CString("SET: %s has set basepl for %s."), GET_NAME(ch), GET_NAME(vict))
	case 65:
		vict.Baseki = value
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set baseki for %s."), GET_NAME(ch), GET_NAME(vict))
		log_imm_action(libc.CString("SET: %s has set baseki for %s."), GET_NAME(ch), GET_NAME(vict))
	case 66:
		vict.Basest = value
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set basest for %s."), GET_NAME(ch), GET_NAME(vict))
		log_imm_action(libc.CString("SET: %s has set basest for %s."), GET_NAME(ch), GET_NAME(vict))
	case 67:
		vict.Droom = int(func() int64 {
	...
	case 68:
		...
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set absorbs for %s."), GET_NAME(ch), GET_NAME(vict))
	...
	case 69:
		...
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set upgrade points for %s."), GET_NAME(ch), GET_NAME(vict))
	...
	case 70:
		...
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set aura for %s."), GET_NAME(ch), GET_NAME(vict))
	...
	case 74:
		...
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set death count for %s."), GET_NAME(ch), GET_NAME(vict))
	...
	case 78:
		...
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set skill slots for %s."), GET_NAME(ch), GET_NAME(vict))
	...
	case 80:
		...
		mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("SET: %s has set transformation class for %s."), GET_NAME(ch), GET_NAME(vict))
...
func do_set(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	retval = int(libc.BoolToInt(perform_set(ch, vict, mode, &buf[0])))
	if retval != 0 {
...
func do_plist(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	for i = 0; i <= top_of_p_table; i++ {
		if IS_SET(uint32(int32(player_table[i].Flags)), 1<<0) {
...
func do_raise(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	char_from_room(vict)
	if vict.Droom != int(-1) && vict.Droom != 0 && vict.Droom != 1 {
	...
	ch.Deathtime = 0
	act(libc.CString("$n's body forms in a pool of @Bblue light@n."), 1, vict, nil, nil, TO_ROOM)
}
func do_chown(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if buf2[0] == 0 {
		...
		for i = 0; i < NUM_WEARS; i++ {
			if (victim.Equipment[i]) != nil && CAN_SEE_OBJ(ch, victim.Equipment[i]) && isname(&buf2[0], (victim.Equipment[i]).Name) {
		...
		act(libc.CString("@n$n makes a magical gesture and $p@n flies from $N to $m."), 0, ch, obj, unsafe.Pointer(victim), TO_NOTVICT)
		act(libc.CString("@n$n makes a magical gesture and $p@n flies away from you to $m."), 0, ch, obj, unsafe.Pointer(victim), TO_VICT)
		act(libc.CString("@nYou make a magical gesture and $p@n flies away from $N to you."), 0, ch, obj, unsafe.Pointer(victim), TO_CHAR)
		obj_from_char(obj)
		obj_to_char(obj, ch)
		save_char(ch)
		save_char(victim)
...
func do_zpurge(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	one_argument(argument, &arg[0])
	if arg[0] == 0 {
		zone = zone_table[world[ch.In_room].Zone].Number
	} else {
		zone = libc.Atoi(libc.GoString(&arg[0]))
	}
	for i = 0; i <= top_of_zone_table && found == 0; i++ {
		if zone_table[i].Number == zone {
			stored = i
			found = 1
		}
	}
	if found == 0 || !can_edit_zone(ch, zone) {
	...
	mudlog(NRM, int(MAX(ADMLVL_GOD, int64(ch.Player_specials.Invis_level))), 1, libc.CString("(GC) %s has purged zone %d."), GET_NAME(ch), zone)
}
...
func do_zcheck(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if buf[0] == 0 || libc.StrCmp(&buf[0], libc.CString(".")) == 0 {
		...
		zrnum = real_zone(libc.Atoi(libc.GoString(&buf[0])))
	}
	if zrnum == int(-1) {
	...
	for i = 0; i < top_of_mobt; i++ {
		if real_zone_by_thing(mob_index[i].Vnum) == zrnum {
			...
			if MOB_FLAGGED(mob, MOB_AGGRESSIVE) && MOB_FLAGGED(mob, uint32(int32(int(MOB_AGGR_GOOD|MOB_AGGR_EVIL)|MOB_AGGR_NEUTRAL))) && (func() int {
				found = 1
				return found
			}()) != 0 {
			...
			if AFF_FLAGGED(mob, uint32(int32(int(AFF_GROUP|AFF_CHARM)|AFF_POISON))) && (func() int {
				found = 1
				return found
			}()) != 0 {
		...
		if world[i].Zone == zrnum {
			for j = 0; j < NUM_OF_DIRS; j++ {
				...
				for k = 0; offlimit_zones[k] != -1; k++ {
					if world[exroom].Zone == real_zone(offlimit_zones[k]) && (func() int {
						found = 1
						return found
					}()) != 0 {
			...
			if ROOM_FLAGGED(i, uint32(int32(int(ROOM_ATRIUM|ROOM_HOUSE)|ROOM_HOUSE_CRASH|ROOM_OLC|ROOM_BFS_MARK))) {
				len_ += uint64(stdio.Snprintf(&buf[len_], int(64936-uintptr(len_)), "- Has illegal affection bits set (%s %s %s %s %s)\r\n", func() string {
					if ROOM_FLAGGED(i, ROOM_ATRIUM) {
						return "ATRIUM"
					}
					return ""
				}(), func() string {
					if ROOM_FLAGGED(i, ROOM_HOUSE) {
						return "HOUSE"
					}
					return ""
				}(), func() string {
					if ROOM_FLAGGED(i, ROOM_HOUSE_CRASH) {
						return "HCRSH"
					}
					return ""
				}(), func() string {
					if ROOM_FLAGGED(i, ROOM_OLC) {
						return "OLC"
					}
					return ""
				}(), func() string {
					if ROOM_FLAGGED(i, ROOM_BFS_MARK) {
						return "*"
					}
					return ""
				}()))
...
func mob_checkload(ch *char_data, mvnum int) {
	var (
		cmd_no int
		count  int = 0
		zone   int
		mrnum  int = real_mobile(mvnum)
	)
	if mrnum == int(-1) {
	...
	for zone = 0; zone <= top_of_zone_table; zone++ {
		for cmd_no = 0; int(zone_table[zone].Cmd[cmd_no].Command) != 'S'; cmd_no++ {
			...
			if zone_table[zone].Cmd[cmd_no].Arg1 == mrnum {
...
func obj_checkload(ch *char_data, ovnum int) {
	var (
		cmd_no     int
		count      int = 0
		zone       int
		ornum      int = real_object(ovnum)
		lastroom_v int = 0
		lastroom_r int = 0
		lastmob_r  int = 0
	)
	...
	for zone = 0; zone <= top_of_zone_table; zone++ {
		for cmd_no = 0; int(zone_table[zone].Cmd[cmd_no].Command) != 'S'; cmd_no++ {
			...
			case 'M':
				...
				lastroom_r = zone_table[zone].Cmd[cmd_no].Arg3
				lastmob_r = zone_table[zone].Cmd[cmd_no].Arg1
			case 'O':
				lastroom_v = world[zone_table[zone].Cmd[cmd_no].Arg3].Number
				lastroom_r = zone_table[zone].Cmd[cmd_no].Arg3
				if zone_table[zone].Cmd[cmd_no].Arg1 == ornum {
			...
			case 'R':
				...
				lastroom_r = zone_table[zone].Cmd[cmd_no].Arg1
				if zone_table[zone].Cmd[cmd_no].Arg2 == ornum {
...
func trg_checkload(ch *char_data, tvnum int) {
	var (
		cmd_no     int
		found      int = 0
		zone       int
		trnum      int = real_trigger(tvnum)
		lastroom_v int = 0
		lastroom_r int = 0
		k          int
		lastmob_r  int = 0
	...
	for zone = 0; zone <= top_of_zone_table; zone++ {
		for cmd_no = 0; int(zone_table[zone].Cmd[cmd_no].Command) != 'S'; cmd_no++ {
			...
			case 'O':
				lastroom_v = world[zone_table[zone].Cmd[cmd_no].Arg3].Number
				lastroom_r = zone_table[zone].Cmd[cmd_no].Arg3
				lastobj_r = zone_table[zone].Cmd[cmd_no].Arg1
			case 'P':
				lastobj_r = zone_table[zone].Cmd[cmd_no].Arg1
			case 'G':
				lastobj_r = zone_table[zone].Cmd[cmd_no].Arg1
			...
			case 'R':
				...
				fallthrough
			case 'T':
				if zone_table[zone].Cmd[cmd_no].Arg2 != trnum {
					break
				}
...
func do_checkloadstatus(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if unicode.ToLower(rune(buf1[0])) == 'm' {
		mob_checkload(ch, libc.Atoi(libc.GoString(&buf2[0])))
		return
	}
	if unicode.ToLower(rune(buf1[0])) == 'o' {
		obj_checkload(ch, libc.Atoi(libc.GoString(&buf2[0])))
		return
	}
	if unicode.ToLower(rune(buf1[0])) == 't' {
		trg_checkload(ch, libc.Atoi(libc.GoString(&buf2[0])))
		return
	}
}
func do_findkey(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if arg[0] == 0 {
		...
		dir = search_block(&arg[0], &dirs[0], 0)
		return dir
	}()) >= 0 || (func() int {
		dir = search_block(&arg[0], &abbr_dirs[0], 0)
		return dir
	}()) >= 0 {
		if (world[ch.In_room].Dir_option[dir]) == nil {
			send_to_char(ch, libc.CString("There's no exit in that direction!\r\n"))
		} else if (func() int {
			key = (world[ch.In_room].Dir_option[dir]).Key
			return key
		}()) == int(-1) || key == 0 {
