	...
	if GET_SKILL(ch, SKILL_SPIRITCONTROL) == 0 {
		...
		if AFF_FLAGGED(ch, AFF_SPIRITCONTROL) {
			...
			if ch.Move < cost {
				...
				ch.Move -= cost
				act(libc.CString("@YYou concentrate and quantify every last bit of your spiritual and mental energies. You have full control of them and can bring them forth in an instant.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@y$n@Y seems to concentrate hard for a moment.@n"), 1, ch, nil, nil, TO_ROOM)
	...
	if num >= 3 {
		...
		if libc.StrCaseCmp(&arg[0], libc.CString("merge")) == 0 {
			...
			if float64(ch.Hit) < float64(gear_pl(ch))*0.4 {
				...
				act(libc.CString("@YYou focus your ki into your body while concentrating on the image of your body splitting into two. @yYou lose your concentration and fail to split though...@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@y$n@Y seems to concentrate really hard for a moment, before relaxing.@n"), 1, ch, nil, nil, TO_ROOM)
				ch.Mana -= cost
				ch.Move -= cost
				improve_skill(ch, SKILL_MULTIFORM, 1)
			} else {
				act(libc.CString("@YYou focus your ki into your body while concentrating on the image of your body splitting into two. Another you splits out of your body!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@YSuddenly @y$n@Y seems to concentrates really and after a brief moment splits into two copies of $mself!@n"), 1, ch, nil, nil, TO_ROOM)
				ch.Mana -= cost
				ch.Move -= cost
				if multi1 == nil {
...
func resolve_song(ch *char_data) {
	...
	if obj2 == nil {
		...
		act(libc.CString("@c$n@C stops playing $s song.@n"), 1, ch, nil, nil, TO_ROOM)
		ch.Powerattack = 0
		return
	}
	...
	if skill > diceroll {
		...
		act(libc.CString("@CYou continue playing your song.@n"), 1, ch, nil, nil, TO_CHAR)
		act(&buf[0], 1, ch, nil, nil, TO_ROOM)
	} else {
		act(libc.CString("@CYou mess up a portion of the song, but continue playing.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@c$n@C messes up a portion of $s song, but continues to play.@n"), 1, ch, nil, nil, TO_ROOM)
		return
	}
	for vict = world[ch.In_room].People; vict != nil; vict = next_v {
				...
				if AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP) || vict == ch {
					if ch == vict.Master || ch.Master == vict || ch.Master == vict.Master || vict == ch {
						if skill > diceroll {
							...
							if vict != ch {
								act(libc.CString("@CYour skillfully playing of the Song of Safety has an effect on @c$N@C.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
							} else {
								act(libc.CString("@CYour skillfully playing of the Song of Safety has an effect on your own body@C.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
							}
							vict.Hit += restore
							...
							if vict != ch {
								act(libc.CString("@c$n's@C soothing Song of Safety has recovered some of your powerlevel, stamina, and limb condition."), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
							}
							act(libc.CString("@c$n@C continues playing $s ocarina!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			...
			if ch.Master != nil && vict.Master != nil {
				if AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP) {
					if ch == vict.Master || ch.Master == vict || ch.Master == vict.Master {
						...
						act(libc.CString("@CYour forboding music has caused @c$N's@C shadows to stitch into $S body, slowing $S actions!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
						act(libc.CString("@c$n's@C forboding music has caused YOUR shadows to stitch into YOUR body, slow YOUR actions down!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
						act(libc.CString("@c$n's@C forboding music has caused @c$N's@C shadows to stitch into $S body, slowing $S actions!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
						if !IS_NPC(vict) {
		...
		case SONG_TELEPORT_EARTH:
			...
			if AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP) {
				if ch == vict.Master || ch.Master == vict || ch.Master == vict.Master {
					if skill > diceroll {
						act(libc.CString("@CYour Teleportation Melody has transported @c$N@C to Earth in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
						act(libc.CString("@c$n's@C Teleportation Melody has transported you to Earth in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
						act(libc.CString("@c$n's@C Teleportation Melody has transported @c$N@C away in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
						char_from_room(vict)
		...
		case SONG_TELEPORT_VEGETA:
			...
			if AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP) {
				if ch == vict.Master || ch.Master == vict || ch.Master == vict.Master {
					if skill > diceroll {
						act(libc.CString("@CYour Teleportation Melody has transported @c$N@C to Vegeta in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
						act(libc.CString("@c$n's@C Teleportation Melody has transported you to Vegeta in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
						act(libc.CString("@c$n's@C Teleportation Melody has transported @c$N@C away in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
						char_from_room(vict)
		...
		case SONG_TELEPORT_FRIGID:
			...
			if AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP) {
				if ch == vict.Master || ch.Master == vict || ch.Master == vict.Master {
					if skill > diceroll {
						act(libc.CString("@CYour Teleportation Melody has transported @c$N@C to Frigid in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
						act(libc.CString("@c$n's@C Teleportation Melody has transported you to Frigid in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
						act(libc.CString("@c$n's@C Teleportation Melody has transported @c$N@C away in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
						char_from_room(vict)
		...
		case SONG_TELEPORT_KONACK:
			...
			if AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP) {
				if ch == vict.Master || ch.Master == vict || ch.Master == vict.Master {
					if skill > diceroll {
						act(libc.CString("@CYour Teleportation Melody has transported @c$N@C to Konack in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
						act(libc.CString("@c$n's@C Teleportation Melody has transported you to Konack in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
						act(libc.CString("@c$n's@C Teleportation Melody has transported @c$N@C away in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
						char_from_room(vict)
		...
		case SONG_TELEPORT_NAMEK:
			...
			if AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP) {
				if ch == vict.Master || ch.Master == vict || ch.Master == vict.Master {
					if skill > diceroll {
						act(libc.CString("@CYour Teleportation Melody has transported @c$N@C to Namek in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
						act(libc.CString("@c$n's@C Teleportation Melody has transported you to Namek in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
						act(libc.CString("@c$n's@C Teleportation Melody has transported @c$N@C away in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
						char_from_room(vict)
		...
		case SONG_TELEPORT_ARLIA:
			...
			if AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP) {
				if ch == vict.Master || ch.Master == vict || ch.Master == vict.Master {
					if skill > diceroll {
						act(libc.CString("@CYour Teleportation Melody has transported @c$N@C to Arlia in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
						act(libc.CString("@c$n's@C Teleportation Melody has transported you to Arlia in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
						act(libc.CString("@c$n's@C Teleportation Melody has transported @c$N@C away in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
						char_from_room(vict)
		...
		case SONG_TELEPORT_AETHER:
			...
			if AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP) {
				if ch == vict.Master || ch.Master == vict || ch.Master == vict.Master {
					if skill > diceroll {
						act(libc.CString("@CYour Teleportation Melody has transported @c$N@C to Aether in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
						act(libc.CString("@c$n's@C Teleportation Melody has transported you to Aether in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
						act(libc.CString("@c$n's@C Teleportation Melody has transported @c$N@C away in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
						char_from_room(vict)
		...
		case SONG_TELEPORT_KANASSA:
			...
			if AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP) {
				if ch == vict.Master || ch.Master == vict || ch.Master == vict.Master {
					if skill > diceroll {
						act(libc.CString("@CYour Teleportation Melody has transported @c$N@C to Kanassa in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
						act(libc.CString("@c$n's@C Teleportation Melody has transported you to Kanassa in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
						act(libc.CString("@c$n's@C Teleportation Melody has transported @c$N@C away in a flash!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
						char_from_room(vict)
		...
		case SONG_SHIELDING:
			if vict == ch || AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP) {
				if ch == vict.Master || ch.Master == vict || ch.Master == vict.Master || vict == ch {
					if skill > diceroll {
						if vict != ch {
							act(libc.CString("@CYour triumphant and soaring music has powered a barrier around @c$N@C!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
							act(libc.CString("@c$n's@C triumphant and soaring music has powered a barrier around you!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
						} else {
							act(libc.CString("@CYour triumphant and soaring music has powered a barrier around yourself@C!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
						}
						act(libc.CString("@c$n's@C triumphant and soaring music has powered a barrier around @c$N@C!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
	...
	if ch.Powerattack >= 4 && skill > diceroll {
		...
		case SONG_TELEPORT_EARTH:
			...
			ch.Powerattack = 0
			act(libc.CString("@CFinally as the last of your comrades has been teleported you teleport yourself to Earth and stop your song.@n"), 1, ch, nil, nil, TO_CHAR)
		case SONG_TELEPORT_VEGETA:
			char_from_room(ch)
			char_to_room(ch, real_room(2234))
			ch.Powerattack = 0
			act(libc.CString("@CFinally as the last of your comrades has been teleported you teleport yourself to Vegeta and stop your song.@n"), 1, ch, nil, nil, TO_CHAR)
		case SONG_TELEPORT_FRIGID:
			char_from_room(ch)
			char_to_room(ch, real_room(4047))
			ch.Powerattack = 0
			act(libc.CString("@CFinally as the last of your comrades has been teleported you teleport yourself to Frigid and stop your song.@n"), 1, ch, nil, nil, TO_CHAR)
		case SONG_TELEPORT_NAMEK:
			char_from_room(ch)
			char_to_room(ch, real_room(0x27C6))
			ch.Powerattack = 0
			act(libc.CString("@CFinally as the last of your comrades has been teleported you teleport yourself to Namek and stop your song.@n"), 1, ch, nil, nil, TO_CHAR)
		case SONG_TELEPORT_KANASSA:
			char_from_room(ch)
			char_to_room(ch, real_room(14910))
			ch.Powerattack = 0
			act(libc.CString("@CFinally as the last of your comrades has been teleported you teleport yourself to Kanassa and stop your song.@n"), 1, ch, nil, nil, TO_CHAR)
		case SONG_TELEPORT_AETHER:
			char_from_room(ch)
			char_to_room(ch, real_room(0x2EF9))
			ch.Powerattack = 0
			act(libc.CString("@CFinally as the last of your comrades has been teleported you teleport yourself to Aether and stop your song.@n"), 1, ch, nil, nil, TO_CHAR)
		case SONG_TELEPORT_ARLIA:
			char_from_room(ch)
			char_to_room(ch, real_room(0x3ED7))
			ch.Powerattack = 0
			act(libc.CString("@CFinally as the last of your comrades has been teleported you teleport yourself to Arlia and stop your song.@n"), 1, ch, nil, nil, TO_CHAR)
		case SONG_TELEPORT_KONACK:
			char_from_room(ch)
			char_to_room(ch, real_room(8003))
			ch.Powerattack = 0
			act(libc.CString("@CFinally as the last of your comrades has been teleported you teleport yourself to Konack and stop your song.@n"), 1, ch, nil, nil, TO_CHAR)
		}
	}
}
func do_song(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	var instrument int = GET_OBJ_VNUM(obj2)
	if ch.Powerattack != 0 {
		act(libc.CString("@cYou stop playing your ocarina.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@c$n stops playing their ocarina.@n"), 1, ch, nil, nil, TO_ROOM)
		ch.Powerattack = 0
		return
	} else {
		var (
		...
		if arg[0] == 0 {
			...
			if modifier == 0 {
				...
				act(libc.CString("@CYou begin to play the Song of Safety! Your fingers lightly glide over the ocarina and as you blow into it sweet music similar to a lullaby issues forth from the intrument.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@c$n@C begins to play a song on $s ocarina. The music seems to be some sort of lullaby.@n"), 1, ch, nil, nil, TO_ROOM)
				...
				act(libc.CString("@CYou begin to play the Shadow Stitch Minuet! Your fingers lightly glide over the ocarina and as you blow into it forboding low toned music issues forth.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@c$n@C begins to play a song on $s ocarina. Depressing low toned music issues forth from the ocarina.@n"), 1, ch, nil, nil, TO_ROOM)
				...
				act(libc.CString("@CYou begin to play the Melody of Teleportation! Your fingers lightly glide over the ocarina and as you blow into it a repeating light hearted melody issues forth.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@c$n@C begins to play a song on $s ocarina. A light hearted melody can be heard sounding from the ocarina as it is played.@n"), 1, ch, nil, nil, TO_ROOM)
				ch.Mana -= cost
				return
				...
				act(libc.CString("@CYou begin to play the Song of Shielding! Your fingers lightly glide over the ocarina and as you blow into it a triumphant series of notes issues forth.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@c$n@C begins to play a song on $s ocarina. A triumphant song full of soaring sounds from the ocarina as it is played.@n"), 1, ch, nil, nil, TO_ROOM)
...
func do_moondust(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if chance > int(ch.Aff_abils.Wis)+rand_number(1, 10) {
		act(libc.CString("@GYou spread your wings and begin to concentrate. Your wings begin to glow a soft sea green color. As you prepare to release a cloud of your charged wing dust you lose focus and the power you had begun to charge into your wings dissipates.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@g$n@G spreads $s wings and seems to concentrate for a moment. Suddenly $s wings begin to glow a soft sea green color. This soft glow grows brighter for a second before fading completely.@n"), 1, ch, nil, nil, TO_ROOM)
		ch.Move -= cost
	...
	ch.Move -= cost
	WAIT_STATE(ch, (int(1000000/OPT_USEC))*1)
	act(libc.CString("@GYou spread your wings and begin to concentrate. Your wings begin to glow a soft sea green color. As your wings grow brighter you focus your charged bio energy in a shockwave the unleashes a cloud of glowing green dust. You breath in the dust and feel it rejuvinate your body's cells!@n"), 1, ch, nil, nil, TO_CHAR)
	act(libc.CString("@g$n@G spreads $s wings and seems to concentrate for a moment. Suddenly $s wings begin to glow a soft sea green color. This soft glow grows brighter and as $e flexes $s wings to their full extent a shockwave of energy explodes outward. Carried on this shockwave is a cloud of glowing dust! You notice some of the dust being breathed in by $s!@n"), 1, ch, nil, nil, TO_ROOM)
	...
	for vict = world[ch.In_room].People; vict != nil; vict = next_v {
		...
		if AFF_FLAGGED(vict, AFF_GROUP) {
			if ch.Master == vict.Master || vict.Master == ch || ch.Master == vict {
				...
				act(libc.CString("@CYou breathe in the dust and are healed by it somewhat!@n"), 1, vict, nil, nil, TO_CHAR)
				act(libc.CString("@c$n@C breathes in the dust and is healed somewhat!@n"), 1, vict, nil, nil, TO_ROOM)
...
func do_shell(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if AFF_FLAGGED(ch, AFF_SHELL) {
		act(libc.CString("@mYou quickly absorb the armor carapace covering your body back inside.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@M$n's@m armored carapce retreats back to its original size.@n"), 1, ch, nil, nil, TO_ROOM)
	...
	if float64(ch.Move) < float64(ch.Max_move)*0.2 {
		...
		act(libc.CString("@mYou crouch down and begin to focus on your body's carapace cells encouraging them to multiply! However your control is lacking and you ultimately fail to grow your armor very much.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@M$n@m crouches down and seems to strain for a moment before giving up and resuming $s normal stance.@n"), 1, ch, nil, nil, TO_ROOM)
		return
	} else {
		act(libc.CString("@mYou crouch down and begin to focus on your body's carapace cells, encouraging them to multiply! Very quickly millions of new carapace cells have been born and your armored carapace extends over all parts of your body!@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@M$n@m crouches down and after a few moments of straining $s body's carapace armor starts to grow thicker and extends to cover all parts of $s body!@n"), 1, ch, nil, nil, TO_ROOM)
...
func do_liquefy(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if AFF_FLAGGED(ch, AFF_LIQUEFIED) {
		act(libc.CString("@MSuddenly large chunks of goo start to hover up slowly. These very same chunks quickly begin to fly into each other, piling on as the ball of goo grows. Suddenly @m$n@M emerges as the ball of goo takes $s shape!@n"), 1, ch, nil, nil, TO_ROOM)
		act(libc.CString("@MYou begin to pull the liquid chunks of your body together. Those chunks hover upward and merge into each other until a large ball of goo is formed. Slowly your body emerges as the pieces of your body take on their old form!@n"), 1, ch, nil, nil, TO_CHAR)
	...
	if libc.StrCaseCmp(&arg[0], libc.CString("hide")) == 0 {
		...
		if axion_dice(0) > GET_LEVEL(ch) {
			act(libc.CString("@MYour body starts to become loose and sag, but you lose focus and it reverts to its original shape!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@m$n@M's body starts to become loose and sag, but $e seems to return normal a moment later.@n"), 1, ch, nil, nil, TO_ROOM)
			ch.Mana -= int64((float64(ch.Max_mana) * 0.002) + 150)
			return
		} else {
			act(libc.CString("@MYour body starts to become loose and sag. It continues to droop down until it begins to run down like a river of goo flowing from where your body was.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@m$n@M's body starts to become loose and sag. Much of $s body begins to pour down and scatter around as pools of goo.@n"), 1, ch, nil, nil, TO_ROOM)
			ch.Mana -= int64((float64(ch.Max_mana) * 0.002) + 150)
		...
		if arg2[0] == 0 {
			...
			return
		} else if !can_kill(ch, vict, nil, 1) {
			...
			act(libc.CString("@MYour body rapidly turns to liquid and flies for @R$N's@M open mouth! However $E easily dodges and avoids your attempt!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@m$n@M's body rapidly turns to liquid and flies for @RYOUR@M open mouth! However you are faster and managed to dodge the attempt.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@m$n@M's body rapidly turns into liquid and flies for @R$N's@M open mouth! However $E easily dodges and avoids @m$n's@M attempt!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			ch.Mana -= int64((float64(ch.Max_mana) * 0.002) + 150)
			if ch.Fighting == nil {
			...
			act(libc.CString("@MYour body rapidly turns to liquid and flies for @R$N's@M open mouth! However as you force yourself in through $S mouth $E successfully resists and forces your back out!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@m$n@M's body rapidly turns to liquid and flies for @RYOUR@M open mouth! However you think quickly and force $m out before $e has a chance to get fully into your body!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@m$n@M's body rapidly turns into liquid and flies for @R$N's@M open mouth! However as $e forces $mself in through @R$N's@M mouth $E manages to resist and force @m$n@M back out!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			ch.Mana -= int64((float64(ch.Max_mana) * 0.002) + 150)
			...
			act(libc.CString("@MYour body rapidly turns to liquid and flies for @R$N's@M open mouth! As you fill $S body you expand outward until $s body explodes into a gory mess!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@m$n@M's body rapidly turns to liquid and flies for @RYOUR@M open mouth! As $e fills your body it begins to expand until it is unable to take the strain any longer and explodes!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@m$n@M's body rapidly turns into liquid and flies for @R$N's@M open mouth! As $e forces $mself in through @R$N's@M mouth $S body begins to expand until it can't take the strain any longer and explodes!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			ch.Mana -= int64((float64(ch.Max_mana) * 0.002) + 150)
			if AFF_FLAGGED(ch, AFF_GROUP) {
				group_gain(ch, vict)
			} else {
...
func do_defend(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if arg[0] == 0 && ch.Defending == nil {
		...
		act(libc.CString("@YYou stop defending @y$N@Y.@n"), 1, ch, nil, unsafe.Pointer(ch.Defending), TO_CHAR)
		act(libc.CString("@y$n@Y stops defending you.@n"), 1, ch, nil, unsafe.Pointer(ch.Defending), TO_VICT)
		act(libc.CString("@y$n@Y stops defending @y$N@Y.@n"), 1, ch, nil, unsafe.Pointer(ch.Defending), TO_NOTVICT)
	...
	if (func() *char_data {
		...
		act(libc.CString("@YYou start defending @y$N@Y.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@y$n@Y starts defending you.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("@y$n@Y starts defending @y$N@Y.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		vict.Defender = ch
		ch.Defending = vict
		return
	}
...
func do_fish(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if libc.StrCaseCmp(&arg[0], libc.CString("cast")) == 0 {
		if PLR_FLAGGED(ch, PLR_FISHING) {
			...
			reveal_hiding(ch, 0)
			act(libc.CString("@CYou pull your arm back and then spring it forward, casting the baited line. A moment later there is a splash as the hook enters the water.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@c$n@C pulls $s arm back and then springs it foward, casting the line of $s fishing pole into the water.@n"), 1, ch, nil, nil, TO_ROOM)
		...
		if !PLR_FLAGGED(ch, PLR_FISHING) {
			...
			reveal_hiding(ch, 0)
			act(libc.CString("@CYou pull hard but the fish spits the hook out a second before you pull! You return to waiting for a bite...@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@c$n@C pulls hard on $s fishing line, but a moment later $e frowns and returns to fishing.@n"), 1, ch, nil, nil, TO_ROOM)
			ch.Fishstate = FISH_NOFISH
			return
		} else {
			reveal_hiding(ch, 0)
			act(libc.CString("@CYou pull hard on your line and feel that you have managed to hook the fish! Better @Greel@C it in!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@c$n@C pulls hard on $s fishing line and starts to struggle with the fish on the other end!@n"), 1, ch, nil, nil, TO_ROOM)
		...
		if !PLR_FLAGGED(ch, PLR_FISHING) {
			...
			reveal_hiding(ch, 0)
			act(libc.CString("@CYou begin reeling the fish in slowly.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@c$n@C begins to reel the line on $s pole in.@n"), 1, ch, nil, nil, TO_ROOM)
		...
		if (ch.Equipment[WEAR_WIELD2]) == nil {
			...
			if int(pole.Type_flag) != ITEM_FISHPOLE {
				...
				if (func() *obj_data {
					...
					reveal_hiding(ch, 0)
					act(libc.CString("@CYou carefully apply the $p@C to your hook.@n"), 1, ch, bait, nil, TO_CHAR)
					act(libc.CString("@c$n@C carefully applies $p@C to $s fishing pole's hook.@n"), 1, ch, bait, nil, TO_ROOM)
		...
		if !PLR_FLAGGED(ch, PLR_FISHING) {
			...
			reveal_hiding(ch, 0)
			act(libc.CString("@CYou reel in your line and stop fishing.@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@c$n@C reels in $s fishing line and stops fishing.@n"), 1, ch, nil, nil, TO_ROOM)
...
func has_pole(ch *char_data) bool {
	if (ch.Equipment[WEAR_WIELD2]) != nil {
		var pole *obj_data = (ch.Equipment[WEAR_WIELD2])
		if int(pole.Type_flag) == ITEM_FISHPOLE {
			return true
...
func fish_update() {
	...
	for i = character_list; i != nil; i = next_char {
		...
		if ROOM_FLAGGED(i.In_room, ROOM_FISHING) {
			if PLR_FLAGGED(i, PLR_FISHING) && has_pole(i) {
				ch = i
				if ch.Accuracy_mod <= 0 && ch.Fishstate == FISH_REELING {
					...
					if ch.Fishstate == FISH_REELING && rand_number(1, 100) <= 80 {
						...
						act(libc.CString("@CYou reel the line on your pole some.@n"), 1, ch, nil, nil, TO_CHAR)
						act(libc.CString("@c$n@C reels the line on $s pole slowly.@n"), 1, ch, nil, nil, TO_ROOM)
						send_to_char(ch, libc.CString("@D[@wDistance@D: @Y%d@D]@n\r\n"), func() int {
						...
						act(libc.CString("@CYou struggle as the fish fights against your attempts to reel it in!@n"), 1, ch, nil, nil, TO_CHAR)
						act(libc.CString("@c$n@C struggles with the fish on the end of $s pole!@n"), 1, ch, nil, nil, TO_ROOM)
					} else if ch.Fishstate == FISH_REELING {
						act(libc.CString("@CYou feel the line go slack and realize you've lost the fish! You reel your line back in...@n"), 1, ch, nil, nil, TO_CHAR)
						act(libc.CString("@c$n@C frowns and then begins to reel in $s line.@n"), 1, ch, nil, nil, TO_ROOM)
						ch.Accuracy_mod = 0
						ch.Fishstate = FISH_NOFISH
						REMOVE_BIT_AR(ch.Act[:], PLR_FISHING)
						if has_pole(ch) {
						...
						act(libc.CString("@CYou feel as if the fish has stopped biting...@n"), 1, ch, nil, nil, TO_CHAR)
						ch.Fishstate = FISH_NOFISH
						...
						act(libc.CString("@CYou feel a fish biting on your line! Better @Ghook@C it!@n"), 1, ch, nil, nil, TO_CHAR)
						ch.Fishstate = FISH_BITE
					}
				}
			} else if PLR_FLAGGED(i, PLR_FISHING) && !has_pole(i) {
...
func catch_fish(ch *char_data, quality int) {
	...
	act(libc.CString("@CYou manage to pull a $p@C from the water and onto the ground in front of you!@n"), 1, ch, fish, nil, TO_CHAR)
	act(libc.CString("@c$n@C manages to pull a $p@C from the water and onto the ground in front of $m!@n"), 1, ch, fish, nil, TO_ROOM)
	var weight int = 1
	...
	if (func() *obj_data {
		...
		if GET_OBJ_VNUM(obj) == 3425 {
			...
			if ch.Mana < cost {
				...
				ch.Mana -= cost
				act(libc.CString("@WWith your ki flowing carefully into your hands you take a hold of the @G$p@W and begin to strip it of its leaves. Once it has been stripped you go to squeeze the ink carefully from the leaves into the bottle, but unfortunately the ink explodes into a mess instead!@n"), 1, ch, obj, nil, TO_CHAR)
				act(libc.CString("@C$n@W takes a hold of the @G$p@W and begins to strip it of its leaves. Once it has been stripped $e bundles up the leaves in $s hands and begins to squeeze. A nasty explosion of a mess is all that follows!@n"), 1, ch, obj, nil, TO_ROOM)
				...
				ch.Mana -= cost
				act(libc.CString("@WWith your ki flowing carefully into your hands you take a hold of the @G$p@W and begin to strip it of its leaves. Once it has been stripped you go to squeeze the ink carefully from the leaves into the bottle, and manage to get every last drop of ink into it.@n"), 1, ch, obj, nil, TO_CHAR)
				act(libc.CString("@C$n@W takes a hold of the @G$p@W and begins to strip it of its leaves. Once it has been stripped $e bundles up the leaves in $s hands and begins to squeeze ink carefully from the leaves into a bottle.@n"), 1, ch, obj, nil, TO_ROOM)
				extract_obj(obj)
				...
				if (bottle.Value[6]) >= 24 {
					...
					ch.Mana -= 0
					act(libc.CString("@GAs the last of the ink fills the bottle you infuse a final burst of ki into the bottle.@n"), 1, ch, filled, nil, TO_CHAR)
					act(libc.CString("@GAs the last of the ink fills the bottle @g$n@G infuses a final burst of ki into the bottle.@n "), 1, ch, filled, nil, TO_ROOM)
				} else {
...
func do_runic(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if (func() *char_data {
		...
		act(libc.CString("@BYou dip your brush into the ink, but as you infuse your ki you balance the flow wrong and end up destroying the ink bottle!@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@b$n@B dips $s runic brush into a bottle filled with shimmering ink. @b$n@B appears to concentrate for a moment before a look of panic dons $s face. Just at that moment the bottle of ink explodes! Strange...@n"), 1, ch, nil, nil, TO_ROOM)
		extract_obj(bottle)
		...
		ch.Mana -= cost
		act(libc.CString("@BYou dip your brush into the ink, but as you infuse your ki you balance the flow wrong and end up evaporating some ink!@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@b$n@B dips $s runic brush into a bottle filled with shimmering ink. @b$n@B appears to concentrate for a moment before some ink evaporates. Strange...@n"), 1, ch, nil, nil, TO_ROOM)
		improve_skill(ch, SKILL_RUNIC, 1)
		...
		if vict == ch {
			ch.Mana -= cost
			act(libc.CString("@BYou dip your brush into the ink and infuse your ki skillfully into it. You pull the brush out and paint the @D'@CKenaz@D'@B rune on your skin!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@b$n@B dips $s brush into a bottle of ink and at the same time the ink starts to glow. Skillfully $e then writes the @D'@CKenaz@D'@B rune on $s skin.@n"), 1, ch, nil, nil, TO_ROOM)
			...
			ch.Mana -= cost
			act(libc.CString("@BYou dip your brush into the ink and infuse your ki skillfully into it. You pull the brush out and paint the @D'@CKenaz@D'@B rune on @b$N's@B skin!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@b$n@B dips $s brush into a bottle of ink and at the same time the ink starts to glow. Skillfully $e then writes the @D'@CKenaz@D'@B rune on @RYOUR@B skin.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@b$n@B dips $s brush into a bottle of ink and at the same time the ink starts to glow. Skillfully $e then writes the @D'@CKenaz@D'@B rune on @b$N's@B skin.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			send_to_char(ch, libc.CString("@D[@B%d@b ink used.@D]@n\r\n"), inkcost)
			var duration int = int(float64(skill) * 0.16)
			if duration < 1 {
				duration = 1
		...
		if (bottle.Value[6]) < inkcost {
			...
			ch.Mana -= cost
			act(libc.CString("@BYou dip your brush into the ink and infuse your ki skillfully into it. You pull the brush out and paint the @D'@CAlgiz@D'@B rune on your skin!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@b$n@B dips $s brush into a bottle of ink and at the same time the ink starts to glow. Skillfully $e then writes the @D'@CAlgiz@D'@B rune on $s skin.@n"), 1, ch, nil, nil, TO_ROOM)
			send_to_char(ch, libc.CString("@D[@B%d@b ink used.@D]@n\r\n"), inkcost)
			...
			ch.Mana -= cost
			act(libc.CString("@BYou dip your brush into the ink and infuse your ki skillfully into it. You pull the brush out and paint the @D'@CAlgiz@D'@B rune on @b$N's@B skin!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@b$n@B dips $s brush into a bottle of ink and at the same time the ink starts to glow. Skillfully $e then writes the @D'@CAlgiz@D'@B rune on @RYOUR@B skin.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@b$n@B dips $s brush into a bottle of ink and at the same time the ink starts to glow. Skillfully $e then writes the @D'@CAlgiz@D'@B rune on @b$N's@B skin.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			send_to_char(ch, libc.CString("@D[@B%d@b ink used.@D]@n\r\n"), inkcost)
			var duration int = int(float64(skill) * 0.05)
			if duration < 1 {
				duration = 1
		...
		if (bottle.Value[6]) < inkcost {
			...
			ch.Mana -= cost
			act(libc.CString("@BYou dip your brush into the ink and infuse your ki skillfully into it. You pull the brush out and paint the @D'@COagaz@D'@B rune on your skin!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@b$n@B dips $s brush into a bottle of ink and at the same time the ink starts to glow. Skillfully $e then writes the @D'@COagaz@D'@B rune on $s skin.@n"), 1, ch, nil, nil, TO_ROOM)
			send_to_char(ch, libc.CString("@D[@B%d@b ink used.@D]@n\r\n"), inkcost)
		...
		if (bottle.Value[6]) < inkcost {
			...
			ch.Mana -= cost
			act(libc.CString("@BYou dip your brush into the ink and infuse your ki skillfully into it. You pull the brush out and paint the @D'@CLaguz@D'@B rune on your skin!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@b$n@B dips $s brush into a bottle of ink and at the same time the ink starts to glow. Skillfully $e then writes the @D'@CLaguz@D'@B rune on $s skin.@n"), 1, ch, nil, nil, TO_ROOM)
			send_to_char(ch, libc.CString("@D[@B%d@b ink used.@D]@n\r\n"), inkcost)
			var duration int = int(float64(skill) * 0.04)
			if duration < 1 {
				duration = 1
			...
			ch.Mana -= cost
			act(libc.CString("@BYou dip your brush into the ink and infuse your ki skillfully into it. You pull the brush out and paint the @D'@COagaz@D'@B rune on @b$N's@B skin!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@b$n@B dips $s brush into a bottle of ink and at the same time the ink starts to glow. Skillfully $e then writes the @D'@COagaz@D'@B rune on @RYOUR@B skin.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@b$n@B dips $s brush into a bottle of ink and at the same time the ink starts to glow. Skillfully $e then writes the @D'@COagaz@D'@B rune on @b$N's@B skin.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			send_to_char(ch, libc.CString("@D[@B%d@b ink used.@D]@n\r\n"), inkcost)
			var duration int = int(float64(skill) * 0.04)
			if duration < 1 {
				duration = 1
		...
		if (bottle.Value[6]) < inkcost {
			...
			ch.Mana -= cost
			act(libc.CString("@BYou dip your brush into the ink and infuse your ki skillfully into it. You pull the brush out and paint the @D'@CWunjo@D'@B rune on your skin!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@b$n@B dips $s brush into a bottle of ink and at the same time the ink starts to glow. Skillfully $e then writes the @D'@CWunjo@D'@B rune on $s skin.@n"), 1, ch, nil, nil, TO_ROOM)
			send_to_char(ch, libc.CString("@D[@B%d@b ink used.@D]@n\r\n"), inkcost)
			...
			ch.Mana -= cost
			act(libc.CString("@BYou dip your brush into the ink and infuse your ki skillfully into it. You pull the brush out and paint the @D'@CWunjo@D'@B rune on @b$N's@B skin!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@b$n@B dips $s brush into a bottle of ink and at the same time the ink starts to glow. Skillfully $e then writes the @D'@CWunjo@D'@B rune on @RYOUR@B skin.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@b$n@B dips $s brush into a bottle of ink and at the same time the ink starts to glow. Skillfully $e then writes the @D'@CWunjo@D'@B rune on @b$N's@B skin.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			send_to_char(ch, libc.CString("@D[@B%d@b ink used.@D]@n\r\n"), inkcost)
			var duration int = int(float64(skill) * 0.08)
			if duration < 1 {
				duration = 1
		...
		if (bottle.Value[6]) < inkcost {
			...
			ch.Mana -= cost
			act(libc.CString("@BYou dip your brush into the ink and infuse your ki skillfully into it. You pull the brush out and paint the @D'@CPurisaz@D'@B rune on your skin!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@b$n@B dips $s brush into a bottle of ink and at the same time the ink starts to glow. Skillfully $e then writes the @D'@CPurisaz@D'@B rune on $s skin.@n"), 1, ch, nil, nil, TO_ROOM)
			send_to_char(ch, libc.CString("@D[@B%d@b ink used.@D]@n\r\n"), inkcost)
			...
			ch.Mana -= cost
			act(libc.CString("@BYou dip your brush into the ink and infuse your ki skillfully into it. You pull the brush out and paint the @D'@CPurisaz@D'@B rune on @b$N's@B skin!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@b$n@B dips $s brush into a bottle of ink and at the same time the ink starts to glow. Skillfully $e then writes the @D'@CPurisaz@D'@B rune on @RYOUR@B skin.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@b$n@B dips $s brush into a bottle of ink and at the same time the ink starts to glow. Skillfully $e then writes the @D'@CPUrisaz@D'@B rune on @b$N's@B skin.@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			send_to_char(ch, libc.CString("@D[@B%d@b ink used.@D]@n\r\n"), inkcost)
			var duration int = int(float64(skill) * 0.06)
			send_to_char(vict, libc.CString("@GYou feel as if your inner energy is more potent! @D(@WLasts@D: @w%d@D)@n\r\n"), duration)
			if duration < 1 {
		...
		if (bottle.Value[6]) < inkcost {
			...
			ch.Mana -= cost
			act(libc.CString("@BYou dip your brush into the ink and infuse your ki skillfully into it. You pull the brush out and paint the @D'@CGebo@D'@B rune on your skin! The rune flashes out of existence immediately!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@b$n@B dips $s brush into a bottle of ink and at the same time the ink starts to glow. Skillfully $e then writes the @D'@CGebo@D'@B rune on $s skin. The rune flashes out of existence immediately!@n"), 1, ch, nil, nil, TO_ROOM)
			send_to_char(ch, libc.CString("@D[@B%d@b ink used.@D]@n\r\n"), inkcost)
			...
			ch.Mana -= cost
			act(libc.CString("@BYou dip your brush into the ink and infuse your ki skillfully into it. You pull the brush out and paint the @D'@CGebo@D'@B rune on @b$N's@B skin! The rune flashes out of existence immediately!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@b$n@B dips $s brush into a bottle of ink and at the same time the ink starts to glow. Skillfully $e then writes the @D'@CGebo@D'@B rune on @RYOUR@B skin. The rune flashes out of existence immediately!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@b$n@B dips $s brush into a bottle of ink and at the same time the ink starts to glow. Skillfully $e then writes the @D'@CGebo@D'@B rune on @b$N's@B skin. The rune flashes out of existence immediately!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			send_to_char(ch, libc.CString("@D[@B%d@b ink used.@D]@n\r\n"), inkcost)
			vict.Player_specials.Class_skill_points[vict.Chclass] += 125
			send_to_char(vict, libc.CString("@GYou feel like you've just gained a lot of knowledge. Now if only you could apply it. @D[@m+125 PS@D]@n\r\n"))
			bottle.Value[6] -= inkcost
			...
			if GET_OBJ_VNUM(obj) == 1306 {
				if axion_dice(0) > int(ch.Aff_abils.Con) {
					if int(ch.Race) != RACE_ANDROID && int(ch.Race) != RACE_DEMON && int(ch.Race) != RACE_ICER {
						...
						act(libc.CString("@RYou choke on the the burning hot @Da@Ws@wh@Dc@Wl@wo@Du@Wd@R!@n"), 1, ch, nil, nil, TO_CHAR)
						act(libc.CString("@r$n@R chokes on the burning hot @Da@Ws@wh@Dc@Wl@wo@Du@Wd@R!@n"), 1, ch, nil, nil, TO_ROOM)
					}
					if int(ch.Race) != RACE_ANDROID && int(ch.Race) != RACE_DEMON && !IS_NPC(ch) {
						if !PLR_FLAGGED(ch, PLR_EYEC) && !AFF_FLAGGED(ch, AFF_BLIND) {
							reveal_hiding(ch, 0)
							act(libc.CString("@DYour eyes sting from the hot ash! You can't see!@n"), 1, ch, nil, nil, TO_CHAR)
							act(libc.CString("@r$n@D eyes appear to have been hurt by the ash!@n"), 1, ch, nil, nil, TO_ROOM)
							var duration int = 1
...
func do_ashcloud(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if ch.Mana < cost {
		...
		reveal_hiding(ch, 0)
		act(libc.CString("@RYou take a handful of ashes, and when you go to blow flames at it you lose focus. The ashes are blown from your hands by your huge gust of breath.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@r$n@R takes a handful of ashes from $s belongings and blows it out of $s hands with a strong gust of air. @YStrange.@n"), 1, ch, nil, nil, TO_ROOM)
		extract_obj(ash)
		ch.Mana -= cost
		return
	} else {
		...
		if level == 3 {
			ch.Mana -= cost
			act(libc.CString("@RYou take a handful of ashes and you create a fierce heat within your lungs. With the heat ready you breathe ki infused flames at the pile of ashes! The flames and ashes mix and fill the surrounding area with a hot burning ash!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@r$n@R takes a handful of ashes and $e breathes ki infused flames at the pile of ashes! The flames and ashes mix and fill the surrounding area with a hot burning ash!@n"), 1, ch, nil, nil, TO_ROOM)
			...
			if !wearable_obj(obj) {
				...
				if ch.Move < obj.Weight+ch.Max_move/40 {
					...
					if obj.Size == SIZE_SMALL {
						...
						act(libc.CString("@WYou carefully adjust the size of @c$p@W.@n"), 1, ch, obj, nil, TO_CHAR)
						act(libc.CString("@C$n@W carefully adjusts the size of @c$p@W.@n"), 1, ch, obj, nil, TO_ROOM)
						obj.Size = SIZE_SMALL
...
func do_healglow(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if ch.Mana < cost {
		...
		if vict == ch {
			act(libc.CString("@CPlacing your hands on your body you begin to focus your energies. Slowly a strong blue glow glistens and shines across your skin!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@c$n@C places $s hands on $s body. Slowly a strong blue glow glistens and shines across $s skin!@n"), 1, ch, nil, nil, TO_ROOM)
			...
			act(libc.CString("@CPlacing your hands on @c$N's@C body you begin to focus your energies. Slowly a strong blue glow glistens and shines across $S skin!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@c$n@C places $s hands on YOUR body. Slowly a strong blue glow glistens and shines across your skin!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@c$n@C places $s hands on @c$N's@C body. Slowly a strong blue glow glistens and shines across $S skin!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			var duration int = int(float64(GET_SKILL(ch, SKILL_HEALGLOW)) * 0.1)
...
func do_amnisiac(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if cost > int(ch.Mana) {
		...
		act(libc.CString("@WYou attempt to grab @C$N@W to kiss $M, but $E evades!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@M$n@W attempts to grab you and leans in with puckered lips, but you managed to evade!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("@M$n@W attempts to grab @C$N@W and leans in with puckered lips, but $E manages to evade!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		ch.Mana -= int64(cost)
		return
	} else {
		act(libc.CString("@WYou reach out quickly, grabbing @C$N@W and pulling them in close to you. Just as quick, you pull their head forcefully towards yours, planting a deep and heavy kiss. The fool wobbles a bit, shocked.  It is unlikely that @c$E@W will be able to focus very well on that skill, not with the thought of your lips on their mind."), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(libc.CString("@M$n@W grabs you, giving you a deep, passionate kiss. Your mind is suddenly overwhelmed, and in your shock you seem to forget a few of your tricks."), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(libc.CString("@WYou see @C$n@W quickly grab @C$N@W, pulling them into a deep, almost passionate kiss. @C$N@W seems shocked, and wobbles a bit, grabbing at @c$s@W head once @C$n@W lets go."), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		ch.Mana -= int64(cost)
...
func do_shimmer(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if tar != nil {
		...
		ch.Mana -= cost
		act(libc.CString("@wYour body begins to fade away almost appearing ghost like, before a ripple passes through your image and your are gone in an instant!@n"), 1, ch, nil, unsafe.Pointer(tar), TO_CHAR)
		act(libc.CString("@w$n@w appears in an instant out of nowhere right next to you!@n"), 1, ch, nil, unsafe.Pointer(tar), TO_VICT)
		act(libc.CString("@w$n@w body begins to fade away almost appearing ghost like, before a ripple passes through $s image and $e is gone in an instant!@n"), 1, ch, nil, unsafe.Pointer(tar), TO_NOTVICT)
...
func do_channel(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if ruby != nil {
		if skill < chance {
			act(libc.CString("@RAs you move your ki through the lava you begin to draw heat away from it into the ruby. You screw up the rate of heating though and cause the ruby to crumble to dust!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@RAs $n@R moves $s ki through the lava $e begins to draw heat away from it into a blood ruby. However $e screws up the rate of heating and causes the ruby to crumble to dust!@n"), 1, ch, nil, nil, TO_ROOM)
			extract_obj(ruby)
		} else {
			act(libc.CString("@RAs you move your ki through the lava you begin to draw heat away from it into the ruby. You do so at an even rate and end up with a glowing red hot blood ruby!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@RAs $n@R moves $s ki through the lava $e begins to draw heat away from it into a blood ruby. The ruby glows red hot as $e finishes the process of channeling the heat!@n"), 1, ch, nil, nil, TO_ROOM)
...
func do_hydromancy(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if libc.StrCaseCmp(&arg[0], libc.CString("spike")) == 0 {
		...
		if skill < chance {
			ch.Mana -= cost
			act(libc.CString("@CYou press your palms together in front of your body but you fail to produce the proper control to form the spike!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@c$n@C presses $s palms together and then slowly pulls them apart. Nothing important appears to have happened."), 1, ch, nil, nil, TO_ROOM)
		...
		ch.Mana -= cost
		act(libc.CString("@CYou press your palms together in front of your body and focusing ki you force water up along your body. That water pools between your palms and as pull your palms apart a @c$p@C forms!@n"), 1, ch, obj, nil, TO_CHAR)
		act(libc.CString("@c$n@C presses $s palms together in front of $s body and water begins to flow up $s body and pools between $s palms. Slowly pulling them apart reveals a @c$p@C as it forms between them!@n"), 1, ch, obj, nil, TO_VICT)
		if obj.Weight+int64(gear_weight(ch)) <= max_carry_weight(ch) {
			...
			act(libc.CString("@C$n@w drops an ice spike.@n"), 1, ch, nil, nil, TO_ROOM)
		...
		attempt = search_block(&arg2[0], &dirs[0], 0)
		if CAN_GO(ch, attempt) {
			...
			var bunn [64936]byte
			if skill < chance {
				act(libc.CString("@BUsing your ki you attempt to create a rush of water! @RYou fail!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@b$n@B seems to attempt to create water with $s ki! @RHowever, $e fails!@n"), 1, ch, nil, nil, TO_ROOM)
				ch.Mana -= cost
				WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
			} else {
				ch.Mana -= cost
				...
				act(&bun[0], 1, ch, nil, nil, TO_CHAR)
				act(&bunn[0], 1, ch, nil, nil, TO_ROOM)
				for vict = world[ch.In_room].People; vict != nil; vict = next_v {
					next_v = vict.Next_in_room
					if vict == ch {
						continue
					...
					if !can_kill(ch, vict, nil, 1) {
						act(libc.CString("@CYou are protected from the water!@n"), 1, vict, nil, nil, TO_VICT)
						act(libc.CString("@C$n@C is protected from the water!@n"), 1, vict, nil, nil, TO_ROOM)
					} else if int(vict.Race) == RACE_KANASSAN {
						act(libc.CString("@CYou effortlessly swim against the current.@n"), 1, vict, nil, nil, TO_CHAR)
						act(libc.CString("@C$n@C effortlessly swims against the current.@n"), 1, vict, nil, nil, TO_ROOM)
					} else if int(vict.Skills[SKILL_BALANCE]) >= axion_dice(-10) {
						act(libc.CString("@CYou manage to keep your balance and are not swept away!@n"), 1, vict, nil, nil, TO_CHAR)
						act(libc.CString("@C$n@C manages to keep $s balance and is not swept away!@n"), 1, vict, nil, nil, TO_ROOM)
					} else if AFF_FLAGGED(ch, AFF_FLYING) {
						act(libc.CString("@CYou fly above the rushing waters and are untouched.@n"), 1, vict, nil, nil, TO_CHAR)
						act(libc.CString("@C$n@C flies above the rushing waters and is untouched.@n"), 1, vict, nil, nil, TO_ROOM)
					} else {
						act(libc.CString("@cYou are caught by the rushing waters and sent tumbling away!@n"), 1, vict, nil, nil, TO_CHAR)
						act(libc.CString("@c$n@c is caught by the rushing waters and sent tumbling away!@n"), 1, vict, nil, nil, TO_ROOM)
...
func do_kanso(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if skill < dice {
		...
		if skill > pdice && !AFF_FLAGGED(vict, AFF_PARA) {
			act(libc.CString("@R$N@W is paralyzed by the attack!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@RYou are paralyzed by the attack!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@R$N@Wis paralyzed by the attack!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
...
func do_instill(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if OBJ_FLAGGED(obj, ITEM_SLOTS_FILLED) {
		...
		act(libc.CString("@GYou instill the token into @g$p@G. It glows @ggreen@G for a moment before returning to normal. The token disappears with the glow.@n"), 1, ch, obj, nil, TO_CHAR)
		act(libc.CString("@g$n@G instills a token into @g$p@G. It glows @ggreen@G for a moment before returning to normal. The token disappears with the glow.@n"), 1, ch, obj, nil, TO_ROOM)
...
func do_hayasa(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if ch.Mana < cost {
		...
		ch.Mana -= cost
		act(libc.CString("@CYou close your eyes for a brief moment and focus your ki around your body as a soft blue glow. The glow disappears though as you fail to maintain the effect...@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@c$n@C closes $s eyes for a brief moment and a soft blue glow begins to form around $s body. The glow disappears a second later though and $e frowns.@n"), 1, ch, nil, nil, TO_ROOM)
		...
		reveal_hiding(ch, 0)
		act(libc.CString("@CYou close your eyes for a brief moment and focus your ki around your body as a soft blue glow. All your movements are faster now!@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@c$n@C closes $s eyes for a brief moment and a soft blue glow begins to form around $s body. The glow pulsates gently as $e opens his eyes and smiles.@n"), 1, ch, nil, nil, TO_ROOM)
		improve_skill(ch, SKILL_HAYASA, 1)
		WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
	}
}
	...
	if libc.StrCaseCmp(&arg[0], libc.CString("bury")) == 0 {
		if arg2[0] == 0 {
			...
			if SECT(ch.In_room) != SECT_DESERT {
				act(libc.CString("@yYou start digging in a spot of soft dirt. Once you have an appropriately sized hole you drop @G$p@y in and then cover it.@n"), 1, ch, obj, nil, TO_CHAR)
				act(libc.CString("@C$n@y starts digging in a spot of soft dirt. Once $e has an appropriately sized hole $e drops @G$p@y in and then covers it.@n"), 1, ch, obj, nil, TO_ROOM)
			} else {
				act(libc.CString("@YYou start digging in a spot of soft sand. Once you have an appropriately sized hole you drop @G$p@Y in and then cover it.@n"), 1, ch, obj, nil, TO_CHAR)
				act(libc.CString("@C$n@Y starts digging in a spot of soft sand. Once $e has an appropriately sized hole $e drops @G$p@Y in and then covers it.@n"), 1, ch, obj, nil, TO_ROOM)
			}
			obj_from_char(obj)
			obj_to_room(obj, ch.In_room)
		...
		if fobj == nil {
			...
			if SECT(ch.In_room) != SECT_DESERT {
				act(libc.CString("@yYou slowly dig and reveal @G$p@y buried in the dirt! You pull it out and set it on the ground before covering the hole back up.@n"), 1, ch, fobj, nil, TO_CHAR)
				act(libc.CString("@C$n@y starts digging and shortly reveals @G$p@y buried in the dirt! Quickly $e pulls it out and sets it on the ground before covering the hole back up.@n"), 1, ch, fobj, nil, TO_ROOM)
			} else {
				act(libc.CString("@YYou slowly dig and reveal @G$p@Y buried in the sand! You pull it out and set it on the ground before covering the hole back up.@n"), 1, ch, fobj, nil, TO_CHAR)
				act(libc.CString("@C$n@Y starts digging and shortly reveals @G$p@Y buried in the sand! Quickly $e pulls it out and sets it on the ground before covering the hole back up.@n"), 1, ch, fobj, nil, TO_ROOM)
			}
...
func do_arena(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if arg[0] == 0 {
		...
		if int(libc.BoolToInt(GET_ROOM_VNUM(ch.In_room))) == 0x45D3 {
			...
			for d = descriptor_list; d != nil; d = d.Next {
				...
				if IN_ARENA(d.Character) {
					...
					act(&buf[0], 1, ch, nil, unsafe.Pointer(d.Character), TO_CHAR)
					found = 1
				}
			}
			if found == 0 {
		...
		if num < 0 {
			...
			if found == 1 {
				act(libc.CString("@wYou start watching the action surrounding that particular fighter in the arena.@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n@w starts watching the action in the arena.@n"), 1, ch, nil, nil, TO_ROOM)
...
func do_ensnare(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if weave == nil {
		...
		if (func() *char_data {
			...
			act(libc.CString("@WYou unwind your bundle of silk and grab a loose end of it. Splitting that end to reveal the sticky innards of the strand you swing the strand at @c$N@W! Unfortunately you miss and lose the bundle...@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@C$n@W unwinds a bundle of silk and grabs a loose end of it. Splitting that end to reveal the sticky innards of the strand $e swings the strand at YOU! Fortunately $e misses and loses the bundle...@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@C$n@W unwinds a bundle of silk and grabs a loose end of it. Splitting that end to reveal the sticky innards of the strand $e swings the strand at @c$N@W! Fortunately $e misses and loses the bundle...@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		} else if AFF_FLAGGED(vict, AFF_ZANZOKEN) && !AFF_FLAGGED(ch, AFF_ZANZOKEN) {
			act(libc.CString("@WYou unwind your bundle of silk and grab a loose end of it. Splitting that end to reveal the sticky innards of the strand you swing the strand at @c$N@W! Unfortunately @c$N@W zanzokens away avoiding it and you lose the bundle...@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@C$n@W unwinds a bundle of silk and grabs a loose end of it. Splitting that end to reveal the sticky innards of the strand $e swings the strand at YOU! Fortunately you zanzoken away avoiding it and @C$n@W loses the bundle...@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@C$n@W unwinds a bundle of silk and grabs a loose end of it. Splitting that end to reveal the sticky innards of the strand $e swings the strand at @c$N@W! Fortunately @c$N@W zanzokens away avoiding it and @C$n@W loses the bundle...@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			REMOVE_BIT_AR(vict.Affected_by[:], AFF_ZANZOKEN)
		} else if AFF_FLAGGED(vict, AFF_ZANZOKEN) && AFF_FLAGGED(ch, AFF_ZANZOKEN) {
			if GET_SPEEDI(ch)+rand_number(1, 100) < GET_SPEEDI(vict)+rand_number(1, 100) {
				act(libc.CString("@WYou unwind your bundle of silk and grab a loose end of it. Splitting that end to reveal the sticky innards of the strand you swing the strand at @c$N@W! You both zanzoken! Unfortunately @c$N@W manages to avoid it and you lose the bundle...@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@C$n@W unwinds a bundle of silk and grabs a loose end of it. Splitting that end to reveal the sticky innards of the strand $e swings the strand at YOU! You both zanzoken! Fortunately you manage to avoid it and @C$n@W loses the bundle...@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@C$n@W unwinds a bundle of silk and grabs a loose end of it. Splitting that end to reveal the sticky innards of the strand $e swings the strand at @c$N@W! They both zanzoken! Fortunately @c$N@W manages to avoid it and @C$n@W loses the bundle...@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
				REMOVE_BIT_AR(vict.Affected_by[:], AFF_ZANZOKEN)
				REMOVE_BIT_AR(ch.Affected_by[:], AFF_ZANZOKEN)
			} else {
				act(libc.CString("@WYou unwind your bundle of silk and grab a loose end of it. Splitting that end to reveal the sticky innards of the strand you swing the strand at @c$N@W! Fortunately you manage to hit $M! You both zanzoken! Quickly you spin around $M and ensnare $S arms with the silk!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
				act(libc.CString("@C$n@W unwinds a bundle of silk and grabs a loose end of it. Splitting that end to reveal the sticky innards of the strand $e swings the strand at YOU! Unfortunately $e manages to hit YOU! You both zanzoken! Quickly $e spins around you and ensnares your arms with the silk!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
				act(libc.CString("@C$n@W unwinds a bundle of silk and grabs a loose end of it. Splitting that end to reveal the sticky innards of the strand $e swings the strand at @c$N@W! Unfortunately $e manages to hit $M! They both zanzoken! Quickly $e spins around @c$N@W and ensnares $S arms with the silk!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
			...
			act(libc.CString("@WYou unwind your bundle of silk and grab a loose end of it. Splitting that end to reveal the sticky innards of the strand you swing the strand at @c$N@W! Unfortunately @c$N@W manages to avoid it and you lose the bundle...@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("@C$n@W unwinds a bundle of silk and grabs a loose end of it. Splitting that end to reveal the sticky innards of the strand $e swings the strand at YOU! Fortunately you manage to avoid it and @C$n@W loses the bundle...@n"), 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("@C$n@W unwinds a bundle of silk and grabs a loose end of it. Splitting that end to reveal the sticky innards of the strand $e swings the strand at @c$N@W! Fortunately @c$N@W manages to avoid it and @C$n@W loses the bundle...@n"), 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		} else {
			act(libc.CString("@WYou unwind your bundle of silk and grab a loose end of it. Splitting that end to reveal the sticky innards of the strand you swing the strand at @c$N@W! Fortunately you manage to hit $M! Quickly you spin around $M and ensnare $S arms with the silk!@n"), 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
...
func valid_silk(obj *obj_data) bool {
	...
	if libc.StrCaseCmp(&arg[0], libc.CString("weave")) == 0 {
		...
		if obj == nil {
			...
			if libc.StrCaseCmp(&arg2[0], libc.CString("head")) == 0 {
				if prob <= perc {
					act(libc.CString("@WYou attempt to weave $p@W into the desired piece but end up ruining the entire bundle instead.@n"), 1, ch, obj, nil, TO_CHAR)
					act(libc.CString("@C$n@W attempts to weave $p@W into some type of clothing but ends up ruining the entire bundle instead.@n"), 1, ch, obj, nil, TO_ROOM)
					extract_obj(obj)
					...
					act(libc.CString("@WYou attempt to weave the bundle and manage to create $p@W!@n"), 1, ch, weaved, nil, TO_CHAR)
					act(libc.CString("@C$n@W attempts to weave a bundle into something and manages to create $p@W!@n"), 1, ch, weaved, nil, TO_ROOM)
		...
		if ch.Mana < cost {
			...
			if super == 1 {
				...
				obj_to_room(obj, ch.In_room)
				act(libc.CString("@YYou concentrate your ki into your silk sacs and begin to spit silk out of your mouth. You gently weave the silk and in no time at all you have a $p@Y piled at your feet!@n"), 1, ch, obj, nil, TO_CHAR)
				send_to_char(ch, libc.CString("@YIt's SUPER grand!@n\r\n"))
				act(libc.CString("@C$n@W seems to concentrate for a moment before spitting out a golden colored silk from $s mouth. Gently $e weaves the silk and in no time at all $e has a $p@W piled at $s feet!@n"), 1, ch, obj, nil, TO_ROOM)
				ch.Mana -= cost
			} else if prob > perc && prob >= 100 {
				obj = read_object(16700, VIRTUAL)
				obj_to_room(obj, ch.In_room)
				act(libc.CString("@WYou concentrate your ki into your silk sacs and begin to spit silk out of your mouth. You gently weave the silk and in no time at all you have a $p@W piled at your feet!@n"), 1, ch, obj, nil, TO_CHAR)
				act(libc.CString("@C$n@W seems to concentrate for a moment before spitting out a golden colored silk from $s mouth. Gently $e weaves the silk and in no time at all $e has a $p@W piled at $s feet!@n"), 1, ch, obj, nil, TO_ROOM)
				ch.Mana -= cost
				...
				act(libc.CString("@WYou concentrate your ki into your silk sacs and begin to spit silk out of your mouth. You end up making a poorly formed puddle of goo...@n"), 1, ch, obj, nil, TO_CHAR)
				act(libc.CString("@C$n@W seems to concentrate for a moment before spitting out a poorly formed puddle of goo...@n"), 1, ch, obj, nil, TO_ROOM)
				ch.Mana -= cost
	...
	if int(ch.Race) != RACE_ARLIAN && (int(ch.Race) != RACE_BIO || int(ch.Race) == RACE_BIO && ((ch.Genome[0]) != 6 && (ch.Genome[1]) != 6)) {
		...
		if arg[0] == 0 || arg2[0] == 0 {
			...
			if libc.StrCaseCmp(&arg[0], libc.CString("pl")) == 0 || libc.StrCaseCmp(&arg[0], libc.CString("PL")) == 0 {
				act(libc.CString("@GYou focus your mind and begin to overwork your powerful adrenal glands and your wounds begin to heal!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@g$n@G seems to concentrate and $s wounds begin to heal!@n"), 1, ch, nil, nil, TO_ROOM)
				...
				act(libc.CString("@GYou focus your mind and begin to overwork your powerful adrenal glands and you feel your ki replenish!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@g$n@G seems to concentrate and $e appears energized!@n"), 1, ch, nil, nil, TO_ROOM)
...
func valid_recipe(ch *char_data, recipe int, type_ int) bool {
	var (
		tomato    int = -1
		cucumber  int = -1
		onion     int = -1
...
func campfire_cook(recipe int) bool {
	switch recipe {
	case RECIPE_STEAK:
		fallthrough
	case RECIPE_GRILLED_NORMFISH:
...
func do_cook(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if arg[0] == 0 {
		...
		if !valid_recipe(ch, recipe, 0) {
			return
		} else if cook_element(ch.In_room) == 1 && !campfire_cook(recipe) {
		...
		if pass == 1 {
			if skill < prob {
				act(libc.CString("@wYou screw up the preparation of the recipe and end up wasting the ingredients!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@C$n@w starts to prepare some food, but ends up ruining the ingredients instead!@n"), 1, ch, nil, nil, TO_ROOM)
			...
			act(libc.CString("@wYou carefully prepare the ingredients and then start cooking them. After a while of patience  and skillful care you successfully make @D'@C$p@D'@w!@n"), 1, ch, meal, nil, TO_CHAR)
			act(libc.CString("@C$n@w carefully prepares some ingredients and starts cooking them. After a while of patience and skillful care $e succeeds in making @D'@C$p@D'@w!@n"), 1, ch, meal, nil, TO_ROOM)
			improve_skill(ch, SKILL_COOKING, 0)
			if psbonus > 0 {
...
func do_fireshield(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	var prob int = axion_dice(0)
	if skill <= prob {
		act(libc.CString("@WYou hold your hands up in front of you on either side and try to summon defensive @rf@Rl@Ya@rm@Re@Ys@W to cover your body. Yet you screw up and the technique fails!@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@c$n@W holds $s hands up in front of $m on either side and tries to summon defensive @rf@Rl@Ya@rm@Re@Ys@W to cover $s body. Yet $e seems to screw up and the technique fails!@n"), 1, ch, nil, nil, TO_ROOM)
		improve_skill(ch, SKILL_FIRESHIELD, 0)
		ch.Mana -= cost
		return
	} else {
		act(libc.CString("@WYou hold your hands up in front of you on either side and try to summon defensive @rf@Rl@Ya@rm@Re@ys@W to cover your body. The ki you have gathered pours out of your body and creates intense black @rf@Rl@Ya@rm@Re@Ys@W that cover your entire body in a protective layer!"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@c$n@W holds $s hands up in front of $m on either side and tries to summon defensive @rf@Rl@Ya@rm@Re@ys@W to cover $s body. The ki $e has gathered pours out of $s body and creates intense black @rf@Rl@Ya@rm@Re@Ys@W that cover $s entire body in a protective layer!"), 1, ch, nil, nil, TO_ROOM)
		improve_skill(ch, SKILL_FIRESHIELD, 0)
		ch.Mana -= cost
...
func do_warppool(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if libc.StrCaseCmp(libc.CString("earth"), &arg[0]) == 0 && ROOM_FLAGGED(ch.In_room, ROOM_EARTH) {
		...
		if prob > perc {
			act(libc.CString("@CYou reach your hand out and begin to swirl nearby water with it. At the same time you release ki into the water and focus your mind on sensing out the distant body of water you wish to travel to. You lose your concentration and the ritual fails!@n"), 1, ch, nil, nil, TO_CHAR)
			act(libc.CString("@c$n@C reaches $s hand out and begins to swirl nearby water with it. The water that is being swirled begins to glow @wbright@B blue@C and has a distinct separation from the rest of the waters. Suddenly a puzzled look comes across @c$n's @Cface and the water returns to normal.@n"), 1, ch, nil, nil, TO_ROOM)
			ch.Mana -= int64(cost)
			...
			act(libc.CString("@c$n@C reaches $s hand out and begins to swirl nearby water with it. The water that is being swirled begins to glow @wbright@B blue@C and has a distinct separation from the rest of the waters. Suddenly @c$n@C vanishes into this water! A moment later the waters return to normal.@n"), 1, ch, nil, nil, TO_ROOM)
			improve_skill(ch, SKILL_WARP, 1)
			char_from_room(ch)
			char_to_room(ch, real_room(850))
			act(libc.CString("@CSuddenly a large whirlpool of flashing water begins to form nearby. After a few seconds @c$n@C pops out of the center of the pool! The water then return to normal a moment laterr...@n"), 1, ch, nil, nil, TO_ROOM)
			ch.Mana -= int64(cost)
		}
...
func do_obstruct(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if (world[ch.In_room].Dir_option[dir]) == nil {
		...
		act(libc.CString("@CYou channel your ki and start to create a wall of water, but lose your concentration and the water promptly disappears.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@c$n@C channels $s ki and starts to create a wall of water, but loses $s concentration and the water promptly disappears.@n"), 1, ch, nil, nil, TO_ROOM)
		ch.Mana -= int64(cost)
		...
		var (
			obj     *obj_data
			newroom int = world[ch.In_room].Dir_option[dir].To_room
		)
		...
		for obj = world[newroom].Contents; obj != nil; obj = obj.Next_content {
			if GET_OBJ_VNUM(obj) == 79 {
				if obj.Cost == dir2 {
					if skill < prob {
						act(libc.CString("@CYou place your hands on the glacial wall and concentrate. You fail to undo the composition of the wall!@n"), 1, ch, nil, nil, TO_CHAR)
						act(libc.CString("@c$n@C places $s hands on the glacial wall and concentrates. Nothing happens...@n"), 1, ch, nil, nil, TO_ROOM)
						ch.Mana -= int64(cost / 2)
					} else {
						act(libc.CString("@CYou place your hands on the glacial wall and concentrate. You unfreeze the wall and evaporate the water effortlessly.@n"), 1, ch, nil, nil, TO_CHAR)
						act(libc.CString("@c$n@C places $s hands on the glacial wall and concentrates. Suddenly the wall melts and then evaporates!@n"), 1, ch, nil, nil, TO_ROOM)
						ch.Mana -= int64(cost / 2)
						extract_obj(obj)
					}
					return
		...
		act(libc.CString("@CYou concentrate and channel your ki. A wall of water starts to form in such a way to block off the direction of your choice. As the wall becomes complete it freezes solid by your will!@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@c$n@C concentrates and channels $s ki. A wall of water starts to form in such a way to block off one of the directions of this area. As the wall becomes complete it freezes solid by @c$n's@C will!@n"), 1, ch, nil, nil, TO_ROOM)
		send_to_room(newroom, libc.CString("@cA wall of water forms slowly upward blocking off the %s direction. This wall of water then freezes instantly once it stops growing.@n\r\n"), dirs[dir2])
		improve_skill(ch, SKILL_HYOGA_KABE, 0)
		ch.Mana -= int64(cost)
		return
	}
...
func do_dimizu(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	var prob int = axion_dice(0)
	if world[ch.In_room].Geffect < 0 {
		act(libc.CString("@CYou concentrate and distabilie the water, separating the hydrogen and oxygen. The gases dissipate quickly."), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@c$n@C concentrates and the water filling the area seems to shudder. Suddenly the water begins to evaporate as the hydrogen and oxygen are separated."), 1, ch, nil, nil, TO_ROOM)
		world[ch.In_room].Geffect = 0
		WAIT_STATE(ch, (int(1000000/OPT_USEC))*1)
		return
		...
		act(libc.CString("@CYou gather your ki and concentrate on creating water from it. Water begins to flow upward around the entire area, but you lose your concentration and it all goes flooding away!@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@c$n@C gathers $s ki and concentrates on creating water from it. Water begins to flow upward around the entire area, but $e loses $s concentration and all the water goes flooding away!@n"), 1, ch, nil, nil, TO_ROOM)
		ch.Mana -= ch.Max_mana / 12
		improve_skill(ch, SKILL_DIMIZU, 0)
		return
	} else {
		act(libc.CString("@CYou gather your ki and concentrate on creating water from it. Water begins to flow upward around the entire area. You form the water into a perfect cube with barely any ripples in its walls. It will maintain this form for a while.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@c$n@C gathers $s ki and concentrates on creating water from it. Water begins to flow upward around the entire area. @c$n@C forms the water into a perfect cube with barely any ripples in its walls. It appears the water will maintain this form for a while.@n"), 1, ch, nil, nil, TO_ROOM)
		ch.Mana -= ch.Max_mana / 12
...
func do_beacon(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if AFF_FLAGGED(ch, AFF_SPIRIT) {
		...
		ch.Droom = int(libc.BoolToInt(GET_ROOM_VNUM(ch.In_room)))
		return
	}
}
func do_feed(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	act(libc.CString("@WYou take $p@W and pop it into @C$N@W's mouth!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_CHAR)
	act(libc.CString("@C$n@W takes $p@W and pops it into YOUR mouth!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_VICT)
	act(libc.CString("@C$n@W takes $p@W and pops it into @c$N@W's mouth!@n"), 1, ch, obj, unsafe.Pointer(vict), TO_NOTVICT)
...
func do_spoil(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if type_ == 0 {
		act(libc.CString("@C$n@W reaches down and @rtears@W the head off of @R$p@W!@n"), 1, ch, obj, nil, TO_ROOM)
		act(libc.CString("@WYou reach down and @rtear@W the head off of @R$p@W!@n"), 1, ch, obj, nil, TO_CHAR)
	} else if type_ == 1 {
		act(libc.CString("@C$n@W reaches down and @rcuts@W the head off of @R$p@W!@n"), 1, ch, obj, nil, TO_ROOM)
		act(libc.CString("@WYou reach down and @rcut@W the head off of @R$p@W!@n"), 1, ch, obj, nil, TO_CHAR)
	} else if type_ == 2 {
		act(libc.CString("@C$n@W reaches down and @rcuts@W the head off of @R$p@W!@n"), 1, ch, obj, nil, TO_ROOM)
		act(libc.CString("@WYou reach down and @rcut@W the head off of @R$p@W!@n"), 1, ch, obj, nil, TO_CHAR)
	}
