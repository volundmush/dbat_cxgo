...
func do_olc(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if (func() int {
		olc_mode = search_block(&mode_arg[0], &olc_modes[0], 0)
		return olc_mode
	}()) < 0 {
	...
	case OLC_ROOM_TYPE:
		if unicode.IsDigit(rune(*argument)) {
			...
			rnum = ch.In_room
			vnum = int(libc.BoolToInt(GET_ROOM_VNUM(ch.In_room)))
...
func olc_interpreter(targ unsafe.Pointer, mode int, arg *byte) {
	...
	if (func() int {
		command = search_block(&command_string[0], &olc_commands[0], 0)
		return command
	}()) < 0 {
...
func can_modify(ch *char_data, vnum int) bool {
	return true
}
...
func olc_bitvector(bv *int, names **byte, arg *byte) {
	...
	for *arg != 0 {
		...
		if (func() int {
			flagnum = search_block(this_name, names, 1)
			return flagnum
		}()) < 0 {
	...
	*bv = newbv
	// todo: fix this
	//sprintbit(uint32(int32(newbv)), ([]*byte)(names), &buf[0], uint64(64936))
