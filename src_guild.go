...
func calculate_skill_cost(ch *char_data, skill int) int {
	var cost int = 0
	if IS_SET(uint32(int32(spell_info[skill].Flags)), 1<<10) {
		cost = 8
	} else if IS_SET(uint32(int32(spell_info[skill].Flags)), 1<<11) {
		cost = 15
	} else if IS_SET(uint32(int32(spell_info[skill].Flags)), 1<<12) {
		if int(ch.Skills[skill]) == 0 {
			cost = 200
		} else {
			cost = 25
		}
	} else if IS_SET(uint32(int32(spell_info[skill].Flags)), 1<<13) {
		if int(ch.Skills[skill]) == 0 {
			cost = 300
		} else {
			cost = 40
...
func do_teach(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if vict.Master == nil {
		...
		act(&tochar[0], 1, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		act(&tovict[0], 1, ch, nil, unsafe.Pointer(vict), TO_VICT)
		act(&toother[0], 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
		for {
...
func print_skills_by_type(ch *char_data, buf *byte, maxsz int, sktype int, argument *byte) int {
	...
	for i = 1; i <= SKILL_TABLE_SIZE; i++ {
		...
		if arg[0] != 0 {
			if libc.Atoi(libc.GoString(&arg[0])) <= 0 && libc.StrStr(spell_info[i].Name, &arg[0]) == nil {
				known = 0
...
func is_guild_open(keeper *char_data, guild_nr int, msg int) bool {
	var buf [200]byte
	buf[0] = 0
...
func is_guild_ok_char(keeper *char_data, ch *char_data, guild_nr int) bool {
	var buf [200]byte
...
func is_guild_ok(keeper *char_data, ch *char_data, guild_nr int) bool {
	if is_guild_open(keeper, guild_nr, 1) {
		return is_guild_ok_char(keeper, ch, guild_nr)
	}
	return false
}
func does_guild_know(guild_nr int, i int) bool {
	return guild_index[guild_nr].Skills[i] != 0
}
func does_guild_know_feat(guild_nr int, i int) bool {
	return guild_index[guild_nr].Feats[i] != 0
}
func sort_spells() {
	var a int
...
func what_does_guild_know(guild_nr int, ch *char_data) {
	...
	for sortpos = 0; sortpos < SKILL_TABLE_SIZE; sortpos++ {
		i = sortpos
		if does_guild_know(guild_nr, i) && skill_type(i) == (1<<1) {
			for func() int {
				canknow = 0
				k = 0
				return func() int {
	...
	for sortpos = 1; sortpos <= NUM_FEATS_DEFINED; sortpos++ {
		...
		if does_guild_know_feat(guild_nr, i) && feat_is_available(ch, i, 0, nil) && feat_list[i].In_game != 0 && feat_list[i].Can_learn != 0 {
	...
	if config_info.Play.Enable_languages != 0 {
		...
		for sortpos = 0; sortpos < SKILL_TABLE_SIZE; sortpos++ {
			i = sortpos
			if does_guild_know(guild_nr, i) && IS_SET(uint32(int32(skill_type(i))), 1<<2) {
				for func() int {
					canknow = 0
					return func() int {
						j = 0
...
func prereq_pass(ch *char_data, snum int) bool {
	...
	} else if IS_SET(uint32(int32(spell_info[snum].Flags)), 1<<10) || IS_SET(uint32(int32(spell_info[snum].Flags)), 1<<11) {
	...
	} else if IS_SET(uint32(int32(spell_info[snum].Flags)), 1<<12) {
	...
	} else if IS_SET(uint32(int32(spell_info[snum].Flags)), 1<<13) {
...
func handle_practice(keeper *char_data, guild_nr int, ch *char_data, argument *byte, skill int) {
	...
	if IS_SET(uint32(int32(spell_info[skill_num].Skilltype)), 1<<1) {
		for func() int {
			learntype = 0
			return func() int {
				i = 0
...
func handle_exp(keeper *char_data, guild_nr int, ch *char_data, argument *byte) {
	...
	if ch.Exp > int64(level_exp(ch, GET_LEVEL(ch)+1)) && GET_LEVEL(ch) != 100 {
		...
		act(libc.CString("@c$n@W spends time training you in $s fighting style.@n"), 1, keeper, nil, unsafe.Pointer(ch), TO_VICT)
		act(libc.CString("@c$n@W spends time training @C$N@W in $s fighting style.@n"), 1, keeper, nil, unsafe.Pointer(ch), TO_NOTVICT)
...
func handle_study(keeper *char_data, guild_nr int, ch *char_data, argument *byte, unused int) {
	...
	ch.Gold -= goldcost
	ch.Player_specials.Class_skill_points[ch.Chclass] += 25
	act(libc.CString("@c$N@W spends time lecturing you on various subjects.@n"), 1, ch, nil, unsafe.Pointer(keeper), TO_CHAR)
	act(libc.CString("@c$N@W spends time lecturing @C$n@W on various subjects.@n"), 1, ch, nil, unsafe.Pointer(keeper), TO_ROOM)
...
func handle_learn(keeper *char_data, guild_nr int, ch *char_data, argument *byte) {
	...
	if !feat_is_available(ch, feat_num, 0, nil) || feat_list[feat_num].In_game == 0 || feat_list[feat_num].Can_learn == 0 {
	...
	case FEAT_GREATER_SPELL_FOCUS:
		...
		*/
		...
		if !feat_is_available(ch, feat_num, subval, nil) {
			send_to_char(ch, libc.CString("You do not satisfy the prerequisites for that feat.\r\n"))
			return
		}
		if sftype == 1 {
			if IS_SET_AR(ch.Combat_feats[subfeat][:], uint32(int32(subval))) {
				send_to_char(ch, libc.CString("You already have that weapon feat.\r\n"))
				return
			}
			SET_BIT_AR(ch.Combat_feats[subfeat][:], uint32(int32(subval)))
		} else if sftype == 2 {
			if IS_SET(uint32(int32(ch.School_feats[subfeat])), uint32(int32(subval))) {
...
func guild(ch *char_data, me unsafe.Pointer, cmd int, argument *byte) bool {
	var (
		arg           [2048]byte
		guild_nr      int
		i             int
	...
	if guild_index[guild_nr].Func != nil {
		if guild_index[guild_nr].Func(ch, me, cmd, &arg[0]) {
			return true
		}
	}
	if !AWAKE(keeper) {
...
func boot_the_guilds(gm_f *stdio.File, filename *byte, rec_count int) {
	...
	for done == 0 {
		if *buf == '#' {
			...
			guild_index[top_guild].Vnum = temp
			...
			guild_index[top_guild].Gm = real_mobile(guild_index[top_guild].Gm)
			...
			if buf != nil && *buf != '#' && *buf != '$' {
				...
				for temp = 1; temp < GW_ARRAY_MAX; temp++ {
					...
					guild_index[top_guild].With_who[temp] = uint32(int32(val))
...
func assign_the_guilds() {
	...
	for gdindex = 0; gdindex <= top_guild; gdindex++ {
		if guild_index[gdindex].Gm == int(-1) {
			continue
		}
		...
		mob_index[guild_index[gdindex].Gm].Func = func(ch *char_data, me unsafe.Pointer, cmd int, argument *byte) bool {
...
func guild_customer_string(guild_nr int, detailed int) *byte {
	...
	for *trade_letters[gindex] != '\n' && len_+1 < uint64(64936) {
		if detailed != 0 {
			if !IS_SET_AR(guild_index[guild_nr].With_who[:], uint32(int32(flag))) {
	...
	if *arg == 0 {
		...
		if gm_num > 0 {
			for gm_nr = 0; gm_nr <= top_guild; gm_nr++ {
				if gm_num == guild_index[gm_nr].Vnum {
					break
				}
			}
...
func list_guilds(ch *char_data, rnum int, vmin int, vmax int) {
	var (
		i       int
		bottom  int
		top     int
	...
	for i = 0; i <= top_guild; i++ {
		if guild_index[i].Vnum >= bottom && guild_index[i].Vnum <= top {
			counter++
			send_to_char(ch, libc.CString("@g%4d@n) [@c%-5d@n]"), counter, guild_index[i].Vnum)
			send_to_char(ch, libc.CString(" @c[@y%d@c]@y %s@n"), func() int {
				if guild_index[i].Gm == int(-1) {
					return -1
				}
...
func count_guilds(low int, high int) int {
	var (
		i int
		j int
	)
	for i = func() int {
		j = 0
		return j
	}(); guild_index[i].Vnum <= high && i <= top_guild; i++ {
		if guild_index[i].Vnum >= low {
			j++
		}
	}
	return j
