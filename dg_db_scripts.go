...
func dg_read_trigger(fp *stdio.File, proto unsafe.Pointer, type_ int) {
	...
	if rnum == int(-1) {
		...
		case WLD_TRIGGER:
			mudlog(BRF, ADMLVL_BUILDER, TRUE, libc.CString("SYSERR: dg_read_trigger: Trigger vnum #%d asked for but non-existant! (room:%d)"), vnum, GET_ROOM_VNUM(room_rnum(((*room_data)(proto)).Number)))
		default:
...
func assign_triggers(i unsafe.Pointer, type_ int) {
	...
	case MOB_TRIGGER:
		...
		for trg_proto != nil {
			...
			if rnum == int(-1) {
				mudlog(BRF, ADMLVL_BUILDER, TRUE, libc.CString("SYSERR: trigger #%d non-existant, for mob #%d"), trg_proto.Vnum, mob_index[mob.Nr].Vnum)
			} else {
				if mob.Script == nil {
