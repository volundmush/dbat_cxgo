...
func can_harvest(plant *obj_data) bool {
	...
	if arg[0] != 0 {
		if libc.StrCaseCmp(&arg[0], libc.CString("collect")) == 0 {
			...
			if ROOM_FLAGGED(ch.In_room, ROOM_FERTILE1) {
				...
				obj_to_char(soil, ch)
				act(libc.CString("@yYou sink your shovel into the soft ground and manage to dig up a pile of fertile soil!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@y sinks $s shovel into the soft ground and manages to dig up a pile of fertile soil!@n"), 1, ch, nil, nil, TO_ROOM)
				soil.Value[0] = 8
				...
				var soil *obj_data = read_object(math.MaxUint8, VIRTUAL)
				obj_to_char(soil, ch)
				act(libc.CString("@yYou sink your shovel into the soft ground and manage to dig up a pile of good soil!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@y sinks $s shovel into the soft ground and manages to dig up a pile of good soil!@n"), 1, ch, nil, nil, TO_ROOM)
				...
				var soil *obj_data = read_object(math.MaxUint8, VIRTUAL)
				obj_to_char(soil, ch)
				act(libc.CString("@yYou sink your shovel into the soft ground and manage to dig up a pile of soil!@n"), 1, ch, nil, nil, TO_CHAR)
				act(libc.CString("@w$n@y sinks $s shovel into the soft ground and manages to dig up a pile of soil!@n"), 1, ch, nil, nil, TO_ROOM)
	...
	if ch.Move < cost {
		...
		if libc.StrCaseCmp(&arg2[0], libc.CString("water")) == 0 {
			...
			if water == nil {
				...
				act(libc.CString("@GAs you go to water @g$p@G you end up sloppily wasting about half of it on the ground.@n"), 1, ch, obj, nil, TO_CHAR)
				act(libc.CString("@g$n@G takes a bottle of grow water and sloshes some of it on @g$p@G.@n"), 1, ch, obj, nil, TO_ROOM)
				ch.Move -= cost
				...
				act(libc.CString("@GYou calmly and expertly pour the grow water on @g$p@G.@n"), 1, ch, obj, nil, TO_CHAR)
				act(libc.CString("@g$n@G calmly and expertly pours some grow water on @g$p@G.@n"), 1, ch, obj, nil, TO_ROOM)
				ch.Move -= cost
			...
			if clippers == nil {
				...
				act(libc.CString("@GAs you go to harvest @g$p@G you end up cutting it in half instead!@n"), 1, ch, obj, nil, TO_CHAR)
				act(libc.CString("@g$n@G attempts to harvest @g$p@G with $s clippers, but accidently cuts the plant in half!@n"), 1, ch, obj, nil, TO_ROOM)
				ch.Move -= cost
				extract_obj(obj)
				WAIT_STATE(ch, (int(1000000/OPT_USEC))*3)
				improve_skill(ch, SKILL_GARDENING, 0)
				...
				act(libc.CString("@GYou calmly and expertly harvest @g$p@G.@n"), 1, ch, obj, nil, TO_CHAR)
				act(libc.CString("@g$n@G calmly and expertly harvests @g$p@G.@n"), 1, ch, obj, nil, TO_ROOM)
				ch.Move -= cost
			...
			if shovel == nil {
				...
				act(libc.CString("@GYou calmly dig up @g$p@G.@n"), 1, ch, obj, nil, TO_CHAR)
				act(libc.CString("@g$n@G calmly digs up @g$p@G.@n"), 1, ch, obj, nil, TO_ROOM)
				ch.Move -= cost
				obj_from_room(obj)
				obj_to_char(obj, ch)
			...
			if soil == nil {
				...
				act(libc.CString("@GYou end up digging a hole too shallow to hold @g$p@G. Better try again.@n"), 1, ch, obj, nil, TO_CHAR)
				act(libc.CString("@g$n@G digs a very shallow hole in one of the planters and then realizes @g$p@G won't fit in it.@n"), 1, ch, obj, nil, TO_ROOM)
				ch.Move -= cost
				WAIT_STATE(ch, (int(1000000/OPT_USEC))*3)
				improve_skill(ch, SKILL_GARDENING, 0)
				return
			} else {
				act(libc.CString("@GYou dig a proper sized hole and plant @g$p@G in it.@n"), 1, ch, obj, nil, TO_CHAR)
				act(libc.CString("@g$n@G digs a proper sized hole in a planter and plants @g$p@G in it.@n"), 1, ch, obj, nil, TO_ROOM)
				obj_from_char(obj)
			...
			if !OBJ_FLAGGED(obj, ITEM_MATURE) {
				...
				act(libc.CString("@GYou end up shredding @g$p@G with your clumsy unskilled hands.@n"), 1, ch, obj, nil, TO_CHAR)
				act(libc.CString("@g$n@G grabs a hold of @g$p@G and shreds it in an attempt to pick it.@n"), 1, ch, obj, nil, TO_ROOM)
				return
			} else {
				act(libc.CString("@GYou grab a hold of @g$p@G and carefully pick it out of the soil.@n"), 1, ch, obj, nil, TO_CHAR)
				act(libc.CString("@g$n@G grabs a hold of @g$p@G and carefully picks it out of the soil.@n"), 1, ch, obj, nil, TO_ROOM)
				obj_from_room(obj)
				obj_to_char(obj, ch)
				ch.Move -= cost
				WAIT_STATE(ch, (int(1000000/OPT_USEC))*3)
...
func do_pack(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if (func() *obj_data {
		...
		if GET_OBJ_VNUM(obj) >= 19090 && GET_OBJ_VNUM(obj) <= 0x4A9B || GET_OBJ_VNUM(obj) == 11 {
			act(libc.CString("@CYou push a hidden button on $p@C and a cloud of smoke erupts and covers it. As the smoke clears a small capsule can be seen on the ground.@n"), 1, ch, obj, nil, TO_CHAR)
			act(libc.CString("@c$n@C pushes a hidden button on $p@C and a cloud of smokes erupts and covers it. As the smoke clears a small capsule can be seen on the ground.@n"), 1, ch, obj, nil, TO_ROOM)
			if GET_OBJ_VNUM(obj) == 11 {
				extract_obj(obj)
				packed = read_object(0x4A8D, VIRTUAL)
				obj_to_room(packed, ch.In_room)
			} else {
				var fnum int = GET_OBJ_VNUM(obj) - 10
			...
			if arg2[0] == 0 {
				...
				_ = cont
				act(libc.CString("@CYou push a hidden button on $p@C and a cloud of smoke erupts and covers it. As the smoke clears a pile of money can be seen on the ground!@n"), 1, ch, obj, nil, TO_CHAR)
				act(libc.CString("@c$n@C pushes a hidden button on $p@C and a cloud of smokes erupts and covers it. As the smoke clears a pile of money can be seen on the ground!@n"), 1, ch, obj, nil, TO_ROOM)
				var money int = 0
				var count int = 0
				var rnum int = GET_OBJ_VNUM(obj)
				if GET_OBJ_VNUM(obj) >= 18800 && GET_OBJ_VNUM(obj) <= 0x49D3 {
					...
					money = 65000
					for count < 4 {
						for world[real_room(rnum)].Contents != nil {
...
func do_deploy(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if capsule == 0 {
		...
		return
	} else if ch.Rp < 10 && furniture == 0 {
		send_to_char(ch, libc.CString("You are required to have (not spend) 10 RPP in order to place a house.\r\n"))
		return
	} else if furniture == 1 && (!ROOM_FLAGGED(ch.In_room, ROOM_HOUSE) || ROOM_FLAGGED(ch.In_room, ROOM_SHIP)) {
		send_to_char(ch, libc.CString("You can't deploy house furniture capsules here.\r\n"))
		return
	} else if furniture == 1 && (ROOM_FLAGGED(ch.In_room, ROOM_GARDEN1) || ROOM_FLAGGED(ch.In_room, ROOM_GARDEN2)) {
		send_to_char(ch, libc.CString("You can't deploy house furniture capsules here.\r\n"))
		return
	} else if furniture == 0 && (SECT(ch.In_room) == SECT_INSIDE || SECT(ch.In_room) == SECT_WATER_NOSWIM || SECT(ch.In_room) == SECT_WATER_SWIM || SECT(ch.In_room) == SECT_SPACE) {
	...
	if furniture == 1 {
		...
		if fnum != 0 {
			var furn *obj_data = read_object(fnum, VIRTUAL)
			act(libc.CString("@CYou click the capsule's button and toss it to the floor. A puff of smoke erupts immediately and quickly dissipates to reveal, $p@C.@n"), 1, ch, furn, nil, TO_CHAR)
			act(libc.CString("@c$n@C clicks a capsule's button and tosses it to the floor. A puff of smoke erupts immediately and quickly dissipates to reveal, $p@C.@n"), 1, ch, furn, nil, TO_ROOM)
	...
	for giveup == 0 && cont == 0 {
		obj3 = find_obj_in_list_vnum(world[real_room(rnum)].Contents, 0x4971)
	...
	if cont == 1 {
		...
		var key *obj_data = read_object(rnum, VIRTUAL)
		obj_to_char(key, ch)
		act(libc.CString("@WYou click the capsule and toss it to the ground. A large cloud of smoke erupts from the capsule and after it clears a house is visible in its place!@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@C$n@W clicks a capsule and then tosses it to the ground. A large cloud of smoke erupts from the capsule and after it clears a house is visible in its place!@n"), 1, ch, nil, nil, TO_ROOM)
...
func do_twohand(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if (ch.Equipment[WEAR_WIELD1]) == nil && !PLR_FLAGGED(ch, PLR_THANDW) {
		...
		act(libc.CString("$n stops wielding $s weapon with both hands."), 1, ch, nil, nil, TO_ROOM)
		REMOVE_BIT_AR(ch.Act[:], PLR_THANDW)
		return
	} else {
		send_to_char(ch, libc.CString("You grab your weapon with both hands.\r\n"))
		act(libc.CString("$n starts wielding $s weapon with both hands."), 1, ch, nil, nil, TO_ROOM)
	...
	if dballtime == 0 {
		...
		for k = object_list; k != nil; k = k.Next {
			...
			if GET_OBJ_VNUM(k) == 20 {
				...
				found7 = 1
			} else if k.In_room != int(-1) && world[k.In_room].Geffect == 6 && !OBJ_FLAGGED(k, ITEM_UNBREAKABLE) {
		...
		if found1 == 0 {
			...
			for load == 0 {
				if real_room(num) != int(-1) {
					if (func() int {
						zone = real_zone_by_thing(real_room(num))
						return zone
					}()) != int(-1) {
			...
			if rand_number(1, 10) > 8 {
				if hunter1 == 0 {
					if (func() int {
						r_num = real_mobile(DBALL_HUNTER1_VNUM)
						return r_num
					}()) == int(-1) {
						return
					}
					...
					if (func() int {
						r_num = real_mobile(DBALL_HUNTER2_VNUM)
						return r_num
					}()) == int(-1) {
						return
					}
...
func do_auction(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if arg1[0] == 0 {
		...
		return
	} else if is_abbrev(&arg1[0], libc.CString("cancel")) || is_abbrev(&arg1[0], libc.CString("stop")) {
	...
	} else if is_abbrev(&arg1[0], libc.CString("stats")) || is_abbrev(&arg1[0], libc.CString("identify")) {
	...
	if libc.StrCaseCmp(&arg[0], libc.CString("list")) == 0 {
		...
		if obj2 == nil {
			...
			if GET_SKILL(ch, SKILL_APPRAISE) < rand_number(1, 101) {
				...
				act(libc.CString("@c$n@w looks stumped about something they viewed on their scouter screen.@n"), 1, ch, nil, nil, TO_ROOM)
				WAIT_STATE(ch, (int(1000000/OPT_USEC))*2)
				return
			} else {
				send_to_char(ch, libc.CString("You look at images of the object on your scouter.\r\n"))
				act(libc.CString("@c$n@w looks at something on their scouter screen.@n"), 1, ch, nil, nil, TO_ROOM)
...
func auc_stat(ch *char_data, obj *obj_data) {
	if aucstat == AUC_NULL_STATE {
		...
		act(&buf[0], 1, ch_selling, obj, unsafe.Pointer(ch), int(TO_VICT|2<<7))
		ch.Gold -= 500
	}
}
func auc_send_to_all(messg *byte, buyer bool) {
	...
	for i = descriptor_list; i != nil; i = i.Next {
		...
		if buyer {
			act(messg, 1, ch_buying, obj_selling, unsafe.Pointer(i.Character), int(TO_VICT|2<<7))
		} else {
			act(messg, 1, ch_selling, obj_selling, unsafe.Pointer(i.Character), int(TO_VICT|2<<7))
		}
	}
}
...
func do_assemble(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if tool != nil {
		act(libc.CString("@WYou open up your toolkit and take out the necessary tools.@n"), 1, ch, nil, nil, TO_CHAR)
		act(libc.CString("@C$n@W opens up $s toolkit and takes out the necessary tools.@n"), 1, ch, nil, nil, TO_ROOM)
	} else {
	...
	if libc.StrCaseCmp(argument, libc.CString("campfire")) != 0 {
		...
		if GET_SKILL(ch, SKILL_BUILD) <= roll {
			if (func() *obj_data {
				pObject = read_object(lVnum, VIRTUAL)
				return pObject
			}()) == nil {
				send_to_char(ch, libc.CString("You can't %s %s %s.\r\n"), complete_cmd_info[cmd].Command, AN(argument), argument)
				return
		...
		if survival <= roll {
			...
			if tool != nil && rand_number(1, 3) == 3 && (tool.Value[VAL_ALL_HEALTH]) > 0 {
				...
				act(libc.CString("@RYour toolset is looking a bit more worn.@n"), 1, ch, nil, nil, TO_CHAR)
	...
	act(&buf[0], 0, ch, pObject, nil, TO_CHAR)
	stdio.Sprintf(&buf[0], "$n %ss $p.", complete_cmd_info[cmd].Command)
	act(&buf[0], 0, ch, pObject, nil, TO_ROOM)
	if assemblyCheckComponents(lVnum, ch, 1) {
		roll = 9001
	}
...
func perform_put(ch *char_data, obj *obj_data, cont *obj_data) {
	...
	if OBJ_FLAGGED(cont, ITEM_FORGED) {
		act(libc.CString("$P is forged and won't hold anything."), 0, ch, nil, unsafe.Pointer(cont), TO_CHAR)
		return
	}
	...
	if int(cont.Type_flag) == ITEM_CONTAINER && (cont.Value[VAL_CONTAINER_CAPACITY]) == 0 {
		act(libc.CString("$p won't fit in $P."), 0, ch, obj, unsafe.Pointer(cont), TO_CHAR)
		...
		act(libc.CString("$p won't fit in $P."), 0, ch, obj, unsafe.Pointer(cont), TO_CHAR)
	} else if OBJ_FLAGGED(obj, ITEM_NODROP) && cont.In_room != int(-1) {
		act(libc.CString("You can't get $p out of your hand."), 0, ch, obj, nil, TO_CHAR)
		...
		if !OBJ_FLAGGED(obj, ITEM_ANTI_HIEROPHANT) {
			act(libc.CString("$n puts $p in $P."), 1, ch, obj, unsafe.Pointer(cont), TO_ROOM)
		} else {
			act(libc.CString("$n puts an @DA@wd@cv@Ce@Wnt @DD@wu@ce@Cl @mC@Ma@Wr@wd@n in $P."), 1, ch, obj, unsafe.Pointer(cont), TO_ROOM)
		}
		if OBJ_FLAGGED(obj, ITEM_NODROP) && !OBJ_FLAGGED(cont, ITEM_NODROP) {
			SET_BIT_AR(cont.Extra_flags[:], ITEM_NODROP)
			act(libc.CString("You get a strange feeling as you put $p in $P."), 0, ch, obj, unsafe.Pointer(cont), TO_CHAR)
		} else {
			act(libc.CString("You put $p in $P."), 0, ch, obj, unsafe.Pointer(cont), TO_CHAR)
		}
		if int(cont.Type_flag) == ITEM_PORTAL || int(cont.Type_flag) == ITEM_VEHICLE {
			obj_from_obj(obj)
			obj_to_room(obj, real_room(cont.Value[VAL_CONTAINER_CAPACITY]))
			if int(cont.Type_flag) == ITEM_PORTAL {
				act(libc.CString("What? $U$p disappears from $P in a puff of smoke!"), 1, ch, obj, unsafe.Pointer(cont), TO_ROOM)
				act(libc.CString("What? $U$p disappears from $P in a puff of smoke!"), 0, ch, obj, unsafe.Pointer(cont), TO_CHAR)
			}
		}
	}
}
func do_put(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if *theobj == 0 {
		...
		if cont == nil {
			...
			act(libc.CString("$p is not a container."), 0, ch, cont, nil, TO_CHAR)
...
func can_take_obj(ch *char_data, obj *obj_data) bool {
	if !OBJWEAR_FLAGGED(obj, ITEM_WEAR_TAKE) {
		act(libc.CString("$p: you can't take that!"), 0, ch, obj, nil, TO_CHAR)
		return false
	} else if int(ch.Carry_items) >= 50 {
		act(libc.CString("$p: your arms are full!"), 0, ch, obj, nil, TO_CHAR)
		return false
	} else if (gear_weight(ch) + int(obj.Weight)) > int(max_carry_weight(ch)) {
		act(libc.CString("$p: you can't carry that much weight."), 0, ch, obj, nil, TO_CHAR)
		return false
	} else if (obj.Weight+int64(world[ch.In_room].Gravity))+int64(gear_weight(ch)) > max_carry_weight(ch) {
		act(libc.CString("$p: you can't carry that much weight because of the gravity."), 0, ch, obj, nil, TO_CHAR)
		return false
	}
	return true
}
func get_check_money(ch *char_data, obj *obj_data) {
	...
	if ch.Gold+value > GOLD_CARRY(ch) {
		...
		act(libc.CString("@w$n @wdrops some onto the ground.@n"), 0, ch, nil, nil, TO_ROOM)
		extract_obj(obj)
		var diff int = 0
...
func perform_get_from_container(ch *char_data, obj *obj_data, cont *obj_data, mode int) {
	if mode == (1<<2) || mode == (1<<5) || can_take_obj(ch, obj) {
		if int(ch.Carry_items) >= 50 {
			act(libc.CString("$p: you can't hold any more items."), 0, ch, obj, nil, TO_CHAR)
			return
		}
		if ch.Sits != nil && GET_OBJ_VNUM(ch.Sits) > 603 && GET_OBJ_VNUM(ch.Sits) < 608 && GET_OBJ_VNUM(ch.Sits)-4 != GET_OBJ_VNUM(cont) && GET_OBJ_VNUM(cont) > 599 && GET_OBJ_VNUM(cont) < 604 {
			...
			obj_to_char(obj, ch)
			if OBJ_FLAGGED(cont, ITEM_SHEATH) {
				act(libc.CString("You draw $p from $P."), 0, ch, obj, unsafe.Pointer(cont), TO_CHAR)
				act(libc.CString("$n draws $p from $P."), 1, ch, obj, unsafe.Pointer(cont), TO_ROOM)
			} else {
				act(libc.CString("You get $p from $P."), 0, ch, obj, unsafe.Pointer(cont), TO_CHAR)
				act(libc.CString("$n gets $p from $P."), 1, ch, obj, unsafe.Pointer(cont), TO_ROOM)
			}
			if OBJ_FLAGGED(obj, ITEM_HOT) {
				if (ch.Bonuses[BONUS_FIREPROOF]) <= 0 && int(ch.Race) != RACE_DEMON {
					...
					act(libc.CString("@RYou are burned by it!@n"), 1, ch, nil, nil, TO_CHAR)
					act(libc.CString("@R$n@R is burned by it!@n"), 1, ch, nil, nil, TO_ROOM)
				}
			}
			if IS_NPC(ch) {
				item_check(obj, ch)
...
func get_from_container(ch *char_data, cont *obj_data, arg *byte, mode int, howmany int) {
	...
	if OBJVAL_FLAGGED(cont, 1<<2) {
		act(libc.CString("$p is closed."), 0, ch, cont, nil, TO_CHAR)
		...
		if (func() *obj_data {
			...
			act(&buf[0], 0, ch, cont, nil, TO_CHAR)
		} else {
		...
		if found == 0 {
			if obj_dotmode == FIND_ALL {
				act(libc.CString("$p seems to be empty."), 0, ch, cont, nil, TO_CHAR)
			} else {
				var buf [64936]byte
...
func perform_get_from_room(ch *char_data, obj *obj_data) {
	if obj.Sitting != nil {
	...
	if can_take_obj(ch, obj) && get_otrigger(obj, ch) {
		obj_from_room(obj)
		obj_to_char(obj, ch)
		act(libc.CString("You get $p."), 0, ch, obj, nil, TO_CHAR)
		act(libc.CString("$n gets $p."), 1, ch, obj, nil, TO_ROOM)
		if OBJ_FLAGGED(obj, ITEM_HOT) {
			if (ch.Bonuses[BONUS_FIREPROOF]) <= 0 && int(ch.Race) != RACE_DEMON {
				ch.Hit -= int64(float64(ch.Hit) * 0.25)
				if (ch.Bonuses[BONUS_FIREPRONE]) > 0 {
...
func do_get(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if arg1[0] == 0 {
	...
	} else if is_number(&arg1[0]) && arg3[0] == 0 {
		...
		if cont_dotmode == FIND_INDIV {
			...
			for cont = ch.Carrying; cont != nil; cont = cont.Next_content {
				if CAN_SEE_OBJ(ch, cont) && (cont_dotmode == FIND_ALL || isname(&arg2[0], cont.Name)) {
...
func perform_drop_gold(ch *char_data, amount int, mode int8, RDR int) {
	var obj *obj_data
	if amount <= 0 {
		...
		if int(mode) != SCMD_JUNK {
			...
			if int(mode) == SCMD_DONATE {
				...
				act(libc.CString("$n throws some gold into the air where it disappears in a puff of smoke!"), 0, ch, nil, nil, TO_ROOM)
				obj_to_room(obj, RDR)
				...
				if !drop_wtrigger(obj, ch) && obj != nil {
					extract_obj(obj)
					return
				}
...
func perform_drop(ch *char_data, obj *obj_data, mode int8, sname *byte, RDR int) int {
	var (
		buf   [64936]byte
		value int
	)
	...
	if int(mode) == SCMD_DROP && !drop_wtrigger(obj, ch) {
		return 0
	}
	if GET_OBJ_VNUM(obj) == 17 || GET_OBJ_VNUM(obj) == 0x464E {
		...
		act(&buf[0], 0, ch, obj, nil, TO_CHAR)
		return 0
	}
	...
	act(&buf[0], 1, ch, obj, nil, TO_ROOM)
	obj_from_char(obj)
	switch mode {
	case SCMD_DROP:
		if !OBJ_FLAGGED(obj, ITEM_UNBREAKABLE) && world[ch.In_room].Geffect == 6 {
			act(libc.CString("$p melts in the lava!"), 0, ch, obj, nil, TO_CHAR)
			act(libc.CString("$p melts in the lava!"), 0, ch, obj, nil, TO_ROOM)
			extract_obj(obj)
		} else if world[ch.In_room].Geffect == 6 {
			act(libc.CString("$p plops down on some cooled lava!"), 0, ch, obj, nil, TO_CHAR)
			act(libc.CString("$p plops down on some cooled lava!"), 0, ch, obj, nil, TO_ROOM)
			obj_to_room(obj, ch.In_room)
			if ch.Admlevel > 0 {
				send_to_imm(libc.CString("IMM DROP: %s dropped %s in room [%d]"), GET_NAME(ch), obj.Short_description, GET_ROOM_VNUM(obj.In_room))
				log_imm_action(libc.CString("IMM DROP: %s dropped %s in room [%d]"), GET_NAME(ch), obj.Short_description, GET_ROOM_VNUM(obj.In_room))
	...
	case SCMD_DONATE:
		obj_to_room(obj, RDR)
		act(libc.CString("$p suddenly appears in a puff a smoke!"), 0, nil, obj, nil, TO_ROOM)
		return 0
	case SCMD_JUNK:
...
func do_drop(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	case SCMD_DONATE:
		...
		mode = SCMD_DONATE
		num_don_rooms = int(libc.BoolToInt(config_info.Room_nums.Donation_room_1 != int(-1)))*2 + int(libc.BoolToInt(config_info.Room_nums.Donation_room_2 != int(-1))) + int(libc.BoolToInt(config_info.Room_nums.Donation_room_3 != int(-1))) + 1
...
func perform_give(ch *char_data, vict *char_data, obj *obj_data) {
	...
	if OBJ_FLAGGED(obj, ITEM_NODROP) {
		act(libc.CString("You can't let go of $p!!  Yeech!"), 0, ch, obj, nil, TO_CHAR)
		return
	}
	if int(vict.Carry_items) >= 50 {
		act(libc.CString("$N seems to have $S hands full."), 0, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		if IS_NPC(ch) {
			act(libc.CString("$n@n drops $p because you can't carry anymore."), 1, ch, obj, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("$n@n drops $p on the ground since $N's unable to carry it."), 1, ch, obj, unsafe.Pointer(vict), TO_NOTVICT)
			obj_from_char(obj)
			obj_to_room(obj, ch.In_room)
		}
		return
	...
	if IS_NPC(vict) && (OBJ_FLAGGED(obj, ITEM_FORGED) || OBJ_FLAGGED(obj, ITEM_FORGED)) {
		act(libc.CString("$n tries to hand $p to $N."), 1, ch, obj, unsafe.Pointer(vict), TO_NOTVICT)
		do_say(vict, libc.CString("I don't want that piece of junk."), 0, 0)
		return
	}
	if obj.Weight+int64(gear_weight(vict)) > max_carry_weight(vict) {
		act(libc.CString("$E can't carry that much weight."), 0, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		if IS_NPC(ch) {
			act(libc.CString("$n@n drops $p because you can't carry anymore."), 1, ch, obj, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("$n@n drops $p on the ground since $N's unable to carry it."), 1, ch, obj, unsafe.Pointer(vict), TO_NOTVICT)
			obj_from_char(obj)
	...
	if (obj.Weight+int64(world[vict.In_room].Gravity))+int64(gear_weight(vict)) > max_carry_weight(vict) {
		act(libc.CString("$E can't carry that much weight because of the gravity."), 0, ch, nil, unsafe.Pointer(vict), TO_CHAR)
		if IS_NPC(ch) {
			act(libc.CString("$n@n drops $p because you can't carry anymore."), 1, ch, obj, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("$n@n drops $p on the ground since $N's unable to carry it."), 1, ch, obj, unsafe.Pointer(vict), TO_NOTVICT)
			obj_from_char(obj)
	...
	if !IS_NPC(vict) && !IS_NPC(ch) {
		if PRF_FLAGGED(vict, PRF_NOGIVE) {
			act(libc.CString("$N refuses to accept $p at this time."), 0, ch, obj, unsafe.Pointer(vict), TO_CHAR)
			act(libc.CString("$n tries to give you $p, but you are refusing to be handed things."), 0, ch, obj, unsafe.Pointer(vict), TO_VICT)
			act(libc.CString("$n tries to give $N, $p, but $E is refusing to be handed things right now."), 0, ch, obj, unsafe.Pointer(vict), TO_NOTVICT)
			return
		}
	}
	obj_from_char(obj)
	obj_to_char(obj, vict)
	act(libc.CString("You give $p to $N."), 0, ch, obj, unsafe.Pointer(vict), TO_CHAR)
	act(libc.CString("$n gives you $p."), 0, ch, obj, unsafe.Pointer(vict), TO_VICT)
	act(libc.CString("$n gives $p to $N."), 1, ch, obj, unsafe.Pointer(vict), TO_NOTVICT)
	if OBJ_FLAGGED(obj, ITEM_HOT) {
		if (vict.Bonuses[BONUS_FIREPROOF]) <= 0 && int(vict.Race) != RACE_DEMON {
			...
			act(libc.CString("@RYou are burned by it!@n"), 1, vict, nil, nil, TO_CHAR)
			act(libc.CString("@R$n@R is burned by it!@n"), 1, vict, nil, nil, TO_ROOM)
		}
	}
}
...
func perform_give_gold(ch *char_data, vict *char_data, amount int) {
	...
	act(&buf[0], 0, ch, nil, unsafe.Pointer(vict), TO_VICT)
	stdio.Snprintf(&buf[0], int(64936), "$n gives %s to $N.", money_desc(amount))
	act(&buf[0], 1, ch, nil, unsafe.Pointer(vict), TO_NOTVICT)
	...
	if arg[0] == 0 {
		...
		if dotmode == FIND_INDIV {
			...
			if ch.Carrying == nil {
				...
				for obj = ch.Carrying; obj != nil; obj = next_obj {
					next_obj = obj.Next_content
					if CAN_SEE_OBJ(ch, obj) && (dotmode == FIND_ALL || isname(&arg[0], obj.Name)) {
						perform_give(ch, vict, obj)
						if ch.Admlevel > 0 && !IS_NPC(vict) {
							send_to_imm(libc.CString("IMM GIVE: %s has given %s to %s."), GET_NAME(ch), obj.Short_description, GET_NAME(vict))
							log_imm_action(libc.CString("IMM GIVE: %s has given %s to %s."), GET_NAME(ch), obj.Short_description, GET_NAME(vict))
...
func name_from_drinkcon(obj *obj_data) {
	...
	if obj.Item_number == int(-1) || obj.Name != obj_proto[obj.Item_number].Name {
...
func do_drink(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if int(temp.Type_flag) != ITEM_DRINKCON && int(temp.Type_flag) != ITEM_FOUNTAIN {
		if GET_OBJ_VNUM(temp) == 86 && on_ground != 1 {
			act(libc.CString("@wYou uncork the $p and tip it to your lips. Drinking it down you feel a warmth flow through your body and your ki returns to full strength .@n"), 1, ch, temp, nil, TO_CHAR)
			act(libc.CString("@C$n@w uncorks the $p and tips it to $s lips. Drinking it down and then smiling.@n"), 1, ch, temp, nil, TO_ROOM)
			obj_from_char(temp)
			extract_obj(temp)
			ch.Mana = ch.Max_mana
	...
	if IS_NPC(ch) {
		act(libc.CString("$n@w drinks from $p."), 1, ch, temp, nil, TO_ROOM)
		obj_from_char(temp)
		extract_obj(temp)
		return
	}
	if int(ch.Player_specials.Conditions[DRUNK]) > 10 && int(ch.Player_specials.Conditions[THIRST]) > 0 {
		...
		act(libc.CString("$n tries to drink but misses $s mouth!"), 1, ch, nil, nil, TO_ROOM)
		return
	}
	...
	if subcmd == SCMD_DRINK {
		...
		act(&buf[0], 1, ch, temp, nil, TO_ROOM)
		send_to_char(ch, libc.CString("You drink the %s.\r\n"), drinks[temp.Value[VAL_DRINKCON_LIQUID]])
		if temp.Action_description != nil {
			act(temp.Action_description, 1, ch, temp, nil, TO_CHAR)
		}
		amount = 4
	} else {
		act(libc.CString("$n sips from $p."), 1, ch, temp, nil, TO_ROOM)
	...
	if (temp.Value[VAL_DRINKCON_POISON]) != 0 && (int(ch.Race) != RACE_MUTANT || (ch.Genome[0]) != 7 && (ch.Genome[1]) != 7) {
		...
		act(libc.CString("$n chokes and utters some strange sounds."), 1, ch, nil, nil, TO_ROOM)
...
func do_eat(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if subcmd == SCMD_EAT {
		act(libc.CString("You eat some of $p."), 0, ch, food, nil, TO_CHAR)
		if food.Action_description != nil {
			act(food.Action_description, 0, ch, food, nil, TO_CHAR)
		}
		act(libc.CString("$n eats some of $p."), 1, ch, food, nil, TO_ROOM)
	} else {
		act(libc.CString("You nibble a little bit of $p."), 0, ch, food, nil, TO_CHAR)
		act(libc.CString("$n tastes a little bit of $p."), 1, ch, food, nil, TO_ROOM)
	...
	if (food.Value[VAL_FOOD_POISON]) != 0 && !ADM_FLAGGED(ch, ADM_NOPOISON) {
		...
		act(libc.CString("$n coughs and utters some strange sounds."), 0, ch, nil, nil, TO_ROOM)
		af.Type = SPELL_POISON
...
func do_pour(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if subcmd == SCMD_FILL {
		...
		if int(to_obj.Type_flag) != ITEM_DRINKCON {
			act(libc.CString("You can't fill $p!"), 0, ch, to_obj, nil, TO_CHAR)
			return
		}
		if arg2[0] == 0 {
			act(libc.CString("What do you want to fill $p from?"), 0, ch, to_obj, nil, TO_CHAR)
			return
		}
		if (func() *obj_data {
		...
		if int(from_obj.Type_flag) != ITEM_FOUNTAIN && !OBJ_FLAGGED(from_obj, ITEM_BROKEN) {
			act(libc.CString("You can't fill something from $p."), 0, ch, from_obj, nil, TO_CHAR)
			return
		} else if int(from_obj.Type_flag) == ITEM_FOUNTAIN && OBJ_FLAGGED(from_obj, ITEM_BROKEN) {
			act(libc.CString("You can't fill something from a broken fountain."), 0, ch, from_obj, nil, TO_CHAR)
			return
		}
	}
	if (from_obj.Value[VAL_DRINKCON_HOWFULL]) == 0 {
		act(libc.CString("The $p is empty."), 0, ch, from_obj, nil, TO_CHAR)
		return
	}
	if subcmd == SCMD_POUR {
		if arg2[0] == 0 {
		...
		if libc.StrCaseCmp(&arg2[0], libc.CString("out")) == 0 {
			if (from_obj.Value[VAL_DRINKCON_CAPACITY]) > 0 {
				act(libc.CString("$n empties $p."), 1, ch, from_obj, nil, TO_ROOM)
				act(libc.CString("You empty $p."), 0, ch, from_obj, nil, TO_CHAR)
	...
	if subcmd == SCMD_FILL {
		act(libc.CString("You gently fill $p from $P."), 0, ch, to_obj, unsafe.Pointer(from_obj), TO_CHAR)
		act(libc.CString("$n gently fills $p from $P."), 1, ch, to_obj, unsafe.Pointer(from_obj), TO_ROOM)
	}
...
func wear_message(ch *char_data, obj *obj_data, where int) {
	...
	act(wear_messages[where][0], 1, ch, obj, nil, TO_ROOM)
	act(wear_messages[where][1], 0, ch, obj, nil, TO_CHAR)
}
...
func perform_wear(ch *char_data, obj *obj_data, where int) {
	...
	if !OBJWEAR_FLAGGED(obj, uint32(int32(wear_bitvectors[where]))) {
		act(libc.CString("You can't wear $p there."), 0, ch, obj, nil, TO_CHAR)
		return
	}
	if !BODY_FLAGGED(ch, uint32(int32(where))) {
...
func find_eq_pos(ch *char_data, obj *obj_data, arg *byte) int {
	...
	if arg == nil || *arg == 0 {
		...
		where = search_block(arg, &keywords[0], 0)
		return where
	}()) < 0 || *arg == '!' {
		...
		for obj = ch.Carrying; obj != nil; obj = next_obj {
			...
			if CAN_SEE_OBJ(ch, obj) && (func() int {
				...
				if GET_LEVEL(ch) < obj.Level {
					act(libc.CString("$p: you are not experienced enough to use that."), 0, ch, obj, nil, TO_CHAR)
					send_to_char(ch, libc.CString("You need to be at least %d level to use it.\r\n"), obj.Level)
				} else if OBJ_FLAGGED(obj, ITEM_BROKEN) {
					act(libc.CString("$p: it seems to be broken."), 0, ch, obj, nil, TO_CHAR)
				} else if OBJ_FLAGGED(obj, ITEM_FORGED) {
					act(libc.CString("$p: it seems to be fake..."), 0, ch, obj, nil, TO_CHAR)
				} else {
					items_worn++
					if !is_proficient_with_armor(ch, obj.Value[VAL_ARMOR_SKILL]) && int(obj.Type_flag) == ITEM_ARMOR {
		...
		if (func() *obj_data {
			...
			for obj != nil {
				...
				if (func() int {
					...
					act(libc.CString("You can't wear $p."), 0, ch, obj, nil, TO_CHAR)
				}
				obj = next_obj
			}
		}
...
func do_wield(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if arg[0] == 0 {
		...
		if !OBJWEAR_FLAGGED(obj, ITEM_WEAR_WIELD) {
			...
			if !IS_NPC(ch) && !is_proficient_with_weapon(ch, obj.Value[VAL_WEAPON_SKILL]) && int(obj.Type_flag) == ITEM_ARMOR {
	...
	if arg[0] == 0 {
		...
		if int(obj.Type_flag) == ITEM_LIGHT {
			...
			if (obj.Value[VAL_LIGHT_HOURS]) > 0 || (obj.Value[VAL_LIGHT_HOURS]) < 0 {
				act(libc.CString("@wYou light $p@w."), 1, ch, obj, nil, TO_CHAR)
				act(libc.CString("@C$n@w lights $p@w."), 1, ch, obj, nil, TO_ROOM)
			}
			if (obj.Value[VAL_LIGHT_HOURS]) == 0 {
				act(libc.CString("@wYou try to light $p@w but it is burnt out."), 1, ch, obj, nil, TO_CHAR)
				act(libc.CString("@C$n@w tries to light $p@w but nothing happens."), 1, ch, obj, nil, TO_ROOM)
			}
		} else {
...
func perform_remove(ch *char_data, pos int) {
	...
	if (func() *obj_data {
		...
		act(libc.CString("You can't remove $p, it must be CURSED!"), 0, ch, obj, nil, TO_CHAR)
	} else if int(ch.Carry_items) >= 50 {
		act(libc.CString("$p: your arms are full!"), 0, ch, obj, nil, TO_CHAR)
	} else {
		...
		act(libc.CString("You stop using $p."), 0, ch, obj, nil, TO_CHAR)
		act(libc.CString("$n stops using $p."), 1, ch, obj, nil, TO_ROOM)
		if previous > ch.Hit {
			var drop [2048]byte
			stdio.Sprintf(&drop[0], "@RYour powerlevel has dropped from removing $p@R! @D[@r-%s@D]\r\n", add_commas(previous-ch.Hit))
			act(&drop[0], 0, ch, obj, nil, TO_CHAR)
		}
	}
}
	...
	if found == 0 {
		...
		if dotmode == FIND_ALL {
			...
			if arg[0] == 0 {
				...
				found = 0
				for i = 0; i < NUM_WEARS; i++ {
					if (ch.Equipment[i]) != nil && CAN_SEE_OBJ(ch, ch.Equipment[i]) && isname(&arg[0], (ch.Equipment[i]).Name) {
						perform_remove(ch, i)
						found = 1
					}
				}
...
func do_sac(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	act(libc.CString("$n sacrifices $p."), 0, ch, j, nil, TO_ROOM)
