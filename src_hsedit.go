...
func hsedit_save_internally(d *descriptor_data) {
	...
	if house_rnum != int(-1) {
		...
		if house_rnum < MAX_HOUSES {
			...
			mudlog(NRM, ADMLVL_BUILDER, 1, libc.CString("HSEDIT: Max houses limit reached - Unable to save OLC data"))
		}
	}
	if real_room(d.Olc.House.Vnum) != int(-1) {
		SET_BIT_AR(world[real_room(d.Olc.House.Vnum)].Room_flags[:], ROOM_HOUSE)
	}
	if real_room(d.Olc.House.Atrium) != int(-1) {
...
func hedit_delete_house(d *descriptor_data, house_vnum int) {
	...
	var real_house int
	if (func() int {
		i = find_house(house_vnum)
		return i
	}()) == int(-1) {
		mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("SYSERR: hsedit: Invalid house vnum in hedit_delete_house\r\n"))
	...
	if (func() int {
		...
		REMOVE_BIT_AR(world[real_house].Room_flags[:], uint32(int32(int(ROOM_HOUSE|ROOM_HOUSE_CRASH))))
	}
...
func hsedit_disp_flags_menu(d *descriptor_data) {
	...
	sprintbit(uint32(int32(d.Olc.House.Bitvector)), house_flags[:], &buf1[0], uint64(64936))
...
func hsedit_dir_menu(d *descriptor_data) {
	...
	house = d.Olc.House
	mudlog(CMP, ADMLVL_BUILDER, 1, libc.CString("(LOG) hsedit_dir_menu: house vnum = %d"), house.Vnum)
...
func hsedit_disp_menu(d *descriptor_data) {
	...
	buf2[0] = '\x00'
	sprintbit(uint32(int32(house.Bitvector)), house_flags[:], &buf1[0], uint64(64936))
...
func hsedit_parse(d *descriptor_data, arg *byte) {
	...
	mudlog(CMP, ADMLVL_BUILDER, 0, libc.CString("(LOG) hsedit_parse: OLC mode %d"), d.Olc.Mode)
	switch d.Olc.Mode {
	case HSEDIT_CONFIRM_SAVESTRING:
		...
		case 'Y':
			...
			mudlog(CMP, ADMLVL_BUILDER, 1, libc.CString("OLC: %s edits house %d"), GET_NAME(d.Character), d.Olc.Number)
	...
	case HSEDIT_MAIN_MENU:
		...
		case '1':
			hsedit_owner_menu(d)
		case '2':
			if d.Olc.House.Vnum == int(-1) || real_room(d.Olc.House.Vnum) == int(-1) {
				send_to_char(d.Character, libc.CString("ERROR: Invalid house VNUM\r\n(Press Enter)\r\n"))
				mudlog(NRM, ADMLVL_GRGOD, 1, libc.CString("SYSERR: Invalid house VNUM in hsedit"))
			} else {
		...
		case '3':
			if d.Olc.House.Vnum == int(-1) || real_room(d.Olc.House.Vnum) == int(-1) {
				send_to_char(d.Character, libc.CString("ERROR: Invalid house VNUM\r\n(Press Enter)\r\n"))
				mudlog(NRM, ADMLVL_BUILDER, 1, libc.CString("SYSERR: Invalid house VNUM in hsedit"))
			} else {
				hsedit_dir_menu(d)
			}
		case '4':
	...
	case HSEDIT_DIR_MENU:
		...
		if (world[id].Dir_option[number]) == nil {
			...
			hsedit_dir_menu(d)
			return
		} else if world[id].Dir_option[number].To_room == int(-1) {
			...
			d.Olc.House.Atrium = world[room_rnum].Number
		}
	case HSEDIT_NOVNUM:
	...
	case HSEDIT_DELETE:
		...
		case 'y':
			fallthrough
		case 'Y':
			hedit_delete_house(d, d.Olc.House.Vnum)
			return
		case 'n':
			fallthrough
		case 'N':
	...
	case HSEDIT_FLAGS:
		...
		if number < 0 || number > HOUSE_NUM_FLAGS {
			...
			if number == 0 {
				...
				if IS_SET(uint32(int32(d.Olc.House.Bitvector)), uint32(int32(1<<(number-1)))) {
		...
		return
	default:
		mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("SYSERR: Reached default case in parse_hsedit"))
	}
	d.Olc.Value = 1
	hsedit_disp_menu(d)
}
...
func do_oasis_hsedit(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	d = ch.Desc
	if d.Olc != nil {
		mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("SYSERR: do_oasis_hsedit: Player already had olc structure."))
	...
	if !can_edit_zone(ch, d.Olc.Zone_num) {
		...
		mudlog(BRF, ADMLVL_BUILDER, 1, libc.CString("OLC: %s tried to edit zone %d allowed zone %d"), GET_NAME(ch), zone_table[d.Olc.Zone_num].Number, ch.Player_specials.Olc_zone)
		libc.Free(unsafe.Pointer(d.Olc))
		d.Olc = nil
		return
	}
	if save != 0 {
		...
		mudlog(CMP, int(MAX(ADMLVL_BUILDER, int64(ch.Player_specials.Invis_level))), 1, libc.CString("OLC: %s saves house info for zone %d."), GET_NAME(ch), zone_table[d.Olc.Zone_num].Number)
		hsedit_save_to_disk()
		libc.Free(unsafe.Pointer(d.Olc))
		d.Olc = nil
		return
	...
	if real_num == int(-1) {
		if num_of_houses >= MAX_HOUSES {
			...
			mudlog(NRM, ADMLVL_BUILDER, 1, libc.CString("HSEDIT: MAX houses limit reached (%d)\r\n"), MAX_HOUSES)
			return
		} else {
			hsedit_setup_new(d)
		}
	...
	mudlog(CMP, ADMLVL_BUILDER, 1, libc.CString("OLC: (hsedit) %s starts editing zone %d allowed zone %d"), GET_NAME(ch), zone_table[d.Olc.Zone_num].Number, ch.Player_specials.Olc_zone)
}
