...
func do_oasis_copy(ch *char_data, argument *byte, cmd int, subcmd int) {
	var (
		...
		oasis_copy_info [6]struct {
			Con_type       int
			Binary_search  func(vnum int) int
			...
			Command        *byte
			Text           *byte
		}{{Con_type: CON_REDIT, Binary_search: real_room, Save_func: redit_save_internally, Setup_existing: redit_setup_existing, Command: libc.CString("rcopy"), Text: libc.CString("room")}, {Con_type: CON_OEDIT, Binary_search: real_object, Save_func: oedit_save_internally, Setup_existing: oedit_setup_existing, Command: libc.CString("ocopy"), Text: libc.CString("object")}, {Con_type: CON_MEDIT, Binary_search: real_mobile, Save_func: medit_save_internally, Setup_existing: medit_setup_existing, Command: libc.CString("mcopy"), Text: libc.CString("mobile")}, {Con_type: CON_SEDIT, Binary_search: real_shop, Save_func: sedit_save_internally, Setup_existing: sedit_setup_existing, Command: libc.CString("scopy"), Text: libc.CString("shop")}, {Con_type: CON_TRIGEDIT, Binary_search: real_trigger, Save_func: trigedit_save, Setup_existing: trigedit_setup_existing, Command: libc.CString("tcopy"), Text: libc.CString("trigger")}, {Con_type: -1, Binary_search: nil, Save_func: nil, Setup_existing: nil, Command: libc.CString("\n"), Text: libc.CString("\n")}}
	)
	...
	if buf1[0] == 0 || buf2[0] == 0 || (!is_number(&buf1[0]) || !is_number(&buf2[0])) {
	...
	for d = descriptor_list; d != nil; d = d.Next {
		if d.Connected == subcmd {
			if d.Olc != nil && d.Olc.Number == dst_vnum {
	...
	d = ch.Desc
	if d.Olc != nil {
		mudlog(BRF, ADMLVL_IMMORT, 1, libc.CString("SYSERR: do_oasis_copy: Player already had olc structure."))
		libc.Free(unsafe.Pointer(d.Olc))
	}
	d.Olc = new(oasis_olc_data)
	if (func() int {
		p := &d.Olc.Zone_num
		d.Olc.Zone_num = real_zone_by_thing(dst_vnum)
		return *p
	}()) == int(-1) {
...
func do_dig(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if (func() int {
		dir = search_block(&sdir[0], &abbr_dirs[0], 0)
		return dir
	}()) < 0 {
		dir = search_block(&sdir[0], &dirs[0], 0)
	}
	...
	if zone == int(-1) || !can_edit_zone(ch, zone) {
	...
	if rrnum == int(-1) {
		if d.Olc != nil {
			mudlog(BRF, ADMLVL_IMMORT, 1, libc.CString("SYSERR: do_dig: Player already had olc structure."))
			libc.Free(unsafe.Pointer(d.Olc))
		}
		d.Olc = new(oasis_olc_data)
		d.Olc.Zone_num = zone
...
func do_rcopy(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if arg2[0] == 0 {
		tvnum = libc.Atoi(libc.GoString(&arg[0]))
		rvnum = int(libc.BoolToInt(GET_ROOM_VNUM(ch.In_room)))
	} else if arg2[0] != 0 {
		rvnum = libc.Atoi(libc.GoString(&arg2[0]))
		tvnum = libc.Atoi(libc.GoString(&arg[0]))
	}
	if rvnum == 0 || tvnum == 0 {
...
func redit_find_new_vnum(zone int) int {
	var (
		vnum int = genolc_zone_bottom(zone)
...
func buildwalk(ch *char_data, dir int) bool {
	var (
		buf  [2048]byte
		vnum int
		rnum int
	...
	if !IS_NPC(ch) && PRF_FLAGGED(ch, PRF_BUILDWALK) && ch.Admlevel >= ADMLVL_IMMORT {
		if !can_edit_zone(ch, world[ch.In_room].Zone) {
			send_cannot_edit(ch, world[ch.In_room].Zone)
		} else if (func() int {
			...
			if d.Olc != nil {
				mudlog(BRF, ADMLVL_IMMORT, 1, libc.CString("SYSERR: buildwalk(): Player already had olc structure."))
				libc.Free(unsafe.Pointer(d.Olc))
			}
			d.Olc = new(oasis_olc_data)
