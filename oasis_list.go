...
func do_oasis_list(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	if subcmd == SCMD_OASIS_ZLIST {
		if smin[0] != 0 && is_number(&smin[0]) != 0 {
...
func do_oasis_links(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	last = int(zone_table[zrnum].Top)
	first = int(zone_table[zrnum].Bot)
		...
		if int(libc.BoolToInt(GET_ROOM_VNUM(nr))) >= first {
			for j = 0; j < NUM_OF_DIRS; j++ {
				if world[nr].Dir_option[j] != nil {
					...
					if to_room != room_rnum(-1) && zrnum != world[to_room].Zone {
...
func list_rooms(ch *char_data, rnum zone_rnum, vmin zone_vnum, vmax zone_vnum) {
	...
	for i = 0; i <= int(top_of_world); i++ {
		if world[i].Number >= room_vnum(bottom) && world[i].Number <= room_vnum(top) {
			counter++
			...
			for j = 0; j < NUM_OF_DIRS; j++ {
				...
				if (world[i].Dir_option[j]).To_room == room_rnum(-1) {
					continue
				}
...
func list_mobiles(ch *char_data, rnum zone_rnum, vmin zone_vnum, vmax zone_vnum) {
	...
	for i = 0; i <= int(top_of_mobt); i++ {
		if mob_index[i].Vnum >= mob_vnum(bottom) && mob_index[i].Vnum <= mob_vnum(top) {
			counter++
...
func list_objects(ch *char_data, rnum zone_rnum, vmin room_vnum, vmax room_vnum) {
	...
	for i = 0; i <= int(top_of_objt); i++ {
		if obj_index[i].Vnum >= mob_vnum(bottom) && obj_index[i].Vnum <= mob_vnum(top) {
			counter++
...
func list_shops(ch *char_data, rnum zone_rnum, vmin shop_vnum, vmax shop_vnum) {
	...
	for i = 0; i <= top_shop; i++ {
		if shop_index[i].Vnum >= room_vnum(bottom) && shop_index[i].Vnum <= room_vnum(top) {
			counter++
			send_to_char(ch, libc.CString("@g%4d@n) [@g%-5d@n]"), counter, shop_index[i].Vnum)
			for j = 0; (shop_index[i].In_room[j]) != room_vnum(-1); j++ {
...
func print_zone(ch *char_data, vnum zone_vnum) {
	...
	for i = 0; i <= largest_table; i++ {
		...
		if i < top_of_trigt {
			if trig_index[i].Vnum >= mob_vnum(bottom) && trig_index[i].Vnum <= mob_vnum(top) {
				size_triggers++
			}
		}
	...
	for i = 0; i < top_of_trigt; i++ {
		if trig_index[i].Vnum >= mob_vnum(bottom) && trig_index[i].Vnum <= mob_vnum(top) {
			...
			if int(trig_index[i].Proto.Attach_type) == OBJ_TRIGGER {
				sprintbit(bitvector_t(int32(trig_index[i].Proto.Trigger_type)), otrig_types[:], &trgtypes[0], uint64(256))
				send_to_char(ch, libc.CString("obj @y%s@n\r\n"), &trgtypes[0])
			} else if int(trig_index[i].Proto.Attach_type) == WLD_TRIGGER {
				sprintbit(bitvector_t(int32(trig_index[i].Proto.Trigger_type)), wtrig_types[:], &trgtypes[0], uint64(256))
				send_to_char(ch, libc.CString("wld @y%s@n\r\n"), &trgtypes[0])
			} else {
				sprintbit(bitvector_t(int32(trig_index[i].Proto.Trigger_type)), trig_types[:], &trgtypes[0], uint64(256))
