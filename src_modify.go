	...
	if PLR_FLAGGED(d.Character, PLR_WRITING) {
		if int(d.Mail_to) >= BOARD_MAGIC {
			if action == STRINGADD_ABORT {
				board = locate_board(int(d.Mail_to) - BOARD_MAGIC)
				fore = func() *board_msg {
					cur = func() *board_msg {
						aft = nil
						return aft
				...
				save_board(locate_board(int(d.Mail_to) - BOARD_MAGIC))
			}
		}
	}
}
...
func do_skillset(ch *char_data, argument *byte, cmd int, subcmd int) {
	...
	mudlog(BRF, ADMLVL_IMMORT, 1, libc.CString("skillset: %s changed %s's '%s' to %d."), GET_NAME(ch), GET_NAME(vict), spell_info[skill].Name, value)
