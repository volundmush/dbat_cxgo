...
func create_new_board(board_vnum obj_vnum) *board_info {
	...
	temp.Vnum = int(board_vnum)
	temp.Num_messages = 0
	temp.Version = CURRENT_BOARD_VER
	temp.Next = nil
	temp.Messages = nil
	if save_board(temp) == 0 {
...
func save_board(ts *board_info) int {
	var (
		message  *board_msg
...
func show_board(board_vnum obj_vnum, ch *char_data) {
	var (
	...
	var obj *obj_data
	var num int = int(board_vnum)
	if (func() obj_vnum {
		board_vnum = obj_vnum(real_object(obj_vnum(num)))
		return board_vnum
	}()) == obj_vnum(-1) {
...
func board_display_msg(board_vnum obj_vnum, ch *char_data, arg int) {
	var (
...
func mesglookup(message *board_msg, ch *char_data, board *board_info) int {
	var (
		mem         int = 0
		mboard_type *board_memory
		tempname    *byte = nil
...
func write_board_message(board_vnum obj_vnum, ch *char_data, arg *byte) {
	var (
		thisboard *board_info = bboards
		message   *board_msg
	)
...
func board_respond(board_vnum int, ch *char_data, mnum int) {
	...
	thisboard = locate_board(obj_vnum(board_vnum))
	if thisboard == nil {
		send_to_char(ch, libc.CString("Error: Your board could not be found. Please report.\n"))
...
func locate_board(board_vnum obj_vnum) *board_info {
	var thisboard *board_info = bboards
	for thisboard != nil {
		if thisboard.Vnum == int(board_vnum) {
			return thisboard
		}
...
func remove_board_msg(board_vnum obj_vnum, ch *char_data, arg int) {
	var (
		thisboard *board_info
		cur       *board_msg
		temp      *board_msg
